...
const getSections = (): Section[] => {
  let id = 0;
  const r = (name: string, desc: string, result: unknown): Row => ({
    id: ++id,
    name,
    desc,
    result,
    ...
    },
    {
      ...
      rows: [
        r(
          ...
          $bm.n08_insert_t({ name: 'Alice', place: 'Paris' })
        ),
        ...
        r(
          'insert(enu(t()))',
          '{count:3,name:"Alice"}(3)',
          $bm.n12_insert_enu_t({ count: 3, name: 'Alice' })(3)
        ),
        r('cond(t()) true', 'true', $bm.n13_cond_t(true)),
        r(
          'cond(insert(t()))',
          "true · {name:'Alice'}",
          $bm.n14_cond_insert_t(true)?.({ name: 'Alice' })
        ),
        r(
          'gender(insert(t()))',
          "female · {name:'Alice'}",
    ...
    },
    {
      ...
      rows: [
        r(
          'nested object',
          'obj.level1.level2',
          $bm.n17_nested_object.level1.level2
        ),
        r(
          "array ['string']",
          "['item1','item2','item3']",
          ($bm.n18_array_strings as string[]).join(', ')
        ),
        r('array [t()]', 'Array of translations', $bm.n19_array_translations),
        r(
          'array [{obj}]',
          'Array of {name,role}',
          $bm.n20_array_objects.map((o) => `${o.name} (${o.role})`).join(' · ')
        ),
    ...
    },
    {
      ...
      rows: [
        ...
        r(
          'useUnit()',
          '5, kilometer, long',
          $unit(5, { unit: 'kilometer', unitDisplay: 'long' })
        ),
