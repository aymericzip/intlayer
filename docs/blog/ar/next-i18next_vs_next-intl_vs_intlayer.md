...
- تأكد من تعيين `lang` و `dir` على `<html>` الجذر في `src/app/[locale]/layout.tsx`.
- قسّم الترجمات إلى مساحات الأسماء (على سبيل المثال `common.json` و `about.json`) تحت `src/locales/<locale>/`.
- قم بتحميل مساحات الأسماء المطلوبة فقط في مكونات العميل باستخدام `useTranslation('<ns>')` وبمحاذاة `I18nProvider` مع نفس مساحات الأسماء.
- حافظ على الصفحات ثابتة قدر الإمكان: صدّر `export const dynamic = 'force-static'` على الصفحات؛ عيّن `dynamicParams = false` وطبّق `generateStaticParams`.
- استخدم مكونات خادم متزامنة متداخلة تحت حدود العميل بواسطة تمرير السلاسل المحسوبة مسبقًا أو دالة `t` والـ `locale`.
- بالنسبة لـ SEO، عيّن `alternates.languages` في البيانات الوصفية، واعرض قوائم عناوين URL المحلية في `sitemap.ts`، وامنع المسارات المحلية المكررة في `robots.ts`.
- فضّل أدوات التنسيق الموعية بالإعدادات المحلية (مثلاً `Intl.NumberFormat(locale)`) وقم بتخزينها مؤقتًا على العميل إذا كنت تستخدم React < 19.

  </Tab>
  <Tab label="next-intl" value="next-intl">

- **عيّن `lang` و `dir` على html**: في `src/app/[locale]/layout.tsx`، احسب `dir` عبر `getLocaleDirection(locale)` وعيّن `<html lang={locale} dir={dir}>`.
- **قسّم الرسائل حسب مساحة الأسماء**: نظّم JSON لكل إعداد محلي ومساحة أسماء (مثلاً `common.json` و `about.json`).
- **قلّل حمولة العميل**: على الصفحات، أرسل مساحات الأسماء المطلوبة فقط إلى `NextIntlClientProvider` (مثلاً `pick(messages, ['common', 'about'])`).
- **فضّل الصفحات الثابتة**: صدّر `export const dynamic = 'force-static'` وأنشئ معاملات ثابتة لجميع `locales`.
- **مكونات الخادم المتزامنة**: حافظ على مكونات الخادم متزامنة بتمرير السلاسل المحسوبة مسبقًا (التسميات المترجمة والأرقام المنسقة) بدلاً من الاستدعاءات غير المتزامنة أو الدوال غير القابلة للتسلسل.

  </Tab>
  <Tab label="intlayer" value="intlayer">

- **المحتوى المعياري**: ضع قواميس المحتوى بجانب المكونات باستخدام ملفات `.content.{ts|js|json}`.
- **سلامة النوع**: استفد من دمج TypeScript للتحقق من المحتوى في وقت التجميع.
- **تحسين وقت البناء**: استخدم أدوات بناء Intlayer لـ tree-shaking التلقائي وتحسين حجم الحزمة.
