...
로케일 감지 및 라우팅을 처리하기 위한 middleware를 추가합니다:
...
export const config = {
  matcher: [
    // 이들로 시작하는 경로와 확장자가 있는 파일을 제외한 모든 경로와 일치
    "/((?!api|_next|static|.*\\..*).*)",
  ],
};
```
...
Intlayer는 `next-intlayer` package 설정을 통한 기본 제공 middleware 처리를 제공합니다.
...
// app 디렉토리의 파일에만 이 middleware를 적용합니다
export const config = {
  matcher: "/((?!api|_next|static|.*\\..*).*)",
};
```

middleware의 설정은 `intlayer.config.ts` 파일에 중앙화되어 있습니다.
...
- `src/app/[locale]/layout.tsx`의 루트 `<html>`에서 `lang`과 `dir`이 설정되어 있는지 확인하세요.
- `src/locales/<locale>/` 아래에서 번역을 네임스페이스로 분할합니다 (예: `common.json`, `about.json`).
- 클라이언트 컴포넌트에서 `useTranslation('<ns>')`를 사용하여 필요한 네임스페이스만 로드하고 `I18nProvider`를 동일한 네임스페이스로 범위를 제한합니다.
- 가능한 경우 페이지를 정적으로 유지합니다: 페이지에서 `export const dynamic = 'force-static'`을 내보내고 `dynamicParams = false`를 설정하고 `generateStaticParams`를 구현합니다.
- 이미 계산된 문자열이나 `t` 함수 및 `locale`을 전달하여 클라이언트 경계 아래에 동기 서버 컴포넌트를 사용합니다.
- SEO의 경우, 메타데이터에서 `alternates.languages`를 설정하고 `sitemap.ts`에서 지역화된 URL을 나열하고 `robots.ts`에서 중복된 지역화된 경로를 비활성화합니다.
- 로케일 인식 포매터 (예: `Intl.NumberFormat(locale)`)를 선호하고 React < 19를 사용하는 경우 클라이언트에서 메모이제이션합니다.

  </Tab>
  <Tab label="next-intl" value="next-intl">

- **html `lang`과 `dir` 설정**: `src/app/[locale]/layout.tsx`에서 `getLocaleDirection(locale)`을 통해 `dir`을 계산하고 `<html lang={locale} dir={dir}>`을 설정합니다.
- **네임스페이스별로 메시지 분할**: 로케일 및 네임스페이스별로 JSON을 구성합니다 (예: `common.json`, `about.json`).
- **클라이언트 페이로드 최소화**: 페이지에서 필요한 네임스페이스만 `NextIntlClientProvider`로 전송합니다 (예: `pick(messages, ['common', 'about'])`).
- **정적 페이지 선호**: `export const dynamic = 'force-static'`을 내보내고 모든 `locales`에 대한 정적 매개변수를 생성합니다.
- **동기 서버 컴포넌트**: 비동기 호출이나 직렬화할 수 없는 함수 대신 사전 계산된 문자열 (번역된 레이블, 형식이 지정된 숫자)을 전달하여 서버 컴포넌트를 동기적으로 유지합니다.

  </Tab>
  <Tab label="intlayer" value="intlayer">

- **모듈식 콘텐츠**: `.content.{ts|js|json}` 파일을 사용하여 콘텐츠 딕셔너리를 컴포넌트와 함께 배치합니다.
- **타입 안전성**: TypeScript 통합을 활용하여 컴파일 타임 콘텐츠 검증을 수행합니다.
- **빌드 타임 최적화**: Intlayer의 빌드 도구를 사용하여 자동 트리 셰이킹 및 번들 최적화를 수행합니다.
