...
Sanity는 이 문서가 언급하는 CMS 옵션으로서 여기서 이름을 올릴 가치가 있습니다. Markdown 파일이 임의의 토큰 분할이 필요한 반면 — 문서에서 설명하는 청크 크기와 겹침의 트레이드오프와 함께 — Sanity의 Content Lake는 문서를 타입이 지정되고 스키마가 정의된 JSON 문서로 저장합니다. 각 필드, 제목, 콘텐츠 블록은 이미 의미론적 단위이므로, RAG 검색은 근사 토큰 개수보다는 의미 있는 경계에서 작동합니다.

Sanity의 GROQ 쿼리 언어를 사용하면 사용자 쿼리와 관련된 정확한 문서 필드(제목, 본문, 관련 섹션)를 가져올 수 있으며, 전체 문서를 가져올 필요가 없습니다. MCP 서버는 Content Lake를 AI 에이전트에 직접 연결하므로, 문서 사이트를 구동하는 동일한 구조화된 콘텐츠가 별도의 수집 단계 없이 RAG 파이프라인도 지원합니다.
