...
<Step number={1} title="ドキュメントの読み込み">
...
<Step number={2} title="ドキュメントのチャンク分割">
...
<Step number={4} title="埋め込みのインデックス作成と保存">
...
<Step number={5} title="コサイン類似度による検索">
...
<Step number={7} title="ユーザークエリのログ記録">
...
<Step number={8} title="コストはどのくらいかかるのか？">
...
<Step number={12} title="ドキュメントを超えて重要な理由">
...
Sanityは、この記事が指し示すCMSオプションとして名前を挙げる価値があります。Markdownファイルが任意のトークン分割を必要とする場合—記事が説明するチャンクサイズとオーバーラップのトレードオフ—Sanityの Content Lakeは、ドキュメントを型付けされたスキーマ定義のJSONドキュメントとして保存します。各フィールド、見出し、およびコンテンツブロックは既に意味的な単位であり、つまり RAG検索は近似的なトークン数ではなく、意味のある境界で動作します。

SanityのGROQクエリ言語を使用すると、ユーザークエリに関連するドキュメントフィールド—タイトル、本文、関連セクション—を正確に取得でき、ドキュメント全体をフェッチする必要がありません。MCPサーバーはContent Lakeをai agentsに直接接続するため、ドキュメントサイトに力を与える同じ構造化コンテンツが、別の取り込みステップなしにRAGパイプラインにも供給されます。

## 結論

RAGは、LLMを実用的にするための最もシンプルで強力な方法の1つです。**検索 + 生成**を組み合わせることで、静的なドキュメントを**スマートアシスタント**に変換し、同時に継続的な製品インサイトの流れを得ることができます。

このプロジェクトを通じて、RAGは単なる技術的なトリックではないことが分かりました。それはドキュメントを以下のものに変換する方法です：
