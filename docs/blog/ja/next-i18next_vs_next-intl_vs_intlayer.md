...
const PUBLIC_FILE = /\.[^/]+$/; // 拡張子を持つファイルを除外
...
export const config = {
  matcher: [
    // これらで始まるパスとファイル拡張子を持つファイルを除いたすべてのパスにマッチ
    "/((?!api|_next|static|.*\\..*).*)",
  ],
};
```
...
Intlayerは`next-intlayer`パッケージの設定を通じて、組み込みのミドルウェア処理を提供します。
...
// このミドルウェアをappディレクトリ内のファイルにのみ適用
export const config = {
  matcher: "/((?!api|_next|static|.*\\..*).*)",
};
```

ミドルウェアのセットアップは`intlayer.config.ts`ファイルで一元管理されます。

  </Tab>
</Tabs>

### セットアップチェックリストとベストプラクティス

<Tabs defaultTab="next-intl" group='techno'>
  <Tab label="next-i18next" value="next-i18next">

- `src/app/[locale]/layout.tsx` のルート `<html>` に `lang` と `dir` が設定されていることを確認してください。
- `src/locales/<locale>/` の下に翻訳を名前空間に分割してください（例：`common.json`、`about.json`）。
- クライアントコンポーネントで `useTranslation('<ns>')` を使用して必要な名前空間のみをロードし、`I18nProvider` を同じ名前空間でスコープしてください。
- 可能な限りページを静的に保つ：ページで `export const dynamic = 'force-static'` をエクスポートし、`dynamicParams = false` を設定して `generateStaticParams` を実装してください。
- クライアント境界の下にネストされた同期サーバーコンポーネントを使用して、既に計算された文字列または `t` 関数と `locale` を渡してください。
- SEO については、メタデータで `alternates.languages` を設定し、`sitemap.ts` でローカライズされた URL をリストアップし、`robots.ts` で重複したローカライズされたルートを禁止してください。
- ロケール対応のフォーマッタ（例：`Intl.NumberFormat(locale)`）を優先し、React < 19 を使用している場合はクライアントでメモ化してください。

  </Tab>
  <Tab label="next-intl" value="next-intl">

- **html の `lang` と `dir` を設定する**：`src/app/[locale]/layout.tsx` で `getLocaleDirection(locale)` を使用して `dir` を計算し、`<html lang={locale} dir={dir}>` を設定してください。
- **メッセージを名前空間で分割する**：ロケールと名前空間ごとに JSON を整理してください（例：`common.json`、`about.json`）。
- **クライアントペイロードを最小化する**：ページで、必要な名前空間のみを `NextIntlClientProvider` に送信してください（例：`pick(messages, ['common', 'about'])`）。
- **静的ページを優先する**：`export const dynamic = 'force-static'` をエクスポートし、すべての `locales` に対して静的パラメータを生成してください。
- **同期サーバーコンポーネント**：非同期呼び出しまたは非シリアル化可能な関数ではなく、事前計算された文字列（翻訳されたラベル、フォーマットされた数字）を渡すことでサーバーコンポーネントを同期に保ってください。

  </Tab>
  <Tab label="intlayer" value="intlayer">

- **モジュラーコンテンツ**：`.content.{ts|js|json}` ファイルを使用してコンテンツ辞書をコンポーネントと共存させてください。
- **タイプセーフティ**：TypeScript 統合を活用してコンパイル時のコンテンツ検証を行ってください。
- **ビルドタイム最適化**：Intlayer のビルドツールを使用して自動的なツリーシェイキングとバンドル最適化を行ってください。
- **統合ツール**：組み込みのルーティング、SEO ヘルパー、ビジュアルエディタサポートを活用してください。
