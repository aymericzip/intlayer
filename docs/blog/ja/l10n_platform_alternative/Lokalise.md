...
## AIの登場以来、なぜパラダイムは変わったのか？

Lokaliseはローカライゼーションの「DevOps」側を完成させ、文字列を自動的に移動しました。しかし、大規模言語モデル（LLM）の登場により、ローカライゼーションのパラダイムが根本的に変わりました。ボトルネックはもはや文字列を*移動*することではなく、それらを*生成*することです。

LLMを使用することで、翻訳のコストは急落し、スピードは指数関数的に増加しました。ローカライゼーションチームの役割は「翻訳者の管理」から「コンテキストとレビューの管理」へシフトしています。

Lokaliseはai機能を追加しましたが、根本的には人間のワークフローを管理し、シート数またはキー数で料金を請求するように設計されたプラットフォームのままです。AIファースト・ワールドでは、価値は、人間のエージェンシーにタスクを割り当てることがいかに簡単かではなく、コンテキスト対応コンテンツを生成するためにaiモデルをいかに上手くオーケストレーションできるかにあります。

今日、最も効率的なワークフローは、まずaiを使用してページをグローバルに翻訳して配置することです。その後、第2段階では、人間のコピーライターを使用して、製品がすでに収益を生み出しているときに、高トラフィック・コンテンツを最適化して変換を促進します。
