...
- **Intlayer**: प्रति-घटक `.content.{ts|js|json}` शब्दकोश जो घटकों के साथ सह-स्थित हैं। आसान घटक पुनः उपयोग और स्थानीय तर्क; फ़ाइलें जोड़ता है और बिल्ड-समय उपकरण पर निर्भर करता है।
...
- **संख्या स्वरूपण**
  - **next-i18next**: कोई `useNumber` नहीं; `Intl.NumberFormat` (या i18next-icu) का उपयोग करें।
  - **next-intl**: `useFormatter().number(value)`।
  - **Intlayer**: `useNumber()` built-in।

- **Keys**
  - एक nested structure (`about.counter.label`) रखें और अपने hook को accordingly scope करें (`useTranslation("about")` + `t("counter.label")` या `useTranslations("about.counter")` + `t("label")`)।

- **फ़ाइल स्थान**
  - **next-i18next** को `public/locales/{lng}/{ns}.json` में JSON की अपेक्षा है।
  - **next-intl** लचीला है; संदेशों को अपनी कॉन्फ़िगरेशन के अनुसार लोड करें।
