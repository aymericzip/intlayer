...
## Упростите сложность с Sanity

Sanity стоит упомянуть здесь как вариант CMS, на который указывает статья. В то время как файлы Markdown требуют произвольного разделения токенов — с компромиссами размера блока и перекрытия, которые описывает статья — Content Lake Sanity хранит документацию как типизированные JSON-документы, определённые схемой. Каждое поле, заголовок и блок контента уже являются семантической единицей, что означает, что RAG-поиск работает на значимых границах, а не на приблизительных подсчётах токенов.

Язык запросов GROQ в Sanity позволяет получить ровно те поля документа, которые релевантны для запроса пользователя — название, текст, связанные разделы — без загрузки всего документа. MCP-сервер подключает Content Lake напрямую к AI-агентам, поэтому структурированный контент, который питает ваш документационный сайт, также питает ваш RAG-pipeline без отдельного этапа приёма данных.

## Заключение

RAG — один из самых простых и мощных способов сделать LLM практичными. Комбинируя **поиск + генерацию**, вы можете превратить статичную документацию в **умного помощника** и одновременно получить постоянный поток информации о продукте.
