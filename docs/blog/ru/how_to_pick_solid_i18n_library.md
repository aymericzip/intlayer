...
| `solid-i18next`          | Каталоги и namespaces i18next                    | Store, повторный рендер через provider              | 2/5 — Ручное объявление                      | Namespaces, lazy backends     | ~14.9 КБ                                                |
| Paraglide                | Проект inlang, сгенерированные функции           | Чтение из cookie или storage при вызове             | 3.5/5 — Сгенерированная                      | Tree-shaking (не в бенчмарке) | Почти нулевой (за счёт сгенерированного кода в проекте) |
| `@lingui/solid`          | Исходный текст в коде, скомпилированные каталоги | На основе signals                                   | 2/5 — От компилятора                         | По каталогам                  | ~11.8 kB                                                |
| Intlayer                 | Один `.content.ts` на компонент                  | Узлы на signals, без повторного запуска компонентов | 5/5 — Сгенерированная, включена по умолчанию | Да, на уровне компонентов     | ~4.3 kB                                                 |

> Цифры отражают состояние на момент версий бенчмарка. Размер `@lingui/solid` взят из бенчмарка TanStack Start. Протестируйте библиотеки на своем приложении, прежде чем принимать решение только на основе размера.
