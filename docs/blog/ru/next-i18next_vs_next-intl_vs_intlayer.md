...
![Теоретическая утечка контента по архитектуре](https://github.com/aymericzip/intlayer/blob/main/docs/assets/theorical_content_leakage.png?raw=true)
...
| ----------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| ![оптимизированный бандл](https://github.com/aymericzip/intlayer/blob/main/docs/assets/bundle.png?raw=true) | ![неоптимизированный бандл](https://github.com/aymericzip/intlayer/blob/main/docs/assets/bundle_no_optimization.png?raw=true) |
...
export const config = {
  matcher: [
    // Совпадение всех путей, кроме начинающихся с этих и файлов с расширениями
    "/((?!api|_next|static|.*\\..*).*)",
  ],
};
```
...
export const config = {
  // Пропустить API, Next внутренние компоненты и статические ассеты
  matcher: ["/((?!api|_next|.*\\..*).*)"],
};
```
...
Intlayer предоставляет встроенную обработку middleware через конфигурацию пакета `next-intlayer`.
...
// применяет этот middleware только к файлам в директории app
export const config = {
  matcher: "/((?!api|_next|static|.*\\..*).*)",
};
```

Конфигурация middleware централизована в файле `intlayer.config.ts`.

  </Tab>
</Tabs>

### Контрольный список настройки и лучшие практики

<Tabs defaultTab="next-intl" group='techno'>
  <Tab label="next-i18next" value="next-i18next">

- Убедитесь, что `lang` и `dir` установлены на корневом `<html>` в `src/app/[locale]/layout.tsx`.
- Разделите переводы на пространства имен (например `common.json`, `about.json`) в `src/locales/<locale>/`.
- Загружайте только требуемые пространства имен в клиентских компонентах с помощью `useTranslation('<ns>')` и обозначив область `I18nProvider` теми же пространствами имен.
- Держите страницы статичными когда возможно: экспортируйте `export const dynamic = 'force-static'` на страницах; установите `dynamicParams = false` и реализуйте `generateStaticParams`.
- Используйте синхронные серверные компоненты, вложенные под клиентские границы, передавая предварительно вычисленные строки или функцию `t` и `locale`.
- Для SEO установите `alternates.languages` в метаданных, укажите локализованные URLs в `sitemap.ts` и запретите дублирующиеся локализованные маршруты в `robots.ts`.
- Предпочитайте форматеры, зависящие от локали (например `Intl.NumberFormat(locale)`) и мемоизируйте их на клиенте, если используется React < 19.

  </Tab>
  <Tab label="next-intl" value="next-intl">

- **Установите html `lang` и `dir`**: В `src/app/[locale]/layout.tsx` вычислите `dir` через `getLocaleDirection(locale)` и установите `<html lang={locale} dir={dir}>`.
- **Разделите сообщения по пространствам имен**: Организуйте JSON по локали и пространству имен (например `common.json`, `about.json`).
- **Минимизируйте полезную нагрузку клиента**: На страницах отправляйте только требуемые пространства имен в `NextIntlClientProvider` (например `pick(messages, ['common', 'about'])`).
- **Предпочитайте статичные страницы**: Экспортируйте `export const dynamic = 'force-static'` и генерируйте статичные параметры для всех `locales`.
- **Синхронные серверные компоненты**: Держите серверные компоненты синхронными, передавая предварительно вычисленные строки (переведенные метки, отформатированные числа) вместо асинхронных вызовов или функций, которые нельзя сериализовать.

  </Tab>
  <Tab label="intlayer" value="intlayer">

- **Модульное содержимое**: Разместите словари содержимого рядом с компонентами, используя файлы `.content.{ts|js|json}`.
- **Безопасность типов**: Используйте интеграцию TypeScript для проверки содержимого во время компиляции.
- **Оптимизация на этапе сборки**: Используйте инструменты сборки Intlayer для автоматического tree-shaking и оптимизации пакета.
- **Встроенные инструменты**: Используйте встроенную маршрутизацию, помощники SEO и поддержку визуального редактора.
...
[![График истории звёзд](https://api.star-history.com/chart?repos=i18next/next-i18next%2Camannn/next-intl%2Caymericzip/intlayer&type=date&legend=top-left)](https://star-history.com/#i18next/next-i18next&amannn/next-intl&aymericzip/intlayer)
