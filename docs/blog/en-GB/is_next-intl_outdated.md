...
Turbopack and Webpack cannot verify which keys inside `UserProfile` are actually called at runtime. To prevent missing-key errors, **the bundler must include the entire namespace in the client chunk**. Intlayer's destructured properties allow the compiler to track references directly and strip unused fields during compilation. See [bundle optimisation](https://intlayer.org/doc/concept/bundle-optimization) for details.
...
- [The Case for Compiler-Driven Internationalisation](https://intlayer.org/blog/compiler-vs-declarative-i18n)
