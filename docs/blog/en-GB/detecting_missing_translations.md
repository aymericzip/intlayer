...
The tooling is portable too. If your messages live in JSON catalogues today, the [Sync JSON plugin](https://github.com/aymericzip/intlayer/blob/main/docs/docs/en-GB/plugins/sync-json.md) points Intlayer at those files, so you get the audit, fill and test commands without moving your content or changing a single import:
...
If you want the runtime API to stay identical as well, the [compat adapters](https://github.com/aymericzip/intlayer/blob/main/docs/docs/en-GB/compat/index.md) alias `useTranslation`, `$t` and friends at the bundler level. Either way, treat the commands below as one implementation of the idea, not as a requirement.
...
| Lint            | Hardcoded strings never sent for translation | Keys missing from a catalogue                 |
...
This layer is necessary and not sufficient. Types describe the shape of your default catalogue. They say nothing about whether Korean has a value for that key.

## Layer 2: lint the strings that never became keys

The translation you cannot find is often the one that was never externalised. A hardcoded label in a component is invisible to every catalogue-based audit, because as far as the tooling is concerned it does not exist.
...
`no-unused-content` is the one that keeps catalogues from growing forever. Dead keys are not a correctness bug, but they are what makes a translation vendor invoice larger than it should be. Full rule list in the [ESLint plugin doc](https://github.com/aymericzip/intlayer/blob/main/docs/docs/en-GB/eslint.md).
...
**Pseudolocalisation.** Run a locale where every string is transformed, for example `[!!! Ĉĥéçķöũţ !!!]`. Anything rendering in plain English is hardcoded. It finds in ten minutes what a catalogue audit structurally cannot see, because it tests the rendered page rather than the catalogue.

**Crawl your own site.** If you serve localised URLs, fetch a sample per locale and grep the HTML for your default-language strings. A page in `/ja/` containing "Add to cart" is either a missing translation or a fallback you did not know about.
...
Once you know what is missing, `intlayer fill` populates the empty entries, and the `autoFill` option can generate per-locale files as content is declared. See [autoFill](https://github.com/aymericzip/intlayer/blob/main/docs/docs/en-GB/autoFill.md).
...
- **Auditing catalogues but never the rendered page.** Hardcoded strings are invisible to a catalogue audit by definition.
...
- [ESLint plugin rules, including `no-raw-text` and `no-unused-content`](https://github.com/aymericzip/intlayer/blob/main/docs/docs/en-GB/eslint.md)
- [autoFill: generating per-locale declaration files](https://github.com/aymericzip/intlayer/blob/main/docs/docs/en-GB/autoFill.md)
- [Configuration reference: `locales`, `requiredLocales`, `defaultLocale`](https://github.com/aymericzip/intlayer/blob/main/docs/docs/en-GB/configuration.md)
- [Benchmark reports across frameworks](https://github.com/aymericzip/intlayer/blob/main/docs/docs/en-GB/benchmark/index.md)
- [Drop-in i18next compat adapter](https://github.com/aymericzip/intlayer/blob/main/docs/docs/en-GB/compat/i18next.md)
- [What internationalisation actually covers](https://github.com/aymericzip/intlayer/blob/main/docs/blog/en-GB/what_is_internationalization.md)
