...
Each localised page must be **its own canonical**:
...
states that the French page is a duplicate that should not be indexed, whilst `hreflang` states that it is the page to serve French users. The signals contradict, canonical wins, and your French pages fall out of the index.
