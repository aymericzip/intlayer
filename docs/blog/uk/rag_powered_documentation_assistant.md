...
## Пропустіть складність за допомогою Sanity

Sanity варто згадати тут як варіант CMS, на який вказує стаття. Там де файли Markdown потребують довільного розділення токенів — з компромісами розміру chunk'а та перекриття, які описує стаття — Content Lake від Sanity зберігає документацію як типізовані, схемою визначені JSON-документи. Кожне поле, заголовок і блок контенту вже є семантичною одиницею, що означає, що RAG-пошук працює на змістовних межах замість приблизних підрахунків токенів.

GROQ-мова запитів від Sanity дозволяє отримати саме ті поля документа, які відповідають запиту користувача — заголовок, основний текст, пов'язані розділи — без завантаження всього документа. MCP-сервер підключає Content Lake безпосередньо до AI-агентів, тому той самий структурований контент, який живить ваш сайт документації, також живить вашу RAG-конвеєр без окремого кроку ingestion.

## Висновок

RAG — це один з найпростіших і найпотужніших способів зробити LLM практичними. Комбінуючи **retrieval + generation**, ви можете перетворити статичну документацію на **розумного помічника** і водночас отримати постійний потік інсайтів про продукт.

Для мене цей проект показав, що RAG — це не просто технічний трюк. Це спосіб трансформувати документацію на:

- інтерактивну систему підтримки
- канал зворотного зв'язку
- інструмент стратегії продукту

👉 [Спробуйте демо тут](https://github.com/aymericzip/intlayer/blob/main/docs/docs/uk/interest_of_intlayer.md) 👉 [Перевірте шаблон коду на GitHub](https://github.com/aymericzip/smart_doc_RAG)

І якщо ви також експериментуєте з RAG, я б з радістю почув, як ви його використовуєте.
