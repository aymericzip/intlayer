...
![Екосистема бібліотек i18n для Solid](https://github.com/aymericzip/intlayer/blob/main/docs/assets/cloud_i18n_logo.png?raw=true)
...
![Теоретичний витік контенту залежно від архітектури](https://github.com/aymericzip/intlayer/blob/main/docs/assets/theorical_content_leakage.png?raw=true)
...
| Бібліотека               | Модель контенту                               | Реактивність при зміні локалі                             | Типи на ключах                          | Scoping та lazy loading       | Розмір бібліотеки |
| :----------------------- | :-------------------------------------------- | :-------------------------------------------------------- | :-------------------------------------- | :---------------------------- | :---------------- |
| `@solid-primitives/i18n` | Плоский власний словник                       | Signal, accessors від translator                          | Виводяться з вихідного словника         | Немає вбудованого             | Дуже малий        |
| `solid-i18next`          | Каталоги та namespaces i18next                | Store, повторний рендеринг через provider                 | Ручне оголошення                        | Namespaces, lazy backends     | ~14.9 кБ          |
| Paraglide                | Проєкт inlang, згенеровані функції            | Читання при кожному виклику з cookie/storage              | Згенеровані                             | Tree-shaking (не в бенчмарку) | Майже нульовий    |
| `@lingui/solid`          | Вихідний текст у коді, скомпільовані каталоги | На основі signal                                          | Від компілятора                         | Для кожного каталогу          | Малий             |
| Intlayer                 | Один `.content.ts` на компонент               | Вузли на основі signal, без повторного запуску компонента | Згенеровані, увімкнені за замовчуванням | Так, на компонент             | Базовий           |

> Числа відображають стан на момент версій бенчмарку. `@lingui/solid` не брав участі в бенчмарку. Перевірте показники на власному застосунку, перш ніж приймати рішення лише на основі розміру.
...
Час від налаштування до першого перекладеного рядка, наявність [LSP](https://github.com/aymericzip/intlayer/blob/main/docs/docs/uk/lsp.md) або [розширення для VS Code](https://github.com/aymericzip/intlayer/blob/main/docs/docs/uk/vs_code_extension.md), яке показує переклад при наведенні та переходить до оголошення, [CLI](https://github.com/aymericzip/intlayer/blob/main/docs/docs/uk/cli/index.md) для заповнення, тестування і відправки, а також можливість редагування контенту без участі розробників ([візуальний редактор](https://github.com/aymericzip/intlayer/blob/main/docs/docs/uk/intlayer_visual_editor.md) або [CMS](https://github.com/aymericzip/intlayer/blob/main/docs/docs/uk/intlayer_CMS.md)) без відкриття pull request.
