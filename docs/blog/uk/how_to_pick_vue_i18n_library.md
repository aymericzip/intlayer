...
| Бібліотека     | Модель контенту                                               | Типи на ключах                         | Формат повідомлень           | Розділення за маршрутами | Розмір бібліотеки |
| :------------- | :------------------------------------------------------------ | :------------------------------------- | :--------------------------- | :----------------------- | :---------------- |
| `vue-i18n`     | Центральні каталоги на локаль, опціональні блоки SFC `<i18n>` | Опціонально через generic схеми        | Власний (множина через pipe) | Ні                       | ~24.3 kB          |
| `@nuxtjs/i18n` | Як у `vue-i18n`, плюс маршрутизація та SEO-теги               | Так само                               | Так само                     | Ні, лише за локалями     | Додатково         |
| `fluent-vue`   | Файли `.ftl` (Mozilla Fluent)                                 | Відсутні                               | Fluent                       | Ні                       | ~29.7 kB          |
| Paraglide      | Проєкт inlang, згенеровані функції                            | Згенеровані                            | Власний                      | Через tree-shaking       | Майже нуль        |
| Intlayer       | Один `.content.ts` на компонент                               | Згенеровані, включені за замовчуванням | Хелпери (`plural`)           | Так, на компонент        | Базовий           |
