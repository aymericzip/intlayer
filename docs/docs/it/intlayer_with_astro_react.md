...
In questa architettura, l'integrazione `intlayer()` registrata in `astro.config.ts` compila i tuoi dizionari e aggiunge un middleware che risolve la locale di ogni richiesta esponendola su `Astro.locals.intlayer`. Le pagine si trovano sotto un segmento rest `src/pages/[...locale]/`, in modo che la locale predefinita venga servita senza prefisso e ogni altra locale ottenga il proprio URL dedicato. I file `.astro` leggono il contenuto con gli hook `useIntlayer` / `useLocale` di `astro-intlayer` per il rendering server, e l'isola React (`src/components/react/ReactIsland.tsx`) riceve la locale rilevata dal server come prop e la passa a `react-intlayer` per la parte interattiva. Le dichiarazioni di contenuto sono posizionate accanto ai tuoi componenti in `src/`.
...
<Step number={6} title="Crea un componente React Island">
...
<Question title="Quali sono le diverse soluzioni disponibili per internazionalizzare un sito Astro con island React?">

L'opzione `i18n` integrata di Astro gestisce i prefissi di locale e i reindirizzamenti ma non il contenuto stesso, quindi ti serve comunque un livello di messaggi, e un'island aggiunge un secondo problema: l'island esegue React, non Astro.

- **`i18n` di Astro più dizionari scritti a mano**, con **`react-i18next`** all'interno dell'island: due fonti di contenuto da tenere sincronizzate, e nessuna tipizzazione condivisa tra loro.
- **`Intlayer`**: un unico livello di contenuto per entrambi. `astro-intlayer` copre le pagine `.astro` e `react-intlayer` copre le island React, leggendo le stesse dichiarazioni.
...
<Question title="Posso migrare da `react-i18next` senza riscrivere i miei componenti?">
...
<Question title="Ho bisogno di una libreria i18n separata all'interno della mia island React?">

No. `react-intlayer` legge gli stessi dizionari del lato Astro, quindi non installi `react-i18next` accanto ad esso. Il passo 6 mostra il componente island che riceve la locale attiva dalla pagina invece di risolverla di nuovo.
