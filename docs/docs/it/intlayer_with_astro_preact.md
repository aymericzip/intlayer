...
In questa architettura, l'integrazione `intlayer()` registrata in `astro.config.ts` compila i tuoi dizionari e aggiunge un middleware che risolve la locale di ogni richiesta esponendola su `Astro.locals.intlayer`. Le pagine si trovano sotto un segmento rest `src/pages/[...locale]/`, in modo che la locale predefinita venga servita senza prefisso e ogni altra locale ottenga il proprio URL dedicato. I file `.astro` leggono il contenuto con gli hook `useIntlayer` / `useLocale` di `astro-intlayer` per il rendering server, e l'isola Preact (`src/components/preact/PreactIsland.tsx`) riceve la locale rilevata dal server come prop e la passa a `preact-intlayer` per la parte interattiva. Le dichiarazioni di contenuto sono posizionate accanto ai tuoi componenti in `src/`.
...
Intlayer utilizza l'augmentation dei moduli per sfruttare i vantaggi di TypeScript e rafforzare il tuo codebase.
...
<Question title="Quali sono le diverse soluzioni disponibili per internazionalizzare un sito Astro con island Preact?">

L'opzione `i18n` integrata di Astro gestisce i prefissi di locale e i reindirizzamenti ma non il contenuto stesso, quindi ti serve comunque un livello di messaggi, e un'island aggiunge un secondo problema: l'island esegue Preact, non Astro.

- **`i18n` di Astro più dizionari scritti a mano**, con **`preact-i18n`** all'interno dell'island: due fonti di contenuto da tenere sincronizzate, e nessuna tipizzazione condivisa tra loro.
- **`Intlayer`**: un unico livello di contenuto per entrambi. `astro-intlayer` copre le pagine `.astro` e `preact-intlayer` copre le island Preact, leggendo le stesse dichiarazioni.
...
<Question title="Ho bisogno di una libreria i18n separata all'interno della mia island Preact?">

No. `preact-intlayer` legge gli stessi dizionari del lato Astro, quindi non installi `preact-i18n` accanto ad esso. Il passo 6 mostra il componente island che riceve la locale attiva dalla pagina invece di risolverla di nuovo.
