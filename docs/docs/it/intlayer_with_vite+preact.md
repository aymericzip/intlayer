...
In parallelo, puoi anche utilizzare `intlayerProxy` per aggiungere il routing lato server alla tua applicazione. Questo plugin rileverà automaticamente la locale corrente in base all'URL e imposterà il cookie locale appropriato. Se non è specificata alcuna locale, il plugin determinerà la locale più appropriata in base alle preferenze della lingua del browser dell'utente. Se non viene rilevata alcuna locale, reindizzerà alla locale predefinita.
...
> A partire da Intlayer v9, `intlayerProxy()` è integrato direttamente nel plugin `intlayer()` e abilitato per impostazione predefinita tramite l'opzione `routing.enableProxy` (`true` per impostazione predefinita). Registrarlo separatamente come mostrato di seguito è ora opzionale — è mantenuto per compatibilità con le versioni precedenti e per setup che necessitano di controllare l'ordine dei plugin. Impostare `routing.enableProxy: false` per disattivarlo. Vedi le [note di rilascio v9](https://github.com/aymericzip/intlayer/blob/main/docs/docs/it/releases/v9.md).
...
<Step number={8} title="Cambia l'URL quando cambia la lingua" isOptional={true}>
...
<Step number={9} title="Cambia gli attributi HTML di lingua e direzione" isOptional={true}>
...
Applicando questi cambiamenti, la tua applicazione:

- Assicurati che l'attributo **language** (`lang`) rifletta correttamente la locale corrente, il che è importante per SEO e comportamento del browser.
- Regola la **direzione del testo** (`dir`) in base alla locale, migliorando la leggibilità e l'usabilità per le lingue con ordini di lettura diversi.
- Fornisci un'esperienza più **accessibile**, poiché le tecnologie assistive dipendono da questi attributi per funzionare in modo ottimale.

</Step>
<Step number={10} title="Creazione di un componente di collegamento localizzato" isOptional={true}>
...
<Step number={11} title="Rendering di Markdown e HTML" isOptional={true}>
...
Intlayer utilizza module augmentation per sfruttare i vantaggi di TypeScript e rendere il tuo codebase più robusto.
...
> Assicurati che il tuo `tsconfig.json` sia configurato per Preact, in particolare `jsx` e `jsxImportSource` o `jsxFactory`/`jsxFragmentFactory` per le versioni precedenti di Preact se non utilizzi i default di `preset-vite`.

### Configurazione Git

Si consiglia di ignorare i file generati da Intlayer. Questo ti permette di evitare di farli scaricare nel tuo repository Git.
...
Per ulteriori dettagli su come utilizzare l'estensione, consulta la [documentazione dell'Estensione Intlayer per VS Code](https://github.com/aymericzip/intlayer/blob/main/docs/docs/it/vs_code_extension.md).
...
<Question title="Quali sono le diverse soluzioni disponibili per internazionalizzare un'app Vite e Preact?">

Vite non ha un'opinione sull'i18n, quindi la scelta viene dall'ecosistema Preact:

- **`preact-i18n`**: una piccola libreria specifica per Preact con un dizionario JSON.
- **`react-i18next`** attraverso `preact/compat`: matura, ma trascina il livello di compatibilità React nel tuo bundle.
...
<Question title="Quanto aggiunge l'i18n alla dimensione del mio bundle Preact?">
...
<Question title="Posso migrare da `preact-i18n` o `react-i18next` senza riscrivere i miei componenti?">

Sì, e ci sono due percorsi. Puoi migrare il contenuto progressivamente con la [guida alla migrazione da react-i18next](https://github.com/aymericzip/intlayer/blob/main/docs/docs/it/migration_from_react-i18next_to_intlayer.md). Oppure puoi mantenere interamente la tua API attuale: gli [adattatori di compatibilità](https://github.com/aymericzip/intlayer/blob/main/docs/docs/it/compat/index.md) espongono esattamente la stessa API di `react-i18next` e `react-intl`, ma servita dai dizionari Intlayer, quindi cambiano gli import e il codice dei componenti no.
...
<Question title="Come uso il contenuto tradotto in un componente Preact?">

Chiama `useIntlayer` nel tuo componente, esattamente come faresti in React. `preact-intlayer` è un binding Preact nativo, quindi non passi attraverso `preact/compat` per esso. Il passo 5 mostra l'uso e il passo 11 copre il contenuto Markdown e HTML.
...
I passi 7 e 8 coprono le rotte localizzate e la riscrittura dell'URL quando la locale cambia, e il passo 10 aggiunge un componente di link localizzato. `routing.mode` decide lo schema dell'URL: `"prefix-no-default"` (il predefinito, `/about` e `/fr/about`), `"prefix-all"`, `"no-prefix"` (risolto da un cookie, un header o un dominio) o `"search-params"` (`/about?locale=fr`). Vedi il [riferimento di configurazione](https://github.com/aymericzip/intlayer/blob/main/docs/docs/it/configuration.md).
