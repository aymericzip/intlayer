...
In questa architettura, `vanilla-intlayer` o il core `intlayer` fornisce API JavaScript per gestire le traduzioni e aggiornare dinamicamente i contenuti del DOM. Le dichiarazioni di contenuto risiedono in `src/`.
...
> A partire da Intlayer v9, `intlayerProxy()` è integrato direttamente nel plugin `intlayer()` ed è abilitato per impostazione predefinita tramite l'opzione `routing.enableProxy` (`true` per impostazione predefinita). Registrarlo separatamente come mostrato di seguito è ora opzionale — è mantenuto per la compatibilità con le versioni precedenti e per le configurazioni che hanno bisogno di controllare l'ordine dei plugin. Impostare `routing.enableProxy: false` per disabilitarlo. Vedi le [note sulla versione v9](https://github.com/aymericzip/intlayer/blob/main/docs/docs/it/releases/v9.md).
...
<Question title="Quali sono le diverse soluzioni disponibili per internazionalizzare un'app Vite e Vanilla JS?">

Vite non ha un'opinione sull'i18n, quindi la scelta viene dall'ecosistema Vanilla JS:

- **Un oggetto dizionario scritto a mano** importato nel tuo entry point: nessuna dipendenza, ma nessuna tipizzazione, nessuna regola di plurale, e nulla che ti dica che manca una traduzione.
- **`i18next`**: maturo e indipendente dal framework, ma aggiunge un runtime e carica i cataloghi come JSON.
...
No. Esegui `npx intlayer extract` e Intlayer legge i tuoi componenti, estrae le stringhe visibili all'utente e scrive un file `.content` accanto a ciascuno, così puoi rivedere un diff invece di copiare le stringhe in un catalogo una alla volta. Il passo 13 di questa guida lo illustra.
...
<Question title="Come uso il contenuto tradotto in un componente Vanilla JS?">

Inizializza Intlayer nel tuo entry point come mostra il passo 4, poi leggi il contenuto con `useIntlayer` e scrivilo nel DOM. Non c'è alcun framework e nessun virtual DOM coinvolto, quindi decidi tu quando aggiornare i nodi. Il passo 8 copre il rendering del contenuto Markdown e HTML.
...
Lo copre il passo 11. `getHTMLTextDir` ritorna `ltr`, `rtl` o `auto` per una locale, così colleghi `lang` e `dir` sull'elemento radice a partire dalla locale attiva e lasci che le proprietà logiche del tuo CSS gestiscano il resto.
...
<Question title="Posso caricare solo la lingua di cui il visitatore ha bisogno?">

Sì. Il passo 12 copre il caricamento lazy dei dizionari per locale, così il payload iniziale porta una lingua e le altre sono recuperate solo se il visitatore cambia. Vedi [dizionari dinamici](https://github.com/aymericzip/intlayer/blob/main/docs/docs/it/dynamic_dictionaries/index.md).
