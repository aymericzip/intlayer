...
Vedi [Application Template](https://github.com/aymericzip/intlayer-sveltekit-template) su GitHub.
...
> `getLocaleFromStorage` controllerà la locale dall'header o dal cookie a seconda della tua configurazione. Vedi [Configurazione](https://intlayer.org/doc/concept/configuration) per maggiori dettagli.
...
<Question title="Quali sono le diverse soluzioni disponibili per internazionalizzare un'app SvelteKit?">

- **`svelte-i18n`** e **`typesafe-i18n`**: cataloghi di messaggi basati su store, collegati alle funzioni load a mano.
- **`Paraglide`**: messaggi compilati con una forte storia di tipizzazione, focalizzato solo sul livello di messaggi.
- **`Intlayer`**: contenuto dichiarato accanto a ogni componente e compilato in fase di build, con routing consapevole delle locale, rilevamento della locale lato server, traduzione AI, un editor visivo e un CMS.
