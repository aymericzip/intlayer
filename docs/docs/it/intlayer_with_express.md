...
<Question title="Quali sono le diverse soluzioni disponibili per internazionalizzare un backend Express?">
...
<Question title="Quanto aggiunge l'i18n alla dimensione del bundle del mio server Express?">
...
<Question title="Funziona con un'app Express esistente e altri middleware?">

Sì. `express-intlayer` è un middleware Express standard, quindi si compone con il tuo stack esistente. Registralo prima delle rotte che leggono contenuto, così la locale è risolta quando un handler chiama `t()` o `getIntlayer()`.
