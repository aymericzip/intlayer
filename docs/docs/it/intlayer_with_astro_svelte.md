...
Dovresti anche aggiungere metadati SEO (come hreflang e link canonici) a ogni pagina e introdurre una Svelte island per i contenuti interattivi lato client.
...
<Question title="Quali sono le diverse soluzioni disponibili per internazionalizzare un sito Astro con island Svelte?">

L'opzione `i18n` integrata di Astro gestisce i prefissi di locale e i reindirizzamenti ma non il contenuto stesso, quindi ti serve comunque un livello di messaggi, e un'island aggiunge un secondo problema: l'island esegue Svelte, non Astro.

- **`i18n` di Astro più dizionari scritti a mano**, con **`svelte-i18n`** all'interno dell'island: due fonti di contenuto da tenere sincronizzate, e nessuna tipizzazione condivisa tra loro.
- **`Intlayer`**: un unico livello di contenuto per entrambi. `astro-intlayer` copre le pagine `.astro` e `svelte-intlayer` copre le island Svelte, leggendo le stesse dichiarazioni.
...
<Question title="Posso migrare da `svelte-i18n` senza riscrivere i miei componenti?">
...
<Question title="Ho bisogno di una libreria i18n separata all'interno della mia island Svelte?">

No. `svelte-intlayer` legge gli stessi dizionari del lato Astro, quindi non installi `svelte-i18n` accanto ad esso. Il passo 6 mostra il componente island che riceve la locale attiva dalla pagina invece di risolverla di nuovo.
