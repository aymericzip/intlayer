...
In questa architettura, l'integrazione `intlayer()` registrata in `astro.config.ts` compila i tuoi dizionari e aggiunge un middleware che risolve la locale di ogni richiesta esponendola su `Astro.locals.intlayer`. Le pagine si trovano sotto un segmento rest `src/pages/[...locale]/`, in modo che la locale predefinita venga servita senza prefisso e ogni altra locale ottenga il proprio URL dedicato. I file `.astro` leggono il contenuto con gli hook `useIntlayer` / `useLocale` di `astro-intlayer` per il rendering server, e l'isola Vue (`src/components/vue/VueIsland.vue`) riceve la locale rilevata dal server come prop e la passa a `vue-intlayer` per la parte interattiva. Le dichiarazioni di contenuto sono posizionate accanto ai tuoi componenti in `src/`.
...
> Le dichiarazioni di contenuto possono be definite in qualsiasi punto della tua applicazione, purché siano incluse nel `contentDir` (per impostazione predefinita `./src`) e corrispondano all'estensione dei file di dichiarazione del contenuto (per impostazione predefinita `.content.{json,ts,tsx,js,jsx,mjs,cjs,md,mdx,yaml,yml}`).
...
<Step number={6} title="Crea un componente Vue Island">
...
<Question title="Quali sono le diverse soluzioni disponibili per internazionalizzare un sito Astro con island Vue?">

L'opzione `i18n` integrata di Astro gestisce i prefissi di locale e i reindirizzamenti ma non il contenuto stesso, quindi ti serve comunque un livello di messaggi, e un'island aggiunge un secondo problema: l'island esegue Vue, non Astro.

- **`i18n` di Astro più dizionari scritti a mano**, con **`vue-i18n`** all'interno dell'island: due fonti di contenuto da tenere sincronizzate, e nessuna tipizzazione condivisa tra loro.
- **`Intlayer`**: un unico livello di contenuto per entrambi. `astro-intlayer` copre le pagine `.astro` e `vue-intlayer` copre le island Vue, leggendo le stesse dichiarazioni.
...
<Question title="Ho bisogno di una libreria i18n separata all'interno della mia island Vue?">

No. `vue-intlayer` legge gli stessi dizionari del lato Astro, quindi non installi `vue-i18n` accanto ad esso. Il passo 6 mostra il componente island che riceve la locale attiva dalla pagina invece di risolverla di nuovo.
