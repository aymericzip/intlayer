...
<AccordionGroup>
<Accordion header="Copertura completamente illuminata">
...
Vedi [Application Template](https://github.com/aymericzip/intlayer-vite-lit-template) su GitHub.
...
In questa architettura, `lit-intlayer` fornisce integrazioni di elementi personalizzati e controller reattivi per gestire le traduzioni all'interno dei Web Components. Le dichiarazioni di contenuto risiedono in `src/` accanto agli elementi.
...
<Step number={7} title="Cambiare la lingua del contenuto" isOptional={true}>
...
<Step number={8} title="Renderizzare contenuti Markdown e HTML" isOptional={true}>
...
<Step number={9} title="Aggiungere il routing localizzato alla tua applicazione" isOptional={true}>
...
<Step number={10} title="Cambiare l'URL quando la lingua cambia" isOptional={true}>
...
<Step number={11} title="Commutare gli attributi di lingua e direzione HTML" isOptional={true}>
...
<Step number={12} title="Estrarre il contenuto dei tuoi componenti" isOptional={true}>
...
<Question title="Quali sono le diverse soluzioni disponibili per internazionalizzare un'app Vite e Lit?">

Vite non ha un'opinione sull'i18n, quindi la scelta viene dall'ecosistema Lit:

- **`@lit/localize`**: l'opzione ufficiale, basata sull'estrazione XLIFF e un bundle compilato per locale, quindi cambiare lingua a runtime richiede la modalità runtime.
- **Un dizionario scritto a mano** importato in ogni elemento: nessuna dipendenza, nessuna tipizzazione, nessuno strumento.
...
<Question title="Come uso il contenuto tradotto in un componente Lit?">

Leggi il contenuto all'interno del metodo `render` del tuo `LitElement` attraverso il binding Intlayer, come mostra il passo 6. Cambiare la locale attiva un aggiornamento degli elementi che lo leggono, così il cambio di lingua non richiede di ricaricare la pagina o inviare un secondo bundle.
