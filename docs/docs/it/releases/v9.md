...
## Plugin Vite: Compilatore e Proxy Integrati

Il plugin Vite `intlayer()` ora integra direttamente il **compilatore** e il **proxy di routing per locale**, per cui la maggior parte dei progetti ha bisogno di un solo plugin in `vite.config.ts`:
...
- **Compilatore**: Si attiva automaticamente quando `compiler.enabled` viene impostato e c'è un percorso in `compiler.output`. Non è più necessario registrare `intlayerCompiler()` separatamente.
- **Proxy**: Si attiva automaticamente in base alla nuova opzione `routing.enableProxy` (`true` per default). Collega il middleware di rilevamento / redirect / rewrite per lo sviluppo, preview e in SSR in produzione. Non è più necessario registrare `intlayerProxy()` separatamente.

### L'opzione `routing.enableProxy`

Una nuova opzione `routing.enableProxy` controlla se il proxy di routing per locale è inserito. Il valore di default è `true`. Disattivalo quando desideri gestire il routing locale da solo:
...
I plugin indipendenti `intlayerCompiler()` e `intlayerProxy()` restano esportati per le configurazioni avanzate. Registrarli insieme a `intlayer()` è sicuro — ogni plugin si deduplica da solo ed entra in esecuzione una sola volta.
