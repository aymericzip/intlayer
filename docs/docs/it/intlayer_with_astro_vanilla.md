...
title: "Astro + Vanilla JS i18n - Guida completa per tradurre la tua applicazione"
description: "Niente più i18next. La guida 2026 per creare un'applicazione Astro + Vanilla JS multilingue (i18n). Traduci con agenti AI e ottimizza la dimensione del bundle, SEO e prestazioni."
keywords:
...
<Step number={6} title="Aggiungere il selettore di lingua">
...
<Question title="Quali sono le diverse soluzioni disponibili per internazionalizzare un sito Astro senza un framework UI?">

L'opzione `i18n` integrata di Astro gestisce i prefissi di locale e i reindirizzamenti, ma lascia il contenuto a te. Da lì:

- **Dizionari scritti a mano**, semplici oggetti JSON o TypeScript importati per pagina: nessuna dipendenza, ma nessuna tipizzazione, nessuna regola di plurale e nessuno strumento per trovare le traduzioni mancanti.
- **`Intlayer`**: contenuto dichiarato accanto alla pagina o al componente che lo renderizza, compilato in fase di build e tipizzato, con traduzione AI, controlli delle traduzioni mancanti in CI, un editor visivo e un CMS.

Senza un framework UI il costo a runtime di una libreria i18n è ciò che vuoi evitare, e Intlayer risolve il contenuto in fase di build, quindi una pagina Astro statica invia HTML tradotto e nessun dizionario. Vedi [perché Intlayer](https://github.com/aymericzip/intlayer/blob/main/docs/docs/it/interest_of_intlayer.md).
...
<Question title="Ho bisogno di JavaScript nella pagina perché le traduzioni funzionino?">

No. Astro renderizza le pagine in fase di build e Intlayer risolve il contenuto durante quel render, quindi l'HTML generato contiene già il testo tradotto. `vanilla-intlayer` serve solo per le parti che rendi interattive nel browser.

</Question>

<Question title="Posso aggiungere un'island interattiva più tardi senza cambiare il mio contenuto?">

Sì, ed è questo il punto di un unico livello di contenuto. Le stesse dichiarazioni sono lette da `astro-intlayer` nella pagina e dal binding del framework di qualunque island tu aggiunga, che sia [React](https://github.com/aymericzip/intlayer/blob/main/docs/docs/it/intlayer_with_astro_react.md), [Vue](https://github.com/aymericzip/intlayer/blob/main/docs/docs/it/intlayer_with_astro_vue.md), [Svelte](https://github.com/aymericzip/intlayer/blob/main/docs/docs/it/intlayer_with_astro_svelte.md), [Solid](https://github.com/aymericzip/intlayer/blob/main/docs/docs/it/intlayer_with_astro_solid.md), [Preact](https://github.com/aymericzip/intlayer/blob/main/docs/docs/it/intlayer_with_astro_preact.md) o [Lit](https://github.com/aymericzip/intlayer/blob/main/docs/docs/it/intlayer_with_astro_lit.md).

</Question>

<Question title="Come configuro il routing localizzato e un selettore di locale?">

Il passo 6 copre il selettore. `routing.mode` decide lo schema dell'URL: `"prefix-no-default"` (il predefinito), `"prefix-all"`, `"no-prefix"` o `"search-params"`, e `routing.domains` associa una locale al proprio dominio. `getLocalizedUrl` riscrive il percorso corrente così cambiare lingua mantiene il lettore sulla stessa pagina. Vedi il [riferimento di configurazione](https://github.com/aymericzip/intlayer/blob/main/docs/docs/it/configuration.md).

</Question>

<Question title="Come genero una sitemap localizzata e i tag hreflang?">

Il passo 7 copre `sitemap.xml` e `robots.txt`. `getMultilingualUrls` costruisce gli alternate per ogni locale dichiarata, incluso `x-default`, che è ciò che i motori di ricerca usano per servire la versione linguistica giusta di una pagina.
