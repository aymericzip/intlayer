...
<Question title="Quali sono le diverse soluzioni disponibili per internazionalizzare un'app Vite e Solid?">

Vite non ha un'opinione sull'i18n, quindi la scelta viene dall'ecosistema Solid:

- **`@solid-primitives/i18n`**: la primitiva della community, un dizionario piatto che assembli e carichi tu stesso.
- **`i18next`** con un wrapper Solid: cataloghi maturi, ma nessuna storia di reattività propria.
...
<Question title="Quanto aggiunge l'i18n alla dimensione del mio bundle Solid?">
...
<Question title="Come uso il contenuto tradotto in un componente Solid?">

Chiama `useIntlayer` nel tuo componente e accedi direttamente ai valori. Il contenuto è supportato da un signal di Solid, quindi cambiare la locale aggiorna solo le parti del DOM che lo leggono, senza un ri-render del componente. Il passo 5 mostra l'uso.
