...
> La funzione `withIntlayer()` è una funzione promise. Permette di preparare i dizionari intlayer prima dell'inizio della build. Se vuoi utilizzarla con altri plugin, puoi attendere. Esempio:
>
> ```ts
...
> ```
>
> Se vuoi utilizzarla in modo sincrono, puoi usare la funzione `withIntlayerSync()`. Esempio:
>
> ```ts
...
<Step number={4} title="Definire le rotte locali dinamiche">
...
<Step number={6} title="Utilizzare il contenuto nel codice">
...
- **`IntlayerProvider`** viene montato una sola volta, nel layout principale. Fornisce la locale sia ai componenti server che client, quindi le pagine non hanno più bisogno di avvolgersi da sole.
- Senza un segmento di percorso `[locale]` la locale proviene sempre dalla richiesta — l'header `x-intlayer-locale` impostato dal proxy Intlayer, poi il cookie della locale — che gli hook del server leggono autonomamente quando il provider non è stato eseguito.
...
> `next-intlayer` è il percorso di import isomorfo: la condizione di export `react-server` fornisce ai componenti server l'implementazione ambient-locale, mentre i componenti client ottengono quella basata sul context. La stessa chiamata funziona su entrambi i lati.
...
<Step number={7} title="Configurare il proxy per il rilevamento della locale" isOptional={true}>
...
<Step number={8} title="Cambiare la lingua del tuo contenuto" isOptional={true}>
...
<Step number={9} title="Ottenere la locale corrente nelle Server Actions" isOptional={true}>
...
<Step number={10} title="Ottimizza la dimensione del bundle" isOptional={true}>
...
- **`next-intl`**, **`next-i18next` / `i18next`** e **`react-intl`**: cataloghi JSON o ICU caricati per namespace.
- **`Lingui`**: guidata dall'estrazione, con messaggi ICU compilati in fase di build.
- **`Intlayer`**: contenuto dichiarato accanto a ogni componente e compilato per componente, completamente tipizzato, con traduzione AI, un editor visivo e un CMS.

Questa guida copre la configurazione senza una locale nel percorso. Vedi [perché Intlayer](https://github.com/aymericzip/intlayer/blob/main/docs/docs/it/interest_of_intlayer.md) e il [benchmark i18n di Next.js](https://github.com/aymericzip/intlayer/blob/main/docs/docs/it/benchmark/nextjs.md).
...
<Question title="Perché dovrei servire la mia app senza una locale nell'URL?">

Perché la locale non fa sempre parte dell'identità della pagina. Una dashboard autenticata, uno strumento interno o un'app dietro un login non hanno motivo di esporre `/fr/` in ogni URL: la lingua è una preferenza dell'utente, non un documento diverso. Eliminare il prefisso mantiene anche le tue rotte, i tuoi link e i tuoi analytics su un unico insieme di percorsi.

</Question>

<Question title="Quali sono le conseguenze SEO di non avere una locale nell'URL?">

Sono reali, quindi scegli deliberatamente. Senza un URL distinto per lingua, i motori di ricerca non hanno una pagina separata da indicizzare per ogni locale, `hreflang` non ha nulla a cui puntare, e un crawler vede solo la lingua che il tuo rilevamento predefinito gli serve. Questo va bene per il contenuto dietro un login, che non è comunque indicizzato, ed è poco adatto a un sito di marketing pubblico o alla documentazione. Se il traffico organico per lingua conta, usa invece la configurazione con prefisso della [guida Next.js 16](https://github.com/aymericzip/intlayer/blob/main/docs/docs/it/intlayer_with_nextjs_16.md).

</Question>

<Question title="Qual è la differenza tra le modalità `no-prefix` e `search-params`?">

`"no-prefix"` tiene la locale completamente fuori dall'URL e la risolve da un cookie, un header o un dominio, così ogni lingua condivide un unico indirizzo. `"search-params"` la mette nella query string come `/dashboard?locale=fr`, il che dà comunque a ogni lingua un URL distinto, collegabile e salvabile nei preferiti senza cambiare il tuo albero delle rotte. Preferisci `"search-params"` quando vuoi link condivisibili per lingua.

</Question>

<Question title="Come viene rilevata la locale allora?">

Dalle fonti elencate in `routing.storage`, per impostazione predefinita prima un cookie e poi l'header `Accept-Language`, ricadendo sulla tua locale predefinita. Il passo 7 aggiunge il proxy che la applica. Una lingua scelta esplicitamente dall'utente viene persistita, quindi sopravvive alla visita successiva. Vedi il [riferimento di configurazione](https://github.com/aymericzip/intlayer/blob/main/docs/docs/it/configuration.md).

</Question>

<Question title="Posso invece associare ogni lingua al proprio dominio?">

Sì, ed è l'opzione da considerare quando hai eliminato il prefisso per ragioni estetiche ma vuoi comunque la SEO. `routing.domains` associa una locale a un hostname, così il dominio identifica la lingua, nessun prefisso viene aggiunto al percorso, e ogni lingua ottiene un URL indicizzabile a cui `hreflang` può puntare.
