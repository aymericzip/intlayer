...
priority: 9
title: "Astro + Solid i18n - Guida completa per tradurre la tua applicazione"
description: "Niente più i18next. La guida 2026 per creare un'applicazione Astro + Solid multilingue (i18n). Traduci con agenti AI e ottimizza la dimensione del bundle, SEO e prestazioni."
keywords:
...
history:
  ...
  - version: 8.7.7
    date: 2026-04-24
    changes: "Documentazione iniziale per Astro + Solid"
author: aymericzip
---

# Tradurre il tuo sito Astro + Solid con Intlayer | Internazionalizzazione (i18n)
...
## Guida passo dopo passo per configurare Intlayer in Astro + Solid
...
- **solid-js**
  Il pacchetto core di Solid.

- **solid-intlayer**
  Pacchetto per integrare Intlayer in applicazioni Solid. Fornisce l'`IntlayerProvider` oltre alle primitive `useIntlayer` e `useLocale` per l'internazionalizzazione in Solid.

- **@astrojs/solid-js**
  Integrazione ufficiale di Astro che consente l'uso di islands di componenti Solid.
...
In questa architettura, l'integrazione `intlayer()` registrata in `astro.config.ts` compila i tuoi dizionari e aggiunge un middleware che risolve la locale di ogni richiesta esponendola su `Astro.locals.intlayer`. Le pagine si trovano sotto un segmento rest `src/pages/[...locale]/`, in modo che la locale predefinita venga servita senza prefisso e ogni altra locale ottenga il proprio URL dedicato. I file `.astro` leggono il contenuto con gli hook `useIntlayer` / `useLocale` di `astro-intlayer` per il rendering server, e l'isola Solid (`src/components/solid/SolidIsland.tsx`) riceve la locale rilevata dal server come prop e la passa a `solid-intlayer` per la parte interattiva. Le dichiarazioni di contenuto sono posizionate accanto ai tuoi componenti in `src/`.
...
│   │       └── SolidIsland.tsx       # Solid island wrapping the tree in IntlayerProvider
│   └── pages
│       ├── [...locale]
│       │   └── index.astro           # Localized page mounting the island
│       ├── robots.txt.ts             # robots.txt endpoint
│       └── sitemap.xml.ts            # Localized sitemap endpoint
├── astro.config.ts                   # Astro config with the intlayer() and solid() integrations
├── intlayer.config.ts
├── package.json
└── tsconfig.json
```
...
Aggiungi il plugin `intlayer` e l'integrazione Solid alla tua configurazione Astro.
...
> L'integrazione `solid()` consente di utilizzare islands di componenti Solid tramite `client:only="solid-js"`.
...
Dovresti anche aggiungere metadati SEO (come hreflang e link canonici) a ogni pagina e introdurre una Solid island per i contenuti interattivi lato client.
...
<html lang={locale} dir={getHTMLTextDir(locale)}>
  ...
  <body>
    <!-- La Solid island renderizza tutti i contenuti interattivi, incluso il selettore di lingua -->
...
<Step number={6} title="Crea un componente Solid Island">

Crea un componente island che racchiuda la tua applicazione Solid e riceva la lingua rilevata dal server:
...
> La prop `locale` viene passata dalla pagina Astro (rilevamento lato server) all'`IntlayerProvider`, rendendola la lingua iniziale per tutte le primitive Solid all'interno dell'albero.

> In Solid, `useIntlayer` restituisce una funzione **accessor** (es. `content.). Devi invocarla per accedere al contenuto reattivo.

</Step>
<Step number={7} title="Aggiungi un selettore di lingua">

Crea un componente Solid `LocaleSwitcher` che legga le lingue disponibili e navighi verso l'URL localizzato quando un utente seleziona una nuova lingua:
...
> **Nota sulla reattività di Solid:**
> In Solid, `locale` è un accessor del segnale reattivo - invoca sempre come `locale()` per ottenere il valore corrente.
...
<Question title="Quali sono le diverse soluzioni disponibili per internazionalizzare un sito Astro con island Solid?">

L'opzione `i18n` integrata di Astro gestisce i prefissi di locale e i reindirizzamenti ma non il contenuto stesso, quindi ti serve comunque un livello di messaggi, e un'island aggiunge un secondo problema: l'island esegue Solid, non Astro.

- **`i18n` di Astro più dizionari scritti a mano**, con **`@solid-primitives/i18n`** all'interno dell'island: due fonti di contenuto da tenere sincronizzate, e nessuna tipizzazione condivisa tra loro.
- **`Intlayer`**: un unico livello di contenuto per entrambi. `astro-intlayer` copre le pagine `.astro` e `solid-intlayer` copre le island Solid, leggendo le stesse dichiarazioni.

Dichiarare un'etichetta una volta e usarla sia in una pagina statica sia in un'island interattiva è il motivo per scegliere un unico livello di contenuto qui. Vedi [perché Intlayer](https://github.com/aymericzip/intlayer/blob/main/docs/docs/it/interest_of_intlayer.md).
...
<Question title="Ho bisogno di una libreria i18n separata all'interno della mia island Solid?">

No. `solid-intlayer` legge gli stessi dizionari del lato Astro, quindi non installi `@solid-primitives/i18n` accanto ad esso. Il passo 6 mostra il componente island che riceve la locale attiva dalla pagina invece di risolverla di nuovo.

</Question>
<Question title="Come fa l'island a sapere quale locale sta renderizzando la pagina?">

La pagina Astro la passa in basso come prop e il provider Intlayer nell'island la raccoglie, così l'island si idrata nella stessa lingua che il server ha renderizzato. È questo che previene il lampo di lingua predefinita che ottieni quando un'island rileva la locale da sola nel browser.

</Question>
<Question title="Il contenuto tradotto viene inviato come HTML statico?">

Sì per le parti Astro, che sono renderizzate in fase di build, quindi le pagine localizzate sono semplice HTML statico che i crawler leggono senza eseguire JavaScript. Solo le island ricevono un dizionario, e solo per la locale che renderizzano.

</Question>
<Question title="Come configuro il routing localizzato e un selettore di locale?">

Il passo 7 copre il selettore. `routing.mode` decide lo schema dell'URL: `"prefix-no-default"` (il predefinito, `/about` e `/fr/about`), `"prefix-all"`, `"no-prefix"` o `"search-params"`, e `routing.domains` associa una locale al proprio dominio. `getLocalizedUrl` riscrive il percorso corrente così cambiare lingua mantiene il lettore sulla stessa pagina. Vedi il [riferimento di configurazione](https://github.com/aymericzip/intlayer/blob/main/docs/docs/it/configuration.md).

</Question>
<Question title="Come genero una sitemap localizzata e i tag hreflang?">

Il passo 8 copre `sitemap.xml` e `robots.txt`. `getMultilingualUrls` costruisce gli alternate per ogni locale dichiarata, incluso `x-default`, che è ciò che i motori di ricerca usano per servire la versione linguistica giusta.

</Question>
<Question title="Come traduco il sito automaticamente con l'AI?">
