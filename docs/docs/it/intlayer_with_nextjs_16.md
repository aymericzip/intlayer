...
<iframe title="La migliore soluzione i18n per Next.js? Scopri Intlayer" class="m-auto aspect-16/9 w-full overflow-hidden rounded-lg border-0" allow="autoplay; gyroscope;" loading="lazy" width="1080" height="auto" src="https://www.youtube.com/embed/e_PPG7PTqGU?autoplay=0&amp;origin=https://intlayer.org&amp;controls=0&amp;rel=1"/>
  ...
  <Tab label="Codice (percorsi di locale)" value="code-locale-path">
...
/>
  ...
  <Tab label="Codice (senza percorso di locale)" value="code-no-locale-path">
...
/>
  ...
  <Tab label="Demo (percorsi di locale)" value="demo">
...
Vedi [Template Applicazione](https://github.com/aymericzip/intlayer-next-16-template) e il [Template Applicazione senza percorso di locale](https://github.com/aymericzip/intlayer-next-no-lolale-path-template) su GitHub.
...
> Il routing locale è utile per la SEO, la dimensione del bundle e le prestazioni. Entrambe le configurazioni, con e senza routing del percorso di locale, sono supportate e trattate in questa guida.
...
Scegli se la tua applicazione deve utilizzare percorsi URL localizzati (ad es. `/it/about`, `/en/about`) o servire contenuti senza un segmento di locale nel percorso (ad es. `/about`, rilevando il locale tramite cookie, intestazioni o parametri di ricerca).

<Tabs group="routing-mode">
<Tab label="Con percorso di locale" value="with-locale-path">

### Architettura

In questa architettura, tutte le pagine localizzate sono nidificate sotto un segmento di route dinamico `[locale]`. Questo approccio è consigliato per la SEO e il rendering statico poiché ogni lingua ha un URL dedicato:
...
Crea un file di configurazione `intlayer.config.ts` per dichiarare le lingue supportate dalla tua applicazione:
...
const config: IntlayerConfig = {
  ...
  routing: {
    mode: "prefix-no-default", // o `prefix-all`
  },
};
...
<Tab label="Senza percorso di locale" value="without-locale-path">

### Architettura

In questa architettura, le rotte non includono un segmento dinamico `[locale]` nel percorso URL. Tutte le pagine risiedono direttamente in `src/app/` e il locale viene determinato tramite cookie, intestazioni o parametri di query. È ideale per dashboard, strumenti interni o app autenticate in cui non è necessario aggiungere un prefisso a ogni URL:
...
Imposta la proprietà `routing.mode` su `"search-params"` (ad es. `/about?locale=it`) o `"no-prefix"` per servire i contenuti senza un prefisso di percorso:
...
> Un singolo `IntlayerProvider` copre entrambi i lati dell'albero: fornisce il contesto server con scope di richiesta letto dagli hook server e monta il provider client affinché i componenti client ricevano la stessa locale.
...
In un'architettura senza percorsi di locale, non esiste alcuna directory `[locale]`. Configura direttamente `src/app/layout.tsx` per leggere il locale dal contesto della richiesta e avvolgi la tua applicazione con `IntlayerProvider`:
...
> Un singolo `IntlayerProvider` copre entrambe le metà dell'albero: fornisce il contesto del server con ambito di richiesta letto dagli hook del server e monta il provider del client in modo che i componenti client ricevano la stessa locale.
...
- **`IntlayerProvider`** viene montato una sola volta, nel locale layout. Fornisce la locale sia ai componenti server che client, quindi le pagine non si avvolgono più su se stesse.
- Gli server hooks risolvono la locale in questo ordine: la locale passata al call site, quindi il server context seminato dal provider, quindi la locale trasportata dalla richiesta (l'header `x-intlayer-locale` impostato dal proxy Intlayer, quindi il cookie locale). Quest'ultimo passaggio è quello che mantiene il contenuto corretto nella navigazione client-side che re-renderizza solo il segmento di pagina, dove il layout , e con esso il provider , non viene rieseguito.
...
export const ServerComponentExample: FC = () => {
  const content = useIntlayer("server-component-example"); // Crea dichiarazione di contenuto correlato
...
> `next-intlayer` è il percorso di import isomorfo: la condizione di export `react-server` fornisce ai server components l'implementazione ambient-locale, mentre i client components ottengono quella context-backed. La stessa chiamata funziona su entrambi i lati.
...
> A partire da Intlayer v9, questo middleware rispetta l'opzione `routing.enableProxy` (`true` per impostazione predefinita). Impostare `routing.enableProxy: false` nella tua configurazione per trasformarlo in un pass-through senza rimuovere questo file. Vedi le [note di rilascio della v9](https://github.com/aymericzip/intlayer/blob/main/docs/docs/it/releases/v9.md).
...
> Nota: Se impostete l'opzione su `importMode: 'dynamic'` o `importMode: 'fetch'` (nella configurazione di `dictionary`), farà affidamento su Suspense, quindi dovrete avvolgere le vostre chiamate a `useIntlayer` in un boundary di `Suspense`. Ciò significa che non potrete utilizzare `useIntlayer` direttamente al livello superiore del vostro componente Page / Layout.
...
A partire da next-intlayer v9.5, `withIntlayer` avvia il content watcher da solo, non è necessaria alcuna configurazione aggiuntiva.
...
<Question title="Quali sono le diverse soluzioni disponibili per internazionalizzare un'app Next.js?">

Next.js non ha un livello di messaggi integrato da quando il campo `i18n` di `next.config.js` ha smesso di applicarsi all'App Router, quindi il livello di localizzazione è sempre una scelta di libreria:

- **`next-intl`**, **`i18next` / `next-i18next`** e **`react-intl`**: le opzioni storiche, basate su cataloghi di messaggi JSON o ICU caricati per namespace.
...
No. Esegui `npx intlayer extract` e Intlayer legge i tuoi componenti, estrae le stringhe visibili all'utente e scrive un file `.content` accanto a ciascuno, così puoi rivedere un diff invece di copiare le stringhe in un catalogo una alla volta. Il passo 14 di questa guida lo illustra.
...
<Question title="Intlayer funziona con l'App Router di Next.js e i React Server Components?">

Sì. `next-intlayer` è costruito per l'App Router: il contenuto è risolto sul server all'interno dei Server Component, quindi nessun dizionario viene inviato al client per il testo renderizzato lato server. I Client Component usano lo stesso hook `useIntlayer` attraverso il provider. Intlayer non blocca il rendering statico, ed è compatibile con Turbopack.

</Question>
<Question title="Quali versioni di Next.js supporta Intlayer?">

Intlayer supporta Next.js 12, 13, 14, 15 e 16. Questa guida copre Next.js 16. Per configurazioni più vecchie, segui la [guida Next.js 15](https://github.com/aymericzip/intlayer/blob/main/docs/docs/it/intlayer_with_nextjs_15.md), la [guida Next.js 14](https://github.com/aymericzip/intlayer/blob/main/docs/docs/it/intlayer_with_nextjs_14.md) o la [guida Pages Router](https://github.com/aymericzip/intlayer/blob/main/docs/docs/it/intlayer_with_nextjs_page_router.md).

</Question>
<Question title="Devo mettere la locale nell'URL, come /fr/about?">

No. Lo schema dell'URL è un'opzione di configurazione, non un vincolo. `routing.mode` accetta:

- `"prefix-no-default"` (predefinito): `/about` per la locale predefinita, `/fr/about` per le altre.
- `"prefix-all"`: ogni locale ha il prefisso, `/en/about` e `/fr/about`.
- `"no-prefix"`: nessuna locale nel percorso, risolta da cookie, header o dominio.
- `"search-params"`: `/about?locale=fr`.

Puoi anche associare ogni locale al proprio dominio con `routing.domains`. Vedi il [riferimento di configurazione](https://github.com/aymericzip/intlayer/blob/main/docs/docs/it/configuration.md) e il [Passaggio 2](#step-2-configure-your-project) per le opzioni della modalità di instradamento.

</Question>
<Question title="Come aggiungo i tag hreflang e i metadati localizzati per la SEO?">

Usa la funzione `generateMetadata` di Next.js insieme a `getMultilingualUrls` di Intlayer. Costruisce la mappa `alternates.languages` per ogni locale dichiarata, inclusa la voce `x-default`, così i motori di ricerca servono la versione linguistica giusta. Lo stesso helper localizza `sitemap.ts` e `robots.ts`. Il passo 8 e il passo 9 di questa guida mostrano il codice completo.

</Question>
<Question title="Come traduco un'app Next.js automaticamente con l'AI?">

Esegui `npx intlayer fill`. La CLI rileva le traduzioni mancanti nei tuoi file di contenuto e le riempie con l'LLM di tua scelta, usando il tuo provider e la tua API key, così paghi il provider direttamente e nulla passa attraverso una terza parte. Vedi il [comando fill](https://github.com/aymericzip/intlayer/blob/main/docs/docs/it/cli/fill.md) e l'[integrazione CI/CD](https://github.com/aymericzip/intlayer/blob/main/docs/docs/it/CI_CD.md).

</Question>
<Question title="Intlayer supporta plurali, genere, condizioni e rich text?">

Sì. Le dichiarazioni di contenuto supportano [forme plurali](https://github.com/aymericzip/intlayer/blob/main/docs/docs/it/dictionary/plurial.md), [contenuto basato sul genere](https://github.com/aymericzip/intlayer/blob/main/docs/docs/it/dictionary/gender.md), condizioni, [inserimenti](https://github.com/aymericzip/intlayer/blob/main/docs/docs/it/dictionary/insertion.md) per valori interpolati, e [Markdown](https://github.com/aymericzip/intlayer/blob/main/docs/docs/it/dictionary/markdown.md) per rich text come pagine legali o corpi di articoli di blog. Numeri, date e valute sono gestiti dai [formattatori](https://github.com/aymericzip/intlayer/blob/main/docs/docs/it/formatters.md).

</Question>
<Question title="Come possono i traduttori e i non sviluppatori modificare il contenuto?">
