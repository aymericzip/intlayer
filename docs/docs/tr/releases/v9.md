...
## Vite Plugin: Gömülü Compiler & Proxy

`intlayer()` Vite eklentisi artık **compiler**'ı ve **locale-routing proxy**'yi doğrudan kendi içinde barındırıyor, bu yüzden çoğu projenin `vite.config.ts` içinde tek bir eklentiye ihtiyacı vardır:
...
- **Compiler**: `compiler.enabled` ayarlandığında ve `compiler.output` yolu yapılandırıldığında otomatik olarak devreye girer. Artık `intlayerCompiler()` eklentisini ayrıca kaydetmenize gerek kalmaz.
- **Proxy**: Yeni `routing.enableProxy` ayarına (varsayılanı `true`) bağlı olarak otomatik devreye girer. Development, preview ve production SSR için dil algılama / redirect / rewrite middleware'ini bağlar. Artık `intlayerProxy()` eklentisini ayrıca kaydetmenize gerek kalmaz.

### `routing.enableProxy` seçeneği

Yeni bir `routing.enableProxy` seçeneği locale-routing proxy'sinin devreye girip girmeyeceğini kontrol eder. Varsayılanı `true`'dur. Locale routing'i kendiniz kontrol etmek isterseniz bu seçeneği kapatın:
...
Bağımsız `intlayerCompiler()` ve `intlayerProxy()` eklentileri gelişmiş kurulumlar için halen export edilmeye devam etmektedir. Bunları `intlayer()` yanına eklemek güvenlidir — her eklenti kendini deduplicate eder (kopyaları ayıklar) ve sadece bir kez çalışır.
