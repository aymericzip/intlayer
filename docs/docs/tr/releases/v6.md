...
Daha fazla seçenek için CLI dokümantasyonuna bakınız: [CLI referansı](https://github.com/aymericzip/intlayer/blob/main/docs/docs/tr/cli/index.md) → "Eksik çevirileri test et". Ayrıca [Test Etme](https://github.com/aymericzip/intlayer/blob/main/docs/docs/tr/testing.md) rehberine de göz atabilirsiniz.
...
Yine de `autoFill` alanını kullanarak sözlük başına ince ayar yapabilirsiniz. Intlayer önce sözlük başına yapılandırmayı dikkate alacak, ardından global yapılandırmaya geri dönecektir. [Auto-fill](https://github.com/aymericzip/intlayer/blob/main/docs/docs/tr/autoFill.md) ve [Content file reference](https://github.com/aymericzip/intlayer/blob/main/docs/docs/tr/dictionary/content_file.md) adresinden tam referansa bakınız.
...
MCP sunucusunu yeni özellikleri desteklemek için güncellendi.

- Otomatik doldurma
- Eksik çevirileri test et
- Canlı güncellemeler
- Logging
- Doğrulama
- Geçiş notları
...
> MCP server, Intlayer kullanan projelerin otomasyonu için yardımcı olur. Otomatik olarak oluşturulan kod için eksik çeviriler test eder. Config'inizi anlamanıza ve Intlayer belgelerini IDE'nize yerleştirmenize yardımcı olur.
...
## Turbopack üzerinde dosyaları izlemek için paralel işlem

`next dev --turbopack` komutuyla Next.js 14 ve üstü sürümlerinde geliştirme sunucusu olarak Turbopack kullanırken, sözlük değişiklikleri varsayılan olarak otomatik olarak algılanmayacaktır.

Bu sınırlama, Turbopack'in içerik dosyalarındaki değişiklikleri izlemek için webpack eklentilerini paralel olarak çalıştıramaması nedeniyle oluşur.

Bunu çözmek için `intlayer watch` komutunu, geliştirme sunucusu ve Intlayer build izleyicisini aynı anda çalıştırmak için `--with` seçeneğini kullanarak bıraktık.
...
## v6'dan v7'ye geçiş notları

Daha fazla bilgi için [v6'dan v7'ye geçiş notlarını](https://github.com/aymericzip/intlayer/blob/main/docs/docs/tr/releases/v7.md) kontrol edin.

---

## Faydalı bağlantılar

- [CLI referansı](https://github.com/aymericzip/intlayer/blob/main/docs/docs/tr/cli/index.md)
