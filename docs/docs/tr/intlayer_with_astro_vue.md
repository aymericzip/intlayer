...
Her sayfaya SEO meta verileri (hreflang ve kurallı bağlantılar gibi) eklemeli ve etkileşimli istemci tarafı içeriği için bir Vue island tanıtmalısınız.
...
<Question title="Vue adaları içeren bir Astro sitesini uluslararasılaştırmak için hangi farklı çözümler mevcuttur?">

Astro'nun yerleşik `i18n` seçeneği yerel ön eklerini ve yönlendirmeleri yönetir ancak içeriğin kendisini yönetmez. Bir ada eklemek ikinci bir sorunu beraberinde getirir: ada Astro değil, Vue çalıştırır.

- **Astro `i18n` artı el yazısı sözlükler**, adada **`vue-i18n`**: senkronize tutulacak iki içerik kaynağı ve aralarında ortak tip denetimi yok.
- **`Intlayer`**: her ikisi için tek bir içerik katmanı. `astro-intlayer` `.astro` sayfalarını, `vue-intlayer` ise aynı bildirimleri okuyarak Vue adalarını kapsar.
...
Büyük ölçüde evet. İçeriği taşımak için [geçiş kılavuzunu](https://github.com/aymericzip/intlayer/blob/main/docs/docs/tr/migration_from_vue-i18n_to_intlayer.md) izleyin. Kademeli olarak da geçiş yapabilirsiniz: [sync JSON eklentisi](https://github.com/aymericzip/intlayer/blob/main/docs/docs/tr/plugins/sync-json.md) mevcut JSON kataloglarınızı doğruluk kaynağı olarak tutar ve bunlardan Intlayer sözlükleri üretir.
...
<Question title="Vue adamın içinde ayrı bir i18n kütüphanesine ihtiyacım var mı?">

Hayır. `vue-intlayer` Astro tarafıyla aynı sözlükleri okur, bu nedenle yanına `vue-i18n` kurmazsınız. Adım 6, ada bileşeninin yereli tekrar çözmek yerine sayfadan doğrudan aldığını gösterir.
