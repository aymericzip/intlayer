...
title: "Astro + Preact i18n - Uygulamanızı çevirmek için eksiksiz kılavuz"
...
Her sayfaya SEO meta verileri (hreflang ve kurallı bağlantılar gibi) eklemeli ve etkileşimli istemci tarafı içeriği için bir Preact island tanıtmalısınız.
...
<Step number={6} title="Bir Preact Island Bileşeni Oluşturun">
...
<Question title="Preact adaları içeren bir Astro sitesini uluslararasılaştırmak için hangi farklı çözümler mevcuttur?">

Astro'nun yerleşik `i18n` seçeneği yerel ön eklerini ve yönlendirmeleri yönetir ancak içeriğin kendisini yönetmez. Bir ada eklemek ikinci bir sorunu beraberinde getirir: ada Astro değil, Preact çalıştırır.

- **Astro `i18n` artı el yazısı sözlükler**, adada **`preact-i18n`**: senkronize tutulacak iki içerik kaynağı ve aralarında ortak tip denetimi yok.
- **`Intlayer`**: her ikisi için tek bir içerik katmanı. `astro-intlayer` `.astro` sayfalarını, `preact-intlayer` ise aynı bildirimleri okuyarak Preact adalarını kapsar.
...
<Question title="`preact-i18n` veya `react-i18next`'den bileşenlerimi yeniden yazmadan geçiş yapabilir miyim?">
...
<Question title="Preact adamın içinde ayrı bir i18n kütüphanesine ihtiyacım var mı?">

Hayır. `preact-intlayer` Astro tarafıyla aynı sözlükleri okur, bu nedenle yanına `preact-i18n` kurmazsınız. Adım 6, ada bileşeninin yereli tekrar çözmek yerine sayfadan doğrudan aldığını gösterir.
