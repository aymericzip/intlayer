...
## Astro + React'te Intlayer'ı Yapılandırmak İçin Adım Adım Kılavuz
...
- **react**, **react-dom**
  React bileşenlerini tarayıcıda oluşturmak için kullanılan temel React paketleri.

- **react-intlayer**
  Intlayer'ı React uygulamalarına entegre etmek için paket. React'te uluslararasılaştırma için `IntlayerProvider`'ın yanı sıra `useIntlayer` ve `useLocale` hook'larını sağlar.

- **@astrojs/react**
  React bileşen islands kullanımına olanak tanıyan resmi Astro entegrasyonu.
...
Astro konfigürasyonunuza `intlayer` eklentisini ve React entegrasyonunu ekleyin.
...
> `react()` entegrasyonu, `client:only="react"` aracılığıyla React bileşen islands kullanımına olanak tanır.
...
Her sayfaya SEO meta verileri (hreflang ve kurallı bağlantılar gibi) eklemeli ve etkileşimli istemci tarafı içeriği için bir React island tanıtmalısınız.
...
<html lang={locale} dir={getHTMLTextDir(locale)}>
  ...
  <body>
    <!-- React island, dil seçici de dahil olmak üzere tüm etkileşimli içeriği oluşturur -->
...
<Step number={6} title="Bir React Island Bileşeni Oluşturun">

React uygulamanızı sarmalayan ve sunucu tarafından algılanan dili alan bir island bileşeni oluşturun:
...
> `locale` prop'u, Astro sayfasından (sunucu algılaması) `IntlayerProvider`'a aktarılır ve ağaç içindeki tüm React hook'ları için başlangıç dili olarak hizmet eder.

</Step>
<Step number={7} title="Bir Dil Seçici Ekleme">

Kullanılabilir dilleri okuyan ve kullanıcı yeni bir dil seçtiğinde yerelleştirilmiş URL'ye yönlendiren bir React `LocaleSwitcher` bileşeni oluşturun:
...
export function LocaleSwitcher() {
  ...
  return (
    <div className="locale-switcher">
      <span className="switcher-label">Dili değiştir:</span>
...
<Question title="React adaları içeren bir Astro sitesini uluslararasılaştırmak için hangi farklı çözümler mevcuttur?">

Astro'nun yerleşik `i18n` seçeneği yerel ön eklerini ve yönlendirmeleri yönetir ancak içeriğin kendisini yönetmez. Bir ada eklemek ikinci bir sorunu beraberinde getirir: ada Astro değil, React çalıştırır.

- **Astro `i18n` artı el yazısı sözlükler**, adada **`react-i18next`**: senkronize tutulacak iki içerik kaynağı ve aralarında ortak tip denetimi yok.
- **`Intlayer`**: her ikisi için tek bir içerik katmanı. `astro-intlayer` `.astro` sayfalarını, `react-intlayer` ise aynı bildirimleri okuyarak React adalarını kapsar.
...
<Question title="`react-i18next`'den bileşenlerimi yeniden yazmadan geçiş yapabilir miyim?">

Büyük ölçüde evet. İçeriği taşımak için [geçiş kılavuzunu](https://github.com/aymericzip/intlayer/blob/main/docs/docs/tr/migration_from_react-i18next_to_intlayer.md) izleyin. Kademeli olarak da geçiş yapabilirsiniz: [sync JSON eklentisi](https://github.com/aymericzip/intlayer/blob/main/docs/docs/tr/plugins/sync-json.md) mevcut JSON kataloglarınızı doğruluk kaynağı olarak tutar ve bunlardan Intlayer sözlükleri üretir.
...
<Question title="React adamın içinde ayrı bir i18n kütüphanesine ihtiyacım var mı?">

Hayır. `react-intlayer` Astro tarafıyla aynı sözlükleri okur, bu nedenle yanına `react-i18next` kurmazsınız. Adım 6, ada bileşeninin yereli tekrar çözmek yerine sayfadan doğrudan aldığını gösterir.
