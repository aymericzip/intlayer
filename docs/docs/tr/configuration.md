...
history:
  ...
  - version: 9.0.0
    date: 2026-07-11
    changes: "`analytics` yapılandırması eklendi"
  - version: 9.0.0
    date: 2026-06-24
...
const config: IntlayerConfig = {
  ...
  routing: {
    ...
    /**
     * Intlayer yerel ayar yönlendirme proxy'sini (middleware) etkinleştirir.
     * dev, preview ve SSR ortamlarında yerel ayar tespiti, yönlendirme ve yeniden yazmayı yönetir.
     * - ayarlanmamış (auto): dev ve preview sunucuları, çerezlerde ve başlıklarda saklanan
     *   yerel ayarı yok sayarak yönlendirmeyi URL odaklı tutar. Önekler yine çözümlenir,
     *   yerel ayar yine kalıcı hale getirilir ve Accept-Language tespiti yine geçerlidir.
     *   Üretimde `true` gibi davranır.
     * - true: her ortamda tam davranış.
     * - false: yerel ayar yönlendirmesi yok.
     * Varsayılan: undefined (auto)
     */
  ...
  /**
   * Analitik (analytics) yapılandırması.
   */
  analytics: {
    /**
    ...
    /**
     * Backend'e otomatik toplu gönderimler arasındaki milisaniye.
     * Varsayılan: 20000
     */
    flushInterval: 20000,

    /**
     * Kaydedilecek oturumların oranı, 0 (hiçbiri) ile 1 (tümü) arasında.
     * Varsayılan: 1
     */
    sampleRate: 1,
  },
  ...
  ai: {
     ...
     * Seçenekler: 'openai', 'anthropic', 'mistral', 'deepseek', 'gemini', 'ollama', 'openrouter', 'alibaba', 'fireworks', 'groq', 'huggingface', 'bedrock', 'googlevertex', 'togetherai', 'lmstudio', 'moonshotai'
     * Varsayılan: 'openai'
     */
    provider: "openai",
  ...
  build: {
    ...
    /**
     * Yerel ayara özgü sözlük parçalarını, onları kullanan kod bölme sınırına göre
     * grupla; böylece geç yüklenen bir sayfa içeriğini tek istekte alır.
     * Varsayılan: true
     *
     * Not:
     * - Yalnızca `importMode: 'dynamic'` kullanan sözlükler için geçerlidir.
     */
    chunkGrouping: true,

    /**
     * Bir sözlüğü, onu kullanan parça render edildikten sonra getirmek yerine o
     * parçayla birlikte yükle. Okumalar askıya alınmak yerine eşzamanlı render
     * edilir, böylece gezinme sırasında artık yükleme durumu görünmez.
     * Varsayılan: true
     *
     * Not:
     * - Yalnızca çözümlenen yerel ayar beklenir, bu nedenle sayfa yalnızca
     *   gösterdiği dili indirir.
     */
...
### Analitik Yapılandırması (Analytics)
...
| `enableProxy` | Intlayer yerel ayar yönlendirme proxy'sini (middleware) etkinleştirir.                                                                                                                                | `boolean` &#124; <br/> `undefined`                                                                                                                                                                           | `undefined` (auto)     | `true`                                                                                                                                                                                      | • Ayarlanmamış (auto): dev ve preview sunucuları çerezlerde/başlıklarda saklanan yerel ayarı yönlendirme kaynağı olarak yok sayar; önekler, kalıcılık ve `Accept-Language` tespiti yine geçerlidir. Üretimde `true` gibi davranır.<br/>• `true`: her yerde tam davranış.<br/>• `false`: yerel ayar yönlendirmesi yok. Next.js’te `intlayerProxy` middleware’i devre dışı geçiş haline gelir. |
...
Bu sözlük yapılandırması iki ana amaca hizmet eder:

1. **Varsayılan Değerler**: İçerik bildirimi dosyaları oluştururken varsayılan değerler tanımlayın
2. **Fallback Davranışı**: Belirli alanlar tanımlanmadığında geri dönüş değerleri sağlayarak sözlük işlemi davranışını küresel olarak tanımlamanıza izin verin
...
| Alan                 | Açıklama                                                                                                                                    | Tür                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Varsayılan  | Örnek                                                         | Not                                                                                                                                                                                          |
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- | ------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `provider`           | Intlayer yapay zeka özellikleri için kullanılacak sağlayıcı.                                                                                | `'openai'` &#124; <br/> `'anthropic'` &#124; <br/> `'mistral'` &#124; <br/> `'deepseek'` &#124; <br/> `'gemini'` &#124; <br/> `'ollama'` &#124; <br/> `'openrouter'` &#124; <br/> `'alibaba'` &#124; <br/> `'fireworks'` &#124; <br/> `'groq'` &#124; <br/> `'huggingface'` &#124; <br/> `'bedrock'` &#124; <br/> `'googleaistudio'` &#124; <br/> `'googlevertex'` &#124; <br/> `'togetherai'` &#124; <br/> `'lmstudio'` &#124; <br/> `'moonshotai'` | `undefined` | `'anthropic'`                                                 | Farklı sağlayıcılar farklı API anahtarları gerektirir ve farklı fiyatlandırmalara sahiptir.                                                                                                  |
...
| `chunkGrouping`       | Yerel ayara özgü sözlük parçalarının, onları kullanan kod bölme sınırına göre gruplanıp gruplanmayacağını belirtir.                | `boolean`                        | `true`                                                                                                                                                                            |                                                                               | • Gruplama olmadan, çok sayıda bileşenden oluşan bir sayfa sözlük başına bir istek gönderir.<br/>• Birden fazla sınırdan erişilen sözlükler paylaşılan bir parçaya taşınır, böylece hiçbir sayfa başka bir sayfanın içeriğini taşımaz.<br/>• Yalnızca `importMode: 'dynamic'` kullanan sözlükler için geçerlidir.<br/>• Yalnızca istemci derlemesi için ve yalnızca paketleme sırasında geçerlidir (geliştirme modunda değil).                                                                                                                                  |
| `dictionariesPreload` | Bir sözlüğün, onu kullanan parça render edildikten sonra getirilmek yerine o parçayla birlikte yüklenip yüklenmeyeceğini belirtir. | `boolean`                        | `true`                                                                                                                                                                            |                                                                               | • Oluşturulan giriş noktası gezinme yerel ayarını en üst düzeyde bekler, bu nedenle geç yüklenen bir rota, içeriği hazır olana kadar yüklenmiş sayılmaz.<br/>• Okumalar askıya alınmak yerine eşzamanlı render edilir, böylece gezinme sırasında artık yükleme durumu görünmez.<br/>• Yalnızca çözümlenen yerel ayar beklenir, bu nedenle sayfa yalnızca gösterdiği dili indirir.<br/>• Yalnızca istemci derlemesinde `importMode: 'dynamic'` kullanan sözlükler için geçerlidir.<br/>• Top-level await destekleyen bir paketleyici gerektirir (Vite, esbuild). |
...
<Question title="intlayer.config.ts dosyası nerede bulunmalıdır?">

Projenizin kök dizininde, `package.json` dosyasının yanında. Intlayer çalışma dizinini ve üst dizinleri `intlayer.config.ts`, `intlayer.config.js`, `intlayer.config.mjs` veya `intlayer.config.cjs` için tarar. CLI komutlarında `--config` bayrağı ile özel bir yol da belirtebilirsiniz.
...
<Question title="Uygulamama yeni bir dili nasıl eklerim?">

Dili `internationalization.locales` dizisine ekleyin, ardından mevcut içeriği bu dile çevirmek için `npx intlayer fill` komutunu çalıştırın. Derleyici bir sonraki çalıştırmada yeni dili otomatik olarak dahil eder.

</Question>

<Question title="URL'lerimden yerel ön ekini nasıl kaldırırım?">

`routing.mode` ayarını yapın. `"no-prefix"` modu tüm diller için ön eki kaldırır ve yereli çerezden veya başlıktan çözer. `"prefix-no-default"` (varsayılan) ise ön eki yalnızca varsayılan dil için kaldırır.

</Question>

<Question title="Her dili kendi alan adından sunabilir miyim?">

Evet. `routing.domains` ayarı her yereli bir alan adına eşler, örneğin `{ tr: 'example.com.tr', en: 'example.com' }`. Alan adı tespiti yol ön eklerinden önce gelir ve `getMultilingualUrls` tam URL'leri üretir.

</Question>

<Question title="Kullanıcının dili nasıl algılanır?">

`routing.storage` üzerinden, öncelik sırasına göre: genellikle önce URL, ardından çerez, daha sonra `Accept-Language` başlığı ve son olarak varsayılan dil.

</Question>

<Question title="routing.enableProxy ne işe yarar?">

Ön ekleri ve yönlendirmeleri çözen yerel yönlendirme ara katman yazılımını (proxy) kontrol eder. Tanımlanmadığında geliştirme ve üretim ortamlarında otomatik çalışır. `false` olarak ayarlanırsa tamamen devre dışı bırakılır.

</Question>

<Question title="importMode static, dynamic ve fetch arasındaki fark nedir?">

`"static"` (varsayılan), sözlükleri statik olarak içe aktarır, böylece pakete dahil edilir ve senkronize okunur. `"dynamic"`, sözlükleri dil başına asenkron yükler. `"fetch"`, sözlükleri çalışma zamanında bir sunucudan veya CMS'den çeker.

</Question>

<Question title="Otomatik çeviri için AI sağlayıcısını ve API anahtarını nereye ayarlamalıyım?">

Yapılandırma dosyasındaki `ai` bölümünde veya komut satırında `--provider`, `--model` ve `--api-key` bayrakları ile. Anahtarınız yerel kalır ve ortamınızdan asla dışarı çıkmaz. Bkz. [AI yapılandırma referansı](https://github.com/aymericzip/intlayer/blob/main/docs/docs/tr/configuration.md).

</Question>

<Question title="Yapılandırmayı değiştirdikten sonra geliştirme sunucusunu yeniden başlatmam gerekir mi?">

Genellikle hayır. Intlayer watcher `intlayer.config.ts` dosyasını izler: kaydettiğinizde yapılandırmayı yeniden yükler ve sözlükleri arka planda günceller.
