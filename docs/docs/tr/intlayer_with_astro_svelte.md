...
Her sayfaya SEO meta verileri (hreflang ve kurallı bağlantılar gibi) eklemeli ve etkileşimli istemci tarafı içeriği için bir Svelte island tanıtmalısınız.
...
<Step number={6} title="Bir Svelte Island Bileşeni Oluşturun">
...
<Question title="Svelte adaları içeren bir Astro sitesini uluslararasılaştırmak için hangi farklı çözümler mevcuttur?">

Astro'nun yerleşik `i18n` seçeneği yerel ön eklerini ve yönlendirmeleri yönetir ancak içeriğin kendisini yönetmez. Bir ada eklemek ikinci bir sorunu beraberinde getirir: ada Astro değil, Svelte çalıştırır.

- **Astro `i18n` artı el yazısı sözlükler**, adada **`svelte-i18n`**: senkronize tutulacak iki içerik kaynağı ve aralarında ortak tip denetimi yok.
- **`Intlayer`**: her ikisi için tek bir içerik katmanı. `astro-intlayer` `.astro` sayfalarını, `svelte-intlayer` ise aynı bildirimleri okuyarak Svelte adalarını kapsar.
...
<Question title="`svelte-i18n`'den bileşenlerimi yeniden yazmadan geçiş yapabilir miyim?">

Büyük ölçüde evet. İçeriği taşımak için [geçiş kılavuzunu](https://github.com/aymericzip/intlayer/blob/main/docs/docs/tr/compat/svelte-i18n.md) izleyin. Kademeli olarak da geçiş yapabilirsiniz: [sync JSON eklentisi](https://github.com/aymericzip/intlayer/blob/main/docs/docs/tr/plugins/sync-json.md) mevcut JSON kataloglarınızı doğruluk kaynağı olarak tutar ve bunlardan Intlayer sözlükleri üretir.
...
<Question title="Svelte adamın içinde ayrı bir i18n kütüphanesine ihtiyacım var mı?">

Hayır. `svelte-intlayer` Astro tarafıyla aynı sözlükleri okur, bu nedenle yanına `svelte-i18n` kurmazsınız. Adım 6, ada bileşeninin yereli tekrar çözmek yerine sayfadan doğrudan aldığını gösterir.
