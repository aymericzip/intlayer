...
> Çevirileri `string` niteliklerinde kullanmak istediğinizde (örneğin `alt`, `title`, `href`, `aria-label`), fonksiyonun

> değerini şu şekilde çağırın:
...
<Question title="Next.js Pages Router uygulamasını uluslararasılaştırmak için hangi farklı çözümler mevcuttur?">

Pages Router, `next.config.js` dosyasının yerleşik `i18n` alanını hala destekler, ancak yalnızca yerel ayar yönlendirmesini ve algılamasını yönetir, çevirilerin kendisini değil; bu nedenle yine de bir içerik katmanı seçersiniz:
