...
priority: 9
title: "Astro + Solid i18n - Uygulamanızı çevirmek için eksiksiz kılavuz"
description: "Artık i18next yok. 2026 yılı için çok dilli (i18n) Astro + Solid uygulaması oluşturma kılavuzu. Yapay zeka ajanlarıyla çevirin ve bundle boyutu, SEO ve performansı optimize edin."
keywords:
...
Her sayfaya SEO meta verileri (hreflang ve kurallı bağlantılar gibi) eklemeli ve etkileşimli istemci tarafı içeriği için bir Solid island tanıtmalısınız.
...
> İçeriğinizi `alt`, `title`, `href`, `aria-label` gibi bir `string` özniteliğinde kullanmak istiyorsanız, işlevin değerini şöyle kullanabilirsiniz:
...
<Question title="Solid adaları içeren bir Astro sitesini uluslararasılaştırmak için hangi farklı çözümler mevcuttur?">

Astro'nun yerleşik `i18n` seçeneği yerel ön eklerini ve yönlendirmeleri yönetir ancak içeriğin kendisini yönetmez. Bir ada eklemek ikinci bir sorunu beraberinde getirir: ada Astro değil, Solid çalıştırır.

- **Astro `i18n` artı el yazısı sözlükler**, adada **`@solid-primitives/i18n`**: senkronize tutulacak iki içerik kaynağı ve aralarında ortak tip denetimi yok.
- **`Intlayer`**: her ikisi için tek bir içerik katmanı. `astro-intlayer` `.astro` sayfalarını, `solid-intlayer` ise aynı bildirimleri okuyarak Solid adalarını kapsar.
