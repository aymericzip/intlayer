...
Evet. Aşamalı geçiş kılavuzlarını izleyebilir veya mevcut JSON dosyalarınızı koruyarak Intlayer sözlüklerine dönüştürebilirsiniz.
...
Fastify eklentisi gelen istekleri çerez, başlıklar veya yol parametreleri üzerinden analiz eder ve sonucu `request.locale` içerisine kaydeder.
...
Evet. `preHandler` kancası (hook) kullanarak kullanıcı JWT belirtecinden veya oturumundan dili belirleyip yereli ayarlayabilirsiniz.

</Question>

<Question title="Fastify rotalarımda yerelleştirilmiş URL ön eklerini nasıl kullanırım?">

Rotalarınıza `/:locale/` parametresi ekleyerek ve Intlayer yerel doğrulayıcısını kullanarak geçersiz dilleri filtreleyebilirsiniz.
