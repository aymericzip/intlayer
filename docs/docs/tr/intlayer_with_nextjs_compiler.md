...
title: "Next.js i18n - Uygulamanızı çevirmek için eksiksiz kılavuz"
description: "Artık i18next yok. 2026 yılı için çok dilli (i18n) Next.js uygulaması oluşturma kılavuzu. Yapay zeka ajanlarıyla çevirin ve bundle boyutu, SEO ve performansı optimize edin."
keywords:
...
Statik analiz kuralları kullanır: JSX metin düğümleri ve `aria-label`, `title`, `placeholder` gibi belirli özellikler yerelleştirilebilir kabul edilirken, CSS sınıf adları ve sistem tanımlayıcıları hariç tutulur.

</Question>

<Question title="Derleyicinin görmediği dizelere ne olur?">

Çalışma zamanında oluşturulan dizeler veya dinamik API yanıtları derleyici tarafından yakalanamaz; bu tür metinler için doğrudan `t()` veya geleneksel `.content` dosyaları kullanılır.

</Question>

<Question title="Derleyiciyi mi kullanmalıyım yoksa içeriği kendim mi bildirmeliyim?">

Yeni başlayan veya hızlı prototip üreten projeler için derleyici zahmetsizdir. Karmaşık iş kuralları, çoğullar veya zengin biçimlendirme içeren projelerde açık `.content` dosyaları daha fazla kontrol sunar.
