...
নিচের গ্রাফটি একটি তাত্ত্বিক অ্যাপের কনটেন্ট পেলোড অনুমান করে, যাতে 1 থেকে 10টি পেজ রয়েছে এবং যা 1 থেকে 10টি ভাষায় অনূদিত, প্রতি পেজে প্রায় 30 KB টেক্সট সহ। locale অনুযায়ী কনটেন্ট ডায়নামিকভাবে লোড করলে ভাষার অক্ষ সরে যায়, কনটেন্টকে কম্পোনেন্ট বা রুটে সীমাবদ্ধ করলে পেজের অক্ষ সরে যায়, এবং কেবল দুটির সমন্বয়েই পেলোড স্থির থাকে।

![আর্কিটেকচার অনুযায়ী তাত্ত্বিক কনটেন্ট লিকেজ](https://github.com/aymericzip/intlayer/blob/main/docs/assets/theorical_content_leakage.webp?raw=true)
...
> `optimize` `false` থাকলে মিনিফিকেশন বাদ দেওয়া হয়। `editor.enabled` `true` থাকলেও এটি চলতে থাকে, তবে ফিল্ডের নাম পরিবর্তনের ধাপ ছাড়া — ভিজ্যুয়াল এডিটর `keyPath` এর মাধ্যমে এডিট রিজলভ করে, তাই মূল ফিল্ডের নামগুলো অক্ষত থাকতে হয়।
...
> `optimize` `false` থাকলে Purge বাদ দেওয়া হয়। `editor.enabled` `true` থাকলেও এটি সক্রিয় থাকে — কারণ পার্জ করা ফিল্ড কোনো কম্পোনেন্ট পড়ে না, তাই এডিটর সেটিকে কখনো রেন্ডার করে না। Next.js-এ `@intlayer/swc` অনুপলব্ধ হলে এবং কম্প্যাট-অ্যাডাপ্টার কলার কনফিগার করা থাকলে এটি অতিরিক্তভাবে এড়িয়ে যাওয়া হয়।
...
"ভারী" JSON ফাইল এবং কোড-স্প্লিটিং-এর সুযোগ চিহ্নিত করার সবচেয়ে সরাসরি উপায় হলো আপনার বান্ডেল বিশ্লেষণ করা। এই টুলগুলো আপনার অ্যাপ্লিকেশনের কম্পাইল করা কোডের একটি ভিজ্যুয়াল ট্রিম্যাপ (treemap) তৈরি করে, যা দিয়ে আপনি ঠিক বুঝতে পারবেন কোন লাইব্রেরিগুলো সবচেয়ে বেশি জায়গা নিচ্ছে।
