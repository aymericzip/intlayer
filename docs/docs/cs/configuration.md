...
const config: IntlayerConfig = {
  ...
  routing: {
    ...
    /**
     * Zapíná proxy pro směrování locale v Intlayeru (middleware).
     * Řeší detekci locale, přesměrování a přepisy URL ve dev, preview a SSR.
     * - nenastaveno (auto): dev a preview servery drží směrování řízené URL a ignorují
     *   locale uloženou v cookies a hlavičkách. Prefixy se stále vyhodnocují, locale
     *   se stále ukládá a detekce Accept-Language stále platí.
     *   V produkci se chová jako `true`.
     * - true: plné chování ve všech prostředích.
     * - false: žádné směrování locale.
     * Výchozí: undefined (auto)
     */
  ...
  ai: {
     ...
     * Možnosti: 'openai', 'anthropic', 'mistral', 'deepseek', 'gemini', 'ollama', 'openrouter', 'alibaba', 'fireworks', 'groq', 'huggingface', 'bedrock', 'googlevertex', 'togetherai', 'lmstudio', 'moonshotai'
     * Výchozí: 'openai'
     */
    provider: "openai",
  ...
  build: {
    ...
    /**
     * Seskupit chunky slovníků podle jazyka na základě hranice dělení kódu, která
     * je používá, aby líně načtená stránka získala svůj obsah jedním požadavkem.
     * Výchozí: true
     *
     * Poznámka:
     * - Platí pouze pro slovníky používající `importMode: 'dynamic'`.
     */
    chunkGrouping: true,

    /**
     * Načíst slovník společně s chunkem, který jej používá, místo jeho stažení až
     * po vykreslení daného chunku. Čtení se vykreslí synchronně místo pozastavení,
     * takže navigace už neproblikne stavem načítání.
     * Výchozí: true
     *
     * Poznámka:
     * - Čeká se pouze na vyhodnocený jazyk, stránka tedy stahuje jen jazyk, který
     *   zobrazuje.
     */
...
| `enableProxy` | Zapíná proxy pro směrování locale v Intlayeru (middleware).                                                                                                                                                                | `boolean` &#124; <br/> `undefined`                                                                                                                                                                           | `undefined` (auto)     | `true`                                                                                                                                                                                            | • Nenastaveno (auto): dev a preview servery ignorují locale uloženou v cookies/hlavičkách jako zdroj přesměrování; prefixy, ukládání locale a detekce `Accept-Language` stále platí. V produkci se chová jako `true`.<br/>• `true`: plné chování všude.<br/>• `false`: žádné směrování locale. V Next.js se middleware `intlayerProxy` stane průchozím. |
...
| `expires`  | Číslo udává dny od vytvoření; datum (nebo ISO řetězec data) je absolutní datum vypršení. Výchozí: `undefined` | `Date` &#124; <br/> `number` &#124; <br/> `string`    |
...
| Pole                 | Popis                                                                                                                        | Typ                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Výchozí     | Příklad                                                       | Poznámka                                                                                                                                                                              |
| -------------------- | ---------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- | ------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `provider`           | Poskytovatel použitý pro AI funkce Intlayer.                                                                                 | `'openai'` &#124; <br/> `'anthropic'` &#124; <br/> `'mistral'` &#124; <br/> `'deepseek'` &#124; <br/> `'gemini'` &#124; <br/> `'ollama'` &#124; <br/> `'openrouter'` &#124; <br/> `'alibaba'` &#124; <br/> `'fireworks'` &#124; <br/> `'groq'` &#124; <br/> `'huggingface'` &#124; <br/> `'bedrock'` &#124; <br/> `'googleaistudio'` &#124; <br/> `'googlevertex'` &#124; <br/> `'togetherai'` &#124; <br/> `'lmstudio'` &#124; <br/> `'moonshotai'` | `undefined` | `'anthropic'`                                                 | Různí poskytovatelé vyžadují různé API klíče a mají různé ceny.                                                                                                                       |
...
| `chunkGrouping`       | Určuje, zda se mají chunky slovníků podle jazyka seskupovat podle hranice dělení kódu, která je používá.               | `boolean`                        | `true`                                                                                                                                                                            |                                                                               | • Bez seskupení vyšle stránka složená z mnoha komponent jeden požadavek na každý slovník.<br/>• Slovníky dosažitelné z více hranic se přesunou do sdíleného chunku, takže žádná stránka nenese obsah jiné stránky.<br/>• Platí pouze pro slovníky používající `importMode: 'dynamic'`.<br/>• Platí pouze pro klientský build a jen při bundlování (ne ve vývojovém režimu).                                                                                                                                                 |
| `dictionariesPreload` | Určuje, zda se má slovník načítat společně s chunkem, který jej používá, místo stažení až po vykreslení daného chunku. | `boolean`                        | `true`                                                                                                                                                                            |                                                                               | • Vygenerovaný vstupní bod čeká na jazyk prohlížení na nejvyšší úrovni, takže líně načtená trasa není považována za načtenou, dokud není k dispozici její obsah.<br/>• Čtení se vykreslí synchronně místo pozastavení, takže navigace už neproblikne stavem načítání.<br/>• Čeká se pouze na vyhodnocený jazyk, stránka tedy stahuje jen jazyk, který zobrazuje.<br/>• Platí pouze pro slovníky používající `importMode: 'dynamic'` v klientském buildu.<br/>• Vyžaduje bundler s podporou top-level await (Vite, esbuild). |
