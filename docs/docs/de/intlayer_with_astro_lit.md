...
<Step number={6} title="Erstellen Sie ein Lit-Custom-Element">
...
<Question title="Welche verschiedenen Lösungen gibt es, um eine Astro-Website mit Lit-Islands zu internationalisieren?">

Die eingebaute `i18n`-Option von Astro kümmert sich um Locale-Präfixe und Weiterleitungen, aber nicht um den Inhalt selbst, sodass Sie weiterhin eine Message-Ebene brauchen, und eine Island bringt ein zweites Problem: Die Island läuft mit Lit, nicht mit Astro.

- **Astro `i18n` plus handgeschriebene Wörterbücher**, mit **`@lit/localize`** innerhalb der Island: zwei Inhaltsquellen, die synchron gehalten werden müssen, und keine gemeinsame Typisierung dazwischen.
- **`Intlayer`**: eine Inhaltsebene für beides. `astro-intlayer` deckt die `.astro`-Seiten ab und `lit-intlayer` die Lit-Islands, die dieselben Deklarationen lesen.
...
<Question title="Kann ich von `@lit/localize` migrieren, ohne meine Komponenten neu zu schreiben?">
...
<Question title="Brauche ich eine separate i18n-Bibliothek innerhalb meiner Lit-Island?">

Nein. `lit-intlayer` liest dieselben Wörterbücher wie die Astro-Seite, sodass Sie `@lit/localize` nicht daneben installieren. Schritt 6 zeigt, wie die Island-Komponente die aktive Locale von der Seite erhält, statt sie erneut aufzulösen.
