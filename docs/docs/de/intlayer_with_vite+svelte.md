...
title: Vite + Svelte i18n - Vollständiger Leitfaden zur Übersetzung Svelte
description: Beste Lösung für Bundle-Größe, SEO, Performance & Wartbarkeit. Machen Sie Ihre Vite and Svelte App 2026 mehrsprachig, LLM-Übersetzung, Agent Skills & MCP.
keywords:
...
> Ihre Inhaltsdeklarationen können überall in Ihrer Anwendung definiert werden, sobald sie in das Verzeichnis `contentDir` (standardmäßig `./src`) aufgenommen werden. Und die Dateiendung der Inhaltsdeklaration muss übereinstimmen (standardmäßig `.content.{json,ts,tsx,js,jsx,mjs,cjs}`).
...
### (Optional) Schritt 9 : Inhalt Ihrer Komponenten extrahieren
