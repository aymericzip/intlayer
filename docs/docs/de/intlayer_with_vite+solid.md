...
In dieser Architektur stellt `solid-intlayer` den in `index.tsx` eingebundenen `IntlayerProvider` bereit, um den Solid-Baum zu umschließen. Inhaltsdeklarationen befinden sich in `src/` neben den Komponenten.
...
<Question title="Welche verschiedenen Lösungen gibt es, um eine Vite-und-Solid-App zu internationalisieren?">

Vite hat keine Meinung zu i18n, sodass die Wahl aus dem Solid-Ökosystem kommt:

- **`@solid-primitives/i18n`**: das Community-Primitiv, ein flaches Wörterbuch, das Sie selbst zusammensetzen und laden.
- **`i18next`** mit einem Solid-Wrapper: ausgereifte Kataloge, aber keine eigene Reaktivität.
...
<Question title="Wie viel trägt i18n zu meiner Solid-Bundle-Größe bei?">
...
<Question title="Wie verwende ich übersetzten Inhalt in einer Solid-Komponente?">

Rufen Sie `useIntlayer` in Ihrer Komponente auf und greifen Sie direkt auf die Werte zu. Der Inhalt wird von einem Solid-Signal gestützt, sodass ein Wechsel der Locale nur die Teile des DOM aktualisiert, die ihn lesen, ohne ein erneutes Rendern der Komponente. Schritt 5 zeigt die Verwendung.
