...
> Seit Intlayer v9 ist der Compiler direkt im `intlayer()`-Plugin gebündelt und aktiviert sich automatisch, sobald `compiler.enabled` mit einem `compiler.output`-Pfad gesetzt ist. Die separate Registrierung von `intlayerCompiler()` wie unten gezeigt ist jetzt optional — sie dedupliziert sich selbst, wenn sie auch hinzugefügt wird. Siehe die [v9-Versionshinweise](https://github.com/aymericzip/intlayer/blob/main/docs/docs/de/releases/v9.md).
...
<Question title="Welche verschiedenen Lösungen gibt es, um eine Vite-und-React-App zu internationalisieren?">

- **`react-i18next` / `i18next`**: JSON-Namespaces, die zur Laufzeit geladen werden, mit von Hand an jeder Aufrufstelle geschriebenen Schlüsseln.
- **`react-intl`** und **`Lingui`**: ICU-Nachrichten mit einem Extraktionsschritt, den Sie selbst ausführen.
...
Dieser Leitfaden verwendet das Compiler-Setup, bei dem Sie weiterhin einfache Strings in Ihren Komponenten schreiben und die Wörterbücher für Sie generiert werden. Siehe [warum Intlayer](https://github.com/aymericzip/intlayer/blob/main/docs/docs/de/interest_of_intlayer.md) und den [Benchmark](https://github.com/aymericzip/intlayer/blob/main/docs/docs/de/benchmark/index.md).
...
Verwenden Sie den Compiler, wenn Sie i18n mit möglichst wenig Aufwand zu einer bestehenden Codebasis hinzufügen möchten: Sie behalten Ihre Komponenten, wie sie sind, und die Wörterbücher folgen. Deklarieren Sie Inhalte selbst, wie der [Standard-Leitfaden für Vite und React](https://github.com/aymericzip/intlayer/blob/main/docs/docs/de/intlayer_with_vite+react.md) zeigt, wenn Sie explizite Kontrolle über Schlüssel, Struktur und Wiederverwendung über Komponenten hinweg möchten. Beide können nebeneinander bestehen: kompilierte Inhalte und deklarierte Inhalte leben in derselben Wörterbuchebene.
...
<Question title="Wie entscheidet der Compiler, was für den Nutzer sichtbarer Text ist?">

Durch Heuristiken über Ihren JSX-Code, weshalb er in beide Richtungen falsch liegen kann: Ein `className`-Wert oder ein Statuscode kann wie Text aussehen, und ein ungewöhnliches Muster kann übersehen werden. In einer großen Codebasis korrigieren Sie die Randfälle mit Annotationen. Wenn dieser Kompromiss Ihnen nicht zusagt, führt `npx intlayer extract` dieselbe Extraktion einmalig aus und lässt das Ergebnis als Diff zum Review. Siehe den [extract-Befehl](https://github.com/aymericzip/intlayer/blob/main/docs/docs/de/cli/extract.md).
...
<Question title="Wie ändere ich die Sprache zur Laufzeit?">

Schritt 6 behandelt das. `useLocale` stellt die aktive Locale, die deklarierten Locales und einen Setter bereit, der die Wahl persistiert, und die Komponenten, die kompilierten Inhalt lesen, rendern in der neuen Sprache neu, ohne Seitenneuladung.
