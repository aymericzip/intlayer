...
In dieser Architektur sind alle lokalisierten Routen unter dem Routensegment `{-$locale}` in Solid verschachtelt. Dieser Ansatz stellt sicher, dass jede Sprache eine dedizierte URL hat, und ermöglicht gleichzeitig ein automatisches Locale-Präfix, Validierung und SEO-Optimierung.
...
> Ihre Inhaltsdeklarationen können an beliebiger Stelle in Ihrer Anwendung definiert werden, solange sie im Verzeichnis `contentDir` (standardmäßig `./app`) enthalten sind. Und der Dateierweiterung für Inhaltsdeklarationen (standardmäßig `.content.{json,ts,tsx,js,jsx,mjs,cjs,md,mdx,yaml,yml}`) entsprechen.
...
> Verwenden Sie standardmäßig **`useIntlayer`**: Es ist der empfohlene Weg, Inhalte innerhalb von Komponenten zu lesen, und der Compiler löst den Aufruf auf die gerenderte Locale auf. Greifen Sie nur außerhalb des Solid-Baums zu `getIntlayer` / `getIntlayerAsync`: im `head` von Routen, in Loadern und Server Functions.
...
Am besten für kleine Metadaten-Dictionaries, wenige Locales oder beim Prototyping.
...
> `head` kann aufgerufen werden, bevor der Loader fertig ist; `loaderData` ist deshalb als möglicherweise `undefined` typisiert. Behalten Sie die Optional-Chaining-Aufrufe bei oder geben Sie einen Fallback-Titel zurück.

Sie behalten den Locale-Chunk, ohne seine Kosten auf dem kritischen Pfad des `head` zu bezahlen. Der Preis ist die Developer Experience: Der Inhalt muss explizit vom Loader über `loaderData` an den `head` durchgereicht werden.
...
<Question title="Welche verschiedenen Lösungen gibt es, um eine TanStack-Start-und-Solid-App zu internationalisieren?">

TanStack Start liefert keine i18n-Ebene mit, und die Optionen im Solid-Ökosystem sind dünn:

- **`@solid-primitives/i18n`**: ein flaches Wörterbuch, das Sie selbst zusammensetzen und in den Router einbinden.
- **`i18next`** mit einem Solid-Wrapper: ausgereifte Kataloge, aber keine Integration mit dem TanStack-Router, der `head`-Funktion oder dem Pre-Render-Durchlauf.
- **`Intlayer`**: Inhalte werden neben jeder Komponente deklariert und zur Build-Zeit kompiliert, mit Locale-bewusstem Routing, Sitemap-Generierung, KI-Übersetzung, visuellem Editor und CMS.
...
<Question title="Unterstützt Intlayer hier serverseitiges Rendering und Pre-Rendering?">

Ja. Inhalte werden während SSR aufgelöst, und Schritt 16 behandelt die Pre-Render-Konfiguration, die ein statisches Dokument pro lokalisierter Route ausgibt, zuzüglich der aus derselben Routentabelle generierten lokalisierten Sitemap.

</Question>
<Question title="Rendert eine Änderung der Locale meine gesamte App neu?">

Nein. Inhalte werden von Solid-Signals gestützt, sodass ein Sprachwechsel nur die DOM-Knoten aktualisiert, die die geänderten Werte lesen.

</Question>
<Question title="Wie füge ich hreflang-Tags und eine lokalisierte Sitemap hinzu?">

Verwenden Sie die eingebaute Funktion `generateSitemap` in einer Route `src/routes/sitemap[.]xml.ts`. Sie gibt den `xhtml:link`-Namespace aus, sodass jede Sprachversion einer Seite bidirektional auf die anderen verweist. Lokalisierte `head`-Metadaten werden in Schritt 12 behandelt.

</Question>
<Question title="Wie handhabe ich 404-Seiten bei lokalisierten Routen?">

Schritt 14 behandelt das. `validatePrefix` sagt Ihnen, ob das Locale-Segment der URL eine deklarierte Locale ist, sodass `/xx/about` einen echten 404 zurückgibt, statt als doppelte Seite indexiert zu werden.
...
Schritt 9 zeigt die Komponente. `useLocale` stellt die aktive Locale, die verfügbaren Locales und einen Setter bereit, der die Wahl persistiert, während `getLocalizedUrl` den aktuellen Pfad in die Zielsprache umschreibt, sodass der Nutzer auf derselben Seite bleibt.
