...
## YAML- & Markdown-Inhaltsdateien (v8.10.0)

Sie können Inhalte jetzt direkt mit den Dateierweiterungen `.content.md` und `.content.yaml` deklarieren. Dies ist perfekt für Langform-Inhalte, Blogs, Dokumentationen und Datenschutzrichtlinien.

Diese Dateien sind vollständig über den Visual Editor bearbeitbar und bieten einen optimierten Workflow für nicht-technische Teammitglieder.

Weitere Details finden Sie in der [Dokumentation zu YAML-Inhaltsdateien](https://github.com/aymericzip/intlayer/blob/main/docs/docs/de/dictionary/content_file.md#yaml-content-file) und der [Dokumentation zu Markdown-Inhaltsdateien](https://github.com/aymericzip/intlayer/blob/main/docs/docs/de/dictionary/markdown.md).
...
## Unicode CLDR Pluralregeln (v8.8.0)

Ein neuer `plural` Node-Helper wurde basierend auf den Pluralregeln des Unicode CLDR Projekts eingeführt, um die Unterstützung für komplexe Sprachen zu verbessern.
...
const openingsContent = {
  ...
  content: {
    totalOpenings: t({
      ...
      de: plural({
        // Added German translation for plural example
        one: "{{count}} offene Stelle",
...
Sie können den Zähler dann dynamisch in Ihrer Framework-Komponente übergeben:
...
const OpeningsComponent: FC<{ count: number }> = ({ count }) => {
  ...
  return (
    <div>
      ...
      {/* In German:                                   */}
      {/*  count=0  → "0 offene Stellen" (other)       */}
      {/*  count=1  → "1 offene Stelle"  (one)         */}
      {/*  count=2  → "2 offene Stellen" (other)       */}
      {/*  count=21 → "21 offene Stellen" (other)      */}
...
Weitere Informationen finden Sie in der [Dokumentation zur Pluralisierung](https://github.com/aymericzip/intlayer/blob/main/docs/docs/de/dictionary/plurial.md).
...
Der `IntlayerNode`-Typ wurde aktualisiert, um das direkte Aufrufen primitiver JavaScript-Methoden auf Inhaltsknoten zu ermöglichen. Dies macht es möglich, mit Ihren lokalisierten Inhalten direkt zu interagieren, als wären sie ein Standard-String-, Zahlen-, Boolean- oder Array-Primitiv.

Unterstützt in React, Preact, Solid, Svelte, Vue und Angular.

### String-Beispiel

```typescript
content.placeholder; // Gibt IntlayerNode<string> zurück
content.placeholder.value; // Gibt string zurück
content.placeholder.toString(); // Gibt string zurück
content.placeholder.toLowerCase(); // Gibt string zurück
String(content.placeholder); // Gibt string zurück
content.placeholder.toUpperCase(); // Gibt großgeschriebenen string zurück
content.placeholder.replace("a", "b"); // Gibt modifizierten string zurück
```

### Array-Beispiel

```typescript
content.myArrayOfString             // Gibt IntlayerNode<Array<string>> zurück
content.myArrayOfString.find(...)   // Gibt Element zurück
content.myArrayOfString.join(', ')  // Gibt verbundenen string zurück
```
...
## Tooling: C++ Watcher & OXC-basierter LSP (v8.12.0)

Intlayer v8.12.0 bringt wesentliche Verbesserungen für die Developer Experience:

- **Parcel Watcher**: Der Content Watcher wurde von `chokidar` zu `@parcel/watcher` migriert, wodurch natives C++ File Watching genutzt wird, um schnellere Updates und einen deutlich geringeren Speicherverbrauch zu ermöglichen.
- **Neues Language Server Protocol (LSP)**: Ein brandneues LSP ist jetzt verfügbar. Basierend auf OXC-Parsing hilft es Ihrer IDE und Ihren AI-Agenten, `useIntlayer('my-key')`-Aufrufe nahtlos mit den entsprechenden `.content`-Dateien zu verknüpfen und umgekehrt.

Weitere Details finden Sie in der [LSP-Dokumentation](https://github.com/aymericzip/intlayer/blob/main/docs/docs/de/lsp.md).
...
## Experimentelle Build-Optimierungen: Minify & Purge (v8.7.0)

Intlayer v8.7.0 führt neue experimentelle Build-Funktionen in Ihrer `intlayer.config.ts` ein, die darauf abzielen, Ihr Produktions-Bundle zu optimieren:

- **Dictionary Minification**: Minifiziert Dictionary-Dateien, um die Größe zu reduzieren.
- **Unused Key Purging**: Scannt und entfernt ungenutzte Übersetzungsschlüssel aus Dictionaries und benennt aktive Schlüssel in kurze Zeichen um (z. B. `"products" -> "a"`, `"pricing" -> "b"`). Dies reduziert die Bundle-Größe um bis zu 5%.
- **Disable TypeScript Checks**: Beschleunigt Builds, indem die TypeScript-Typüberprüfung während des Kompilierungsschritts deaktiviert wird.
...
const config: IntlayerConfig = {
  ...
  build: {
    /**
     * Minifiziert die Dictionaries, um die Bundle-Größe zu reduzieren.
     */
    minify: true,

    /**
     * Entfernt ungenutzte Schlüssel in Dictionaries.
     */
    purge: true,

    /**
     * Gibt an, ob der Build TypeScript-Typen überprüfen soll.
     */
    checkTypes: false,
  },
};
...
## Angular-Unterstützung: Vite Bundler (v8.11.0)

Für Angular-Benutzer unterstützt Intlayer jetzt nativ den Vite Bundler durch ein neues benutzerdefiniertes esbuild-Plugin, was die Entwicklungs- und Build-Zeiten beschleunigt.

Details zur Konfiguration finden Sie in der [Angular Environment Dokumentation](https://github.com/aymericzip/intlayer/blob/main/docs/docs/de/intlayer_with_angular_21.md).

## Sitemap-Generierung (v8.6.0)

Intlayer generiert jetzt automatisch Sitemaps basierend auf Ihren Locales und dem Routing-Modus. Dies ist besonders nützlich für die SEO-Optimierung in Router-Architekturen wie TanStack Router:
