...
- 30 % kleinere Bundle-Größe des Visual Editors
...
## Automatische Code-Formatierung: formatCommand-Konfiguration

v7 führt die `formatCommand`-Option in der Editor-Konfiguration ein, die es Ihnen ermöglicht, Inhaltsdateien automatisch zu formatieren, wenn sie von Intlayer geschrieben werden. Dies gewährleistet einen konsistenten Code-Stil und eine konsistente Formatierung in Ihren Content-Deklarationsdateien.

Falls nicht gesetzt, versucht Intlayer, den Format-Befehl automatisch zu erkennen. Durch den Versuch, die folgenden Befehle aufzulösen: prettier, biome, eslint.

### Konfiguration

Die `formatCommand`-Option akzeptiert eine String-Vorlage, wobei `{{file}}` durch den tatsächlichen Dateipfad ersetzt wird:
...
### Unterstützte Formatter

Sie können jeden Code-Formatter verwenden, der Dateipfade als Argumente akzeptiert:
...
- **Konsistente Formatierung**: Alle Inhaltsdateien werden automatisch gemäß den Stilrichtlinien Ihres Projekts formatiert
- **Developer Experience**: Keine manuelle Formatierung von Dateien nach dem Schreiben durch Intlayer erforderlich
- **Team-Konsistenz**: Stellt sicher, dass alle Teammitglieder die gleiche Formatierung auf Inhaltsdateien anwenden
- **CI/CD-Integration**: Inhaltsdateien behalten konsistente Formatierung in automatisierten Workflows bei

### Funktionsweise

Wenn Intlayer eine Content-Deklarationsdatei (`.content.ts`, `.content.js`, etc.) schreibt oder aktualisiert, führt es automatisch den angegebenen Format-Befehl auf der Datei aus. Der Platzhalter `{{file}}` wird durch den tatsächlichen Dateipfad ersetzt, und der Befehl wird im Basisverzeichnis des Projekts ausgeführt.

---

## Dictionary-Konfiguration: Bessere Organisation und Struktur

v7 führt einen neuen dedizierten `dictionary`-Konfigurationsbereich ein, der eine bessere Organisation für wörterbuchbezogene Einstellungen und verbessertes Content-Management bietet.

### Neue Dictionary-Konfigurationsstruktur

Die `fill`-Eigenschaft wurde aus dem `content`-Abschnitt in einen neuen `dictionary`-Abschnitt verschoben, was eine klarere Trennung der Verantwortlichkeiten ermöglicht:
...
- **Klarere Organisation**: Dictionary-spezifische Einstellungen sind nun zusammengefasst
- **Bessere Separation of Concerns**: Content Discovery vs. Dictionary-Operationen sind klar getrennt
- **Verbesserte Wartbarkeit**: Einfacher zu verstehen und zu modifizieren Dictionary-bezogene Konfigurationen
- **Zukünftige Erweiterbarkeit**: Der Dictionary-Bereich kann zusätzliche Dictionary-spezifische Einstellungen aufnehmen
- **Umfassendes Dictionary-Management**: Umfasst Eigenschaften wie `title`, `live`, `priority`, `tags`, `version` und `description` zum Erstellen und Verwalten neuer Dictionaries

### Konfigurationsverwendung

Die Wörterbuchkonfiguration dient zwei Hauptzwecken:

1. **Standardwerte**: Definieren Sie Standardwerte beim Erstellen von Content-Deklarationsdateien
2. **Fallback-Verhalten**: Stellen Sie Fallback-Werte bereit, wenn bestimmte Felder nicht definiert sind, sodass Sie das Wörterbuch-Betriebsverhalten global definieren können

Der Wörterbuchbereich umfasst umfassende Eigenschaften zur Wörterbuchverwaltung:

- **`fill`**: Auto-Fill-Verhalten für die Content-Generierung
- **`title`**: Standardtitel für neue Wörterbücher
- **`live`**: Live-Sync-Konfiguration für Echtzeit-Updates
- **`priority`**: Prioritätseinstellungen für die Wörterbuchauflösung
- **`tags`**: Standard-Tags für die Content-Organisation
- **`version`**: Versionsverwaltung für Wörterbuch-Updates
- **`description`**: Standardbeschreibung für neue Inhalte
...
- **`editor.formatCommand`**: Neue Option für automatische Code-Formatierung von Content-Dateien
