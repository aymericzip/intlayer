...
## Vite Plugin: Gebündelter Compiler & Proxy

Das `intlayer()` Vite-Plugin bündelt nun den **Compiler** und den **Locale-Routing Proxy** direkt, sodass die meisten Projekte nur noch ein einziges Plugin in `vite.config.ts` benötigen:
...
- **Compiler**: Wird automatisch aktiviert, wenn `compiler.enabled` gesetzt ist und ein `compiler.output`-Pfad konfiguriert ist. Du musst `intlayerCompiler()` nicht mehr separat registrieren.
- **Proxy**: Wird automatisch basierend auf der neuen Option `routing.enableProxy` aktiviert (`true` als Standard). Es verdrahtet die Middleware zur Locale-Erkennung / Umleitung / Umschreibung für Development, Preview und Production SSR. Du musst `intlayerProxy()` nicht mehr separat registrieren.

### Die `routing.enableProxy` Option

Eine neue Option `routing.enableProxy` steuert, ob der Locale-Routing Proxy eingesteckt wird. Der Standard ist `true`. Deaktiviere sie, wenn du das Locale-Routing selbst übernehmen möchtest:
...
Die eigenständigen Plugins `intlayerCompiler()` und `intlayerProxy()` bleiben für erweiterte Setups exportiert. Sie neben `intlayer()` zu registrieren ist sicher — jedes Plugin dedupliziert sich selbst und wird nur einmal ausgeführt.
