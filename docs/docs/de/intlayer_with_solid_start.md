...
<Question title="Welche verschiedenen Lösungen gibt es, um eine Solid-Start-App zu internationalisieren?">

- **`@solid-primitives/i18n`**: das Community-Primitiv, ein flaches Wörterbuch, das Sie selbst zusammensetzen, laden und typisieren.
- **`i18next`** mit einem Solid-Wrapper: ausgereifte Kataloge, aber nichts für Locale-bewusstes Routing oder Server-Rendering in Solid Start.
- **`Intlayer`**: Inhalte werden neben jeder Komponente deklariert und zur Build-Zeit kompiliert, mit lokalisierten Routen, serverseitiger Locale-Auflösung, Canonical- und hreflang-Links, einer mehrsprachigen Sitemap, KI-Übersetzung, visuellem Editor und CMS.
