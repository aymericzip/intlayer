...
<AccordionGroup>
<Accordion header="Vollständige analoge Abdeckung">
...
<Step number={6} title="Ändern der Sprache Ihrer Inhalte" isOptional={true}>
...
<Question title="Welche verschiedenen Lösungen gibt es, um eine Analog-App zu internationalisieren?">

Analog ist ein Angular-Meta-Framework auf Basis von Vite, sodass es die Angular-Optionen erbt und die Vite-Optionen hinzufügt:

- **`@angular/localize`**: XLIFF-Extraktion mit einem kompilierten Build pro Locale, was schlecht zu einem dateibasierten Router und Server-Rendering passt.
- **`ngx-translate`** und **`Transloco`**: JSON-Kataloge zur Laufzeit über einen Service, ohne Integration mit dem Routing oder dem serverseitigen Rendering von Analog.
- **`Intlayer`**: Inhalte werden neben jeder Komponente deklariert und vom Vite-Plugin zur Build-Zeit kompiliert, vollständig typisiert, mit Locale-Wechsel zur Laufzeit, KI-Übersetzung, visuellem Editor und CMS.

Siehe [warum Intlayer](https://github.com/aymericzip/intlayer/blob/main/docs/docs/de/interest_of_intlayer.md) und den [Angular-Leitfaden](https://github.com/aymericzip/intlayer/blob/main/docs/docs/de/intlayer_with_angular_21.md) für die Angular-spezifischen APIs.

</Question>

<Question title="Wie viel trägt i18n zu meiner Analog-Bundle-Größe bei?">
...
<Question title="Funktioniert es mit dem serverseitigen Rendering von Analog und Vite?">

Ja. Das `intlayer()`-Vite-Plugin kompiliert Ihre Inhalte und überwacht sie in der Entwicklung, und die Locale wird auf dem Server aufgelöst, sodass die erste HTML-Antwort bereits in der richtigen Sprache ist. Vorgerenderte Routen lösen ihren Inhalt zur Build-Zeit auf.
