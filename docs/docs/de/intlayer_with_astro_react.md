...
Sie sollten außerdem SEO-Metadaten (wie hreflang und Canonical-Links) zu jeder Seite hinzufügen und ein React-Island für interaktive clientseitige Inhalte einführen.
...
<Step number={6} title="Erstellen Sie eine React-Island-Komponente">
...
<Question title="Welche verschiedenen Lösungen gibt es, um eine Astro-Website mit React-Islands zu internationalisieren?">

Die eingebaute `i18n`-Option von Astro kümmert sich um Locale-Präfixe und Weiterleitungen, aber nicht um den Inhalt selbst, sodass Sie weiterhin eine Message-Ebene brauchen, und eine Island bringt ein zweites Problem: Die Island läuft mit React, nicht mit Astro.

- **Astro `i18n` plus handgeschriebene Wörterbücher**, mit **`react-i18next`** innerhalb der Island: zwei Inhaltsquellen, die synchron gehalten werden müssen, und keine gemeinsame Typisierung dazwischen.
- **`Intlayer`**: eine Inhaltsebene für beides. `astro-intlayer` deckt die `.astro`-Seiten ab und `react-intlayer` die React-Islands, die dieselben Deklarationen lesen.
...
<Question title="Kann ich von `react-i18next` migrieren, ohne meine Komponenten neu zu schreiben?">
...
<Question title="Brauche ich eine separate i18n-Bibliothek innerhalb meiner React-Island?">

Nein. `react-intlayer` liest dieselben Wörterbücher wie die Astro-Seite, sodass Sie `react-i18next` nicht daneben installieren. Schritt 6 zeigt, wie die Island-Komponente die aktive Locale von der Seite erhält, statt sie erneut aufzulösen.
