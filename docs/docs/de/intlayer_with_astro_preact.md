...
priority: 9
title: "Astro + Preact i18n - Vollständiger Leitfaden zur Übersetzung Ihrer App"
description: "Kein i18next mehr. Der 2026-Leitfaden zum Erstellen einer mehrsprachigen (i18n) Astro + Preact-App. Übersetzen Sie mit KI-Agenten und optimieren Sie Bundle-Größe, SEO und Performance."
keywords:
...
Sie sollten außerdem SEO-Metadaten (wie hreflang und Canonical-Links) zu jeder Seite hinzufügen und ein Preact-Island für interaktive clientseitige Inhalte einführen.
...
<Question title="Welche verschiedenen Lösungen gibt es, um eine Astro-Website mit Preact-Islands zu internationalisieren?">

Die eingebaute `i18n`-Option von Astro kümmert sich um Locale-Präfixe und Weiterleitungen, aber nicht um den Inhalt selbst, sodass Sie weiterhin eine Message-Ebene brauchen, und eine Island bringt ein zweites Problem: Die Island läuft mit Preact, nicht mit Astro.

- **Astro `i18n` plus handgeschriebene Wörterbücher**, mit **`preact-i18n`** innerhalb der Island: zwei Inhaltsquellen, die synchron gehalten werden müssen, und keine gemeinsame Typisierung dazwischen.
- **`Intlayer`**: eine Inhaltsebene für beides. `astro-intlayer` deckt die `.astro`-Seiten ab und `preact-intlayer` die Preact-Islands, die dieselben Deklarationen lesen.
...
<Question title="Brauche ich eine separate i18n-Bibliothek innerhalb meiner Preact-Island?">

Nein. `preact-intlayer` liest dieselben Wörterbücher wie die Astro-Seite, sodass Sie `preact-i18n` nicht daneben installieren. Schritt 6 zeigt, wie die Island-Komponente die aktive Locale von der Seite erhält, statt sie erneut aufzulösen.
