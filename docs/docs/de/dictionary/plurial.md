...
| Anwendungsfall                                                           | Helper   |
| ------------------------------------------------------------------------ | -------- |
| Sprachspezifische grammatikalische Pluralformen (ein Apfel / zwei Äpfel) | `plural` |
| Benutzerdefinierte Zahlenbereiche (`<5`, `>=10`) oder Nicht-CLDR-Gruppen | `enu`    |

Wenn Sie nur Englisch unterstützen (das nur `one` / `other` hat), funktioniert beides. Für jede Sprache mit `few` / `many` / `two` Unterscheidungen ist `plural` vorzuziehen.
...
Um Pluralinhalt in Next.js Client Components zu verwenden, rufen Sie ihn über den `useIntlayer` Hook ab und rufen Sie ihn mit einer Anzahl auf. Hier ist ein Beispiel:
...
Um Pluralinhalte in Vue-Komponenten zu verwenden, rufen Sie diese über den `useIntlayer`-Hook ab und rufen Sie sie mit einer Anzahl auf. Hier ist ein Beispiel:
...
Um Pluralinhalt in Svelte-Komponenten zu verwenden, rufen Sie ihn über den `useIntlayer` Hook ab und rufen ihn mit einer Anzahl auf. Der Store wird mit `$` aufgerufen. Hier ist ein Beispiel:
...
Um Pluralinhalte in Preact-Komponenten zu verwenden, rufen Sie diese über den `useIntlayer`-Hook ab und rufen Sie ihn mit einer Anzahl auf. Hier ist ein Beispiel:
...
Um Pluralinhalt in SolidJS-Komponenten zu verwenden, rufen Sie ihn über den `useIntlayer`-Hook ab und rufen ihn mit einer Anzahl auf. Hier ist ein Beispiel:
...
Um Pluralinhalt in Angular-Komponenten zu verwenden, rufen Sie ihn über den `useIntlayer` Hook ab und rufen Sie ihn mit einer Anzahl auf. Hier ist ein Beispiel:
