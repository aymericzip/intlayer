...
<Step number={8} title="Ändern der URL, wenn sich die Lokalisierung ändert" isOptional={true}>
...
<Question title="Welche verschiedenen Lösungen gibt es, um ein Create-React-App-Projekt zu internationalisieren?">
...
- **`Intlayer`**: Inhalte werden neben jeder Komponente deklariert und zur Build-Zeit über `react-scripts-intlayer` kompiliert, vollständig typisiert, mit KI-Übersetzung, visuellem Editor und CMS.
...
<Question title="Create React App wird nicht mehr gepflegt. Sollte ich zuerst zu Vite wechseln?">
