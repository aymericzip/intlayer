...
In dieser Architektur stellt `preact-intlayer` den in `main.tsx` eingebundenen `IntlayerProvider` bereit, um den Preact-Baum zu umschließen. Inhaltsdeklarationen befinden sich in `src/` neben den Komponenten.
