...
In dieser Architektur sind alle lokalisierten Routen unter dem Routensegment `{-$locale}` verschachtelt. Dieser Ansatz stellt sicher, dass jede Sprache eine dedizierte URL hat, und ermöglicht gleichzeitig ein automatisches Locale-Präfix, Validierung und SEO-Optimierung.
...
<Step number={15} title="Inhalt Ihrer Komponenten extrahieren" isOptional={true}> isOptional={true}>
