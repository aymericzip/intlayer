...
title: "Astro + Solid i18n - Vollständiger Leitfaden zur Übersetzung Ihrer App"
description: "Kein i18next mehr. Der 2026-Leitfaden zum Erstellen einer mehrsprachigen (i18n) Astro + Solid-App. Übersetzen Sie mit KI-Agenten und optimieren Sie Bundle-Größe, SEO und Performance."
keywords:
...
# Übersetzen Sie Ihre Astro + Solid Website mit Intlayer | Internationalisierung (i18n)
...
## Schritt-für-Schritt-Anleitung zur Konfiguration von Intlayer in Astro + Solid
...
- **solid-js**
  Das Kern-Solid-Paket.

- **solid-intlayer**
  Paket zur Integration von Intlayer in Solid-Anwendungen. Es stellt den `IntlayerProvider` sowie die Primitiven `useIntlayer` und `useLocale` für die Internationalisierung in Solid bereit.

- **@astrojs/solid-js**
  Offizielle Astro-Integration, die die Verwendung von Solid-Komponenten-Islands ermöglicht.
...
Fügen Sie das `intlayer`-Plugin und die Solid-Integration zu Ihrer Astro-Konfiguration hinzu.
...
> Die `solid()`-Integration ermöglicht es Ihnen, Solid-Komponenten-Islands über `client:only="solid-js"` zu verwenden.
...
Sie sollten außerdem SEO-Metadaten (wie hreflang und Canonical-Links) zu jeder Seite hinzufügen und ein Solid-Island für interaktive clientseitige Inhalte einführen.
...
<html lang={locale} dir={getHTMLTextDir(locale)}>
  ...
  <body>
    <!-- Das Solid-Island rendert alle interaktiven Inhalte, einschließlich des Sprachumschalters -->
...
<Step number={6} title="Erstellen Sie eine Solid-Island-Komponente">

Erstellen Sie eine Island-Komponente, die Ihre Solid-Anwendung umschließt und die vom Server erkannte Sprache empfängt:
...
Erstellen Sie eine Solid-Komponente `LocaleSwitcher`, die die verfügbaren Sprachen liest und bei Auswahl einer neuen Sprache durch den Benutzer zur lokalisierten URL navigiert:
...
> **Hinweis zur Solid-Reaktivität:**
> In Solid ist `locale` ein Reaktivitäts-Signal-Accessor - rufen Sie ihn immer als `locale()` auf, um den aktuellen Wert zu erhalten.
...
<Question title="Welche verschiedenen Lösungen gibt es, um eine Astro-Website mit Solid-Islands zu internationalisieren?">

Die eingebaute `i18n`-Option von Astro kümmert sich um Locale-Präfixe und Weiterleitungen, aber nicht um den Inhalt selbst, sodass Sie weiterhin eine Message-Ebene brauchen, und eine Island bringt ein zweites Problem: Die Island läuft mit Solid, nicht mit Astro.

- **Astro `i18n` plus handgeschriebene Wörterbücher**, mit **`@solid-primitives/i18n`** innerhalb der Island: zwei Inhaltsquellen, die synchron gehalten werden müssen, und keine gemeinsame Typisierung dazwischen.
- **`Intlayer`**: eine Inhaltsebene für beides. `astro-intlayer` deckt die `.astro`-Seiten ab und `solid-intlayer` die Solid-Islands, die dieselben Deklarationen lesen.
...
<Question title="Brauche ich eine separate i18n-Bibliothek innerhalb meiner Solid-Island?">

Nein. `solid-intlayer` liest dieselben Wörterbücher wie die Astro-Seite, sodass Sie `@solid-primitives/i18n` nicht daneben installieren. Schritt 6 zeigt, wie die Island-Komponente die aktive Locale von der Seite erhält, statt sie erneut aufzulösen.

</Question>
<Question title="Woher weiß die Island, welche Locale die Seite rendert?">

Die Astro-Seite reicht sie als Prop weiter und der Intlayer-Provider in der Island greift sie auf, sodass die Island in derselben Sprache hydratisiert, die der Server gerendert hat. Das verhindert das Aufblitzen der Standardsprache, das entsteht, wenn eine Island die Locale im Browser selbst erkennt.

</Question>
<Question title="Wird der übersetzte Inhalt als statisches HTML ausgeliefert?">

Ja für die Astro-Teile, die zur Build-Zeit gerendert werden, sodass lokalisierte Seiten reines statisches HTML sind, das Crawler ohne Ausführung von JavaScript lesen. Nur die Islands erhalten ein Wörterbuch, und nur für die Locale, die sie rendern.
...
Schritt 8 behandelt `sitemap.xml` und `robots.txt`. `getMultilingualUrls` erstellt die Alternativen für jede deklarierte Locale, einschließlich `x-default`, was Suchmaschinen verwenden, um die richtige Sprachversion auszuliefern.
