...
## Pliki zawartości YAML i Markdown (v8.10.0)

Możesz teraz deklarować zawartość bezpośrednio przy użyciu rozszerzeń plików `.content.md` i `.content.yaml`. Jest to idealne rozwiązanie dla długich treści, blogów, dokumentacji i polityk prywatności.

Pliki te są w pełni edytowalne z poziomu Edytora Wizualnego (Visual Editor), co zapewnia usprawniony przepływ pracy dla nietechnicznych członków zespołu.

Więcej szczegółów znajdziesz w [Dokumentacji plików zawartości YAML](https://github.com/aymericzip/intlayer/blob/main/docs/docs/pl/dictionary/content_file.md#yaml-content-file) oraz [Dokumentacji plików zawartości Markdown](https://github.com/aymericzip/intlayer/blob/main/docs/docs/pl/dictionary/markdown.md).
...
## Reguły liczby mnogiej Unicode CLDR (v8.8.0)

Wprowadzono nowy helper węzła `plural` oparty na regułach liczby mnogiej zdefiniowanych przez projekt Unicode CLDR, co poprawia obsługę złożonych języków.
...
Możesz następnie przekazać liczbę dynamicznie w komponencie swojego frameworka:
...
Więcej informacji znajdziesz w [Dokumentacji pluralizacji](https://github.com/aymericzip/intlayer/blob/main/docs/docs/pl/dictionary/plurial.md).
...
## TypeScript: Akcesory prymitywów IntlayerNode

Typ `IntlayerNode` został zaktualizowany, aby umożliwić wywoływanie prymitywnych metod JavaScript bezpośrednio na węzłach zawartości. Umożliwia to interakcję ze zlokalizowaną zawartością bezpośrednio tak, jakby była standardowym typem prymitywnym string, number, boolean lub array.

Wspierane w React, Preact, Solid, Svelte, Vue i Angular.

### Przykład dla String

```typescript
content.placeholder; // Zwraca IntlayerNode<string>
content.placeholder.value; // Zwraca string
content.placeholder.toString(); // Zwraca string
content.placeholder.toLowerCase(); // Zwraca string
String(content.placeholder); // Zwraca string
content.placeholder.toUpperCase(); // Zwraca string z wielkimi literami
content.placeholder.replace("a", "b"); // Zwraca zmodyfikowany string
```

### Przykład dla Array

```typescript
content.myArrayOfString             // Zwraca IntlayerNode<Array<string>>
content.myArrayOfString.find(...)   // Zwraca element
content.myArrayOfString.join(', ')  // Zwraca połączony string
```
...
## Narzędzia: Watcher C++ i LSP oparty na OXC (v8.12.0)

Intlayer v8.12.0 przynosi znaczące ulepszenia w doświadczeniu deweloperskim:

- **Parcel Watcher**: Watcher zawartości został zmigrowany z `chokidar` do `@parcel/watcher`, wykorzystując natywne śledzenie plików w C++, aby zapewnić szybsze aktualizacje i znacznie niższe zużycie pamięci.
- **Nowy Language Server Protocol (LSP)**: Dostępny jest teraz zupełnie nowy LSP. Zbudowany w oparciu o parsowanie OXC, pomaga Twojemu IDE i agentom AI bezproblemowo łączyć wywołania `useIntlayer('my-key')` z odpowiadającymi im plikami `.content` i odwrotnie.

Więcej szczegółów znajdziesz w [Dokumentacji LSP](https://github.com/aymericzip/intlayer/blob/main/docs/docs/pl/lsp.md).
...
## Eksperymentalne optymalizacje budowania: Minify i Purge (v8.7.0)

Intlayer v8.7.0 wprowadza nowe eksperymentalne funkcje budowania w Twoim `intlayer.config.ts`, zaprojektowane w celu optymalizacji produkcyjnego bundle'a:

- **Minifikacja słowników (Dictionary Minification)**: Minifikuje pliki słowników w celu zmniejszenia ich rozmiaru.
- **Usuwanie nieużywanych kluczy (Unused Key Purging)**: Skanuje i usuwa nieużywane klucze tłumaczeń ze słowników, zmieniając nazwy aktywnych kluczy na krótkie znaki (np. `"products" -> "a"`, `"pricing" -> "b"`). Zmniejsza to rozmiar bundle'a nawet o 5%.
- **Wyłączenie sprawdzania TypeScript**: Przyspiesza budowanie poprzez wyłączenie sprawdzania typów TypeScript podczas etapu kompilacji.
...
Jeśli wcześniej ustawiłeś `contentDir`, Intlayer v8 użyje go również jako domyślnego dla `codeDir`, ale zaloguje ostrzeżenie. Powiniensy jawnie zdefiniować `codeDir` w swojej konfiguracji.
...
## Wsparcie dla Angulara: Vite Bundler (v8.11.0)

Dla użytkowników Angulara, Intlayer wspiera teraz natywnie bundler Vite poprzez nowy, niestandardowy plugin esbuild, co przyspiesza czas programowania i budowania.

Szczegóły dotyczące konfiguracji znajdziesz w [Dokumentacji środowiska Angular](https://github.com/aymericzip/intlayer/blob/main/docs/docs/pl/intlayer_with_angular_21.md).

## Generowanie mapy strony - Sitemap (v8.6.0)

Intlayer generuje teraz automatycznie mapy stron (sitemaps) na podstawie Twoich lokalizacji (locales) i trybu routingu. Jest to szczególnie przydatne do optymalizacji SEO w architekturach routerów takich jak TanStack Router:
