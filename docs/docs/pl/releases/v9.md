...
## Plugin Vite: Wbudowany Compiler i Proxy

Plugin Vite `intlayer()` ma teraz wbudowany bezpośrednio **compiler** i **locale-routing proxy**, dzięki czemu większość projektów potrzebuje tylko jednego pluginu w `vite.config.ts`:
...
- **Compiler**: Aktywuje się automatycznie po ustawieniu opcji `compiler.enabled` oraz podaniu ścieżki w `compiler.output`. Nie musisz już oddzielnie rejestrować `intlayerCompiler()`.
- **Proxy**: Aktywuje się automatycznie w oparciu o nową opcję `routing.enableProxy` (domyślnie `true`). Konfiguruje on middleware do detekcji / redirectu / przepisywania języków w development, preview i production SSR. Nie musisz już oddzielnie rejestrować `intlayerProxy()`.

### Opcja `routing.enableProxy`

Nowa opcja `routing.enableProxy` kontroluje wpięcie proxy do routingu dla locales. Domyślnie wynosi `true`. Wyłącz ją, jeśli chcesz zarządzać routingiem samodzielnie:
...
Samodzielne pluginy `intlayerCompiler()` i `intlayerProxy()` wciąż pozostają eksportowane dla zaawansowanych środowisk. Zarejestrowanie ich obok `intlayer()` jest w pełni bezpieczne — każdy plugin deduplikuje się sam i jest uruchamiany tylko raz.
