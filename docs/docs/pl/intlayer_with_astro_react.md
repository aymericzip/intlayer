...
- **react**, **react-dom**
  Podstawowe pakiety React używane do renderowania komponentów React w przeglądarce.

- **react-intlayer**
  Pakiet do integracji Intlayer z aplikacjami React. Zapewnia `IntlayerProvider` oraz hooki `useIntlayer` i `useLocale` dla międzynarodowości w React.

- **@astrojs/react**
  Oficjalna integracja Astro pozwalająca na używanie islandów komponentów React.
...
<Step number={6} title="Utworzenie komponentu React Island">
...
<Question title="Jakie są różne rozwiązania dostępne do internacjonalizacji witryny Astro z wyspami (islands) React?">
...
<Question title="Czy mogę zmigrować z `react-i18next` bez przepisywania moich komponentów?">
...
<Question title="Czy potrzebuję osobnej biblioteki i18n wewnątrz mojej wyspy (island) React?">

Nie. Pakiet `react-intlayer` czyta te same słowniki co strona Astro, więc nie instalujesz obok `react-i18next`. Krok 6 pokazuje komponent wyspy odbierający aktywny język ze strony zamiast rozwiązywać go ponownie.
