...
<AccordionGroup>
<Accordion header="Pełne pokrycie Preact">
...
W tej architekturze `preact-intlayer` zapewnia `IntlayerProvider` zamontowany w `main.tsx`, aby opakować drzewo Preact. Deklaracje zawartości znajdują się w `src/` obok komponentów.
...
Intlayer wykorzystuje moduł augmentacji, aby czerpać korzyści z TypeScript i wzmocnić swoją bazę kodu.

![Autokompletion](https://github.com/aymericzip/intlayer/blob/main/docs/assets/autocompletion.png?raw=true)

![Błąd tłumaczenia](https://github.com/aymericzip/intlayer/blob/main/docs/assets/translation_error.png?raw=true)

Upewnij się, że konfiguracja TypeScript zawiera autogenerowane typy.
...
```json5 fileName="tsconfig.json"
{
  ...
  "compilerOptions": {
    // ...
    "jsx": "react-jsx",
    "jsxImportSource": "preact", // Rekomendowane dla Preact 10+
    // ...
  },
  "include": [
    // ... Twoje istniejące konfiguracje TypeScript
...
> Upewnij się, że `tsconfig.json` jest skonfigurowany dla Preact, szczególnie `jsx` i `jsxImportSource` lub `jsxFactory`/`jsxFragmentFactory` dla starszych wersji Preact, jeśli nie używasz domyślnych ustawień `preset-vite`.
...
<Question title="Jakie są różne rozwiązania dostępne do internacjonalizacji aplikacji Vite i Preact?">

Vite nie narzuca podejścia do i18n, więc wybór zależy od ekosystemu Preact:

- **`preact-i18n`** lub **`react-i18next`**: popularne biblioteki oparte na przestrzeniach nazw ładowanych w czasie wykonywania.
