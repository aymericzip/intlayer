...
title: Vite + Svelte i18n - Kompletny przewodnik po tłumaczeniu Svelte
description: Najlepsze rozwiązanie dla rozmiaru bundle, SEO, wydajności & utrzymania. Uczyń swój Vite and Svelte aplikację wielojęzycznym w 2026, tłumaczenie LLM, Agent Skills & MCP.
keywords:
  - Internacjonalizacja
  - Dokumentacja
  - Intlayer
...
> Twoje deklaracje zawartości mogą być zdefiniowane w dowolnym miejscu w Twojej aplikacji, pod warunkiem, że zostaną umieszczone w katalogu `contentDir` (domyślnie `./src`). I będą miały rozszerzenie pliku deklaracji zawartości (domyślnie `.content.{json,ts,tsx,js,jsx,mjs,cjs}`).
