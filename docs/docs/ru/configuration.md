...
const config: IntlayerConfig = {
  ...
  analytics: {
     ...
     * Требует, чтобы был задан `editor.clientId` для атрибуции.
     * По умолчанию: false
     */
    enabled: true,
...
Аналитика строго опциональна (opt-in): ничего не собирается, если `analytics.enabled` явно не установлен в `true` **и** не настроен ключ проекта (`editor.clientId`) для атрибуции. При отключении (по умолчанию) вся интеграция аналитики удаляется из сборки вашего приложения (dead-code elimination).

| Поле            | Описание                                                                   | Тип       | По умолчанию | Пример  | Примечание                                                                                                                        |
| --------------- | -------------------------------------------------------------------------- | --------- | ------------ | ------- | --------------------------------------------------------------------------------------------------------------------------------- |
| `enabled`       | Включает сбор аналитики (просмотры страниц, показы контента, события A/B). | `boolean` | `false`      | `true`  | Требует, чтобы был задан `editor.clientId` для атрибуции; иначе аналитика остаётся отключённой, даже если `enabled` равно `true`. |
