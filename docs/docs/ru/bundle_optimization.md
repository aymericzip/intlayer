...
| **`minify`**          | `boolean`                        | `false`                         | Указывает, нужно ли минифицировать словари для уменьшения размера бандла.                                                                                                                              |
| **`purge`**           | `boolean`                        | `false`                         | Указывает, нужно ли удалять неиспользуемые ключи в словарях.                                                                                                                                           |
...
## Минификация и очистка словарей

Intlayer предоставляет опции для дальнейшей оптимизации вашего бандла путем минификации словарей и очистки неиспользуемых ключей (purge).

### Минификация (Minification)

Минификация словарей удаляет ненужные пробелы, комментарии и уменьшает размер JSON-контента. Это особенно полезно для больших словарей.
...
Очистка гарантирует, что в финальный бандл словарей будут включены только те ключи, которые действительно используются в вашем коде. Это может значительно сократить размер вашего бандла, если у вас есть большие словари со множеством ключей, которые не используются во всех частях вашего приложения.
...
> Примечание: Очистка игнорируется, если `optimize` отключен.
