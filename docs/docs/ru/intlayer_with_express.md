...
<Question title="Какие существуют решения для интернационализации бэкенда на Express?">

Исторический вариант — это `i18next` с `i18next-http-middleware`, который загружает каталоги JSON для каждого пространства имён и хранит локаль на запросе. Альтернатива — `Intlayer` через `express-intlayer`, который объявляет контент в типизированных файлах, общих с вашим фронтендом, разрешает локаль для каждого запроса и добавляет ИИ-перевод и CMS.
...
<Question title="Работает ли это с существующим приложением Express и другими middleware?">

Да. `express-intlayer` — это стандартный middleware Express, поэтому он компонуется с вашим существующим стеком. Регистрируйте его перед маршрутами, которые читают контент, чтобы локаль была разрешена к моменту, когда обработчик вызывает `t()` или `getIntlayer()`.
