...
title: "Vite + Preact i18n - Полное руководство по переводу вашего приложения"
description: "Больше никакого i18next. Руководство 2026 по созданию многоязычного (i18n) приложения Vite + Preact. Переводите с помощью ИИ-агентов и оптимизируйте размер бандла, SEO и производительность."
keywords:
  - Интернационализация
  - Документация
  - Intlayer
...
<AccordionGroup>
<Accordion header="Полное покрытие Preact">
...
В этой архитектуре `preact-intlayer` предоставляет `IntlayerProvider`, смонтированный в `main.tsx` для оборачивания дерева Preact. Объявления контента находятся в `src/` рядом с компонентами.
...
> Ваши объявления контента могут быть определены в любом месте вашего приложения, при условии, что они включены в каталог `contentDir` (по умолчанию `./src`). И соответствуют расширению файла объявлений контента (по умолчанию `.content.{json,ts,tsx,js,jsx,mjs,cjs,md,mdx,yaml,yml}`).
...
<Step number={5} title="Использование Intlayer в коде">
...
<Step number={6} title="Изменение языка контента" isOptional={true}>
...
<Step number={10} title="Создание компонента локализованной ссылки" isOptional={true}>
...
<Step number={11} title="Рендеринг Markdown и HTML" isOptional={true}>
...
Intlayer использует модульное расширение, чтобы получить преимущества TypeScript и сделать ваш codebase более надежным.
...
> Убедитесь, что ваш `tsconfig.json` настроен для Preact, особенно `jsx` и `jsxImportSource` или `jsxFactory`/`jsxFragmentFactory` для более старых версий Preact, если вы не используете значения по умолчанию `preset-vite`.
...
<Question title="Какие существуют решения для интернационализации приложения Vite и Preact?">

У Vite нет мнения об i18n, поэтому выбор идёт из экосистемы Preact:

- **`preact-i18n`**: небольшая библиотека, специфичная для Preact, со словарём JSON.
- **`react-i18next`** через `preact/compat`: зрелая, но она тянет слой совместимости с React в ваш бандл.
...
<Question title="Насколько i18n увеличивает размер бандла моего Preact?">
