...
В этой архитектуре `angular-intlayer` предоставляет `provideIntlayer()`, зарегистрированный в `src/app/app.config.ts`, чтобы каждый компонент мог читать свой контент через сигнал `useIntlayer`. Angular CLI переключается на кастомный сборщик esbuild, а `esbuild.plugins.ts` регистрирует плагин Intlayer в сборке для отслеживания и пересборки объявлений контента. Объявления контента размещаются рядом с компонентами в `src/app/`.
...
<Step number={4} title="Объявите свой контент">
...
Подробнее о том, как использовать расширение, см. [в документации к расширению Intlayer для VS Code](https://github.com/aymericzip/intlayer/blob/main/docs/docs/ru/vs_code_extension.md).
...
<Question title="Какие существуют решения для интернационализации приложения Angular?">
...
Нет. Это модель `@angular/localize`. С Intlayer единая сборка обслуживает каждую объявленную локаль, активный язык разрешается во время выполнения из URL, cookie или заголовка `Accept-Language`, и загружается только контент отрендеренной локали.
...
<Question title="Как автоматически перевести приложение Angular с помощью ИИ?">
