...
- `'local&remote'`: Словарь управляется как локально, так и удалённо. После отправки в CMS он останется синхронизированным—изменения из локального файла отправляются в CMS, а удалённые изменения могут быть получены обратно в локальный файл.
...
- `'live'`: Словарь импортируется динамически с использованием API живой синхронизации.
