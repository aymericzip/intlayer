...
{
  key: "about-page",
  location: "local", // Контент остаётся только в вашей кодовой базе
  content: {
    title: "About Us"
  }
}
...
- `'static'`: Словарь импортируется статически во время сборки (build time). Это режим по умолчанию.
...
Если установлено, это свойство переопределяет глобальный `importMode`, определённый в `the `dictionary`property of`intlayer.config.ts``.
...
Для удалённых словарей этот массив содержит идентификаторы проектов, которые могут использовать этот словарь. Удалённый словарь может быть общим для нескольких проектов.
...
3. **Производительность**:
   ...
   - Убедитесь, что плагин трансформации сборки (`@intlayer/swc` или `@intlayer/babel`) включен для оптимизации словаря во время сборки (build time)
