...
# Содержимое файлов / Встраивание файлов в Intlayer

В Intlayer функция `file` позволяет встраивать содержимое внешних файлов в словарь. Такой подход гарантирует, что Intlayer распознает исходный файл, обеспечивая безупречную интеграцию с Visual Editor и CMS Intlayer.
...
При использовании функции `file` вы можете использовать разные типы путей для указания файла, который нужно встроить.

- `file("./path/to/file.txt")` - Относительный путь к текущему файлу
- `file("path/to/file.txt")` - Относительный путь к корневому каталогу проекта
- `file("/users/username/path/to/file.txt")` - Абсолютный путь
