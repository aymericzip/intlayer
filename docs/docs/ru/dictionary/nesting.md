...
В Intlayer вложенность достигается через функцию `nest`, которая позволяет вам ссылаться на содержимое из другого словаря и переиспользовать его. Вместо дублирования содержимого, вы можете указать на существующий модуль содержимого по его ключу.
