...
В этой архитектуре `svelte-intlayer` предоставляет реактивные хранилища и утилиты для управления переводами внутри компонентов Svelte. Объявления контента находятся в `src/` рядом с компонентами.
