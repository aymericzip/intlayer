...
title: Vite + Svelte i18n - Полное руководство по переводу Svelte
description: Лучшее решение для размера бандла, SEO, производительности & поддерживаемости. Сделайте Vite and Svelte приложение многоязычным в 2026, перевод LLM, Agent Skills & MCP.
keywords:
  - Интернационализация
  - Документация
  - Intlayer
...
> Ваши объявления контента могут быть определены в любом месте вашего приложения, как только они будут включены в директорию `contentDir` (по умолчанию, `./src`). И соответствовать расширению файла объявления контента (по умолчанию, `.content.{json,ts,tsx,js,jsx,mjs,cjs}`).
