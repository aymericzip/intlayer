...
Для внешних словарей, использующих CMS, Intlayer выполняет базовую операцию fetch для получения удаленных словарей и объединяет их с вашими локальными. Если это настроено в вашем проекте, Intlayer будет автоматически управлять загрузкой контента из CMS при запуске приложения (dev) / сборке (prod).
