...
- **Intlayer**: Обеспечивает лучшую производительность и наименьший размер бандла (v9.5.10) для TanStack Start.
...
- [`react-intlayer`](https://github.com/aymericzip/intlayer/blob/main/docs/docs/ru/packages/react-intlayer/exports.md) (v9.5.10)
- [`@intlayer/use-intl`](https://github.com/aymericzip/intlayer/blob/main/docs/docs/ru/compat/next-intl.md) (v9.5.10)
- [`@intlayer/lingui`](https://github.com/aymericzip/intlayer/blob/main/docs/docs/ru/compat/lingui.md) (v9.5.10)
- [`@intlayer/react-i18next`](https://github.com/aymericzip/intlayer/blob/main/docs/docs/ru/compat/react-i18next.md) (v9.5.10)
...
Я запустил одно и то же многоязычное приложение в реальном браузере для каждого стека, а затем записал, что фактически показалось в сети и сколько времени это заняло. Размеры указаны **после стандартного веб-сжатия**, потому что это ближе к тому, что люди скачивают, чем к подсчёту исходного кода.

- **Размер библиотеки интернационализации**: После bundling, tree-shaking и minification размер библиотеки i18n — это размер поставщиков (например `NextIntlClientProvider`) + код hooks (например `useTranslations`) в пустом компоненте. Это не включает загрузку файлов переводов. Это показывает, насколько дорогой является библиотека, прежде чем в игру вступит ваш контент.

- **JavaScript на странице**: Для каждого маршрута тестирования, сколько скрипта браузер загружает при каждом посещении, усреднённые по страницам в наборе (и по локалям, где отчёт их объединяет). Тяжёлые страницы — медленные страницы.

- **Утечка из других локалей**: Это контент той же страницы, но на другом языке, который по ошибке загружается на проверяемую страницу. Этот контент не нужен и его следует избегать. (например, контент страницы `/fr/about` в пакете страницы `/en/about`)

- **Утечка из других маршрутов**: То же самое для **других экранов** приложения: загружается ли их копия вместе с тем, когда вы открыли только одну страницу. (например, контент страницы `/en/about` в пакете страницы `/en/contact`). Высокий результат указывает на слабое разбиение или слишком широкие пакеты.

- **Средний размер пакета компонента**: Обычные элементы пользовательского интерфейса измеряются **по одному** вместо того, чтобы скрываться внутри одного гигантского числа приложения. Это показывает, скрывает ли интернационализация незаметно раздуванием обычные компоненты. Например, если ваш компонент переуходит, он загружает все эти данные из памяти. Привязка большого JSON к любому компоненту — это как подключение большого хранилища неиспользуемых данных, которое замедлит производительность ваших компонентов.

- **Скорость переключения языка**: Я переключаю язык с помощью собственного элемента управления приложения и замеряю, сколько времени проходит до того, как страница явно переключится, что заметит посетитель, а не лабораторный микро-шаг.

- **Работа по рендерингу после смены языка**: Более узкое продолжение: сколько усилий затратил интерфейс на перерисовку нового языка после того, как переключение началось. Полезно, когда "ощущаемое" время и стоимость framework расходятся.

- **Время первоначальной загрузки страницы**: От навигации до того момента, когда браузер считает страницу полностью загруженной для тестируемых сценариев. Хорошо подходит для сравнения холодных запусков.

- **Время гидратации**: Когда приложение это предоставляет, как долго клиент потратит на превращение серверного HTML во что-то, на что вы действительно можете нажать. Прочерк в таблицах означает, что эта реализация не предоставила надежный показатель гидратации в этом тесте.
...
- Для приложения весом около 111 КБ `gt-react` может добавить более 170 КБ сверху (порядок величины, наблюдаемый в реализации Next.js в том же бенчмарке).
...
Пакет довольно тяжелый (~11.1 kb, что более чем в 2.9 раза больше чем `react-intlayer`).
...
Пакет особенно тяжелый (~18.4kb, что примерно в 3.5× раз больше `react-intlayer`).
...
Пакет также тяжёлый (~14.4kb, что примерно в 3 раза больше, чем `react-intlayer`).
