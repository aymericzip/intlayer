...
description: Сравните библиотеки интернационализации (i18n) для Vue, такие как vue-i18n, fluent-vue, Tolgee и Intlayer. Подробный отчет о производительности, размере бандла, утечках и реактивности.
keywords:
  - benchmark
  - i18n
  - intl
...
history:
  ...
  - version: 9.5.7
    date: 2026-09-23
    changes: "Обновление результатов бенчмарка и добавление Tolgee"
  - version: 9.5.1
    date: 2026-09-11
    changes: "Обновление результатов бенчмарка"
  - version: 8.9.8
...
- **Intlayer**: Самое легкое решение (v9.5.10) со встроенным разделением (scoping) и динамической загрузкой.
- **Tolgee**: Эффективная динамическая загрузка с нулевыми утечками в динамическом режиме, но более тяжелое решение (~3.0× Intlayer) без встроенной типобезопасности при компиляции.
...
- [`vue-intlayer`](https://github.com/aymericzip/intlayer/blob/main/docs/docs/ru/packages/vue-intlayer/exports.md) (v9.5.10)
- [`@intlayer/vue-i18n`](https://github.com/aymericzip/intlayer/blob/main/docs/docs/ru/compat/vue-i18n.md) (v9.5.10)
...
`Tolgee` решает многие из упомянутых ранее проблем, предлагая динамическую загрузку, которая успешно устраняет утечки локалей и страниц (снижая JS страницы примерно до 58.8 КБ). Однако он не обеспечивает встроенной типобезопасности для ключей во время компиляции, что затрудняет обнаружение отсутствующих ключей. Кроме того, размер библиотеки относительно велик (~13.8 КБ, что примерно в 3.7 раза больше `vue-intlayer`).
