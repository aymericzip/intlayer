...
history:
  - version: 9.4.0
    date: 2026-08-22
    changes: "Обновление до архитектуры Next.js >= 9.4.0"
  - version: 10.0.0
    date: 2026-06-23
...
| ------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------ |
| `IntlayerProvider`       | Единый провайдер для Next.js App Router. Монтируется один раз в макете локали, инициализирует серверный контекст области запроса _и_ монтирует клиентский провайдер. (Intlayer >= 9.4)  | -                  |
| `IntlayerClientProvider` | **Устарело** — используйте `IntlayerProvider` из `next-intlayer/server`. Провайдер для клиентских компонентов в Next.js App Router. Оборачивает `IntlayerProvider` из `react-intlayer`. | -                  |
| `IntlayerServerProvider` | **Устарело** — используйте `IntlayerProvider` из `next-intlayer/server`. Предоставляет контекст локали на сервере. (Intlayer < 9.4)                                                     | -                  |
...
| `MarkdownRenderer`       | Рендерит контент в markdown с использованием кастомных компонентов.                                                                                                                     | -                  |
