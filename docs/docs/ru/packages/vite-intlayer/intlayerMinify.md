...
Когда `editor.enabled` имеет значение `true`, плагин продолжает работать, но **пропускает переименование полей**: переименование переписывает ключи контента, по которым проходит интерпретатор, поэтому `keyPath`, получаемый визуальным редактором, больше не будет совпадать с не объединёнными словарями, которые он редактирует. Сжатие пробелов и удаление метаданных верхнего уровня остаются активными.
