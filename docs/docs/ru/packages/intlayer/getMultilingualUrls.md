...
- `url: string`
  - **Description**: Исходная строка URL, которая будет префиксирована локалями.
  - **Type**: `string`
  - **Required**: Да
...
- **Режимы маршрутизации:**
  - `'prefix-no-default'`: Локаль по умолчанию без префикса, остальные с префиксом (например, `/dashboard`, `/fr/dashboard`)
  - `'prefix-all'`: Все локали с префиксами (например, `/en/dashboard`, `/fr/dashboard`)
  - `'no-prefix'`: Отсутствие префиксов локалей в URL (все локали возвращают одинаковый URL)
  - `'search-params'`: Локаль указывается через параметр запроса (например, `/dashboard?locale=fr`)
