...
Компилятору Intlayer требуется Babel для извлечения и оптимизации вашего контента. Обновите свой `babel.config.js` (или `babel.config.json`), чтобы включить плагины Intlayer:
