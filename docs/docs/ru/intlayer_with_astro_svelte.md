...
title: "Astro + Svelte i18n - Полное руководство по переводу вашего приложения"
description: "Больше никакого i18next. Руководство 2026 по созданию многоязычного (i18n) приложения Astro + Svelte. Переводите с помощью ИИ-агентов и оптимизируйте размер бандла, SEO и производительность."
keywords:
  - Интернационализация
  - Документация
  - Intlayer
...
history:
  ...
  - version: 8.7.7
    date: 2026-04-24
    changes: "Начальная документация для Astro + Svelte"
author: aymericzip
---

# Переводите ваш сайт Astro + Svelte с помощью Intlayer | Интернационализация (i18n)
...
- **svelte**
  Основной пакет Svelte.

- **svelte-intlayer**
  Пакет, который интегрирует Intlayer с приложениями Svelte. Он предоставляет `setupIntlayer`, `useIntlayer`, и хранилища (stores) `useLocale` для интернационализации в Svelte.

- **@astrojs/svelte**
  Официальная интеграция Astro, которая позволяет использовать острова (islands) компонентов Svelte.
...
В этой архитектуре интеграция `intlayer()`, зарегистрированная в `astro.config.ts`, собирает ваши словари и добавляет middleware, который определяет локаль каждого запроса и предоставляет её в `Astro.locals.intlayer`. Страницы располагаются в сегменте rest `src/pages/[...locale]/`, благодаря чему локаль по умолчанию отдается без префикса, а каждая другая локаль получает собственный выделенный URL. Файлы `.astro` читают контент с помощью хуков `useIntlayer` / `useLocale` из `astro-intlayer` для серверного рендеринга, а остров Svelte (`src/components/svelte/SvelteIsland.svelte`) получает определённую сервером локаль в качестве пропа и передаёт её в `svelte-intlayer` для интерактивной части. Объявления контента размещаются рядом с вашими компонентами в `src/`.
...
Добавьте плагин intlayer и интеграцию Svelte в вашу конфигурацию.
...
> Интеграция `svelte()` позволяет использовать острова компонентов Svelte через `client:only="svelte"`.
...
Вам также следует добавить SEO-метаданные, такие как hreflang и канонические ссылки, на каждую страницу и встроить остров Svelte для интерактивного контента на стороне клиента.
...
<html lang={locale} dir={getHTMLTextDir(locale)}>
  ...
  <body>
    <!-- Остров Svelte отрисовывает весь интерактивный контент, включая переключатель языков -->
    <SvelteIsland locale={locale} client:only="svelte" />
  </body>
</html>
```

> Если вы хотите использовать ваш контент в `string` атрибуте, таком как `alt`, `title`, `href`, `aria-label` и т.д., вы можете использовать значение функции, как:
...
<Step number={6} title="Создание компонента Острова Svelte">

Создайте компонент острова, который оборачивает ваше приложение Svelte. Функция `setupIntlayer` должна быть вызвана с локалью, определенной сервером, до обращения к любому хранилищу (store).
...
<div>
  <h1>{$content.title}</h1>
  <!-- Переключатель языка отрисовывается внутри острова -->
...
> Проп `locale` передается со страницы Astro (определенной сервером) и используется для инициализации `setupIntlayer`, что делает её начальной локалью для всех хранилищ в компоненте.

</Step>
<Step number={7} title="Добавление переключателя языков">

Переключатель языков встроен непосредственно в остров Svelte (показано в Шаге 6). Он использует `useLocale` из `svelte-intlayer` и переходит по локализованному URL, когда пользователь выбирает новый язык:
...
<script lang="ts">
  ...
  // Используется та же настройка locale/setupIntlayer, что и в Шаге 6 выше...
...
Для получения более подробной информации об использовании расширения см. [документацию расширения Intlayer для VS Code](https://github.com/aymericzip/intlayer/blob/main/docs/docs/ru/vs_code_extension.md).
...
<Question title="Какие существуют решения для интернационализации сайта Astro с островами Svelte?">

Встроенная опция `i18n` Astro обрабатывает префиксы локалей и редиректы, но не сам контент, поэтому вам всё ещё нужен слой сообщений, а остров добавляет вторую проблему: остров выполняет Svelte, а не Astro.

- **Astro `i18n` плюс написанные вручную словари** с **`svelte-i18n`** внутри острова: два источника контента, которые нужно синхронизировать, и никакой общей типизации между ними.
- **`Intlayer`**: один слой контента для обоих. `astro-intlayer` покрывает страницы `.astro`, а `svelte-intlayer` покрывает острова Svelte, читая одни и те же объявления.
...
<Question title="Могу ли я мигрировать с `svelte-i18n`, не переписывая свои компоненты?">
...
<Question title="Нужна ли мне отдельная библиотека i18n внутри моего острова Svelte?">

Нет. `svelte-intlayer` читает те же словари, что и сторона Astro, поэтому вы не устанавливаете `svelte-i18n` рядом с ним. Шаг 6 показывает компонент острова, получающий активную локаль от страницы, а не разрешающий её снова.
