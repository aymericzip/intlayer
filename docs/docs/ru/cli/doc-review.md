...
- **`--mode [mode]`**: Как выполняется проверка. По умолчанию `apply`.

  - `apply`: перевести расходящиеся блоки с помощью ИИ и записать файлы.
  - `report`: не вызывать ИИ; записывать блоки, требующие внимания (с номерами строк и содержимым) для базовой и целевой локалей, чтобы помочь другому агенту сгенерировать переводы.
  - `synthesis`: не вызывать ИИ и пропустить вывод поблочно; вывести только итоговый отчет о документах, которые актуальны, и тех, которые содержат блоки для редактирования.

  > Пример: `npx intlayer doc review --mode report`

  > Пример: `npx intlayer doc review --mode synthesis`

Независимо от режима, запуск завершается синтезом, в котором перечисляются документы, согласованные с их базовым документом, и документы, которые все еще содержат блоки для редактирования.
