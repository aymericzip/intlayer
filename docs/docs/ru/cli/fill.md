...
- Разделяет большие JSON файлы на chunks, чтобы оставаться в пределах контекстного окна AI модели.
- Повторяет перевод, если формат вывода некорректен.
- Включает контекст приложения и файла для улучшенной точности перевода.
- Сохраняет существующие переводы, не перезаписывая их.
- Обрабатывает файлы, chunks и locales параллельно, используя систему очередей для увеличения скорости.
