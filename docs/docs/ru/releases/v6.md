...
См. больше опций в документации CLI: [Справочник CLI](https://github.com/aymericzip/intlayer/blob/main/docs/docs/ru/cli/index.md) → "Проверка отсутствующих переводов". А также руководство [Тестирование](https://github.com/aymericzip/intlayer/blob/main/docs/docs/ru/testing.md).
...
Автоматически заполните ваши отсутствующие переводы, используя команду `npx intlayer fill`.
...
Обновлен MCP server для поддержки новых функций.
...
> MCP server помогает автоматизировать проекты, использующие Intlayer. Тестирование недостающих переводов для автогенерируемого кода. Помогает понять вашу конфигурацию и встроить документацию Intlayer в вашу IDE.
...
Система логирования была улучшена для предоставления более подробной информации о том, что происходит во время сборки (build time) и выполнения трансформаций.

> Включите подробные логи, чтобы лучше понимать, что происходит во время сборки (build time) и выполнения трансформаций.
...
## Параллельный процесс для отслеживания файлов на turbopack

При использовании Turbopack на Next.js 14 и выше в качестве сервера разработки с командой `next dev --turbopack` изменения словарей не будут автоматически обнаруживаться по умолчанию.

Это ограничение возникает потому, что Turbopack не может запускать webpack plugins параллельно для отслеживания изменений в ваших файлах контента.

Чтобы обойти это, мы использовали команду `intlayer watch` с опцией `--with` для одновременного запуска сервера разработки и наблюдателя сборки Intlayer.
...
## Примечания миграции с v6 на v7

Дополнительную информацию см. в [примечаниях миграции с v6 на v7](https://github.com/aymericzip/intlayer/blob/main/docs/docs/ru/releases/v7.md).

---

## Полезные ссылки

- [Справочник CLI](https://github.com/aymericzip/intlayer/blob/main/docs/docs/ru/cli/index.md)
