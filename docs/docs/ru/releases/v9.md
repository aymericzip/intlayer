...
description: Узнайте, что нового в Intlayer v9. Представляем пакеты совместимости drop-in для популярных библиотек i18n, а также поддержку Collections, Variants и Dynamic Records.
keywords:
  - Intlayer
  - Совместимость
  - Миграция
...
## Vite Plugin: Встроенный компилятор и прокси

Плагин Vite `intlayer()` теперь включает в себя напрямую **компилятор** и **прокси маршрутизации локалей**, так что большинству проектов нужен только один плагин в `vite.config.ts`:
...
- **Компилятор**: Активируется автоматически, если задан `compiler.enabled` и путь `compiler.output`. Вам больше не нужно регистрировать `intlayerCompiler()` отдельно.
- **Прокси**: Активируется автоматически на основе новой опции `routing.enableProxy` (`true` по умолчанию). Он подключает middleware обнаружения / перенаправления / перезаписи локали в development, preview и production SSR. Вам больше не нужно регистрировать `intlayerProxy()` отдельно.

### Опция `routing.enableProxy`

Новая опция `routing.enableProxy` управляет тем, подключен ли прокси маршрутизации локалей. Значение по умолчанию — `true`. Отключите ее, если вы хотите обрабатывать маршрутизацию локалей самостоятельно:
...
Отдельные плагины `intlayerCompiler()` и `intlayerProxy()` по-прежнему экспортируются для продвинутых конфигураций. Их регистрация вместе с `intlayer()` безопасна — каждый плагин дедублицируется автоматически и запускается только один раз.
...
// Получить один элемент по индексу
...
- [Динамические словари — Collections, Variants и Dynamic Records](https://github.com/aymericzip/intlayer/blob/main/docs/docs/ru/dynamic_dictionaries/index.md)
