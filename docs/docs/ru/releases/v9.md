...
- **Compiler**: Активируется автоматически, когда задан параметр `compiler.enabled` и настроена путь `compiler.output`. Вам больше не нужно регистрировать `intlayerCompiler()` отдельно.
