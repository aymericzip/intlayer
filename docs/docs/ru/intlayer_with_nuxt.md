...
В этой архитектуре маршруты страниц в `pages/` автоматически локализуются модулем `nuxt-intlayer`. Страницы, макеты и компоненты используют локализованные объявления контента, определенные в каталоге `content/`.
