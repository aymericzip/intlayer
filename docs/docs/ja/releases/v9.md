...
新しいオプションパッケージ [`@intlayer/analytics`](https://github.com/aymericzip/intlayer/blob/main/docs/docs/ja/analytics.md) は、実際にどのコンテンツが訪問者に表示されているかを教えてくれます。そして **Collections & Variants** の輪を完成させ、コンテンツの A/B テストとバリアントごとのコンバージョン計測を可能にします。
...
現在 React、Next.js、React Native（`react-intlayer` 経由）で利用可能です。他のフレームワークバインディング（Vue、Svelte、Angular など）は、`@intlayer/editor` と同じ展開パターンに沿って計画されています。完全なガイドは [Analytics ドキュメント](https://github.com/aymericzip/intlayer/blob/main/docs/docs/ja/analytics.md) を参照してください。
...
- [Analytics（分析） - コンテンツ露出の計測とA/Bテスト](https://github.com/aymericzip/intlayer/blob/main/docs/docs/ja/analytics.md)
