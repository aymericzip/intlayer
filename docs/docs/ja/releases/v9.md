...
Intlayer v9は、単一のコマンドで独自のIntlayerインスタンスを実行するためのファーストクラスサポートを提供します。Intlayer Cloudアカウントは必要ありません。

```sh
curl -fsSL https://intlayer.org/install.sh | sh
```

インストーラーは`docker-compose.yml`と`.env`をダウンロードし、必要なシークレットを自動生成し、`docker compose up -d`を実行します。ダッシュボード、API、データベース、オブジェクトストレージ、トランザクションメールのすべてがコンテナ内でローカルに実行されます。
