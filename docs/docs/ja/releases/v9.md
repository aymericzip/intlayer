...
## Viteプラグイン: バンドルされたCompilerとProxy

`intlayer()` Viteプラグインは現在、**compiler**と**locale-routing proxy**を直接バンドルしており、ほとんどのプロジェクトでは `vite.config.ts` に単一のプラグインが必要です:
...
- **Compiler**: `compiler.enabled` が設定され `compiler.output` パスが構成されると自動的にアクティブになります。もはや `intlayerCompiler()` を個別に登録する必要はありません。
- **Proxy**: 新しい `routing.enableProxy` オプション (デフォルト `true`) に基づいて自動的にアクティブになります。開発、プレビュー、本番SSRでのロケール検出 / リダイレクト / リライトミドルウェアを接続します。もはや `intlayerProxy()` を個別に登録する必要はありません。

### `routing.enableProxy` オプション

新しい `routing.enableProxy` オプションは、ロケールルーティングプロキシを接続するかどうかを制御します。デフォルトは `true` です。ロケールルーティングを自分で処理したい場合は無効にしてください:
...
独立した `intlayerCompiler()` および `intlayerProxy()` プラグインは高度なセットアップのために引き続きエクスポートされています。それらを `intlayer()` と一緒に登録しても安全です — 各プラグインは自身を重複排除し、一度だけ実行されます。
