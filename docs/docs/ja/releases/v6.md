...
CLIの詳細オプションについては、CLIドキュメントをご覧ください: [CLIリファレンス](https://github.com/aymericzip/intlayer/blob/main/docs/docs/ja/cli/index.md) → 「欠落翻訳のテスト」。また、[テスト](https://github.com/aymericzip/intlayer/blob/main/docs/docs/ja/testing.md)ガイドも参照してください。
...
`autoFill`フィールドを使用して、コンテンツファイル内の辞書ごとに微調整することもできます。Intlayerはまず辞書ごとの設定を考慮し、その後グローバル設定にフォールバックします。完全なリファレンスは[Auto-fill](https://github.com/aymericzip/intlayer/blob/main/docs/docs/ja/autoFill.md)と[Content file reference](https://github.com/aymericzip/intlayer/blob/main/docs/docs/ja/dictionary/content_file.md)を参照してください。
...
> MCP server は Intlayer を使用するプロジェクトの自動化に役立ちます。自動生成されたコードの欠落している翻訳をテストします。config を理解し、IDE に Intlayer のドキュメントを組み込むのに役立ちます。
...
## Turbopackでファイルを監視するための並列プロセス

`next dev --turbopack`コマンドを使用してNext.js 14以上でTurbopackを開発サーバーとして使用する場合、辞書の変更はデフォルトでは自動的に検出されません。

この制限は、Turbopackがコンテンツファイルの変更を監視するためにwebpackプラグインを並列で実行できないために発生します。

これを回避するために、`intlayer watch`コマンドに`--with`オプションを使用して、開発サーバーとIntlayerビルドウォッチャーを同時に実行するようにしました。
...
詳細については、[v6 から v7 への移行ノート](https://github.com/aymericzip/intlayer/blob/main/docs/docs/ja/releases/v7.md)を確認してください。

---

## 便利なリンク

- [CLIリファレンス](https://github.com/aymericzip/intlayer/blob/main/docs/docs/ja/cli/index.md)
