...
- `apply`: AI を使用して異なるブロックを翻訳し、ファイルを作成します。
- `report`: AI を呼び出さず、ベースと対象ロケールのブロック（行番号とコンテンツ付き）で注意が必要なものをログして、別のエージェントが翻訳を生成できるようにします。
- `synthesis`: AI を呼び出さず、ブロックごとの出力をスキップし、最新のドキュメントと編集が必要なドキュメントの最終的な概要のみをログします。

  > 例: `npx intlayer doc review --log`

  > 例: `npx intlayer doc review --mode synthesis`

どのモードであっても、実行は、ベースドキュメントと一致しているドキュメントと、まだ編集するブロックがあるドキュメントをリストアップするサマリーで終了します。
