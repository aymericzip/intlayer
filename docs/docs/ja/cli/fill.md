...
- 大規模なJSONファイルをチャンクに分割して、AIモデルのコンテキストウィンドウ制限内に収まるようにします。
- 出力形式が正しくない場合は翻訳を再試行します。
- アプリケーション固有とファイル固有のコンテキストを組み込んで、翻訳精度を向上させます。
- 既存の翻訳を保持し、上書きしません。
- キューシステムを使用してファイル、チャンク、ロケールを並列処理して、速度を向上させます。
