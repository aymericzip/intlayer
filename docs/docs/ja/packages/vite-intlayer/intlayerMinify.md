...
`editor.enabled` が `true` の場合、プラグインは引き続き実行されますが、**フィールドのリネームはスキップされます**。リネームはインタープリタが辿るコンテンツキーを書き換えるため、ビジュアルエディタが受け取る `keyPath` が、編集対象であるマージ前の辞書と一致しなくなってしまいます。空白の圧縮とトップレベルのメタデータ削除は引き続き有効です。
