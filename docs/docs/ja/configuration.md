...
history:
  - version: 9.1.3
    date: 2026-08-06
    changes: "`routing.enableProxy` を三状態化: 未設定 (auto)、`true`、`false`"
  - version: 9.0.0
    date: 2026-07-11
    changes: "`analytics` 設定を追加"
  - version: 9.0.0
    date: 2026-06-24
...
const config: IntlayerConfig = {
  ...
  routing: {
    ...
    /**
     * Intlayer のロケールルーティングプロキシ（ミドルウェア）を有効にします。
     * dev、preview、SSR でのロケール検出・リダイレクト・リライトを担います。
     * - 未設定 (auto): dev および preview サーバーは Cookie やヘッダーに保存された
     *   ロケールを無視し、ルーティングを URL 駆動に保ちます。プレフィックスの解決、
     *   ロケールの永続化、Accept-Language 検出は引き続き動作します。
     *   本番環境では `true` と同じ挙動になります。
     * - true: すべての環境で完全な挙動。
     * - false: ロケールルーティングを行いません。
     * デフォルト: undefined (auto)
     */
  ...
  /**
   * アナリティクス（analytics）設定。
   */
  analytics: {
    /**
     * アナリティクス収集を有効にするかどうか（ページビュー、コンテンツの露出、A/Bイベント）。
     * 帰属（attribution）のために `editor.clientId` の設定が必要です。
     * デフォルト: false
     */
    enabled: true,

    /**
     * バックエンドへの自動バッチ送信間のミリ秒数。
     * デフォルト: 20000
     */
    flushInterval: 20000,

    /**
     * 記録するセッションの割合。0（なし）から1（すべて）まで。
     * デフォルト: 1
     */
    sampleRate: 1,
  },
  ...
  dictionary: {
     ...
     * - "dynamic": Suspenseを使用して動的にインポート。
     * - "fetch": ライブ同期APIを介して動的に取得。
     */
    importMode: "static",

    /**
     * プロジェクト内のすべての辞書のデフォルトのメッセージ形式。
     * - 'intlayer': ネイティブの intlayer 形式 (デフォルト)。
     * - 'icu': ICU メッセージ形式。
     * - 'i18next': i18next 形式。
     * - 'vue-i18n': Vue I18n 形式。
     * - 'po': GNU Gettext PO 形式。
     */
    format: "icu",
  },
...
アナリティクスは厳密にオプトイン方式です。`analytics.enabled` が明示的に `true` に設定され、**かつ** 帰属のためのプロジェクトキー（`editor.clientId`）が設定されない限り、何も収集されません。無効の場合（デフォルト）、アナリティクスの統合全体がアプリケーションのバンドルから削除されます（デッドコード除去）。

| フィールド      | 説明                                                                              | 型        | デフォルト | 例      | 備考                                                                                                                                       |
| --------------- | --------------------------------------------------------------------------------- | --------- | ---------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| `enabled`       | アナリティクス収集を有効にします（ページビュー、コンテンツの露出、A/Bイベント）。 | `boolean` | `false`    | `true`  | 帰属のために `editor.clientId` の設定が必要です。設定されていない場合、`enabled` が `true` でもアナリティクスは無効のままです。            |
...
| `enableProxy` | Intlayer のロケールルーティングプロキシ（ミドルウェア）を有効にします。                                                                                                                                                    | `boolean` &#124; <br/> `undefined`                                                                                                                                                                           | `undefined` (auto)     | `true`                                                                                                                                                                                               | • 未設定 (auto): dev / preview サーバーは Cookie・ヘッダーに保存されたロケールをリダイレクト元として無視します。プレフィックス、永続化、`Accept-Language` 検出は引き続き動作します。本番環境では `true` と同じ挙動です。<br/>• `true`: どこでも完全な挙動。<br/>• `false`: ロケールルーティングなし。Next.js では `intlayerProxy` ミドルウェアがパススルーになります。 |
...
| ------------------------- | --------------------------------------------------------------------- | -------- | --------------------------------- | -------------------- | ------------------------------------------------------------ |
| `baseDir`                 | プロジェクトのベースディレクトリ。                                    | `string` | `process.cwd()`                   | `'/path/to/project'` | すべてのIntlayer関連ディレクトリを解決するために使用される。 |
| `dictionariesDir`         | ローカライゼーション辞書を保存するディレクトリパス。                  | `string` | `'.intlayer/dictionary'`          |                      |                                                              |
| `moduleAugmentationDir`   | モジュール拡張用ディレクトリ。IDEの提案とタイプチェックを向上させる。 | `string` | `'.intlayer/types'`               | `'intlayer-types'`   | `tsconfig.json`に含めることを確認する。                      |
| `unmergedDictionariesDir` | マージされていない辞書を保存するディレクトリ。                        | `string` | `'.intlayer/unmerged_dictionary'` |                      |                                                              |
| `typesDir`                | 辞書タイプを保存するディレクトリ。                                    | `string` | `'.intlayer/types'`               |                      |                                                              |
| `mainDir`                 | メインアプリケーションファイルが保存されるディレクトリ。              | `string` | `'.intlayer/main'`                |                      |                                                              |
| `configDir`               | 設定ファイルが保存されるディレクトリ。                                | `string` | `'.intlayer/config'`              |                      |                                                              |
| `cacheDir`                | キャッシュファイルが保存されるディレクトリ。                          | `string` | `'.intlayer/cache'`               |                      |                                                              |
...
| `format`                    | プロジェクト内のすべての辞書のデフォルトのメッセージ形式。                                                                                    | `'intlayer'` &#124; <br/> `'icu'` &#124; <br/> `'i18next'` &#124; <br/> `'vue-i18n'` &#124; <br/> `'po'`        | `'intlayer'` | `'icu'`                                                                                     | • `'intlayer'`: ネイティブ intlayer 形式。<br/>• `'icu'`: ICU メッセージ形式。<br/>• `'i18next'`: i18next 形式。<br/>• `'vue-i18n'`: Vue I18n 形式。<br/>• `'po'`: GNU Gettext PO 形式。                                                                                                                                                                                                                  |
