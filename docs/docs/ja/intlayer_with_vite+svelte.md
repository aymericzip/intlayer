...
title: Vite + Svelte i18n - 完全な翻訳ガイド： Svelte
description: バンドルサイズ、SEO、パフォーマンス、保守性のための最良のソリューション。2026年にVite and Svelte アプリを多言語化しましょう。LLM翻訳、Agent Skills & MCP。
keywords:
  - 国際化
  - ドキュメント
  - Intlayer
...
> コンテンツ宣言は、アプリケーション内のどこにでも定義できます。`contentDir`ディレクトリ（デフォルトは`./src`）に含まれていれば有効です。また、コンテンツ宣言ファイルの拡張子（デフォルトは`.content.{json,ts,tsx,js,jsx,mjs,cjs}`）に一致する必要があります。
