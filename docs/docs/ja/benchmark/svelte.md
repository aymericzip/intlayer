...
実際のブラウザで各スタックの同じ多言語アプリを実行し、実際にネットワークに送信されたものと処理時間を記録しました。サイズは**通常のウェブ圧縮後**に報告されています。これはユーザーがダウンロードする実際のサイズに近いため、生のソースコード数よりも適切です。

- **国際化ライブラリのサイズ**: バンドリング、ツリーシェイキング、ミニフィケーション後の i18n ライブラリのサイズは、空のコンポーネント内のプロバイダー + ストアコードのサイズです。翻訳ファイルの読み込みは含まれていません。これはコンテンツが含まれる前にライブラリにかかるコストを示します。

- **ページごとの JavaScript**: 各ベンチマークルートについて、そのアクセス時にブラウザが取得するスクリプトの量。スイート内のページとロケール全体で平均化されています（レポートにまとめられている場合）。重いページは遅いページです。

- **他のロケールからのリーク**: 監査対象ページに誤って読み込まれる、別の言語での同じページのコンテンツです。このコンテンツは不要であり、避けるべきです。（例：`/en/about` ページバンドル内の `/fr/about` ページコンテンツ）

- **他のルートからのリーク**: **他の画面**についても同じ考え方です。1 つのページを開いただけなのに、そのコピーが一緒に読み込まれるかどうかです。（例：`/en/contact` ページバンドル内の `/en/about` ページコンテンツ）。高いスコアは分割が弱いか、バンドルが広すぎることを示唆しています。

- **コンポーネントバンドルサイズの平均**: 一般的な UI パーツは 1 つずつ測定され、1 つの巨大なアプリ数に隠れていません。これは国際化がいかに日常的なコンポーネントを静かに膨張させるかを示しています。例えば、コンポーネントが再レンダリングされる場合、メモリからそのすべてのデータを読み込みます。巨大な JSON をコンポーネントに接続することは、コンポーネントのパフォーマンスを低下させる大量の未使用データストアに接続するようなものです。

- **言語切り替えの応答性**: アプリの独自のコントロールを使用して言語を切り替え、ページが明らかに切り替わるまでにかかる時間を計測します。これは訪問者が気付く、ラボのマイクロステップではありません。

- **言語変更後のレンダリング作業**: より狭い範囲のフォローアップです。スイッチが実行中になった後、新しい言語に対して再描画するためにインターフェースがどれだけの労力を費やしたかです。「体感」時間とフレームワークのコストが異なる場合に役立ちます。

- **初期ページ読み込み時間**: ナビゲーションからブラウザがテストシナリオのページが完全に読み込まれたと判断するまで。コールドスタートを比較するのに最適です。

- **ハイドレーション時間**: アプリが公開している場合、クライアントがサーバー HTML を実際にクリックできるものに変える際にかかる時間です。テーブル内のダッシュは、その実装がこのベンチマークで信頼できるハイドレーション数値を提供しなかったことを意味します。
...
このソリューションは、Svelteプロジェクトにおけるi18nのすべてのニーズに応えます。しかし、i18nextや他の主要なi18nソリューションと同様に、少し重いです（~16.6kb。これは `svelte-intlayer` の約4.5倍です）。
