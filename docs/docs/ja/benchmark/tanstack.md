...
私は、すべてのスタックに対して同じ多言語アプリを実際のブラウザで実行し、実際にネットワークを通じて何が表示されたか、どのくらい時間がかかったかを記録しました。サイズは**通常のウェブ圧縮後**に報告されています。これは生ソースコード数よりも、人々が実際にダウンロードするものに近いためです。

- **国際化ライブラリサイズ**: バンドリング、ツリーシェイキング、ミニファイ後の i18n ライブラリのサイズは、空のコンポーネント内のプロバイダー（例：`NextIntlClientProvider`）+ フック（例：`useTranslations`）コードのサイズです。翻訳ファイルの読み込みは含まれません。これにより、コンテンツが入る前にライブラリがどの程度コストがかかるかが分かります。

- **ページごとの JavaScript**: 各ベンチマークルートについて、その訪問時にブラウザが取得するスクリプトの量を、スイート内のページ全体にわたり、またレポートがロールアップする場所のロケール全体にわたって平均化したものです。重いページは遅いページです。

- **他のロケールからのリーク**: 同じページであっても、別の言語であり、監査されたページに誤って読み込まれるコンテンツです。このコンテンツは不要であり、回避する必要があります。（例：`/en/about` ページバンドルの `/fr/about` ページコンテンツ）

- **他のルートからのリーク**: アプリ内の**他の画面**についても同じ考え方です。1 つのページのみを開いたときに、そのコピーが一緒に読み込まれるかどうかです。（例：`/en/contact` ページバンドルの `/en/about` ページコンテンツ）。スコアが高い場合は、分割が弱いか、バンドルが過度に広いことを示唆しています。

- **平均コンポーネントバンドルサイズ**: 一般的な UI コンポーネントは、1 つの巨大なアプリ番号内に隠れるのではなく、**1 つずつ**測定されます。国際化が日常的なコンポーネントを静かに膨張させているかどうかを示します。例えば、コンポーネントが再レンダリングされる場合、メモリからすべてのデータを読み込みます。巨大な JSON をコンポーネントに添付することは、コンポーネントのパフォーマンスを低下させる、使用されていない大量のデータストアに接続するようなものです。

- **言語切り替えレスポンシビティ**: アプリ独自の制御を使用して言語を切り替え、ページが明確に切り替わるまでにかかる時間を計測します。訪問者が気付くこと、ラボの微視的なステップではありません。

- **言語変更後のレンダリング作業**: より狭い追跡です。スイッチが実行中になった後、新しい言語のために、インターフェイスが再描画するのに要した努力がどの程度であるかです。「感じられた」時間とフレームワークコストが異なる場合に役立ちます。

- **初期ページロード時間**: ナビゲーションから、テストしたシナリオについてページが完全に読み込まれたと見なすブラウザまで。コールドスタートを比較するのに適しています。

- **ハイドレーション時間**: アプリが公開している場合、クライアントがサーバー HTML を実際にクリックできるものに変換するのにかかる時間です。テーブルのダッシュ記号は、その実装がこのベンチマークで信頼性の高いハイドレーション数値を提供しなかったことを意味します。
