...
| **`minify`**          | `boolean`                        | `false`                         | バンドルサイズを削減するために辞書を最小化するかどうか。                                                                                                                                                     |
| **`purge`**           | `boolean`                        | `false`                         | 辞書内の未使用のキーをパージするかどうか。                                                                                                                                                                   |
...
Intlayerは、辞書を最小化（Minify）し、未使用のキーをパージ（Purge）することで、バンドルをさらに最適化するオプションを提供します。

### 最小化（Minification）

辞書の最小化により、不必要な空白やコメントが削除され、JSONコンテンツのサイズが削減されます。これは、大規模な辞書で特に有効です。
...
> 注意：`optimize`が無効な場合、またはビジュアルエディタが有効な場合（エディタが編集を可能にするために完全なコンテンツを必要とするため）、最小化は無視されます。

### パージ（Purging）

パージを行うことで、コード内で実際に使用されているキーのみが最終的な辞书バンドルに含まれるようになります。これにより、アプリケーションの各部分で使用されない多くのキーを含む大規模な辞書がある場合に、バンドルサイズを大幅に削減できます。
