...
Intlayerを使用すると、単一のロケールのみをローカルで宣言し、すべての翻訳をCMSを介してリモートで管理するワークフローを採用できます。これにより、コンテンツと翻訳をコードベースから完全に分離でき、コンテンツエディターに対しより高い柔軟性を提供し、Live Sync（アプリケーションを再構築することなく変更を適用）を有効にします。
