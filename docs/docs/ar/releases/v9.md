...
## إضافة Vite: دمج Compiler و Proxy

تدمج إضافة `intlayer()` لـ Vite الآن **المترجم (compiler)** و **وكيل التوجيه المحلي (proxy)** مباشرة، لذا فمعظم المشاريع تحتاج فقط إلى إضافة واحدة في `vite.config.ts`:
...
- **المترجم (Compiler)**: يتم تنشيطه تلقائيًا عند تعيين `compiler.enabled` وتهيئة مسار `compiler.output`. لم تعد بحاجة إلى تسجيل `intlayerCompiler()` بشكل منفصل.
- **الوكيل (Proxy)**: يتم تنشيطه تلقائيًا بناءً على خيار `routing.enableProxy` الجديد (`true` افتراضيًا). حيث يربط البرامج الوسيطة لاكتشاف / إعادة توجيه / إعادة كتابة اللغة في Development و Preview و Production SSR. لم تعد بحاجة إلى تسجيل `intlayerProxy()` بشكل منفصل.

### خيار `routing.enableProxy`

يتحكم خيار `routing.enableProxy` الجديد في تشغيل وكيل التوجيه المحلي أم لا. الوضع الافتراضي هو `true`. قم بتعطيله عندما ترغب في التعامل مع التوجيه المحلي بنفسك:
...
const config: IntlayerConfig = {
  routing: {
    enableProxy: false, // افتراضي: true
  },
};
...
لا تزال الإضافات المستقلة `intlayerCompiler()` و `intlayerProxy()` مصدرة للإعدادات المتقدمة. تسجيلها بجوار `intlayer()` آمن — حيث تقوم كل إضافة بإلغاء النسخ المكررة من نفسها وتعمل مرة واحدة فقط.
