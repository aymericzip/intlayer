...
## `@intlayer/analytics`: تتبع عرض المحتوى واختبار A/B

حزمة اختيارية جديدة، [`@intlayer/analytics`](https://github.com/aymericzip/intlayer/blob/main/docs/docs/en/analytics.md)، تخبرك بالمحتوى الذي يشاهده زوارك فعلياً — وتُكمل حلقة **Collections & Variants** من خلال تمكينك من اختبار المحتوى بطريقة A/B وقياس التحويلات لكل نسخة.

- **التتبع على مستوى Provider**: يسجّل `IntlayerProvider` (React / Next.js / React Native) تلقائياً أحداث `page_view` عند التحميل، وتغيير المسار، وتغيير اللغة.
- **التتبع على مستوى العقدة**: يُبلغ كل تحليل لـ `useIntlayer` عن حدث `content_exposure` — يوضح أي مفتاح قاموس، ومسار المفتاح، واللغة، وعند الاقتضاء أي **نسخة** A/B تم عرضها. يتم دمج حالات العرض المتكررة لنفس العقدة ضمن نافذة الإرسال في حدث واحد.
- **تخصيص حتمي لنسخ A/B**: تُخصّص `getVariant(experimentKey, variants)` نسخة لكل جلسة **دون أي اتصال بالخادم**، لذا لا يحدث أي وميض قبل أول عرض.
- **`useConversion()`**: تنسب هدفاً (تسجيل، نقرة، شراء…) إلى النسخة التي تعرّضت لها الجلسة.
- **بدون أي تكلفة عند عدم التثبيت**: نفس نمط tree-shakeable المستخدم في `@intlayer/editor` — `import()` ديناميكي مُغلّف بـ `try/catch`، إضافة إلى متغيّر بيئة وقت البناء (`INTLAYER_ANALYTICS_ENABLED`) يزيل التكامل بالكامل من الحزمة للتطبيقات التي لا تُفعّله.
- **دون أي مخطط تهيئة جديد**: يعيد استخدام `editor.backendURL` (نقطة الإرسال / الاستضافة الذاتية) و`editor.clientId` (مفتاح المشروع، وهو أيضاً مفتاح التفعيل).
- **يحترم الخصوصية**: تجزئة مجهولة للجلسة (SHA-256، دون تخزين المعرّفات الأصلية)، دون تخزين عناوين IP، وتحديد جغرافي تقريبي على مستوى الدولة فقط، وإرسال الطلبات على دفعات كل 20 ثانية تقريباً.
- **لوحة التحكم**: صفحة **Analytics** جديدة (تظهر في الشريط الجانبي بعد اختيار مشروع) تعرض المستخدمين النشطين، ومستخدمي اليوم، والمستخدمين خلال آخر 7/30/90 يوماً، ورسماً بيانياً لتطور الزوار، وتوزيعاً حسب اللغة والموقع.
...
متوفرة اليوم لـ React وNext.js وReact Native (عبر `react-intlayer`)؛ ويُخطَّط لدعم أطر عمل أخرى (Vue، Svelte، Angular، …) باتباع نفس نمط الطرح المُستخدم في `@intlayer/editor`. راجع [توثيق Analytics](https://github.com/aymericzip/intlayer/blob/main/docs/docs/en/analytics.md) للحصول على الدليل الكامل.
...
- [Analytics - تتبع عرض المحتوى واختبار A/B](https://github.com/aymericzip/intlayer/blob/main/docs/docs/en/analytics.md)
