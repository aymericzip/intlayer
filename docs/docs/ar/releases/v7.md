...
## تنسيق الكود التلقائي: تكوين formatCommand

يقدم الإصدار v7 خيار `formatCommand` في تكوين المحرر، مما يسمح لك بتنسيق ملفات المحتوى تلقائياً عند كتابتها بواسطة Intlayer. وهذا يضمن أسلوب كود متسق وتنسيق موحد عبر ملفات إعلان المحتوى الخاصة بك.

إذا لم يتم تعيينه، سيحاول Intlayer الكشف عن أمر التنسيق تلقائياً. من خلال محاولة حل الأوامر التالية: prettier, biome, eslint.

### الإعدادات

تقبل خيار `formatCommand` قالب نص حيث سيتم استبدال `{{file}}` بمسار الملف الفعلي:
...
يمكنك استخدام أي منسق أكواد يقبل مسارات الملفات كمعاملات:
...
- **تنسيق متسق**: يتم تنسيق جميع ملفات المحتوى تلقائياً وفقاً لإرشادات نمط مشروعك
- **تجربة المطور**: لا حاجة لتنسيق الملفات يدوياً بعد كتابة Intlayer لها
- **اتساق الفريق**: يضمن أن جميع أعضاء الفريق لديهم نفس التنسيق المطبق على ملفات المحتوى
- **تكامل CI/CD**: تحافظ ملفات المحتوى على تنسيق متسق في سير العمل الآلي

### كيف يعمل

عندما يكتب Intlayer أو يحدث ملف إعلان محتوى (`.content.ts`، `.content.js`، إلخ)، فإنه يقوم تلقائياً بتشغيل أمر التنسيق المحدد على الملف. يتم استبدال العنصر النائب `{{file}}` بمسار الملف الفعلي، ويتم تنفيذ الأمر في دليل قاعدة المشروع.

---

## تكوين القاموس: تنظيم وهيكلة أفضل

يقدم الإصدار v7 قسم تكوين `dictionary` مخصصًا جديدًا يوفر تنظيمًا أفضل لإعدادات القاموس وإدارة محسنة للمحتوى.

### هيكل تكوين القاموس الجديد

تم نقل خاصية `fill` من قسم `content` إلى قسم `dictionary` جديد، مما يوفر فصلاً أوضح للمسؤوليات:
...
- **تنظيم أوضح**: يتم الآن تجميع الإعدادات الخاصة بالقاموس معًا
- **فصل أفضل للاهتمامات**: يتم فصل اكتشاف المحتوى عن عمليات القاموس بوضوح
- **قابلية صيانة محسّنة**: يسهل فهم وتعديل التكوينات المتعلقة بالقاموس
- **قابلية التوسع في المستقبل**: يمكن لقسم القاموس أن يستوعب إعدادات إضافية خاصة بالقاموس
- **إدارة قاموس شاملة**: تتضمن خصائص مثل `title` و `live` و `priority` و `tags` و `version` و `description` لإنشاء وإدارة قواميس جديدة

### استخدام التكوين

يخدم تكوين القاموس غرضين رئيسيين:

1. **القيم الافتراضية**: تحديد القيم الافتراضية عند إنشاء ملفات إعلان المحتوى
2. **سلوك الرجوع**: توفير قيم بديلة عندما لا يتم تحديد حقول معينة، مما يسمح لك بتحديد سلوك تشغيل القاموس بشكل عام

يتضمن قسم القاموس خصائص شاملة لإدارة القاموس:

- **`fill`**: سلوك الملء التلقائي لإنشاء المحتوى
- **`title`**: العنوان الافتراضي للقواميیس الجديدة
- **`live`**: تكوين المزامنة الحية للتحديثات في الوقت الفعلي
- **`priority`**: إعدادات الأولویة لحل القاموس
- **`tags`**: الوسوم الافتراضية لتنظيم المحتوى
- **`version`**: إدارة الإصدار لتحديثات القاموس
- **`description`**: الوصف الافتراضي للمحتوى الجديد

للمزید من المعلومات حول ملفات إعلان المحتوى وكيف يتم تطبيق قيم التكوين، راجع [توثيق ملف المحتوى](https://github.com/aymericzip/intlayer/blob/main/docs/docs/ar/dictionary/content_file.md).
...
- **`editor.formatCommand`**: خيار جديد لتنسيق الكود تلقائياً لملفات المحتوى
