...
title: "تدويل Astro + Preact - الدليل الكامل لترجمة تطبيقك"
...
يجب عليك أيضاً إضافة بيانات تعريف SEO مثل hreflang والروابط الأساسية (canonical) إلى كل صفحة وتضمين جزيرة Preact للمحتوى التفاعلي من جانب العميل.
...
<Step number={6} title="إنشاء مكون جزيرة Preact">
...
يستخدم Intlayer module augmentation للاستفادة من فوائد TypeScript وجعل codebase أقوى.
...
<Question title="ما هي الحلول المختلفة المتاحة لتدويل موقع Astro مع جزر Preact؟">

يتعامل خيار `i18n` المدمج في Astro مع البادئات وإعادة التوجيه ولكنه لا يدير المحتوى. يطرح إضافة الجزر تحديًا إضافيًا: الجزر لا تشغل Astro بل تشغل Preact.

- **Astro `i18n` بالإضافة إلى قواميس مخصصة**، وفي الجزر استخدام **`preact-i18n`**: مصدران منفصلان للمحتوى دون أنواع مشتركة.
- **`Intlayer`**: طبقة محتوى موحدة واحدة لكليهما. يخدم `astro-intlayer` صفحات `.astro`، ويقرأ `preact-intlayer` نفس الإعلان لجزر Preact.
...
إلى حد كبير نعم. اتبع [دليل الترحيل](https://github.com/aymericzip/intlayer/blob/main/docs/docs/ar/migration_from_react-i18next_to_intlayer.md). يمكنك أيضًا الترحيل تدريجيًا: تحافظ [مكونة مزامنة JSON](https://github.com/aymericzip/intlayer/blob/main/docs/docs/ar/plugins/sync-json.md) على كتالوجات JSON كمصدر للحقيقة وتنشئ قواميس Intlayer.
...
<Question title="هل أحتاج إلى مكتبة i18n منفصلة داخل جزر Preact الخاصة بي؟">

لا. تقرأ الحزمة `preact-intlayer` نفس القواميس مثل Astro، لذلك لا تقوم بتثبيت `preact-i18n` بجانبها. توضح الخطوة 6 كيف تستقبل مكونات الجزيرة اللغة مباشرة من الصفحة.
