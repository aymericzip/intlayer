...
<AccordionGroup>
<Accordion header="تغطية Angular كاملة">
...
في هذه البنية، يوفر `angular-intlayer` الدالة `provideIntlayer()`، المسجلة في `src/app/app.config.ts` حتى يتمكن كل مكون من قراءة محتواه من خلال إشارة `useIntlayer`. يتم تبديل واجهة سطر أوامر Angular (CLI) إلى أداة بناء esbuild المخصصة، ويقوم `esbuild.plugins.ts` بتسجيل مكون Intlayer الإضافي في البناء لمراقبة تصريحات المحتوى وإعادة بنائها. توضع تصريحات المحتوى بجانب المكونات في `src/app/`.
...
![خطأ ترجمة](https://github.com/aymericzip/intlayer/blob/main/docs/assets/translation_error.webp?raw=true)
...
لمزيد من التفاصيل حول كيفية استخدام الإضافة، راجع [وثائق إضافة Intlayer VS Code Extension](https://github.com/aymericzip/intlayer/blob/main/docs/docs/ar/vs_code_extension.md).
...
<Question title="ما هي الحلول المختلفة المتاحة لتدويل تطبيقات Angular؟">

- **`@angular/localize` المدمج**: ترجمة منفصلة لكل لغة.
- **`ngx-translate`** و **`Transloco`**: كتالوجات وقت التشغيل.
- **`Intlayer`**: مكونات مستقلة، مدخلات إشارات Signal inputs، وتدفق بناء حديث يعتمد على Vite.
...
<Question title="هل يعمل Intlayer مع إشارات Angular Signals؟">

نعم. تُرجع `useIntlayer` إشارة Angular تعمل بسلاسة في بنية Angular بدون مناطق (Zoneless).

</Question>
<Question title="هل يتطلب تغيير اللغة إعادة تحميل الصفحة؟">

لا. بالاعتماد على الإشارات، يظهر تبديل اللغة على الفور في العرض.

</Question>
<Question title="كيف أقوم بإعداد التوجيه مع اللغة في Angular؟">

أعلن عن المعلمة `:locale` في المسارات وقم بمزامنتها مع موفر Intlayer.

</Question>
<Question title="كيف أتعامل مع اللغات من اليمين إلى اليسار في Angular؟">

استخدم `getHTMLTextDir` لتعيين `lang` و `dir` على العنصر الجذري للتطبيق.
