...
- **`Intlayer`**: استخراج تلقائي للنصوص في وقت البناء، دعم HMR، أنواع TypeScript كاملة، وترجمة بالذكاء الاصطناعي.
...
يُضمن المترجم فقط السلاسل التي يستخدمها المكون بالفعل، مما يحافظ على الحجم في حده الأدنى. انظر [تحسين الحزم](https://github.com/aymericzip/intlayer/blob/main/docs/docs/ar/bundle_optimization.md).

</Question>

<Question title="هل يمكنني الترحيل من react-i18next أو react-intl دون إعادة كتابة مكوناتي؟">

نعم، عبر محولات التوافق أو تمكين المترجم مباشرة على الكود الحالي.
...
للتطوير السريع، يعد المترجم مثاليًا. بالنسبة لقواعد الجمع المعقدة أو المنطق المخصص، توفر ملفات المحتوى الصريحة تحكمًا أفضل.

</Question>

<Question title="ماذا يحدث للنصوص التي لا يمكن للمترجم رؤيتها؟">

يمكن معالجة النصوص الديناميكية التي تم إنشاؤها في وقت التشغيل باستخدام `t()` أو ملف `.content` قياسي.

</Question>

<Question title="كيف يقرر المترجم ما هو نص المستخدم؟">

يستهدف المترجم عقد نصوص JSX والسمات الشائعة مثل `aria-label` أو `placeholder`، مع استبعاد فئات CSS ومعرفات النظام.

</Question>

<Question title="كيف أملأ الترجمات المفقودة؟">

تغطي الخطوة 7 ذلك. يرسل الأمر `npx intlayer fill` المحتوى المستخرج إلى نموذج اللغة الذي تختاره، ويقصر `--git-diff` التغييرات على الفرع الحالي.

</Question>

<Question title="كيف أغير اللغة في وقت التشغيل؟">

تغطي الخطوة 6 ذلك. يوفر خطاف `useLocale` اللغة النشطة وقائمة اللغات ودالة التعيين؛ يتم إعادة عرض المكونات باللغة الجديدة دون إعادة تحميل الصفحة.
