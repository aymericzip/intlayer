...
يجب عليك أيضاً إضافة بيانات تعريف SEO مثل hreflang والروابط الأساسية (canonical) إلى كل صفحة وتضمين جزيرة React للمحتوى التفاعلي من جانب العميل.
...
<Question title="ما هي الحلول المختلفة المتاحة لتدويل موقع Astro مع جزر React؟">

يتعامل خيار `i18n` المدمج في Astro مع البادئات وإعادة التوجيه ولكنه لا يدير المحتوى. يطرح إضافة الجزر تحديًا إضافيًا: الجزر لا تشغل Astro بل تشغل React.

- **Astro `i18n` بالإضافة إلى قواميس مخصصة**، وفي الجزر استخدام **`react-i18next`**: مصدران منفصلان للمحتوى دون أنواع مشتركة.
- **`Intlayer`**: طبقة محتوى موحدة واحدة لكليهما. يخدم `astro-intlayer` صفحات `.astro`، ويقرأ `react-intlayer` نفس الإعلان لجزر React.
...
<Question title="هل يمكنني الترحيل من `react-i18next` دون إعادة كتابة المكونات؟">
...
<Question title="هل أحتاج إلى مكتبة i18n منفصلة داخل جزر React الخاصة بي؟">

لا. تقرأ الحزمة `react-intlayer` نفس القواميس مثل Astro، لذلك لا تقوم بتثبيت `react-i18next` بجانبها. توضح الخطوة 6 كيف تستقبل مكونات الجزيرة اللغة مباشرة من الصفحة.
