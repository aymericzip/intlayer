...
> **`source`** يعيّن بيئة محلية إلى مسار ملف JSON الخاص بها. **`location`** يخبر مراقب Intlayer أي مجلد للبحث عن التغييرات. يضمن الخيار `format: 'vue-i18n'` أن الملاصقات يتم تحليلها بشكل صحيح لـ `vue-i18n`.
