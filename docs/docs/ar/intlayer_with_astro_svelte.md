...
priority: 9
title: "تدويل Astro + Svelte - الدليل الكامل لترجمة تطبيقك"
...
history:
  ...
  - version: 8.7.7
    date: 2026-04-24
    changes: "التوثيق الأولي لـ Astro + Svelte"
author: aymericzip
---

# ترجمة موقع Astro + Svelte الخاص بك باستخدام Intlayer | التدويل (i18n)
...
## دليل خطوة بخطوة لتهيئة Intlayer في Astro + Svelte
...
- **svelte**
  حزمة Svelte الأساسية.

- **svelte-intlayer**
  حزمة لدمج Intlayer في تطبيقات Svelte. توفر `setupIntlayer` بالإضافة إلى مخازن (stores) `useIntlayer` و `useLocale` للتدويل في Svelte.

- **@astrojs/svelte**
  إضافة Astro الرسمية التي تتيح استخدام جزر (islands) مكونات Svelte.
...
أضف إضافة `intlayer` وتكامل Svelte إلى تكوين Astro الخاص بك.
...
> يتيح تكامل `svelte()` استخدام جزر مكونات Svelte عبر `client:only="svelte"`.
...
يجب عليك أيضاً إضافة بيانات تعريف SEO مثل hreflang والروابط الأساسية (canonical) إلى كل صفحة وتضمين جزيرة Svelte للمحتوى التفاعلي من جانب العميل.
...
<html lang={locale} dir={locale === 'ar' ? 'rtl' : 'ltr'}>
  ...
  <body>
    <!-- جزيرة Svelte تقوم برندر جميع المحتويات التفاعلية بما في ذلك مبدل اللغة -->
...
> تعتمد بنية الدليل التي تستخدمها على إعداد `middleware.routing` in `intlayer.config.ts`:
>
...
<Step number={6} title="إنشاء مكون جزيرة Svelte">

أنشئ مكون جزيرة يلف تطبيق Svelte الخاص بك. يجب عليك استدعاء `setupIntlayer` باللغة المكتشفة من الخادم قبل الوصول إلى المخازن (stores).
...
<div>
  <h1>{$content.title}</h1>
  <!-- يتم رندر مبدل اللغة مباشرة داخل الجزيرة -->
...
> يتم تمرير خاصية `locale` من صفحة Astro (اكتشاف الخادم) وتستخدم لتهيئة `setupIntlayer` ، مما يحدد اللغة الأولية لجميع المخازن (stores) في المكون.

</Step>
<Step number={7} title="إضافة مبدل اللغة">

تتوفر وظيفة تبديل اللغة مباشرة داخل جزيرة Svelte (انظر الخطوة 6 أعلاه). وهي تستخدم مخزن `useLocale` من `svelte-intlayer` وتنتقل إلى الرابط المترجم عندما يختار المستخدم لغة جديدة:
...
<script lang="ts">
  ...
  // إعادة استخدام نفس تهيئة اللغة (locale/setupIntlayer) كما هو موضح في الخطوة 6...
...
<Question title="ما هي الحلول المختلفة المتاحة لتدويل موقع Astro مع جزر Svelte؟">

يتعامل خيار `i18n` المدمج في Astro مع البادئات وإعادة التوجيه ولكنه لا يدير المحتوى. يطرح إضافة الجزر تحديًا إضافيًا: الجزر لا تشغل Astro بل تشغل Svelte.

- **Astro `i18n` بالإضافة إلى قواميس مخصصة**، وفي الجزر استخدام **`svelte-i18n`**: مصدران منفصلان للمحتوى دون أنواع مشتركة.
- **`Intlayer`**: طبقة محتوى موحدة واحدة لكليهما. يخدم `astro-intlayer` صفحات `.astro`، ويقرأ `svelte-intlayer` نفس الإعلان لجزر Svelte.
...
<Question title="هل أحتاج إلى مكتبة i18n منفصلة داخل جزر Svelte الخاصة بي؟">

لا. تقرأ الحزمة `svelte-intlayer` نفس القواميس مثل Astro، لذلك لا تقوم بتثبيت `svelte-i18n` بجانبها. توضح الخطوة 6 كيف تستقبل مكونات الجزيرة اللغة مباشرة من الصفحة.
