...
> يتم تخطي التصغير عندما يكون `optimize` في وضع `false`. عندما يكون `editor.enabled` في وضع `true`، يستمر التصغير في العمل، ولكن دون خطوة إعادة تسمية الحقول — يقوم المحرر المرئي بحل التعديلات عبر `keyPath`، لذا يجب أن تبقى أسماء الحقول الأصلية كما هي.
...
> يتم تخطي التطهير (Purge) عندما يكون `optimize` في وضع `false`. ويظل نشطًا عندما يكون `editor.enabled` في وضع `true` — إذ إن الحقل الذي تمت إزالته لا يقرأه أي مكوّن، لذا لا يعرضه المحرر مطلقًا. وفي Next.js تُتخطى العملية كذلك عندما تكون `@intlayer/swc` غير متاحة، وعند تهيئة مستدعِيات محوّلات التوافق.
