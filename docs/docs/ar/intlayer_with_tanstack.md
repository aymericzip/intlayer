...
تعمل دوال `head` الخاصة بالمسارات **خارج** شجرة React، لذا لا يتوفر `useIntlayer` هناك. لديك ثلاث طرق لقراءة قاموس من داخل `head`، وكل منها يوازن بين حجم الحزمة وسرعة جهوزية `head` المستند.
