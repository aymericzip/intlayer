...
عندما يكون `editor.enabled` في وضع `true`، يستمر المكوّن الإضافي في العمل لكنه **يتخطى إعادة تسمية الحقول**: إذ تعيد إعادة التسمية كتابة مفاتيح المحتوى التي يتنقل بينها المفسِّر، مما يجعل `keyPath` الذي يتلقاه المحرر المرئي غير مطابق بعد الآن للقواميس غير المدمجة التي يعدّلها. يبقى ضغط المسافات البيضاء وإزالة البيانات الوصفية من المستوى الأعلى نشطين.
