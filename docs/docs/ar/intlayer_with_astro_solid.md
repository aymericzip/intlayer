...
priority: 9
title: "تدويل Astro + Solid - الدليل الكامل لترجمة تطبيقك"
description: "لا مزيد من i18next. دليل 2026 لبناء تطبيق Astro + Solid متعدد اللغات (i18n). ترجم باستخدام وكلاء الذكاء الاصطناعي وحسّن حجم الحزمة وتحسين محركات البحث والأداء."
keywords:
  - التدويل
  - توثيق
  - Intlayer
...
يجب عليك أيضاً إضافة بيانات تعريف SEO مثل hreflang والروابط الأساسية (canonical) إلى كل صفحة وتضمين جزيرة Solid للمحتوى التفاعلي من جانب العميل.
...
<Question title="ما هي الحلول المختلفة المتاحة لتدويل موقع Astro مع جزر Solid؟">

يتعامل خيار `i18n` المدمج في Astro مع البادئات وإعادة التوجيه ولكنه لا يدير المحتوى. يطرح إضافة الجزر تحديًا إضافيًا: الجزر لا تشغل Astro بل تشغل Solid.

- **Astro `i18n` بالإضافة إلى قواميس مخصصة**، وفي الجزر استخدام **`@solid-primitives/i18n`**: مصدران منفصلان للمحتوى دون أنواع مشتركة.
- **`Intlayer`**: طبقة محتوى موحدة واحدة لكليهما. يخدم `astro-intlayer` صفحات `.astro`، ويقرأ `solid-intlayer` نفس الإعلان لجزر Solid.
...
إلى حد كبير نعم. اتبع [دليل الترحيل](https://github.com/aymericzip/intlayer/blob/main/docs/docs/ar/migration_from_i18next_to_intlayer.md). يمكنك أيضًا الترحيل تدريجيًا: تحافظ [مكونة مزامنة JSON](https://github.com/aymericzip/intlayer/blob/main/docs/docs/ar/plugins/sync-json.md) على كتالوجات JSON كمصدر للحقيقة وتنشئ قواميس Intlayer.
...
<Question title="هل أحتاج إلى مكتبة i18n منفصلة داخل جزر Solid الخاصة بي؟">

لا. تقرأ الحزمة `solid-intlayer` نفس القواميس مثل Astro، لذلك لا تقوم بتثبيت `@solid-primitives/i18n` بجانبها. توضح الخطوة 6 كيف تستقبل مكونات الجزيرة اللغة مباشرة من الصفحة.
