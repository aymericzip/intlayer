...
<Question title="ما هي الحلول المختلفة المتاحة لتدويل تطبيقات Elysia؟">

- **قواميس أساسية**: بدون أمان نوعي أو أدوات مساعدة.
- **`Intlayer`**: مخصص لـ Bun و Elysia، ترجمة وقت البناء، أنواع TypeScript صارمة، وأداء استثنائي.
...
<Question title="كم يضيف i18n إلى حجم حزمة خادم Elysia لدي؟">
...
<Question title="هل يمكنني الترحيل من مكتبات i18n الأخرى دون إعادة كتابة المعالجات؟">

نعم، باتباع دليل الترحيل ومكون مزامنة JSON الإضافي.
...
يقرأ مكون Elysia الإضافي الترويسات وملفات تعريف الارتباط في مرحلة `onRequest` أو `derive`، ويحقن اللغة في `context.locale`.
...
<Question title="هل يتوافق Intlayer تمامًا مع بيئة تشغيل Bun؟">

نعم. يعمل Intlayer أصلاً على Bun، مستفيدًا من تحميل الوحدات السريع والتنفيذ المباشر لـ TypeScript.

</Question>

<Question title="هل يمكنني استخدام رسائل خطأ مترجمة في مخططات Elysia TypeBox؟">

نعم. في خطاف `onError`، يمكنك التقاط أخطاء التحقق من صحة المخطط وإرجاع استجابة مترجمة عبر Intlayer.

</Question>

<Question title="كيف أدير التوجيه المعتمد على اللغة في عناوين URL؟">

باستخدام معلمة المسار `/:locale/` في المسارات وإرجاع 404 للغات غير المعروفة.
