...
لقد قمت بتشغيل نفس التطبيق متعدد اللغات في متصفح حقيقي لكل مجموعة، ثم دونت ما ظهر فعلاً على السلك والوقت الذي استغرقه. يتم الإبلاغ عن الأحجام **بعد ضغط الويب العادي**، لأن ذلك أقرب إلى ما يقوم الأشخاص بتنزيله من عدد المصادر الخام.

- **حجم مكتبة التدويل**: بعد التجميع والتخلص من الرموز غير المستخدمة والتصغير، فإن حجم مكتبة i18n هو حجم رمز الموفرين والمتاجر في مكون فارغ. لا يتضمن تحميل ملفات الترجمة. تجيب على السؤال عن تكلفة المكتبة قبل دخول المحتوى الخاص بك.

- **JavaScript لكل صفحة**: لكل مسار معيار، مقدار الرمز الذي يسحبه المتصفح في تلك الزيارة، بمتوسط الصفحات في المجموعة (وعبر اللغات حيث يجمع التقرير بينها). الصفحات الثقيلة هي صفحات بطيئة.

- **التسرب من اللغات الأخرى**: إنه محتوى نفس الصفحة ولكن بلغة أخرى سيتم تحميله بطريق الخطأ في الصفحة المدققة. هذا المحتوى غير ضروري ويجب تجنبه. (على سبيل المثال، محتوى صفحة `/fr/about` في حزمة صفحة `/en/about`)

- **التسرب من الطرق الأخرى**: نفس الفكرة بالنسبة **للشاشات الأخرى** في التطبيق: ما إذا كان نصها يركب على الرغم من فتح صفحة واحدة فقط. (على سبيل المثال، محتوى صفحة `/en/about` في حزمة صفحة `/en/contact`). تشير النتيجة العالية إلى انقسام ضعيف أو حزم واسعة النطاق.

- **حجم حزمة المكون العادي**: يتم قياس قطع واجهة المستخدم الشائعة **واحدة تلو الأخرى** بدلاً من الاختباء داخل رقم تطبيق عملاق. يوضح ما إذا كان التدويل يضخم بصمت المكونات اليومية. على سبيل المثال، إذا أعاد المكون الرسم، فسيتم تحميل جميع تلك البيانات من الذاكرة. إرفاق JSON ضخم لأي مكون يشبه توصيل متجر كبير من البيانات غير المستخدمة التي ستؤدي إلى إبطاء أداء مكوناتك.

- **استجابة تبديل اللغة**: أقوم بتبديل اللغة باستخدام عنصر التحكم الخاص بالتطبيق وأقيس المدة التي تستغرقها الصفحة حتى تتحول بوضوح، ما قد يلاحظه الزائر، وليس خطوة دقيقة في المعمل.

- **عمل الرسم بعد تغيير اللغة**: متابعة أضيق: مقدار الجهد الذي استغرقته الواجهة لإعادة الرسم باللغة الجديدة بمجرد أن يكون التبديل جارياً. مفيد عندما يختلف الوقت "المحسوس" وتكلفة الإطار.

- **وقت تحميل الصفحة الأولي**: من الملاحة إلى المتصفح يعتبر الصفحة مثقلة تماماً للسيناريوهات التي اختبرتها. جيد للمقارنة بين البدايات الباردة.

- **وقت الترطيب**: عندما يعرضه التطبيق، المدة التي يقضيها العميل في تحويل HTML من الخادم إلى شيء يمكنك فعلاً النقر عليه. يعني الشرطة في الجداول أن هذا التنفيذ لم يوفر رقماً موثوقاً للترطيب في هذا المعيار.
