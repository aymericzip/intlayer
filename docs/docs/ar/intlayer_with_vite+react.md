...
title: Vite + React i18n - الدليل الكامل لترجمة React
description: أفضل حل لحجم البندل وتحسين محركات البحث والأداء والصيانة. اجعل Vite and React تطبيق متعدد اللغات في 2026، ترجمة LLM، Agent Skills & MCP.
keywords:
  - التدويل
  - التوثيق
  - Intlayer
...
> يمكن تعريف إعلانات المحتوى الخاصة بك في أي مكان في تطبيقك بمجرد تضمينها في دليل `contentDir` (افتراضيًا، `./src`). ويجب أن تطابق امتداد ملف إعلان المحتوى (افتراضيًا، `.content.{json,ts,tsx,js,jsx,mjs,cjs}`).
