...
- **`i18next` / `i18next-http-middleware`**: حل وقت التشغيل الشائع باستخدام ملفات JSON.
- **قواميس مخصصة**: بدون فحص للأنواع أو أدوات للأتمتة.
- **`Intlayer`**: برمجية `express-intlayer` الوسيطة، أنواع TypeScript كاملة، ترجمة بالذكاء الاصطناعي، نظام CMS، وقواميس مشتركة مع الواجهة الأمامية.
