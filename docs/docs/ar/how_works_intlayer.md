...
3. توليد أنواع القاموس

4. إنشاء أنواع القواميس
...
- الخادم هو تطبيق Express بسيط يستمع إلى طلبات العميل ويسترجع محتوى تطبيقك، مثل `dictionaries` والإعدادات لجعله متاحًا على جانب العميل.
- من ناحية أخرى، العميل هو تطبيق React يُستخدم للتفاعل مع محتواك باستخدام واجهة بصرية.
...
تعمل مكونات Babel و SWC من خلال تحليل شجرة بناء الجملة المجردة (AST) لتطبيقك لاستبدال استدعاءات دوال Intlayer بكود محسّن. تجعل هذه العملية حزمتك النهائية أخف وزناً في الإنتاج من خلال التأكد من أن قواميس المعجم التي تُستخدم فعلاً فقط يتم استيرادها، مع تحسين التقسيم والحد من حجم الحزمة.
