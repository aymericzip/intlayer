...
<Step number={8} title="إنشاء مكون لتبديل اللغة">
...
<Step number={9} title="إضافة إدارة سمات HTML">
...
- **`Intlayer`**: متكامل تمامًا مع التوجيه المعتمد على نظام الملفات، إعلان المحتوى بجانب المكون، تحسين وقت البناء، ترجمة بالذكاء الاصطناعي، ومحرر مرئي.
...
أقل بكثير من أنظمة الكتالوجات التقليدية، لأن الصفحة تُحمل فقط المحتوى الذي تعرضه بالفعل. انظر [تحسين الحزم](https://github.com/aymericzip/intlayer/blob/main/docs/docs/ar/bundle_optimization.md).
...
لا. قم بتشغيل `npx intlayer extract` وسيقرأ Intlayer مكوناتك، ويسحب السلاسل النصية الموجهة للمستخدم، ويكتب ملف `.content` بجانب كل منها، بحيث تراجع diff بدلاً من نسخ السلاسل إلى كتالوج يدويًا. يوضح الخطوة 11 من هذا الدليل ذلك.

لأتمتة كاملة، يقوم [Intlayer Compiler](https://github.com/aymericzip/intlayer/blob/main/docs/docs/ar/compiler.md) بالشيء نفسه في وقت البناء: يمسح كود JSX و TSX و Vue و Svelte عند كل تغيير، وينشئ القواميس ويزامنها من خلال الاستبدال السريع للوحدات (HMR)، بحيث لا توجد مفاتيح تتم صيانتها يدويًا على الإطلاق.
...
<Question title="ما الفرق بين هذا الدليل ودليل React Router المعتمد على الإعدادات؟">

فقط في طريقة الإعلان عن المسارات: يستخدم هذا الدليل تسميات الملفات بدلاً من شجرة المسارات في `routes.ts`. إعلان المحتوى والخطافات متطابقة تمامًا.

</Question>

<Question title="كيف أضيف مقطع اللغة في مسارات نظام الملفات؟">

يصبح مقطع اللغة جزءًا ديناميكيًا من اسم الملف. يقوم `validatePrefix` بتصفية اللغات غير المعروفة، ويقوم `getLocalizedUrl` بتحويل العناوين.

</Question>

<Question title="هل يعمل هذا في وضع إطار عمل React Router مع SSR ومحملات البيانات؟">

نعم. تحل دوال الرندر واللوادر في الخادم اللغة مباشرة وترسل كود HTML المكتمل إلى العميل.

</Question>

<Question title="كيف أضيف وسوم hreflang والبيانات الوصفية المترجمة لـ SEO؟">

استخدم `getMultilingualUrls` داخل تصدير `meta` للمسار لتعيين اللغات البديلة.

</Question>

<Question title="كيف أنشئ مكون رابط مترجم؟">

من خلال إنشاء غلاف على `Link` باستخدام `getLocalizedUrl`، والذي يضيف تلقائيًا بادئة اللغة النشطة إلى الروابط الداخلية.
