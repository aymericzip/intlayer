...
title: "تدويل Vite + Preact - الدليل الكامل لترجمة تطبيقك"
...
في هذه البنية، يوفر `preact-intlayer` موفر `IntlayerProvider` المثبت في `main.tsx` لتغليف شجرة Preact. توجد إعلانات المحتوى في `src/` بجانب المكونات.
...
> يمكن تعريف إعلانات المحتوى الخاصة بك في أي مكان في تطبيقك طالما كانت مدرجة في دليل `contentDir` (افتراضيًا، `./src`). وتتطابق مع امتداد ملف إعلان المحتوى (افتراضيًا، `.content.{json,ts,tsx,js,jsx,mjs,cjs,md,mdx,yaml,yml}`).
...
<Question title="كيف أستخدم المحتوى المترجم داخل مكون Preact؟">

استدعِ `useIntlayer` في مكونك تمامًا كما تفعل في React. `preact-intlayer` هو الربط الأصلي لـ Preact. توضح الخطوة 5 الاستخدام وتغطي الخطوة 11 Markdown و HTML.
