...
| --------------------------------------------------------------------- | --------------- |
| صيغ الجمع النحوية الخاصة باللغة (تفاحة واحدة / تفاحتان / 5 تفاحات)    | `plural`        |
| النطاقات العددية المخصصة (`<5`، `>=10`) أو الفئات غير التابعة لـ CLDR | `enu`           |

إذا كنت تستهدف اللغة الإنجليزية فقط (التي تحتوي على `one` / `other` فقط)، فكلاهما يعمل. لأي لغة بها تمييز `few` / `many` / `two`، يُفضل استخدام `plural`.
