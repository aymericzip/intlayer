...
- `'local&remote'`: يتم إدارة القاموس محليًا وعن بُعد. بعد الإرسال إلى CMS، سيبقى متزامنًا—يتم إرسال التغييرات من الملف المحلي إلى CMS، ويمكن سحب التغييرات البعيدة مرة أخرى إلى الملف المحلي.
...
- `'live'`: يتم استيراد القاموس ديناميكيًا باستخدام واجهة برمجة تطبيقات المزامنة الحية.

إذا تم تعيينه، فإن هذه الخاصية تتجاوز `importMode` العام المحدد في `intlayer.config.ts`.
