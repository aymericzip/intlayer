...
So với các giải pháp chính như `@solid-primitives/i18n` hoặc `i18next`, Intlayer là giải pháp đi kèm với các tính năng tối ưu hóa tích hợp như:

<AccordionGroup>
<Accordion header="Độ che phủ toàn diện">

Intlayer được tối ưu hóa để hoạt động hoàn hảo với Solid bằng cách cung cấp **phạm vi nội dung cấp thành phần**, **bản dịch phản ứng** và tất cả các tính năng cần thiết để mở rộng quy mô quốc tế hóa (i18n).
...
<Step number={10} title="Tạo thành phần Liên kết Địa phương hóa" isOptional={true}>
...
Nên bỏ qua các tệp được tạo bởi Intlayer. Điều này cho phép bạn tránh commit chúng vào kho Git của mình.

Để thực hiện điều này, bạn có thể thêm các hướng dẫn sau vào tệp `.gitignore` của mình:
...
<Question title="Những giải pháp khác nhau nào có sẵn để quốc tế hóa ứng dụng Vite và Solid?">

Vite không có định kiến nào về i18n, vì vậy các tùy chọn thuộc hệ sinh thái Solid:

- **`@solid-primitives/i18n`**: các primitive cơ bản cho Solid.
- **`Intlayer`**: tích hợp đầy đủ với tính phản ứng của Solid, biên dịch lúc build, dịch thuật AI, visual editor và CMS.

Xem [lý do chọn Intlayer](https://github.com/aymericzip/intlayer/blob/main/docs/docs/vi/interest_of_intlayer.md).

</Question>

<Question title="i18n làm tăng kích thước bundle Solid của tôi bao nhiêu?">
...
<Question title="Làm cách nào để sử dụng nội dung dịch bên trong component Solid?">

Gọi `useIntlayer` trong component của bạn. Nội dung được hỗ trợ bởi các signal của Solid, vì vậy việc thay đổi locale chỉ cập nhật các DOM node đọc nội dung mà không re-render toàn bộ component.
