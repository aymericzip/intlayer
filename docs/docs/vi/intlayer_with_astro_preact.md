...
title: "Astro + Preact i18n - Hướng dẫn đầy đủ để dịch ứng dụng của bạn"
description: "Không còn i18next nữa. Hướng dẫn 2026 để xây dựng ứng dụng Astro + Preact đa ngôn ngữ (i18n). Dịch với các AI agent và tối ưu hóa kích thước bundle, SEO và hiệu suất."
keywords:
  - đa ngôn ngữ
  - tài liệu
  - Intlayer
...
- **@astrojs/preact**
  Tích hợp chính thức của Astro cho phép sử dụng các Preact component islands.
...
Trong kiến trúc này, tích hợp `intlayer()` được đăng ký trong `astro.config.ts` sẽ xây dựng các từ điển của bạn và thêm một middleware giúp xử lý locale của mỗi yêu cầu và hiển thị nó trên `Astro.locals.intlayer`. Các trang nằm dưới phân đoạn rest `src/pages/[...locale]/`, do đó locale mặc định được phân phối mà không có tiền tố và mọi locale khác đều có URL riêng biệt. Các tệp `.astro` đọc nội dung bằng các hook `useIntlayer` / `useLocale` của `astro-intlayer` để render phía máy chủ, và đảo Preact (`src/components/preact/PreactIsland.tsx`) nhận locale do máy chủ phát hiện dưới dạng prop và chuyển nó tới `preact-intlayer` cho phần tương tác. Các khai báo nội dung được đặt cùng với các component của bạn trong `src/`.
...
Thêm plugin `intlayer` vào cấu hình Astro và tích hợp Preact của bạn.
...
> Tích hợp `preact()` cho phép sử dụng các Preact component islands thông qua `client:only="preact"`.
...
> Nếu file nội dung của bạn chứa mã TSX, bạn có thể cần nhập `import { h } from "preact";` hoặc đảm bảo pragma JSX của bạn được cấu hình đúng cho Preact.
...
Bạn cũng nên thêm metadata cho SEO (như hreflang và liên kết canonical) vào mỗi trang và giới thiệu một Preact island cho nội dung tương tác phía client.
...
<html lang={locale} dir={getHTMLTextDir(locale)}>
  ...
  <body>
    <!-- Preact island render tất cả các nội dung tương tác, bao gồm cả bộ chuyển đổi ngôn ngữ -->
...
<Step number={6} title="Tạo một Preact Island Component">

Tạo một thành phần island bao bọc ứng dụng Preact của bạn và nhận ngôn ngữ được máy chủ phát hiện:
...
> Prop `locale` được truyền từ trang Astro (phát hiện phía máy chủ) đến `IntlayerProvider`, đóng vai trò là ngôn ngữ ban đầu cho tất cả các Preact hooks bên trong cây thành phần.

> Lưu ý: Trong Preact, thuộc tính HTML `class` được sử dụng thay vì `className`.

</Step>
<Step number={7} title="Thêm bộ chuyển đổi ngôn ngữ">

Tạo một thành phần Preact `LocaleSwitcher` để đọc các ngôn ngữ có sẵn và điều hướng đến URL bản địa hóa khi người dùng chọn ngôn ngữ mới:
...
```json5 fileName="tsconfig.json"
{
  compilerOptions: {
    // ...
    jsx: "react-jsx",
    jsxImportSource: "preact", // Khuyên dùng cho Preact 10+
  },
  "include": [
...
<Question title="Những giải pháp khác nhau nào có sẵn để quốc tế hóa trang Astro có các đảo Preact?">

Tùy chọn `i18n` tích hợp của Astro xử lý tiền tố và chuyển hướng nhưng không quản lý nội dung. Việc thêm đảo đặt ra một thách thức bổ sung: đảo không chạy Astro mà chạy Preact.

- **`i18n` của Astro cộng với các từ điển thủ công**, và trong các đảo sử dụng **`preact-i18n`**: hai nguồn nội dung riêng biệt không có kiểu dữ liệu chia sẻ.
- **`Intlayer`**: một lớp nội dung duy nhất cho cả hai. `astro-intlayer` phục vụ các trang `.astro`, và `preact-intlayer` đọc cùng một khai báo cho các đảo Preact.
...
<Question title="Tôi có cần một thư viện i18n riêng bên trong các đảo Preact của mình không?">

Không. Gói `preact-intlayer` đọc cùng các từ điển như Astro, vì vậy bạn không cần cài đặt `preact-i18n` đi kèm. Bước 6 cho thấy cách các component đảo nhận locale trực tiếp từ trang.
