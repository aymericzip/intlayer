...
Trong kiến trúc này, `vue-intlayer` cung cấp plugin được cài đặt trong `main.ts`. Các thành phần Vue khai báo và sử dụng các bản dịch cục bộ bằng cách sử dụng các composable, với tính năng tải lại nóng của Vite được xử lý bởi `vite-intlayer`.
...
<Question title="Những giải pháp khác nhau nào có sẵn để quốc tế hóa ứng dụng Vue?">

- **`vue-i18n`**: thư viện tiêu chuẩn với các catalog thông báo toàn cục.
- **`Intlayer`**: khai báo cạnh component, plugin Vite biên dịch lúc build, kiểu dữ liệu TypeScript đầy đủ, dịch thuật AI và visual editor.

Xem [lý do chọn Intlayer](https://github.com/aymericzip/intlayer/blob/main/docs/docs/vi/interest_of_intlayer.md) và [benchmark cho Vue](https://github.com/aymericzip/intlayer/blob/main/docs/docs/vi/benchmark/vue.md).

</Question>
<Question title="i18n làm tăng kích thước bundle Vue của tôi bao nhiêu?">
...
<Question title="Tôi có thể di chuyển từ vue-i18n mà không cần viết lại component không?">

Có. Sử dụng [hướng dẫn di chuyển vue-i18n](https://github.com/aymericzip/intlayer/blob/main/docs/docs/vi/migration_from_vue-i18n_to_intlayer.md) hoặc các adapter tương thích.
...
<Question title="Intlayer có hoạt động với Vue Composition API và script setup không?">

Có. `useIntlayer` là composable tiêu chuẩn cho `<script setup>`, và giá trị trả về có tính phản ứng, giúp việc chuyển đổi ngôn ngữ cập nhật giao diện mà không cần tải lại trang.

</Question>
<Question title="Làm cách nào để thiết lập định tuyến được bản địa hóa trong Vue SPA?">

Cấu hình tham số locale trong các tuyến đường Vue Router của bạn hoặc sử dụng tham số tìm kiếm; Intlayer sẽ đồng bộ hóa ngôn ngữ hoạt động với URL. Xem [tài liệu cấu hình](https://github.com/aymericzip/intlayer/blob/main/docs/docs/vi/configuration.md).

</Question>
<Question title="Làm cách nào để đặt các thuộc tính html lang và dir cho ngôn ngữ từ phải sang trái?">

Bước 9 giải thích điều này. Hàm `getHTMLTextDir` trả về `rtl` cho tiếng Ả Rập hoặc tiếng Do Thái, cho phép bạn gắn `lang` và `dir` trên phần tử gốc.

</Question>
<Question title="Làm cách nào để tự động dịch ứng dụng Vue bằng AI?">

Chạy `npx intlayer fill`. Công cụ này phát hiện các bản dịch còn thiếu và điền chúng bằng LLM bạn chọn. Xem [lệnh fill](https://github.com/aymericzip/intlayer/blob/main/docs/docs/vi/cli/fill.md).
