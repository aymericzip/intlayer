...
Trong kiến trúc này, `angular-intlayer` cung cấp `provideIntlayer()`, được đăng ký trong `src/app/app.config.ts` để mọi component đều có thể đọc nội dung của nó thông qua signal `useIntlayer`. Angular CLI được chuyển sang builder esbuild tùy chỉnh, và `esbuild.plugins.ts` đăng ký plugin Intlayer vào bản dựng để theo dõi và xây dựng lại các khai báo nội dung của bạn. Các khai báo nội dung được đặt cùng với các component trong `src/app/`.
...
<Question title="Những giải pháp khác nhau nào có sẵn để quốc tế hóa ứng dụng Angular?">

- **`@angular/localize` tích hợp**: biên dịch riêng theo từng ngôn ngữ.
- **`ngx-translate`** và **`Transloco`**: các catalog runtime.
- **`Intlayer`**: Standalone components, Signal inputs, và pipeline build mới dựa trên Vite.
...
<Question title="Intlayer có hoạt động với Angular Signals không?">

Có. `useIntlayer` trả về một signal của Angular hoạt động mượt mà trong kiến trúc Zoneless của Angular.

</Question>
<Question title="Việc thay đổi ngôn ngữ có yêu cầu tải lại trang không?">

Không. Vì dựa trên signal, việc chuyển đổi ngôn ngữ được phản ánh ngay lập tức trên giao diện.

</Question>
<Question title="Làm cách nào để thiết lập định tuyến có locale trong Angular?">

Khai báo tham số `:locale` trong các tuyến và đồng bộ hóa với provider của Intlayer.

</Question>
<Question title="Làm cách nào để xử lý các ngôn ngữ từ phải sang trái trong Angular?">

Sử dụng `getHTMLTextDir` để đặt `lang` và `dir` trên phần tử gốc của ứng dụng.
