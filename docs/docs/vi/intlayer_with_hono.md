...
description: "Không còn i18next nữa. Hướng dẫn 2026 để xây dựng ứng dụng Hono đa ngôn ngữ (i18n). Dịch với các AI agent và tối ưu hóa kích thước bundle, SEO và hiệu suất."
keywords:
  - Quốc tế hóa
  - Tài liệu
  - Intlayer
...
Có, thông qua di chuyển dần dần hoặc đồng bộ hóa tệp dịch.
...
Middleware Hono đọc cookie và tiêu đề `Accept-Language` qua `c.req`, cung cấp ngôn ngữ hoạt động tại `c.get('locale')`.
...
<Question title="Intlayer có hoạt động trên Cloudflare Workers và môi trường edge không?">

Có. Intlayer tạo mã JavaScript thuần túy mà không phụ thuộc vào hệ thống tệp cục bộ khi runtime, vì vậy nó chạy mượt mà trên Cloudflare Workers, Deno và Vercel Edge.

</Question>

<Question title="Làm cách nào để sử dụng tiền tố URL được bản địa hóa trong các tuyến đường Hono?">

Sử dụng phân đoạn `/:locale/` hoặc tạo các sub-router trong Hono.
