...
title: "Astro + Svelte i18n - Hướng dẫn đầy đủ để dịch ứng dụng của bạn"
description: "Không còn i18next nữa. Hướng dẫn 2026 để xây dựng ứng dụng Astro + Svelte đa ngôn ngữ (i18n). Dịch với các AI agent và tối ưu hóa kích thước bundle, SEO và hiệu suất."
keywords:
  - đa ngôn ngữ
  - tài liệu
  - Intlayer
...
## Hướng dẫn từng bước để cấu hình Intlayer trong Astro + Svelte
...
- **svelte-intlayer**
  Gói để tích hợp Intlayer vào các ứng dụng Svelte. Nó cung cấp `setupIntlayer` cũng như các store `useIntlayer` và `useLocale` cho đa ngôn ngữ trong Svelte.

- **@astrojs/svelte**
  Tích hợp chính thức của Astro cho phép sử dụng các Svelte component islands.
...
Trong kiến trúc này, tích hợp `intlayer()` được đăng ký trong `astro.config.ts` sẽ xây dựng các từ điển của bạn và thêm một middleware giúp xử lý locale của mỗi yêu cầu và hiển thị nó trên `Astro.locals.intlayer`. Các trang nằm dưới phân đoạn rest `src/pages/[...locale]/`, do đó locale mặc định được phân phối mà không có tiền tố và mọi locale khác đều có URL riêng biệt. Các tệp `.astro` đọc nội dung bằng các hook `useIntlayer` / `useLocale` của `astro-intlayer` để render phía máy chủ, và đảo Svelte (`src/components/svelte/SvelteIsland.svelte`) nhận locale do máy chủ phát hiện dưới dạng prop và chuyển nó tới `svelte-intlayer` cho phần tương tác. Các khai báo nội dung được đặt cùng với các component của bạn trong `src/`.
...
Thêm plugin `intlayer` vào cấu hình Astro và tích hợp Svelte của bạn.
...
> Tích hợp `svelte()` cho phép sử dụng các Svelte component islands thông qua `client:only="svelte"`.
...
Bạn cũng nên thêm metadata cho SEO (như hreflang và liên kết canonical) vào mỗi trang và giới thiệu một Svelte island cho nội dung tương tác phía client.
...
<html lang={locale} dir={getHTMLTextDir(locale)}>
  ...
  <body>
    <!-- Svelte island render tất cả các nội dung tương tác, bao gồm cả bộ chuyển đổi ngôn ngữ -->
...
<Step number={6} title="Tạo một Svelte Island Component">

Tạo một thành phần island bao bọc ứng dụng Svelte của bạn. Bạn phải gọi `setupIntlayer` với ngôn ngữ được máy chủ phát hiện trước khi truy cập bất kỳ store nào.
...
<div>
  <h1>{$content.title}</h1>
  <!-- Bộ chuyển đổi ngôn ngữ được render trực tiếp bên trong island -->
...
> Prop `locale` được truyền từ trang Astro (phát hiện phía máy chủ) và được sử dụng để khởi tạo `setupIntlayer`, thiết lập ngôn ngữ ban đầu cho tất cả các stores bên trong thành phần.

</Step>
<Step number={7} title="Thêm bộ chuyển đổi ngôn ngữ">

Chức năng chuyển đổi ngôn ngữ được tích hợp trực tiếp vào Svelte island (xem Bước 6 ở trên). Nó sử dụng store `useLocale` từ `svelte-intlayer` và điều hướng đến URL bản địa hóa khi người dùng chọn ngôn ngữ mới:
...
<script lang="ts">
  ...
  // Giả sử logic locale/setupIntlayer giống như Bước 6...
...
<Question title="Những giải pháp khác nhau nào có sẵn để quốc tế hóa trang Astro có các đảo Svelte?">

Tùy chọn `i18n` tích hợp của Astro xử lý tiền tố và chuyển hướng nhưng không quản lý nội dung. Việc thêm đảo đặt ra một thách thức bổ sung: đảo không chạy Astro mà chạy Svelte.

- **`i18n` của Astro cộng với các từ điển thủ công**, và trong các đảo sử dụng **`svelte-i18n`**: hai nguồn nội dung riêng biệt không có kiểu dữ liệu chia sẻ.
- **`Intlayer`**: một lớp nội dung duy nhất cho cả hai. `astro-intlayer` phục vụ các trang `.astro`, và `svelte-intlayer` đọc cùng một khai báo cho các đảo Svelte.
...
<Question title="Tôi có cần một thư viện i18n riêng bên trong các đảo Svelte của mình không?">

Không. Gói `svelte-intlayer` đọc cùng các từ điển như Astro, vì vậy bạn không cần cài đặt `svelte-i18n` đi kèm. Bước 6 cho thấy cách các component đảo nhận locale trực tiếp từ trang.
