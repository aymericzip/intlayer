...
> Các khai báo nội dung của bạn có thể được định nghĩa ở bất kỳ đâu trong ứng dụng miễn là chúng được bao gồm trong thư mục `contentDir` (mặc định là `./src`). Và phù hợp với phần mở rộng tệp khai báo nội dung (mặc định là `.content.{json,ts,tsx,js,jsx,mjs,cjs,md,mdx,yaml,yml}`).
...
Để biết thêm chi tiết về cách sử dụng tiện ích mở rộng, tham khảo [tài liệu Tiện ích mở rộng Intlayer cho VS Code](https://github.com/aymericzip/intlayer/blob/main/docs/docs/vi/vs_code_extension.md).
...
<Question title="Những giải pháp khác nhau nào có sẵn để quốc tế hóa ứng dụng NestJS?">

- **`nestjs-i18n`**: module phổ biến cho NestJS dùng JSON và YAML.
- **`Intlayer`**: tương thích hoàn toàn với Dependency Injection và interceptor, kiểm tra kiểu tại build time, dịch thuật AI và từ điển dùng chung với frontend.
...
<Question title="i18n làm tăng kích thước bundle server NestJS của tôi bao nhiêu?">
...
<Question title="Tôi có thể di chuyển từ nestjs-i18n mà không cần viết lại handler và service không?">

Phần lớn là có. [Plugin sync JSON](https://github.com/aymericzip/intlayer/blob/main/docs/docs/vi/plugins/sync-json.md) giữ nguyên các tệp hiện có trong khi tạo từ điển Intlayer.
...
Interceptor hoặc middleware của NestJS kiểm tra tiêu đề và cookie, gắn locale vào request context.
...
<Question title="Intlayer hoạt động như thế nào với Dependency Injection (DI) trong NestJS?">

Các service và interceptor của Intlayer có thể được tiêm vào container IoC của NestJS, giúp dễ dàng truy cập locale trong controller và provider.

</Question>
<Question title="Tôi có thể bản địa hóa thông báo xác thực NestJS DTO không?">

Có. Trong các custom decorator của `class-validator` hoặc exception filter, bạn có thể gọi `t()` hoặc `getIntlayer()`.
