...
<Question title="Những giải pháp khác nhau nào có sẵn để quốc tế hóa ứng dụng Solid Start?">

- **`@solid-primitives/i18n`**: từ điển đơn giản dựa trên signal với hỗ trợ SSR hạn chế.
- **`Intlayer`**: hỗ trợ SSR và prerender, phân đoạn tuyến đường, signal của Solid, dịch thuật AI và visual editor.

Xem [lý do chọn Intlayer](https://github.com/aymericzip/intlayer/blob/main/docs/docs/vi/interest_of_intlayer.md).

</Question>

<Question title="i18n làm tăng kích thước bundle Solid Start của tôi bao nhiêu?">
...
<Question title="Intlayer có hoạt động với server side rendering (SSR) trong Solid Start không?">

Có. Nội dung được phân giải trong quá trình SSR, và bước 16 bao gồm cấu hình prerender để tạo HTML tĩnh cho từng locale.

</Question>

<Question title="Việc chuyển đổi ngôn ngữ có làm re-render toàn bộ ứng dụng không?">

Không. Nội dung được hỗ trợ bởi các signal của Solid, do đó việc thay đổi ngôn ngữ chỉ cập nhật các DOM node văn bản thay đổi mà không cần dựng lại cây component.

</Question>

<Question title="Làm cách nào để thêm liên kết canonical và thẻ hreflang?">

Trong sitemap sử dụng `generateSitemap` hoặc `getMultilingualUrls`, tạo các liên kết thay thế `xhtml:link` cho các công cụ tìm kiếm.

</Question>

<Question title="Làm cách nào để xử lý các trang 404 trên các tuyến đường được bản địa hóa?">

Bước 14 giải thích điều này. `validatePrefix` kiểm tra xem phân đoạn ngôn ngữ trong URL có hợp lệ hay không, trả về trạng thái 404 cho các tuyến không xác định.

</Question>

<Question title="Tôi có bắt buộc phải đưa locale vào URL không?">

Không. Cài đặt `routing.mode` chấp nhận `"prefix-no-default"` (mặc định), `"prefix-all"`, `"no-prefix"`, và `"search-params"`.

</Question>

<Question title="Làm cách nào để lấy locale trong hàm server?">

Trong các hàm server của Solid Start, hàm `getIntlayer` tự động phân giải locale từ request context.
