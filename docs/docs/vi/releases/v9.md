...
## Vite Plugin: Tích hợp Compiler & Proxy

Plugin Vite `intlayer()` giờ đây đã tích hợp trực tiếp **compiler** và **locale-routing proxy**, vì vậy hầu hết các dự án chỉ cần một plugin duy nhất trong `vite.config.ts`:
...
- **Compiler**: Tự động kích hoạt khi `compiler.enabled` được thiết lập và có một đường dẫn `compiler.output`. Bạn không cần phải đăng ký `intlayerCompiler()` riêng biệt nữa.
- **Proxy**: Tự động kích hoạt dựa trên tùy chọn mới `routing.enableProxy` (mặc định là `true`). Nó cấu hình middleware phát hiện / chuyển hướng / ghi đè ngôn ngữ trong development, preview, và production SSR. Bạn không cần phải đăng ký `intlayerProxy()` riêng biệt nữa.

### Tùy chọn `routing.enableProxy`

Một tùy chọn mới `routing.enableProxy` kiểm soát xem proxy định tuyến ngôn ngữ có được cắm vào hay không. Mặc định là `true`. Tắt nó đi khi bạn muốn tự xử lý định tuyến ngôn ngữ:
...
Các plugin độc lập `intlayerCompiler()` và `intlayerProxy()` vẫn được xuất ra cho các thiết lập nâng cao. Việc đăng ký chúng cùng với `intlayer()` là an toàn — mỗi plugin sẽ tự động loại bỏ trùng lặp và chỉ chạy một lần.
