...
> `getLocaleFromStorage` akan memeriksa locale dari header atau cookie tergantung pada konfigurasi Anda. Lihat [Configuration](https://intlayer.org/doc/concept/configuration) untuk detail lebih lanjut.
...
<Question title="Apa saja solusi berbeda yang tersedia untuk menginternasionalkan aplikasi SvelteKit?">

- **`svelte-i18n`** dan **`typesafe-i18n`**: katalog berbasis store.
- **`Paraglide`**: kompiler pesan.
- **`Intlayer`**: deklarasi di sebelah komponen, dukungan SSR dan SSG, reaktivitas Svelte penuh, dan terjemahan AI.
