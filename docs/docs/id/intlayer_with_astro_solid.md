...
Dalam arsitektur ini, integrasi `intlayer()` yang didaftarkan di `astro.config.ts` membangun kamus Anda dan menambahkan middleware yang menyelesaikan locale setiap permintaan serta mengeksposnya di `Astro.locals.intlayer`. Halaman berada di bawah segmen rest `src/pages/[...locale]/`, sehingga locale default disajikan tanpa awalan dan setiap locale lainnya mendapatkan URL khusus sendiri. File `.astro` membaca konten dengan hook `useIntlayer` / `useLocale` dari `astro-intlayer` untuk rendering server, dan pulau Solid (`src/components/solid/SolidIsland.tsx`) menerima locale yang terdeteksi server sebagai prop dan meneruskannya ke `solid-intlayer` untuk bagian interaktif. Deklarasi konten ditempatkan bersama komponen Anda di `src/`.
...
Anda juga harus menambahkan metadata SEO (seperti hreflang dan tautan kanonikal) di setiap halaman dan memperkenalkan island Solid untuk konten interaktif di sisi klien.
...
<Question title="Apa saja solusi berbeda yang tersedia untuk menginternasionalkan situs Astro dengan pulau Solid?">

Opsi bawaan `i18n` Astro menangani prefix dan redirect tetapi tidak mengelola konten. Menambahkan pulau menimbulkan pertanyaan tambahan: pulau tersebut tidak menjalankan Astro, melainkan Solid.

- **Astro `i18n` plus kamus buatan sendiri**, dan di pulau menggunakan **`@solid-primitives/i18n`**: dua sumber konten terpisah tanpa typing bersama.
- **`Intlayer`**: satu lapisan konten untuk keduanya. `astro-intlayer` melayani halaman `.astro`, dan `solid-intlayer` membaca deklarasi yang sama untuk pulau Solid.
...
Sebagian besar ya. Ikuti [panduan migrasi](https://github.com/aymericzip/intlayer/blob/main/docs/docs/id/migration_from_i18next_to_intlayer.md). Anda juga dapat bermigrasi secara bertahap: [plugin sync JSON](https://github.com/aymericzip/intlayer/blob/main/docs/docs/id/plugins/sync-json.md) menjaga katalog JSON Anda sebagai sumber kebenaran dan menghasilkan kamus Intlayer.
...
<Question title="Apakah saya memerlukan library i18n terpisah di dalam pulau Solid saya?">

Tidak. Paket `solid-intlayer` membaca kamus yang sama dengan Astro, jadi Anda tidak menginstal `@solid-primitives/i18n` di sampingnya. Langkah 6 menunjukkan bagaimana komponen pulau menerima locale langsung dari halaman.
