...
<Question title="Apa saja solusi berbeda yang tersedia untuk menginternasionalkan situs Astro dengan pulau React?">

Opsi bawaan `i18n` Astro menangani prefix dan redirect tetapi tidak mengelola konten. Menambahkan pulau menimbulkan pertanyaan tambahan: pulau tersebut tidak menjalankan Astro, melainkan React.

- **Astro `i18n` plus kamus buatan sendiri**, dan di pulau menggunakan **`react-i18next`**: dua sumber konten terpisah tanpa typing bersama.
- **`Intlayer`**: satu lapisan konten untuk keduanya. `astro-intlayer` melayani halaman `.astro`, dan `react-intlayer` membaca deklarasi yang sama untuk pulau React.
...
<Question title="Bisakah saya bermigrasi dari `react-i18next` tanpa menulis ulang komponen?">
...
<Question title="Apakah saya memerlukan library i18n terpisah di dalam pulau React saya?">

Tidak. Paket `react-intlayer` membaca kamus yang sama dengan Astro, jadi Anda tidak menginstal `react-i18next` di sampingnya. Langkah 6 menunjukkan bagaimana komponen pulau menerima locale langsung dari halaman.
