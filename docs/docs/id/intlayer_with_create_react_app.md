...
<Question title="Apa saja solusi berbeda yang tersedia untuk menginternasionalkan proyek Create React App?">

- **`react-i18next` / `i18next`**: namespace JSON di runtime.
- **`react-intl`** dan **`Lingui`**: solusi berbasis ICU.
- **`Intlayer`**: kompilasi build time via `react-scripts-intlayer`, typing TypeScript ketat, AI terjemahan, dan visual editor.
