...
Anda juga harus menambahkan metadata SEO (seperti hreflang dan tautan kanonikal) di setiap halaman dan memperkenalkan island Preact untuk konten interaktif di sisi klien.
...
<Question title="Apa saja solusi berbeda yang tersedia untuk menginternasionalkan situs Astro dengan pulau Preact?">

Opsi bawaan `i18n` Astro menangani prefix dan redirect tetapi tidak mengelola konten. Menambahkan pulau menimbulkan pertanyaan tambahan: pulau tersebut tidak menjalankan Astro, melainkan Preact.

- **Astro `i18n` plus kamus buatan sendiri**, dan di pulau menggunakan **`preact-i18n`**: dua sumber konten terpisah tanpa typing bersama.
- **`Intlayer`**: satu lapisan konten untuk keduanya. `astro-intlayer` melayani halaman `.astro`, dan `preact-intlayer` membaca deklarasi yang sama untuk pulau Preact.
...
Sebagian besar ya. Ikuti [panduan migrasi](https://github.com/aymericzip/intlayer/blob/main/docs/docs/id/migration_from_react-i18next_to_intlayer.md). Anda juga dapat bermigrasi secara bertahap: [plugin sync JSON](https://github.com/aymericzip/intlayer/blob/main/docs/docs/id/plugins/sync-json.md) menjaga katalog JSON Anda sebagai sumber kebenaran dan menghasilkan kamus Intlayer.
...
<Question title="Apakah saya memerlukan library i18n terpisah di dalam pulau Preact saya?">

Tidak. Paket `preact-intlayer` membaca kamus yang sama dengan Astro, jadi Anda tidak menginstal `preact-i18n` di sampingnya. Langkah 6 menunjukkan bagaimana komponen pulau menerima locale langsung dari halaman.
