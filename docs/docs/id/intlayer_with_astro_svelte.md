...
# Terjemahkan Situs Astro + Svelte Anda dengan Intlayer | Internasionalisasi (i18n)
...
## Panduan Langkah-demi-Langkah untuk Mengonfigurasi Intlayer di Astro + Svelte
...
- **svelte-intlayer**
  Paket untuk mengintegrasikan Intlayer ke dalam aplikasi Svelte. Paket ini menyediakan `setupIntlayer` serta store `useIntlayer` dan `useLocale` untuk internasionalisasi di Svelte.

- **@astrojs/svelte**
  Integrasi resmi Astro yang memungkinkan penggunaan island komponen Svelte.
...
Dalam arsitektur ini, integrasi `intlayer()` yang didaftarkan di `astro.config.ts` membangun kamus Anda dan menambahkan middleware yang menyelesaikan locale setiap permintaan serta mengeksposnya di `Astro.locals.intlayer`. Halaman berada di bawah segmen rest `src/pages/[...locale]/`, sehingga locale default disajikan tanpa awalan dan setiap locale lainnya mendapatkan URL khusus sendiri. File `.astro` membaca konten dengan hook `useIntlayer` / `useLocale` dari `astro-intlayer` untuk rendering server, dan pulau Svelte (`src/components/svelte/SvelteIsland.svelte`) menerima locale yang terdeteksi server sebagai prop dan meneruskannya ke `svelte-intlayer` untuk bagian interaktif. Deklarasi konten ditempatkan bersama komponen Anda di `src/`.
...
Tambahkan plugin `intlayer` ke konfigurasi Astro Anda dan integrasi Svelte.
...
> Integrasi `svelte()` memungkinkan penggunaan island komponen Svelte melalui `client:only="svelte"`.
...
Anda juga harus menambahkan metadata SEO (seperti hreflang dan tautan kanonikal) di setiap halaman dan memperkenalkan island Svelte untuk konten interaktif di sisi klien.
...
<html lang={locale} dir={getHTMLTextDir(locale)}>
  ...
  <body>
    <!-- Island Svelte merender semua konten interaktif, termasuk pengalih bahasa -->
...
<Step number={6} title="Membuat Komponen Island Svelte">

Buat komponen island yang membungkus aplikasi Svelte Anda. Anda harus memanggil `setupIntlayer` dengan bahasa yang dideteksi server sebelum mengakses store mana pun.
...
<div>
  <h1>{$content.title}</h1>
  <!-- Pengalih bahasa dirender langsung di dalam island -->
...
> Prop `locale` diteruskan dari halaman Astro (deteksi sisi server) dan digunakan untuk menginisialisasi `setupIntlayer`, menyetel bahasa awal untuk semua store di dalam komponen.

</Step>
<Step number={7} title="Menambahkan Pengalih Bahasa">

Fungsionalitas peralihan bahasa terintegrasi langsung di dalam island Svelte (lihat Langkah 6 di atas). Pengalih ini menggunakan store `useLocale` dari `svelte-intlayer` dan menavigasi ke URL yang dilokalkan saat pengguna memilih bahasa baru:
...
<script lang="ts">
  ...
  // Asumsikan logika locale/setupIntlayer yang sama dari Langkah 6...
...
<Question title="Apa saja solusi berbeda yang tersedia untuk menginternasionalkan situs Astro dengan pulau Svelte?">

Opsi bawaan `i18n` Astro menangani prefix dan redirect tetapi tidak mengelola konten. Menambahkan pulau menimbulkan pertanyaan tambahan: pulau tersebut tidak menjalankan Astro, melainkan Svelte.

- **Astro `i18n` plus kamus buatan sendiri**, dan di pulau menggunakan **`svelte-i18n`**: dua sumber konten terpisah tanpa typing bersama.
- **`Intlayer`**: satu lapisan konten untuk keduanya. `astro-intlayer` melayani halaman `.astro`, dan `svelte-intlayer` membaca deklarasi yang sama untuk pulau Svelte.

Mampu mendeklarasikan string sekali dan menggunakannya di halaman statis maupun di pulau interaktif adalah alasan utama memilih satu lapisan konten. Lihat [mengapa Intlayer](https://github.com/aymericzip/intlayer/blob/main/docs/docs/id/interest_of_intlayer.md).
...
<Question title="Bisakah saya bermigrasi dari `svelte-i18n` tanpa menulis ulang komponen?">

Sebagian besar ya. Ikuti [panduan migrasi](https://github.com/aymericzip/intlayer/blob/main/docs/docs/id/compat/svelte-i18n.md). Anda juga dapat bermigrasi secara bertahap: [plugin sync JSON](https://github.com/aymericzip/intlayer/blob/main/docs/docs/id/plugins/sync-json.md) menjaga katalog JSON Anda sebagai sumber kebenaran dan menghasilkan kamus Intlayer.
...
<Question title="Apakah saya memerlukan library i18n terpisah di dalam pulau Svelte saya?">

Tidak. Paket `svelte-intlayer` membaca kamus yang sama dengan Astro, jadi Anda tidak menginstal `svelte-i18n` di sampingnya. Langkah 6 menunjukkan bagaimana komponen pulau menerima locale langsung dari halaman.

</Question>
<Question title="Bagaimana pulau mengetahui bahasa apa yang di-render halaman?">

Halaman Astro meneruskan locale sebagai prop, dan provider Intlayer di pulau menangkapnya, sehingga pulau terhidrasi dalam bahasa yang sama yang dirender server. Ini menghindari kedipan bahasa default (flash of default language).
