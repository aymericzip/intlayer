...
## Pemformatan Kode Otomatis: konfigurasi formatCommand

v7 memperkenalkan opsi `formatCommand` dalam konfigurasi editor, memungkinkan Anda untuk secara otomatis memformat file konten ketika ditulis oleh Intlayer. Ini memastikan gaya kode dan pemformatan yang konsisten di seluruh file deklarasi konten Anda.

Jika tidak diatur, Intlayer akan mencoba mendeteksi perintah format secara otomatis. Dengan mencoba menyelesaikan perintah berikut: prettier, biome, eslint.

### Konfigurasi

Opsi `formatCommand` menerima template string di mana `{{file}}` akan diganti dengan path file yang sebenarnya:
...
### Pemformat yang Didukung

Anda dapat menggunakan pemformat kode apa pun yang menerima jalur file sebagai argumen:
...
**Menggunakan pemformat bawaan Bun:**
...
- **Pemformatan konsisten**: Semua file konten diformat secara otomatis sesuai dengan panduan gaya proyek Anda
- **Pengalaman pengembang**: Tidak perlu memformat file secara manual setelah Intlayer menulisnya
- **Konsistensi tim**: Memastikan semua anggota tim memiliki pemformatan yang sama diterapkan pada file konten
- **Integrasi CI/CD**: File konten mempertahankan pemformatan konsisten dalam alur kerja otomatis

### Cara kerjanya

Ketika Intlayer menulis atau memperbarui file deklarasi konten (`.content.ts`, `.content.js`, dll.), ia secara otomatis menjalankan perintah format yang ditentukan pada file tersebut. Placeholder `{{file}}` diganti dengan jalur file aktual, dan perintah dijalankan di direktori dasar proyek.

---

## Konfigurasi Dictionary: Organisasi dan struktur yang lebih baik

v7 memperkenalkan bagian konfigurasi `dictionary` yang baru dan dedicated yang menyediakan organisasi yang lebih baik untuk pengaturan terkait dictionary dan manajemen konten yang ditingkatkan.

### Struktur konfigurasi dictionary baru

Properti `fill` telah dipindahkan dari bagian `content` ke bagian `dictionary` yang baru, memberikan pemisahan kepedulian yang lebih jelas:
...
- **Organisasi yang lebih jelas**: Pengaturan khusus kamus sekarang dikelompokkan bersama
- **Pemisahan tanggung jawab yang lebih baik**: Penemuan konten vs. operasi kamus dipisahkan dengan jelas
- **Kemudahan pemeliharaan yang ditingkatkan**: Lebih mudah untuk memahami dan memodifikasi konfigurasi terkait kamus
- **Ekstensibilitas di masa depan**: Bagian kamus dapat mengakomodasi pengaturan khusus kamus tambahan
- **Manajemen kamus yang komprehensif**: Mencakup properti seperti `title`, `live`, `priority`, `tags`, `version`, dan `description` untuk membuat dan mengelola kamus baru

### Penggunaan Konfigurasi

Konfigurasi kamus melayani dua tujuan utama:

1. **Nilai Default**: Tentukan nilai default saat membuat file deklarasi konten
2. **Perilaku Fallback**: Sediakan nilai fallback ketika field spesifik tidak didefinisikan, memungkinkan Anda mendefinisikan perilaku operasi kamus secara global

Bagian kamus mencakup properti komprehensif untuk manajemen kamus:

- **`fill`**: Perilaku auto-fill untuk pembuatan konten
- **`title`**: Judul default untuk kamus baru
- **`live`**: Konfigurasi sinkronisasi langsung untuk pembaruan real-time
- **`priority`**: Pengaturan prioritas untuk resolusi kamus
- **`tags`**: Tag default untuk organisasi konten
- **`version`**: Manajemen versi untuk pembaruan kamus
- **`description`**: Deskripsi default untuk konten baru
...
- **`editor.formatCommand`**: Opsi baru untuk pemformatan kode otomatis pada file konten
