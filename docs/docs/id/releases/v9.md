...
## Vite Plugin: Bundel Compiler & Proxy

Plugin Vite `intlayer()` kini mem-bundel langsung **compiler** dan **locale-routing proxy**, sehingga sebagian besar proyek hanya memerlukan satu plugin di `vite.config.ts`:
...
- **Compiler**: Aktif secara otomatis saat `compiler.enabled` disetel dan jalur `compiler.output` dikonfigurasi. Anda tidak perlu lagi mendaftarkan `intlayerCompiler()` secara terpisah.
- **Proxy**: Aktif secara otomatis berdasarkan opsi baru `routing.enableProxy` (default `true`). Ini menghubungkan middleware deteksi / pengalihan / penulisan ulang bahasa di development, preview, dan produksi SSR. Anda tidak perlu lagi mendaftarkan `intlayerProxy()` secara terpisah.

### Opsi `routing.enableProxy`

Opsi baru `routing.enableProxy` mengontrol apakah proxy perutean lokal terpasang. Default-nya adalah `true`. Nonaktifkan saat Anda ingin menangani perutean lokal Anda sendiri:
...
Plugin mandiri `intlayerCompiler()` dan `intlayerProxy()` tetap diekspor untuk pengaturan tingkat lanjut. Mendaftarkannya bersamaan dengan `intlayer()` aman — setiap plugin akan menduplikasi dirinya sendiri dan berjalan hanya sekali.
