...
Intlayer v9 menyediakan dukungan first-class untuk menjalankan instance Intlayer Anda sendiri dengan satu perintah. Tidak ada akun Intlayer Cloud yang diperlukan.

```sh
curl -fsSL https://intlayer.org/install.sh | sh
```

Installer mengunduh `docker-compose.yml` dan `.env`, auto-generate secret yang diperlukan, dan menjalankan `docker compose up -d`. Semuanya — dashboard, API, database, object storage, dan transactional email — berjalan secara lokal dalam container.
