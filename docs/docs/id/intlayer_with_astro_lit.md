...
Elemen kustom Lit diimpor melalui blok `<script>` sisi klien dan ditempatkan di body.
...
// Konten kamus 'lit-demo' untuk lokal tersebut
...
<Step number={6} title="Membuat Elemen Kustom Lit">
...
<Question title="Apa saja solusi berbeda yang tersedia untuk menginternasionalkan situs Astro dengan pulau Lit?">

Opsi bawaan `i18n` Astro menangani prefix dan redirect tetapi tidak mengelola konten. Menambahkan pulau menimbulkan pertanyaan tambahan: pulau tersebut tidak menjalankan Astro, melainkan Lit.

- **Astro `i18n` plus kamus buatan sendiri**, dan di pulau menggunakan **`@lit/localize`**: dua sumber konten terpisah tanpa typing bersama.
- **`Intlayer`**: satu lapisan konten untuk keduanya. `astro-intlayer` melayani halaman `.astro`, dan `lit-intlayer` membaca deklarasi yang sama untuk pulau Lit.
...
Sebagian besar ya. Ikuti [panduan migrasi](https://github.com/aymericzip/intlayer/blob/main/docs/docs/id/compat/index.md). Anda juga dapat bermigrasi secara bertahap: [plugin sync JSON](https://github.com/aymericzip/intlayer/blob/main/docs/docs/id/plugins/sync-json.md) menjaga katalog JSON Anda sebagai sumber kebenaran dan menghasilkan kamus Intlayer.
...
<Question title="Apakah saya memerlukan library i18n terpisah di dalam pulau Lit saya?">

Tidak. Paket `lit-intlayer` membaca kamus yang sama dengan Astro, jadi Anda tidak menginstal `@lit/localize` di sampingnya. Langkah 6 menunjukkan bagaimana komponen pulau menerima locale langsung dari halaman.
