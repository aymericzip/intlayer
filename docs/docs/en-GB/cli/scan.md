...
The `scan` command fetches a public URL, measures the total page size, and audits the page's i18n and SEO health. It produces a scored report (0–100) covering HTML attributes, canonical links, hreflang tags, robots.txt, sitemap.xml, localised internal links, and JavaScript bundle locale weight.
...
Checks:
  ...
  ✓ hreflang tags
  ✓ x-default hreflang
  ✓ localised internal links
  ⚠ all internal links localised
...
| `localised links`         | At least one internal link includes a locale segment     | 5            |
| `all links localised`     | Every internal link includes a locale segment            | 5            |
