...
- **`Intlayer`**: the most advanced solution. Content declared anywhere in your codebase ([next to each component or centralized](https://github.com/aymericzip/intlayer/blob/main/docs/blog/en-GB/per-component_vs_centralized_i18n.md)) and compiled by the Vite plugin at build time, fully typed, with runtime locale switching, AI translation, a visual editor and a CMS.
