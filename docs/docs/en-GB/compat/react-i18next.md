...
During initialisation, Intlayer will install `@intlayer/react-i18next` and create `intlayer.config.ts`. In your bundler (like Vite), apply the Intlayer plugin:
...
- **Plurals & Context:** Handles i18next's suffix-based pluralisation (`key_one`, `key_other`) using native `Intl.PluralRules` and context suffixes (`key_male`).
