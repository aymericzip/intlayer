...
Initialise Intlayer in your project:
...
- **Interpolation & Plurals:** It relies closely on the `react-i18next` adapter behaviour. Handled dynamically are `{{var}}` placeholders and pluralisation mapped from suffixes like `key_0`, `key_one`, and `key_other`.
