...
description: "No more i18next. The 2026 guide to building a multilingual (i18n) Astro + Solid app. Translate with AI agents and optimise bundle size, SEO and performances."
keywords:
...
# Translate your Astro + Solid site with Intlayer | Internationalisation (i18n)
...
## Step-by-Step Guide to Configure Intlayer in Astro + Solid
...
- **solid-intlayer**
  Package to integrate Intlayer into Solid applications. It provides the `IntlayerProvider` as well as the `useIntlayer` and `useLocale` primitives for internationalisation in Solid.

- **@astrojs/solid-js**
  Official Astro integration that allows the use of Solid component islands.
...
Add the `intlayer` plugin and Solid integration to your Astro configuration.
...
> The `solid()` integration allows for using Solid component islands via `client:only="solid-js"`.
...
You should also add SEO metadata (such as hreflang and canonical links) to every page and introduce a Solid island for interactive client-side content.
...
<html lang={locale} dir={getHTMLTextDir(locale)}>
  ...
  <body>
    <!-- The Solid island renders all interactive content, including the language switcher -->
...
<Step number={6} title="Create a Solid Island Component">

Create an island component that wraps your Solid application and receives the server-detected locale:
...
> The `locale` prop is passed from the Astro page (server detection) to `IntlayerProvider`, making it the initial language for all Solid primitives within the tree.

> In Solid, `useIntlayer` returns an **accessor** function (e.g. `content.). You must invoke it to access the reactive content.

</Step>
<Step number={7} title="Add a Language Switcher">

Create a Solid `LocaleSwitcher` component that reads available languages and navigates to the localised URL when a user selects a new language:
...
> In Solid, `locale` is a reactive signal accessor - always invoke as `locale()` to get the current value.
...
<Question title="What are the different solutions available to internationalize an Astro site with Solid islands?">

Astro's built-in `i18n` option handles locale prefixes and redirects but not the content itself, so you still need a message layer, and an island adds a second problem: the island runs Solid, not Astro.

- **Astro `i18n` plus hand written dictionaries**, with **`@solid-primitives/i18n`** inside the island: two content sources to keep in sync, and no shared typing between them.
- **`Intlayer`**: one content layer for both. `astro-intlayer` covers the `.astro` pages and `solid-intlayer` covers the Solid islands, reading the same declarations.
...
<Question title="Do I need a separate i18n library inside my Solid island?">

No. `solid-intlayer` reads the same dictionaries as the Astro side, so you do not install `@solid-primitives/i18n` alongside it. Step 6 shows the island component receiving the active locale from the page instead of resolving it again.
