...
description: "No more i18next. The 2026 guide to building a multilingual (i18n) Hono app. Translate with AI agents and optimise bundle size, SEO and performances."
keywords:
...
<Question title="What are the different solutions available to internationalize a Hono backend?">

Hono has no i18n layer of its own, so the options are a generic library such as `i18next` wired manually into a middleware, or `Intlayer` through `hono-intlayer`, which registers the middleware for you, resolves the locale per request, and shares the same declared content as your frontend.
...
<Question title="How much does i18n add to my Hono server bundle size?">
...
<Question title="Does it work on edge runtimes such as Cloudflare Workers, Deno or Bun?">

Hono targets all of them, and Intlayer resolves content from dictionaries compiled at build time rather than reading catalogue files from disk at runtime, which is what usually breaks on edge runtimes. Keep `dictionary.importMode` at its default `"static"` so the content is bundled with the worker.
...
Yes, and that is the usual setup. `hono-intlayer` works alongside `react-intlayer`, `next-intlayer` and `vite-intlayer` on the same declared content, so a label used both in an API response and in a page is declared once. See [how Intlayer works](https://github.com/aymericzip/intlayer/blob/main/docs/docs/en-GB/how_works_intlayer.md).
