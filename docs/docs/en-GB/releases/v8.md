...
## YAML & Markdown Content Files (v8.10.0)

You can now declare content directly using the `.content.md` and `.content.yaml` file extensions. This is perfect for long-form content, blogs, documentation, and privacy policies.

These files are fully editable from the Visual Editor, providing a streamlined workflow for non-technical team members.

For more details, see the [YAML Content Files Documentation](https://github.com/aymericzip/intlayer/blob/main/docs/docs/en-GB/dictionary/content_file.md#yaml-content-file) and [Markdown Content Files Documentation](https://github.com/aymericzip/intlayer/blob/main/docs/docs/en-GB/dictionary/markdown.md).
...
A new `plural` node helper has been introduced based on the plural rules defined by the Unicode CLDR Project, improving support for complex languages.
...
You can then pass the count dynamically in your framework component:
...
For more information, see the [Pluralization Documentation](https://github.com/aymericzip/intlayer/blob/main/docs/docs/en-GB/dictionary/plurial.md).
...
The `IntlayerNode` type has been updated to allow calling primitive JavaScript methods directly on content nodes. This makes it possible to interact with your localised content directly as if it were a standard string, number, boolean, or array primitive.

Supported across React, Preact, Solid, Svelte, Vue, and Angular.
...
## Tooling: C++ Watcher & OXC-based LSP (v8.12.0)

Intlayer v8.12.0 brings major improvements to the developer experience:

- **Parcel Watcher**: The content watcher has been migrated from `chokidar` to `@parcel/watcher`, leveraging native C++ file watching to provide faster updates and significantly lower memory consumption.
- **New Language Server Protocol (LSP)**: A brand new LSP is now available. Built with OXC-based parsing, it helps your IDE and AI agents seamlessly link `useIntlayer('my-key')` calls with their corresponding `.content` files, and vice versa.

For more details, see the [LSP Documentation](https://github.com/aymericzip/intlayer/blob/main/docs/docs/en-GB/lsp.md).
...
## Experimental Build Optimizations: Minify & Purge (v8.7.0)

Intlayer v8.7.0 introduces new experimental build features in your `intlayer.config.ts` designed to optimise your production bundle:

- **Dictionary Minification**: Minifies dictionary files to decrease size.
- **Unused Key Purging**: Scans and removes unused translation keys from dictionaries, renaming active keys to short characters (e.g. `"products" -> "a"`, `"pricing" -> "b"`). This reduces bundle size by up to 5%.
- **Disable TypeScript Checks**: Speed up builds by disabling TypeScript typechecking during the compilation step.
...
## Angular Support: Vite Bundler (v8.11.0)

For Angular users, Intlayer now natively supports the Vite bundler through a new custom esbuild plugin, speeding up development and build times.

For details on configuration, see the [Angular Environment Documentation](https://github.com/aymericzip/intlayer/blob/main/docs/docs/en-GB/intlayer_with_angular_21.md).

## Sitemap Generation (v8.6.0)

Intlayer now generates sitemaps automatically based on your locales and routing mode. This is particularly useful for SEO optimisation in router architectures like TanStack Router:
