...
You should also add SEO metadata (such as hreflang and canonical links) to every page and introduce a React island for interactive client-side content.
...
<Step number={6} title="Create a React Island Component">
...
<Question title="What are the different solutions available to internationalize an Astro site with React islands?">
...
- **Astro `i18n` plus hand written dictionaries**, with **`react-i18next`** inside the island: two content sources to keep in sync, and no shared typing between them.
- **`Intlayer`**: one content layer for both. `astro-intlayer` covers the `.astro` pages and `react-intlayer` covers the React islands, reading the same declarations.
...
<Question title="Do I need a separate i18n library inside my React island?">

No. `react-intlayer` reads the same dictionaries as the Astro side, so you do not install `react-i18next` alongside it. Step 6 shows the island component receiving the active locale from the page instead of resolving it again.
