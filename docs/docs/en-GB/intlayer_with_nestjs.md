...
<Question title="What are the different solutions available to internationalize a NestJS backend?">

NestJS has `nestjs-i18n`, which is the common choice and covers JSON or YAML catalogues with a request scoped service. The alternative is `Intlayer` through `express-intlayer`, which uses the same declared content as your frontend, is typed against your dictionaries, and comes with AI translation and a CMS.
...
<Question title="How much does i18n add to my NestJS server bundle size?">
...
<Question title="Can I inject translations into a NestJS service or controller?">

Yes. Call `getIntlayer("app")` inside the service or controller, as shown above. There is no module to register per feature and no token to inject, because the active locale comes from the request context that the middleware installed.
...
Yes, and that is the usual setup. `express-intlayer` works alongside `react-intlayer`, `next-intlayer` and `vite-intlayer` on the same declared content, so a label used both in an API response and in a page is declared once. See [how Intlayer works](https://github.com/aymericzip/intlayer/blob/main/docs/docs/en-GB/how_works_intlayer.md).
