...
history:
  - version: 9.3.3
    date: 2026-08-22
    changes: "`@intlayer/analytics` انسٹال ہونے پر اینالیٹکس بطورِ ڈیفالٹ فعال کیا گیا"
  - version: 9.1.3
    date: 2026-08-06
    changes: "`routing.enableProxy` کو تین حالتوں والا بنایا گیا: غیر مقرر (auto)، `true`، `false`"
  - version: 9.0.0
    date: 2026-07-11
    changes: "`analytics` کنفیگریشن شامل کی گئی"
  - version: 9.0.0
    date: 2026-06-24
...
const config: IntlayerConfig = {
  ...
  routing: {
    ...
    /**
     * Intlayer لوکیل-روٹنگ پراکسی (مڈل ویئر) کو فعال کرتا ہے۔
     * dev، preview اور SSR میں لوکیل کی شناخت، ری ڈائریکٹ اور ری رائٹ سنبھالتا ہے۔
     * - غیر مقرر (auto): dev اور preview سرورز کوکیز اور ہیڈرز میں محفوظ لوکیل کو
     *   نظر انداز کرتے ہوئے روٹنگ کو URL پر مبنی رکھتے ہیں۔ پریفکس اب بھی حل ہوتے ہیں،
     *   لوکیل اب بھی محفوظ ہوتا ہے، اور Accept-Language کی شناخت اب بھی لاگو ہوتی ہے۔
     *   پروڈکشن میں یہ `true` کی طرح برتاؤ کرتا ہے۔
     * - true: ہر ماحول میں مکمل برتاؤ۔
     * - false: کوئی لوکیل روٹنگ نہیں۔
     * ڈیفالٹ: undefined (auto)
     */
  ...
  /**
   * اینالیٹکس (analytics) کنفیگریشن۔
   */
  analytics: {
    /**
     * کیا اینالیٹکس ڈیٹا اکٹھا کرنا فعال ہے (پیج ویوز، کنٹینٹ ایکسپوژر، A/B ایونٹس)۔
     * `@intlayer/analytics` کا انسٹال ہونا اور ایٹریبیوشن کے لیے `editor.clientId` سیٹ ہونا ضروری ہے۔
     * ڈیفالٹ: true
     */
    enabled: true,

    /**
     * بیک اینڈ کو خودکار بیچ ارسال کے درمیان ملی سیکنڈز۔
     * ڈیفالٹ: 20000
     */
    flushInterval: 20000,

    /**
     * ریکارڈ کیے جانے والے سیشنز کا تناسب، 0 (کوئی نہیں) سے 1 (تمام) تک۔
     * ڈیفالٹ: 1
     */
    sampleRate: 1,
  },
  ...
  ai: {
     ...
     * اختیارات: 'openai', 'anthropic', 'mistral', 'deepseek', 'gemini', 'ollama', 'openrouter', 'alibaba', 'fireworks', 'groq', 'huggingface', 'bedrock', 'googlevertex', 'togetherai', 'lmstudio', 'moonshotai'
     * ڈیفالٹ: 'openai'
     */
    provider: "openai",
...
### اینالیٹکس کنفیگریشن (Analytics)

Intlayer اینالیٹکس سے متعلق ترتیبات کی وضاحت کرتا ہے: صارفین کو حقیقت میں دکھائے جانے والے مواد (پیج ویوز، کنٹینٹ ایکسپوژر) کو اکٹھا کرنا اور مواد پر A/B ٹیسٹنگ کو فعال بنانا۔

اینالیٹکس بطورِ ڈیفالٹ فعال ہے (opt-out): جیسے ہی `@intlayer/analytics` پیکج انسٹال ہوتا ہے **اور** ایٹریبیوشن کے لیے پراجیکٹ کی (`editor.clientId`) کنفیگر ہوتی ہے، ڈیٹا اکٹھا ہونا شروع ہو جاتا ہے۔ `analytics.enabled` کو `false` کر دیں — یا پیکج انسٹال نہ کریں — تو پورا اینالیٹکس انٹیگریشن آپ کے ایپلیکیشن بنڈل سے ہٹا دیا جاتا ہے (ڈیڈ کوڈ ایلیمینیشن)۔

| فیلڈ            | تفصیل                                                                              | قسم       | ڈیفالٹ  | مثال    | نوٹ                                                                                                                                                                 |
| --------------- | ---------------------------------------------------------------------------------- | --------- | ------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `enabled`       | اینالیٹکس ڈیٹا اکٹھا کرنے کو فعال کرتا ہے (پیج ویوز، کنٹینٹ ایکسپوژر، A/B ایونٹس)۔ | `boolean` | `true`  | `false` | `@intlayer/analytics` کا انسٹال ہونا اور ایٹریبیوشن کے لیے `editor.clientId` سیٹ ہونا ضروری ہے؛ ورنہ `enabled` کے `true` ہونے کے باوجود اینالیٹکس غیر فعال رہتا ہے۔ |
| `flushInterval` | بیک اینڈ کو خودکار بیچ ارسال کے درمیان ملی سیکنڈز۔                                 | `number`  | `20000` | `10000` |                                                                                                                                                                     |
| `sampleRate`    | ریکارڈ کیے جانے والے سیشنز کا تناسب، `0` (کوئی نہیں) سے `1` (تمام) تک۔             | `number`  | `1`     | `0.5`   | سیمپلنگ فی سیشن یقینی (deterministic) ہے، اس لیے ایک ریکارڈ شدہ سیشن اپنے تمام ایونٹس کی رپورٹ کرتا ہے۔                                                             |
...
| `enableProxy` | Intlayer لوکیل-روٹنگ پراکسی (مڈل ویئر) کو فعال کرتا ہے۔                                                                                                                                                          | `boolean` &#124; <br/> `undefined`                                                                                                                                                                           | `undefined` (auto)     | `true`                                                                                                                                                                                             | • غیر مقرر (auto): dev اور preview سرورز کوکیز/ہیڈرز میں محفوظ لوکیل کو ری ڈائریکٹ کے ماخذ کے طور پر نظر انداز کرتے ہیں؛ پریفکس، محفوظ کرنا اور `Accept-Language` کی شناخت اب بھی لاگو ہوتی ہے۔ پروڈکشن میں `true` کی طرح۔<br/>• `true`: ہر جگہ مکمل برتاؤ۔<br/>• `false`: کوئی لوکیل روٹنگ نہیں۔ Next.js میں `intlayerProxy` مڈل ویئر پاس-تھرو بن جاتا ہے۔ |
...
| `expires`  | ایک `number` تخلیق سے دنوں کی نمائندگی کرتا ہے؛ ایک `Date` (یا ISO تاریخ سٹرنگ) مطلق میعاد ختم ہونے کی تاریخ ہے۔ ڈیفالٹ: `undefined` | `Date` &#124; <br/> `number` &#124; <br/> `string`    |
...
| فیلڈ                 | وضاحت                                                                                                                           | قسم                                                                                                                                                                                                                                                                                                                                                                                                                                                  | ڈیفالٹ      | مثال                                                          | نوٹ                                                                                                                                                         |
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- | ------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `provider`           | Intlayer AI خصوصیات کے لیے استعمال کیا جانے والا فراہم کنندہ۔                                                                   | `'openai'` &#124; <br/> `'anthropic'` &#124; <br/> `'mistral'` &#124; <br/> `'deepseek'` &#124; <br/> `'gemini'` &#124; <br/> `'ollama'` &#124; <br/> `'openrouter'` &#124; <br/> `'alibaba'` &#124; <br/> `'fireworks'` &#124; <br/> `'groq'` &#124; <br/> `'huggingface'` &#124; <br/> `'bedrock'` &#124; <br/> `'googleaistudio'` &#124; <br/> `'googlevertex'` &#124; <br/> `'togetherai'` &#124; <br/> `'lmstudio'` &#124; <br/> `'moonshotai'` | `undefined` | `'anthropic'`                                                 | مختلف فراہم کنندگان کے لیے مختلف API کلیدوں کی ضرورت ہوتی ہے اور ان کی قیمتوں کا ڈھانچہ بھی مختلف ہوتا ہے۔                                                  |
