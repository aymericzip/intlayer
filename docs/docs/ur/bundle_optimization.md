...
نیچے دیا گیا گراف ایک نظریاتی ایپ کے کنٹینٹ پے لوڈ کا اندازہ لگاتا ہے، جس میں 1 سے 10 صفحات ہیں اور جسے 1 سے 10 زبانوں میں ترجمہ کیا گیا ہے، فی صفحہ تقریباً 30 KB متن کے ساتھ۔ locale کے مطابق کنٹینٹ کو ڈائنامک طور پر لوڈ کرنے سے زبان کا محور ختم ہو جاتا ہے، کنٹینٹ کو کمپوننٹ یا روٹ تک محدود کرنے سے صفحات کا محور ختم ہو جاتا ہے، اور صرف دونوں کے امتزاج سے ہی پے لوڈ مستحکم رہتا ہے۔

![آرکیٹیکچر کے لحاظ سے نظریاتی کنٹینٹ لیکیج](https://github.com/aymericzip/intlayer/blob/main/docs/assets/theorical_content_leakage.webp?raw=true)
...
> جب `optimize` کی ویلیو `false` ہو تو منی فیکیشن چھوڑ (skipped) دی جاتی ہے۔ جب `editor.enabled` کی ویلیو `true` ہو تو یہ پھر بھی چلتی ہے، لیکن فیلڈ کا نام تبدیل کرنے کا مرحلہ چھوڑ دیا جاتا ہے — ویژول ایڈیٹر ترامیم کو `keyPath` کے ذریعے حل کرتا ہے، اس لیے اصل فیلڈ کے ناموں کا برقرار رہنا ضروری ہے۔
...
> جب `optimize` کی ویلیو `false` ہو تو پرجنگ (Purging) چھوڑ دی جاتی ہے۔ جب `editor.enabled` کی ویلیو `true` ہو تب بھی یہ فعال رہتی ہے — کیونکہ ہٹائی گئی فیلڈ کو کوئی بھی کمپوننٹ نہیں پڑھتا، اس لیے ایڈیٹر اسے کبھی رینڈر نہیں کرتا۔ Next.js پر یہ تب بھی چھوڑ دیا جاتا ہے جب `@intlayer/swc` دستیاب نہ ہو، اور جب کمپیٹ اڈاپٹر کالرز ترتیب دیے گئے ہوں۔
