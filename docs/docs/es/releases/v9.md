...
## `@intlayer/analytics`: seguimiento de exposición de contenido y pruebas A/B

Un nuevo paquete opcional, [`@intlayer/analytics`](https://github.com/aymericzip/intlayer/blob/main/docs/docs/en/analytics.md), te indica qué contenido ven realmente tus visitantes — y cierra el círculo con **Collections & Variants**, permitiéndote hacer pruebas A/B de contenido y medir conversiones por variante.

- **Seguimiento a nivel de provider**: `IntlayerProvider` (React / Next.js / React Native) registra automáticamente eventos `page_view` al cargar, al cambiar de ruta y al cambiar de idioma.
- **Seguimiento a nivel de nodo**: cada resolución de `useIntlayer` reporta un evento `content_exposure` — qué clave de diccionario, ruta de clave, idioma y, cuando aplica, qué **variante** A/B se mostró. Las exposiciones repetidas del mismo nodo dentro de una ventana de envío se combinan en un solo evento.
- **Asignación determinista de A/B**: `getVariant(experimentKey, variants)` asigna una variante a cada sesión **sin ida y vuelta al servidor**, por lo que no hay parpadeo antes del primer renderizado.
- **`useConversion()`**: atribuye un objetivo (registro, clic, compra…) a la variante a la que estuvo expuesta la sesión.
- **Coste cero si no está instalado**: mismo patrón tree-shakeable que `@intlayer/editor` — `import()` dinámico envuelto en `try/catch`, más una variable de entorno en tiempo de compilación (`INTLAYER_ANALYTICS_ENABLED`) que elimina por completo la integración del bundle para las apps que no la activan.
- **Sin nuevo esquema de configuración**: reutiliza `editor.backendURL` (endpoint de envío / auto-alojamiento) y `editor.clientId` (clave del proyecto, que también actúa como interruptor de activación).
- **Respetuoso con la privacidad**: hash anónimo de sesión (SHA-256, sin almacenar ids en bruto), sin almacenamiento de IP, geolocalización aproximada solo por país, solicitudes enviadas por lotes aproximadamente cada 20 segundos.
- **Panel**: una nueva página **Analytics** (visible en la barra lateral una vez seleccionado un proyecto) muestra usuarios activos, usuarios de hoy, usuarios de los últimos 7/30/90 días, un gráfico de evolución de visitantes y desgloses por idioma y ubicación.
...
Disponible hoy para React, Next.js y React Native (a través de `react-intlayer`); otras integraciones de frameworks (Vue, Svelte, Angular, …) están planificadas, siguiendo el mismo patrón de despliegue que `@intlayer/editor`. Consulta la [documentación de Analytics](https://github.com/aymericzip/intlayer/blob/main/docs/docs/en/analytics.md) para la guía completa.
...
- [Analytics - Seguimiento de exposición de contenido y pruebas A/B](https://github.com/aymericzip/intlayer/blob/main/docs/docs/en/analytics.md)
