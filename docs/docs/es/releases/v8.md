...
## Archivos de Contenido YAML y Markdown (v8.10.0)

Ahora puede declarar contenido directamente utilizando las extensiones de archivo `.content.md` y `.content.yaml`. Esto es perfecto para contenido de formato largo, blogs, documentación y políticas de privacidad.

Estos archivos son completamente editables desde el Editor Visual, lo que proporciona un flujo de trabajo optimizado para los miembros del equipo no técnicos.

Para más detalles, consulte la [Documentación de Archivos de Contenido YAML](https://github.com/aymericzip/intlayer/blob/main/docs/docs/es/dictionary/content_file.md#yaml-content-file) y la [Documentación de Archivos de Contenido Markdown](https://github.com/aymericzip/intlayer/blob/main/docs/docs/es/dictionary/markdown.md).
...
## Reglas de Pluralización Unicode CLDR (v8.8.0)

Se ha introducido un nuevo helper de nodo `plural` basado en las reglas de pluralización definidas por el Proyecto Unicode CLDR, mejorando el soporte para idiomas complejos.
...
Luego puede pasar el conteo dinámicamente en el componente de su framework:
...
Para más información, consulte la [Documentación de Pluralización](https://github.com/aymericzip/intlayer/blob/main/docs/docs/es/dictionary/plurial.md).
...
## TypeScript: Accesores Primitivos de IntlayerNode

El tipo `IntlayerNode` ha sido actualizado para permitir llamar métodos primitivos de JavaScript directamente en los nodos de contenido. Esto hace posible interactuar con su contenido localizado directamente como si fuera un primitivo estándar de cadena, número, booleano o array.

Compatible con React, Preact, Solid, Svelte, Vue y Angular.
...
## Herramientas: Watcher de C++ y LSP basado en OXC (v8.12.0)

Intlayer v8.12.0 trae importantes mejoras a la experiencia del desarrollador:

- **Parcel Watcher**: El observador de contenido ha sido migrado de `chokidar` a `@parcel/watcher`, aprovechando la observación de archivos nativa de C++ para proporcionar actualizaciones más rápidas y un consumo de memoria significativamente menor.
- **Nuevo Protocolo de Servidor de Lenguaje (LSP)**: Un LSP completamente nuevo ya está disponible. Construido con parsing basado en OXC, ayuda a su IDE y agentes de IA a vincular sin problemas las llamadas a `useIntlayer('my-key')` con sus archivos `.content` correspondientes, y viceversa.

Para más detalles, consulte la [Documentación de LSP](https://github.com/aymericzip/intlayer/blob/main/docs/docs/es/lsp.md).
...
## Optimizaciones Experimentales de Construcción: Minify y Purge (v8.7.0)

Intlayer v8.7.0 introduce nuevas características experimentales de construcción en su `intlayer.config.ts` diseñadas para optimizar su bundle de producción:

- **Minificación de Diccionarios**: Minifica los archivos de diccionario para reducir su tamaño.
- **Purga de Claves No Utilizadas**: Escanea y elimina las claves de traducción no utilizadas de los diccionarios, renombrando las claves activas a caracteres cortos (por ejemplo, `"products" -> "a"`, `"pricing" -> "b"`). Esto reduce el tamaño del bundle hasta en un 5%.
- **Deshabilitar Verificaciones de TypeScript**: Acelera las construcciones deshabilitando la verificación de tipos de TypeScript durante el paso de compilación.
...
## Soporte para Angular: Bundler Vite (v8.11.0)

Para los usuarios de Angular, Intlayer ahora soporta de forma nativa el bundler Vite a través de un nuevo plugin personalizado de esbuild, acelerando los tiempos de desarrollo y construcción.

Para detalles sobre la configuración, consulte la [Documentación del Entorno de Angular](https://github.com/aymericzip/intlayer/blob/main/docs/docs/es/intlayer_with_angular_21.md).

## Generación de Sitemap (v8.6.0)

Intlayer ahora genera sitemaps automáticamente basándose en sus locales y modo de enrutamiento. Esto es particularmente útil para la optimización SEO en arquitecturas de enrutadores como TanStack Router:
