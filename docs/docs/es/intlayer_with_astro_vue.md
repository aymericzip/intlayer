...
<Step number={6} title="Crear un componente de isla de Vue">
...
<Question title="¿Qué soluciones existen para internacionalizar un sitio Astro con islas de Vue?">

La opción `i18n` integrada de Astro gestiona los prefijos de idioma y las redirecciones, pero no el contenido en sí, así que todavía necesitas una capa de mensajes, y una isla añade un segundo problema: la isla ejecuta Vue, no Astro.

- **La `i18n` de Astro más diccionarios escritos a mano**, con **`vue-i18n`** dentro de la isla: dos fuentes de contenido que mantener sincronizadas y sin tipado compartido entre ellas.
- **`Intlayer`**: una única capa de contenido para ambas. `astro-intlayer` cubre las páginas `.astro` y `vue-intlayer` cubre las islas de Vue, leyendo las mismas declaraciones.
...
<Question title="¿Necesito una biblioteca de i18n aparte dentro de mi isla de Vue?">

No. `vue-intlayer` lee los mismos diccionarios que el lado de Astro, así que no instalas `vue-i18n` junto a él. El paso 6 muestra el componente de la isla recibiendo el idioma activo desde la página en lugar de volver a resolverlo.
