...
<Question title="¿Qué soluciones existen para internacionalizar un backend de Express?">

La opción histórica es `i18next` con `i18next-http-middleware`, que carga catálogos JSON por espacio de nombres y almacena el idioma en la solicitud. La alternativa es `Intlayer` mediante `express-intlayer`, que declara el contenido en archivos tipados compartidos con tu frontend, resuelve el idioma por solicitud y añade traducción con IA y un CMS.
...
<Question title="¿Funciona con una aplicación Express existente y otro middleware?">

Sí. `express-intlayer` es un middleware estándar de Express, así que se compone con tu stack existente. Regístralo antes de las rutas que leen contenido, para que el idioma esté resuelto cuando un manejador llama a `t()` o `getIntlayer()`.
