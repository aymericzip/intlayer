...
También deberías añadir metadatos SEO (como hreflang y enlaces canónicos) a cada página e introducir una isla de React para el contenido interactivo del lado del cliente.
...
<Step number={6} title="Crear un componente de isla de React">
...
<Question title="¿Qué soluciones existen para internacionalizar un sitio Astro con islas de React?">

La opción `i18n` integrada de Astro gestiona los prefijos de idioma y las redirecciones, pero no el contenido en sí, así que todavía necesitas una capa de mensajes, y una isla añade un segundo problema: la isla ejecuta React, no Astro.

- **La `i18n` de Astro más diccionarios escritos a mano**, con **`react-i18next`** dentro de la isla: dos fuentes de contenido que mantener sincronizadas y sin tipado compartido entre ellas.
- **`Intlayer`**: una única capa de contenido para ambas. `astro-intlayer` cubre las páginas `.astro` y `react-intlayer` cubre las islas de React, leyendo las mismas declaraciones.
...
<Question title="¿Puedo migrar desde `react-i18next` sin reescribir mis componentes?">
...
<Question title="¿Necesito una biblioteca de i18n aparte dentro de mi isla de React?">

No. `react-intlayer` lee los mismos diccionarios que el lado de Astro, así que no instalas `react-i18next` junto a él. El paso 6 muestra el componente de la isla recibiendo el idioma activo desde la página en lugar de volver a resolverlo.
