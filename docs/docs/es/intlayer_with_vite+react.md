...
<AccordionGroup>
<Accordion header="Soporte completo de Vite y React">
...
Vite no tiene una opinión sobre i18n, así que las opciones son las del ecosistema de React:
...
- **`Intlayer`**: contenido declarado junto a cada componente, compilado por un plugin de Vite en tiempo de compilación, totalmente tipado, con traducción con IA, un editor visual y un CMS.

En Vite concretamente, Intlayer se integra en la build para que las traducciones se resuelvan y se haga tree shaking en tiempo de compilación en lugar de obtenerse como JSON en tiempo de ejecución. Consulta [por qué Intlayer](https://github.com/aymericzip/intlayer/blob/main/docs/docs/es/interest_of_intlayer.md) y el [benchmark](https://github.com/aymericzip/intlayer/blob/main/docs/docs/es/benchmark/index.md).
...
<Question title="¿Intlayer funciona con el servidor de desarrollo de Vite y HMR?">

Sí. El plugin `intlayer()` de Vite vigila tus archivos `.content.ts` y reconstruye los diccionarios al guardar, así que las ediciones aparecen sin reiniciar el servidor de desarrollo. Las definiciones de tipos se regeneran al mismo tiempo, lo que mantiene el autocompletado sincronizado.

</Question>

<Question title="¿Cómo configuro el enrutamiento por idioma en una SPA de Vite y React?">

Intlayer se encarga de la resolución del idioma y deja el enrutamiento a tu router. Para React Router, sigue la [guía de React Router v7](https://github.com/aymericzip/intlayer/blob/main/docs/docs/es/intlayer_with_react_router_v7.md) dedicada. Para una aplicación sin router, `routing.mode` se puede establecer en `"no-prefix"` o `"search-params"` para que el idioma se almacene en una cookie o un parámetro de consulta en lugar de en la ruta. Consulta la [referencia de configuración](https://github.com/aymericzip/intlayer/blob/main/docs/docs/es/configuration.md).

</Question>

<Question title="¿Cómo gestiono los metadatos de SEO en una aplicación Vite renderizada en el cliente?">

Establece los atributos `lang` y `dir` en el elemento `html` a partir del idioma activo, y emite alternativas `hreflang` para cada idioma declarado con `getMultilingualUrls`. Para las páginas que necesitan rastrearse de forma fiable, prefiere una configuración prerenderizada o renderizada en el servidor como [TanStack Start](https://github.com/aymericzip/intlayer/blob/main/docs/docs/es/intlayer_with_tanstack.md) o [React Router v7](https://github.com/aymericzip/intlayer/blob/main/docs/docs/es/intlayer_with_react_router_v7.md).

</Question>

<Question title="¿Cómo traduzco una aplicación Vite automáticamente con IA?">

Ejecuta `npx intlayer fill`. La CLI detecta las traducciones que faltan y las rellena con el LLM de tu elección usando tu propio proveedor y tu clave de API. `--git-diff` restringe la ejecución al contenido modificado en la rama. Consulta el [comando fill](https://github.com/aymericzip/intlayer/blob/main/docs/docs/es/cli/fill.md) y la [integración de CI/CD](https://github.com/aymericzip/intlayer/blob/main/docs/docs/es/CI_CD.md).
...
A través del [editor visual](https://github.com/aymericzip/intlayer/blob/main/docs/docs/es/intlayer_visual_editor.md), autoalojado, que permite que cualquiera edite texto en su sitio en la aplicación en ejecución, o del [CMS](https://github.com/aymericzip/intlayer/blob/main/docs/docs/es/intlayer_CMS.md) para el contenido que debe cambiar sin un despliegue.
