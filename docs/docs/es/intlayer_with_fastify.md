...
<Question title="¿Qué soluciones existen para internacionalizar un backend de Fastify?">

La opción genérica es `i18next` con `fastify-i18next` o un hook escrito a mano, que carga catálogos JSON por espacio de nombres y almacena el idioma en la solicitud. La alternativa es `Intlayer` mediante `fastify-intlayer`, que registra el plugin por ti, resuelve el idioma por solicitud y comparte el mismo contenido tipado que tu frontend.
...
<Question title="¿Cuánto añade la i18n al tamaño del bundle de mi servidor Fastify?">
...
<Question title="¿Funciona con el ciclo de vida y la encapsulación de plugins de Fastify?">

Sí. `fastify-intlayer` se registra como un plugin estándar de Fastify, así que sigue las reglas de encapsulación habituales. Regístralo en la raíz, o dentro del alcance que lo necesite, antes de las rutas que leen contenido.
