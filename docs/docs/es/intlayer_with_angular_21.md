...
<Question title="¿Qué soluciones existen para internacionalizar una aplicación Angular?">
...
El motivo habitual para abandonar `@angular/localize` es el modelo de una build por idioma. Intlayer mantiene una sola build y cambia de idioma en tiempo de ejecución. Consulta [por qué Intlayer](https://github.com/aymericzip/intlayer/blob/main/docs/docs/es/interest_of_intlayer.md).
...
No. Ese es el modelo de `@angular/localize`. Con Intlayer una sola build sirve a todos los idiomas declarados, el idioma activo se resuelve en tiempo de ejecución a partir de la URL, una cookie o la cabecera `Accept-Language`, y solo se descarga el contenido del idioma renderizado.
...
<Question title="¿Cómo traduzco una aplicación Angular automáticamente con IA?">
