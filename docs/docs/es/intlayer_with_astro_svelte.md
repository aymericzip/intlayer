...
- **svelte**
  El paquete core de Svelte.

- **svelte-intlayer**
  Paquete para integrar Intlayer con aplicaciones de Svelte. Proporciona `setupIntlayer`, así como los stores `useIntlayer` y `useLocale` para la internacionalización en Svelte.

- **@astrojs/svelte**
  Integración oficial de Astro que permite el uso de islas (islands) de componentes Svelte.
...
También deberías añadir metadatos SEO (como hreflang y enlaces canónicos) a cada página e introducir una isla de Svelte para el contenido interactivo del lado del cliente.
...
<html lang={locale} dir={getHTMLTextDir(locale)}>
  ...
  <body>
    <!-- La isla de Svelte renderiza todo el contenido interactivo, incluyendo el selector de idioma -->
...
<Step number={6} title="Crear un componente de isla de Svelte">
...
<div>
  <h1>{$content.title}</h1>
  <!-- El selector de idioma se renderiza directamente en la isla -->
...
<script lang="ts">
  ...
  // Reutilizar la misma configuración de locale/setupIntlayer como se muestra en el Paso 6...
...
<Question title="¿Qué soluciones existen para internacionalizar un sitio Astro con islas de Svelte?">

La opción `i18n` integrada de Astro gestiona los prefijos de idioma y las redirecciones, pero no el contenido en sí, así que todavía necesitas una capa de mensajes, y una isla añade un segundo problema: la isla ejecuta Svelte, no Astro.

- **La `i18n` de Astro más diccionarios escritos a mano**, con **`svelte-i18n`** dentro de la isla: dos fuentes de contenido que mantener sincronizadas y sin tipado compartido entre ellas.
- **`Intlayer`**: una única capa de contenido para ambas. `astro-intlayer` cubre las páginas `.astro` y `svelte-intlayer` cubre las islas de Svelte, leyendo las mismas declaraciones.
...
<Question title="¿Puedo migrar desde `svelte-i18n` sin reescribir mis componentes?">
...
<Question title="¿Necesito una biblioteca de i18n aparte dentro de mi isla de Svelte?">

No. `svelte-intlayer` lee los mismos diccionarios que el lado de Astro, así que no instalas `svelte-i18n` junto a él. El paso 6 muestra el componente de la isla recibiendo el idioma activo desde la página en lugar de volver a resolverlo.
