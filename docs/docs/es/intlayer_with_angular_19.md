...
En esta arquitectura, `angular-intlayer` proporciona `provideIntlayer()`, registrado en `src/app/app.config.ts` para que cada componente pueda leer su contenido a través de la señal `useIntlayer`. La CLI de Angular se cambia al builder personalizado de Webpack, y `webpack.config.ts` combina el plugin de Intlayer en la compilación para observar y reconstruir tus declaraciones de contenido. Las declaraciones de contenido se ubican junto a los componentes en `src/app/`.
