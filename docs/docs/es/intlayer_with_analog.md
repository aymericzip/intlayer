...
<AccordionGroup>
<Accordion header="Soporte completo de analógica">
...
En esta arquitectura, `angular-intlayer` proporciona `provideIntlayer()`, registrado en `src/app/app.config.ts` para que cada componente y página basada en archivos pueda leer su contenido a través de la señal `useIntlayer`. Analog se basa en Vite, por lo que el plugin `intlayer()` de `vite-intlayer` se agrega junto a `analog()` en `vite.config.ts` para observar y reconstruir tus declaraciones de contenido y ejecutar el proxy de locale. Las declaraciones de contenido se ubican junto a los componentes y páginas en `src/app/`.
...
<Question title="¿Qué soluciones existen para internacionalizar una aplicación Analog?">
...
Consulta [por qué Intlayer](https://github.com/aymericzip/intlayer/blob/main/docs/docs/es/interest_of_intlayer.md) y la [guía de Angular](https://github.com/aymericzip/intlayer/blob/main/docs/docs/es/intlayer_with_angular_21.md) para las API específicas de Angular.

</Question>
<Question title="¿Cuánto añade la i18n al tamaño del bundle de mi aplicación Analog?">
...
<Question title="¿Funciona con el renderizado en el servidor de Analog y con Vite?">

Sí. El plugin `intlayer()` de Vite compila tu contenido y lo vigila en desarrollo, y el idioma se resuelve en el servidor para que la primera respuesta HTML ya esté en el idioma correcto. Las rutas prerenderizadas resuelven su contenido en tiempo de compilación.
