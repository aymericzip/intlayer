...
title: "Astro + Solid i18n - Guía completa para traducir tu aplicación"
description: "Sin más i18next. La guía 2026 para crear una aplicación Astro + Solid multilingüe (i18n). Traduce con agentes de IA y optimiza el tamaño del bundle, SEO y rendimiento."
keywords:
...
También deberías añadir metadatos SEO (como hreflang y enlaces canónicos) a cada página e introducir una isla de Solid para el contenido interactivo del lado del cliente.
...
<Question title="¿Qué soluciones existen para internacionalizar un sitio Astro con islas de Solid?">

La opción `i18n` integrada de Astro gestiona los prefijos de idioma y las redirecciones, pero no el contenido en sí, así que todavía necesitas una capa de mensajes, y una isla añade un segundo problema: la isla ejecuta Solid, no Astro.

- **La `i18n` de Astro más diccionarios escritos a mano**, con **`@solid-primitives/i18n`** dentro de la isla: dos fuentes de contenido que mantener sincronizadas y sin tipado compartido entre ellas.
- **`Intlayer`**: una única capa de contenido para ambas. `astro-intlayer` cubre las páginas `.astro` y `solid-intlayer` cubre las islas de Solid, leyendo las mismas declaraciones.
...
<Question title="¿Necesito una biblioteca de i18n aparte dentro de mi isla de Solid?">

No. `solid-intlayer` lee los mismos diccionarios que el lado de Astro, así que no instalas `@solid-primitives/i18n` junto a él. El paso 6 muestra el componente de la isla recibiendo el idioma activo desde la página en lugar de volver a resolverlo.
