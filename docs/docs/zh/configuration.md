...
| `description`               | `string`                                                                                        | 帮助理解字典在编辑器和 CMS 中的用途，也用作 AI 翻译生成的上下文。默认值: `undefined`                      | `'User profile section'` |                                                                                                                                                                                                                                                          |
| `locale`                    | `LocalesValues`                                                                                 | 将字典转换为按语言环境格式。声明的每个字段都成为翻译节点。如果缺失，字典被视为多语言。默认值: `undefined` | `'en'`                   | 当字典特定于单个语言环境而非包含多个语言环境的翻译时使用此项。                                                                                                                                                                                           |
| `contentAutoTransformation` | `boolean &#124; { markdown?: boolean; html?: boolean; insertion?: boolean }`                    | 自动将内容字符串转换为类型化节点（markdown、HTML 或插入）。默认值: `false`                                | `true`                   | Markdown: `### Title` → `md('### Title')`。HTML: `<div>Title</div>` → `html('<div>Title</div>')`。Insertion: `Hello {{name}}` → `insert('Hello {{name}}')`。                                                                                             |
| `location`                  | `'local' &#124; 'remote' &#124; 'hybrid' &#124; 'plugin' &#124; string`                         | 指示字典文件存储位置及其 CMS 同步模式。默认值: `'local'`                                                  | `'hybrid'`               | `'local'`: 仅本地管理。`'remote'`: 仅远程管理（CMS）。`'hybrid'`: 本地和远程均管理。`'plugin'` 或自定义字符串: 由插件或自定义来源管理。                                                                                                                  |
...
| `priority`                  | `number`                                                                                        | 字典的优先级。在解决字典之间的冲突时，值越高优先级越高。默认值: `undefined`                               | `1`                      |                                                                                                                                                                                                                                                          |
| `live`                      | `boolean`                                                                                       | 已弃用 — 请改用 `importMode: 'fetch'`。表示字典内容是否通过实时同步 API 动态获取。默认值: `undefined`     |                          | 在 v8.0.0 中重命名为 `importMode: 'fetch'`。                                                                                                                                                                                                             |
| `schema`                    | `'https://intlayer.org/schema.json'`                                                            | 由 Intlayer 自动生成，用于 JSON 模式验证。默认值: 自动生成                                                |                          | 请勿手动修改。                                                                                                                                                                                                                                           |
| `title`                     | `string`                                                                                        | 帮助在编辑器和 CMS 中识别字典。默认值: `undefined`                                                        | `'User Profile'`         |                                                                                                                                                                                                                                                          |
| `tags`                      | `string[]`                                                                                      | 对字典进行分类，并为编辑器和 AI 提供上下文或说明。默认值: `undefined`                                     | `['user', 'profile']`    |                                                                                                                                                                                                                                                          |
| `version`                   | `string`                                                                                        | 远程字典的版本；帮助追踪当前使用的版本。默认值: `undefined`                                               | `'1.0.0'`                | 可在 CMS 上管理。请勿在本地修改。                                                                                                                                                                                                                        |
