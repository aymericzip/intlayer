...
Intlayer v9 提供一流的支持，使您可以通过单个命令运行自己的 Intlayer 实例。无需 Intlayer Cloud 账户。

```sh
curl -fsSL https://intlayer.org/install.sh | sh
```

安装程序下载 `docker-compose.yml` 和 `.env`，自动生成所需的密钥，并运行 `docker compose up -d`。一切内容 — dashboard、API、数据库、对象存储和事务性电子邮件 — 都在容器中本地运行。
