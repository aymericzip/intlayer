...
## Vite 插件：绑定的 Compiler 和 Proxy

`intlayer()` Vite 插件现在直接捆绑了 **compiler** 和 **locale-routing proxy**，因此大多数项目在 `vite.config.ts` 中只需要一个插件：
...
- **Compiler**：当设置了 `compiler.enabled` 并配置了 `compiler.output` 路径时自动激活。您不再需要单独注册 `intlayerCompiler()`。
- **Proxy**：基于新的 `routing.enableProxy` 选项（默认为 `true`）自动激活。它会在开发、预览和生产 SSR 中连接区域设置检测 / 重定向 / 重写中间件。您不再需要单独注册 `intlayerProxy()`。

### `routing.enableProxy` 选项

新的 `routing.enableProxy` 选项控制是否插入区域路由代理。默认值为 `true`。当您想自行处理区域路由时，请将其禁用：
...
独立的 `intlayerCompiler()` 和 `intlayerProxy()` 插件仍然会被导出用于高级设置。将它们与 `intlayer()` 一起注册是安全的 — 每个插件都会自行去重，并且只会执行一次。
