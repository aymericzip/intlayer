...
Intlayer v8 引入了 `html()` 函数，允许你在字典中嵌入类似 HTML 的内容，并在运行时将标签映射到自定义组件。对于需要超出 Markdown 能力的富文本，或需要在内容中注入交互组件的场景，这非常理想。
...
export default {
  key: "my-key",
  content: {
    myRichText: html("点击 <CustomLink>此处</CustomLink> 获取更多信息"),
  },
};
```
...
欲了解更多详情，请参阅 [HTML Content Documentation](https://github.com/aymericzip/intlayer/blob/main/docs/docs/zh/dictionary/html.md)。
...
有关更多信息和集成指南，请参阅 [自定义 URL 重写 文档](https://github.com/aymericzip/intlayer/blob/main/docs/docs/zh/custom_url_rewrites.md)。

---

## Markdown MDX 与 HTML：`.use()`

Intlayer v8 为富内容统一了 API。用于 v7 Markdown 的 `.set()` 方法已重命名为 [`.use()`](https://github.com/aymericzip/intlayer/blob/main/packages/react-intlayer/src/useIntlayer.tsx)，以在 Markdown 和 HTML 之间提供一致的体验。

### 跨框架的 Markdown 解析器

Intlayer 现在集成了一个强大的、跨框架的 Markdown 解析器，支持 **MDX**。这意味着你可以像在 HTML 中一样，直接在 Markdown 内容中使用组件。

- **MDX 支持**：在你的 Markdown 中使用 React/Vue/Svelte 组件。
- **支持 SSR**：解析器针对服务端渲染（SSR）进行了优化，确保快速的首屏加载和对搜索引擎友好的富内容。
- **一致的 API**：使用 `.use()` 将 Markdown/MDX 标签映射到你的应用组件。
...
参见 [Markdown 文档](https://github.com/aymericzip/intlayer/blob/main/docs/docs/zh/dictionary/markdown.md) 了解更多信息。

---

### 增强的插入值

在 v8 中，插入值现在除了字符串和数字之外，还能**接受 React 元素（或 Vue 节点）**。这允许您将丰富的交互式组件直接注入到插入模板中。
...
Intlayer v8 将内容源配置与内部系统和输出路径分离。这使得 `content` 属性更简洁，并且明确哪些设置由用户管理，哪些由 Intlayer 系统管理。
...
export default {
  content: {
    ...
    codeDir: ["src", "@packages/design-system"], // 仅在此扫描用于代码转换的代码（例如 @packages/design-system/src/*.content.* 文件）
  },
};
```
...
- **`live` 属性**：字典中的 `live` 属性已被弃用。请改用 `importMode: 'fetch'`。
- **从 `.set()` 改为 `.use()`**：用于 Markdown 和 HTML 覆盖的 `.set()` 方法已重命名为 `.use()`，以便与其他框架 API 保持一致。
...
- [Configuration Reference](https://github.com/aymericzip/intlayer/blob/main/docs/docs/zh/configuration.md)
- [Content File Documentation](https://github.com/aymericzip/intlayer/blob/main/docs/docs/zh/dictionary/content_file.md)
- [HTML Content Documentation](https://github.com/aymericzip/intlayer/blob/main/docs/docs/zh/dictionary/html.md)
