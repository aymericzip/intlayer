...
查看更多 CLI 选项请参阅文档：[CLI 参考](https://github.com/aymericzip/intlayer/blob/main/docs/docs/zh/cli/index.md) → “测试缺失的翻译”。以及 [测试](https://github.com/aymericzip/intlayer/blob/main/docs/docs/zh/testing.md) 指南。
...
你仍然可以使用内容文件中的 `autoFill` 字段对每个字典进行微调。Intlayer 将首先考虑每个字典的配置，然后回退到全局配置。请参阅 [Auto-fill](https://github.com/aymericzip/intlayer/blob/main/docs/docs/zh/autoFill.md) 和 [Content file reference](https://github.com/aymericzip/intlayer/blob/main/docs/docs/zh/dictionary/content_file.md) 中的完整参考。
...
I'm ready to audit the translation. I can see that:
...
> MCP server 帮助自动化使用 Intlayer 的项目。测试自动生成代码的缺失翻译。它帮助理解你的配置，并将 Intlayer 的文档嵌入到你的 IDE 中。
...
## 使用 turbopack 监视文件的并行进程

当在 Next.js 14 及以上版本中使用 Turbopack 作为开发服务器，并使用 `next dev --turbopack` 命令时，默认情况下字典更改不会被自动检测。

这个限制发生是因为 Turbopack 无法运行 webpack 插件来并行监视内容文件中的更改。

为了解决这个问题，我们使用 `--with` 选项来运行 `intlayer watch` 命令，同时运行开发服务器和 Intlayer 构建监视器。
...
查看 [从 v6 到 v7 的迁移说明](https://github.com/aymericzip/intlayer/blob/main/docs/docs/zh/releases/v7.md) 了解更多信息。

---

## 有用的链接

- [CLI 参考](https://github.com/aymericzip/intlayer/blob/main/docs/docs/zh/cli/index.md)
