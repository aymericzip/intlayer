...
使用 Intlayer，你可以采用一种工作流程，其中只需在本地声明单一语言环境，而所有翻译都通过 CMS 远程管理。这允许内容和翻译与代码库完全分离，为内容编辑提供更多灵活性，并支持 Live Sync（无需重新构建应用程序即可应用更改）。

## 使用 CMS

使用 Intlayer，您可以采用一种工作流程，其中仅在本地声明单个语言环境，而所有翻译都通过 CMS 远程管理。这样可以使内容和翻译完全与代码库分离，为内容编辑人员提供更多灵活性，并支持实时同步（无需重建应用程序即可应用更改）。
