...
我在真实浏览器中为每个栈运行相同的多语言应用，然后记录了实际在网络上显示的内容以及花费的时间。大小报告是**在正常网络压缩后**的，因为这比原始源代码计数更接近人们实际下载的内容。

- **国际化库大小**：在打包、tree-shaking 和 minification 之后，i18n 库的大小是空组件中提供程序 + 存储代码的大小。它不包括翻译文件的加载。它回答了库在你的内容进入之前有多昂贵的问题。

- **每页 JavaScript**：对于每个基准测试路由，浏览器在该访问中拉取多少脚本，在套件中的页面之间平均（以及在报告汇总的区域设置之间）。页面重则加载慢。

- **其他区域设置的泄漏**：同一页面的内容但以另一种语言形式，会被错误地加载到审核页面中。此内容是不必要的，应该避免。（例如 `/fr/about` 页面内容出现在 `/en/about` 页面包中）

- **其他路由的泄漏**：对于**应用中的其他屏幕**也是同样的想法：当你只打开一个页面时，它们的副本是否会被带入。（例如 `/en/about` 页面内容出现在 `/en/contact` 页面包中）。高分数表示分割较弱或包范围过宽。

- **平均组件包大小**：常见的 UI 组件**一次一个**地测量，而不是隐藏在一个巨大的应用数字中。它展示了国际化是否悄悄地膨胀了日常组件。例如，如果你的组件重新渲染，它会从内存中加载所有这些数据。将一个巨大的 JSON 附加到任何组件就像连接一个大的未使用数据存储，这会减慢你的组件性能。

- **语言切换响应性**：我使用应用自身的控件翻转语言，并计时直到页面清晰地切换为止，这是访问者会注意到的，而不是实验室微步骤。

- **语言更改后的渲染工作**：一个更窄的后续跟进：一旦切换进行中，界面为新语言重新绘制花费了多少工作量。在"感受"时间和框架成本不一致时很有用。

- **初始页面加载时间**：从导航到浏览器认为我测试的场景中的页面完全加载。适合比较冷启动。

- **Hydration 时间**：当应用暴露它时，客户端花费多长时间将服务器 HTML 转换为你实际可以点击的内容。表格中的破折号表示该实现在此基准测试中没有提供可靠的 hydration 数字。
