...
我在真实浏览器中为每个技术栈运行相同的多语言应用，然后记录了实际传输的内容以及所花费的时间。大小是在**正常网络压缩后**报告的，因为这比原始源代码数更接近人们下载的内容。

- **国际化库大小**：在捆绑、树抖动和最小化后，i18n 库的大小是提供程序（例如 `NextIntlClientProvider`）+ 钩子（例如 `useTranslations`）在空组件中的代码大小。它不包括加载翻译文件。它回答了在内容进入之前库的成本是多少。

- **每页 JavaScript**：对于每个基准路由，浏览器在该访问期间拉取多少脚本，在套件中的页面上取平均值（以及在报告汇总的地方跨地区）。页面越重，页面越慢。

- **来自其他地区的泄漏**：这是同一页面但用另一种语言编写的内容，会被错误地加载到审计页面中。此内容是不必要的，应该避免。（例如，`/en/about` 页面包中的 `/fr/about` 页面内容）

- **来自其他路由的泄漏**：对于应用中的**其他屏幕**有相同的想法：当你只打开一个页面时，它们的副本是否会随之传输。（例如，`/en/contact` 页面包中的 `/en/about` 页面内容）。高分提示弱分割或过宽的包。

- **平均组件捆绑大小**：常见 UI 片段被**一次测量一个**而不是隐藏在一个巨大的应用编号中。它展示了国际化是否悄悄地膨胀日常组件。例如，如果你的组件重新渲染，它将从内存中加载所有该数据。将一个巨大的 JSON 附加到任何组件就像连接一个大的未使用数据存储，这会减慢你的组件性能。

- **语言切换响应速度**：我使用应用自己的控件来切换语言，并计时直到页面明确切换需要多长时间，这是访问者会注意到的，而不是实验室微步。

- **语言更改后的渲染工作**：一个更窄的后续问题：一旦切换进行中，界面为新语言重绘需要多少工作量。当"感觉"时间和框架成本不同时很有用。

- **初始页面加载时间**：从导航到浏览器认为页面完全加载的时间，用于我测试的场景。很好用于比较冷启动。

- **水合时间**：当应用公开时，客户端将服务器 HTML 转换为可以实际点击的内容所花费的时间。表中的破折号意味着该实现在此基准中没有提供可靠的水合数据。
