...
- **@astrojs/preact**
  官方 Astro 集成，允许使用 Preact 组件岛 (islands)。
...
在此架构中，注册在 `astro.config.ts` 中的 `intlayer()` 集成会构建你的字典，并添加一个中间件来解析每个请求的语言环境，并将其暴露在 `Astro.locals.intlayer` 上。页面位于 `src/pages/[...locale]/` rest 参数段下，因此默认语言环境不带前缀提供服务，而其他每个语言环境都拥有其专用的 URL。`.astro` 文件使用 `astro-intlayer` 的 `useIntlayer` / `useLocale` 钩子进行服务端渲染，Preact 孤岛 (`src/components/preact/PreactIsland.tsx`) 接收服务端检测到的语言环境作为 prop，并将其传递给 `preact-intlayer` 用于交互部分。内容声明文件与你的组件一起放置在 `src/` 中。
...
将 intlayer 插件和 Preact 集成添加到您的 Astro 配置中。
...
> `preact()` 集成允许通过 `client:only="preact"` 使用 Preact 组件岛。
...
> 如果您的内容文件包含 TSX 代码，您可能需要导入 `import { h } from "preact";` 或确保您的 JSX pragma 已正确配置为 Preact。
...
<Step number={6} title="创建 Preact 岛组件">

创建一个包装 Preact 应用程序并接收服务端检测到的语言的岛组件：
...
> `locale` 属性从 Astro 页面（服务端检测）传递到 `IntlayerProvider`，使其成为树中所有 Preact 钩子的初始语言。

> 注意：在 Preact 中，HTML 属性使用 `class` 而不是 `className`。

</Step>
<Step number={7} title="添加语言切换器">

创建一个 Preact 组件 `LocaleSwitcher`，该组件读取可用语言，并在用户选择新语言时导航到本地化 URL：
...
Intlayer 使用模块扩展来获得 TypeScript 的优势并使您的 codebase 更强大。
...
<Question title="国际化带有 Preact 孤岛 (islands) 的 Astro 站点有哪些不同的解决方案？">

Astro 内置的 `i18n` 选项仅处理语言前缀和重定向，并不包含内容层本身，因此您仍然需要消息层。此外，孤岛带来了第二个问题：孤岛运行的是 Preact，而不是 Astro。

- **Astro `i18n` 加上手写字典，并在孤岛内部使用 `preact-i18n`**：需要维护两个保持同步的内容源，且两者之间没有共享的类型支持。
- **`Intlayer`**：为两者提供统一的内容层。`astro-intlayer` 负责 `.astro` 页面，`preact-intlayer` 负责 Preact 孤岛，共同读取相同的内容声明。

只需声明一次文本标签，即可在静态页面和交互式孤岛中共同使用，这是在此类场景下选择统一内容层的关键原因。请参阅 [为什么选择 Intlayer](https://github.com/aymericzip/intlayer/blob/main/docs/docs/zh/interest_of_intlayer.md)。
...
<Question title="我是否需要在 Preact 孤岛内单独安装 i18n 库？">

不需要。`preact-intlayer` 读取与 Astro 侧相同的字典，因此您无需在孤岛中安装 `preact-i18n`。第 6 步展示了孤岛组件直接从页面接收活动语言环境，而无需在浏览器中重新检测。

</Question>
<Question title="孤岛如何知道页面正在渲染哪种语言？">

Astro 页面将语言环境作为 prop 向下传递，孤岛中的 Intlayer Provider 接收该值，因此孤岛会以与服务端渲染相同的语言进行水合 (hydration)。这有效防止了孤岛在浏览器中自行检测语言环境时出现的默认语言闪烁现象。

</Question>
<Question title="翻译内容是否会以静态 HTML 输出？">

对于 Astro 部分是的，它们在构建时渲染，因此本地化页面是搜索引擎爬虫无需执行 JavaScript 即可读取的纯静态 HTML。只有孤岛才会接收字典，并且仅接收其渲染的语言环境字典。

</Question>
<Question title="如何设置支持语言环境的路由和语言切换器？">

第 7 步介绍了切换器。`routing.mode` 决定 URL 方案：`"prefix-no-default"`（默认，`/about` 与 `/fr/about`）、`"prefix-all"`、`"no-prefix"` 或 `"search-params"`，而 `routing.domains` 可将语言环境映射到其独立域名。`getLocalizedUrl` 会重写当前路径，使切换语言时读者保留在同一页面。请参阅 [配置参考](https://github.com/aymericzip/intlayer/blob/main/docs/docs/zh/configuration.md)。

</Question>
<Question title="如何生成本地化站点地图和 hreflang 标签？">

第 8 步涵盖了 `sitemap.xml` 和 `robots.txt`。`getMultilingualUrls` 为每个声明的语言环境构建备用链接（包括 `x-default`），搜索引擎以此为不同受众提供对应语言版本。
