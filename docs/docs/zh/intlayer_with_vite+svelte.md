...
title: Vite + Svelte i18n - 完整翻译指南： Svelte
description: 最佳的包体积、SEO、性能和可维护性解决方案。让您的 Vite and Svelte 应用在 2026 年实现多语言化，LLM 翻译，Agent Skills & MCP。
keywords:
  - 国际化
  - 文档
  - Intlayer
...
> 您的内容声明可以定义在应用程序中的任何位置，只要它们被包含在 `contentDir` 目录中（默认是 `./src`），并且文件扩展名符合内容声明文件扩展名（默认是 `.content.{json,ts,tsx,js,jsx,mjs,cjs}`）。
