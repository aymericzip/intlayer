...
> 只要您的内容声明包含在 `contentDir` 目录（默认为 `./src` 或 `./app`）中，就可以在应用程序的任何位置定义。并且符合内容声明文件扩展名（默认为 `.content.{json,ts,tsx,js,jsx,mjs,cjs,md,mdx,yaml,yml}`）。
...
<Question title="国际化 AdonisJS 后端有哪些不同的解决方案？">
...
<Question title="i18n 会给我的 AdonisJS 服务端体积增加多少？">
...
默认情况下，`adonis-intlayer` 读取传入请求的 `Accept-Language` 请求头，并选择最匹配的已声明语言环境，若无匹配则回退到默认语言环境。您可以通过 `routing.storage` 自定义来源，例如自定义标头或由前端设置的 Cookie，从而使 API 按照用户实际选择的语言响应，而非浏览器默认宣称的语言。请参阅 [配置参考](https://github.com/aymericzip/intlayer/blob/main/docs/docs/zh/configuration.md)。
...
<Question title="如何在控制器或服务中使用翻译？">

使用您的字典键调用 `getIntlayer`，或者用 `t()` 包裹一段文本。活动语言环境来源于中间件安装的请求上下文，因此不需要注入服务，也不需要在参数中向下传递语言环境。

</Question>

<Question title="它是否适用于 Edge 模板？">

可以。在控制器中解析内容并将其传递给视图，这样模板渲染的就是已经翻译好的值，而无需在模板内部自行解析键名。
...
可以，这正是推荐的架构模式。`adonis-intlayer` 可以与 `react-intlayer`、`next-intlayer` 和 `vite-intlayer` 共同基于完全相同的声明内容工作，因此在 API 响应和页面展示中使用的文案只需声明一次。请参阅 [Intlayer 工作原理](https://github.com/aymericzip/intlayer/blob/main/docs/docs/zh/how_works_intlayer.md)。
