...
在此架构中，注册在 `astro.config.ts` 中的 `intlayer()` 集成会构建你的字典，并添加一个中间件来解析每个请求的语言环境，并将其暴露在 `Astro.locals.intlayer` 上。页面位于 `src/pages/[...locale]/` rest 参数段下，因此默认语言环境不带前缀提供服务，而其他每个语言环境都拥有其专用的 URL。`.astro` 文件使用 `astro-intlayer` 的 `useIntlayer` / `useLocale` 钩子进行服务端渲染，Lit 孤岛 (`src/components/lit/LitDemo.ts`) 接收服务端检测到的语言环境作为 prop，并将其传递给 `lit-intlayer` 用于交互部分。内容声明文件与你的组件一起放置在 `src/` 中。
...
如果你想在 `string` 属性中使用你的内容，例如 `alt`、`title`、`href`、`aria-label` 等，你可以使用函数的值，如下：
...
<Step number={6} title="创建 Lit 自定义元素">
...
<Question title="国际化带有 Lit 孤岛 (islands) 的 Astro 站点有哪些不同的解决方案？">

Astro 内置的 `i18n` 选项仅处理语言前缀和重定向，并不包含内容层本身，因此您仍然需要消息层。此外，孤岛带来了第二个问题：孤岛运行的是 Lit，而不是 Astro。

- **Astro `i18n` 加上手写字典，并在孤岛内部使用 `@lit/localize`**：需要维护两个保持同步的内容源，且两者之间没有共享的类型支持。
- **`Intlayer`**：为两者提供统一的内容层。`astro-intlayer` 负责 `.astro` 页面，`lit-intlayer` 负责 Lit 孤岛，共同读取相同的内容声明。
...
基本可以。请参考 [兼容性适配器概览](https://github.com/aymericzip/intlayer/blob/main/docs/docs/zh/compat/index.md) 迁移内容。您也可以逐步迁移：[JSON 同步插件](https://github.com/aymericzip/intlayer/blob/main/docs/docs/zh/plugins/sync-json.md) 将现有的 JSON 目录作为单一真实来源（source of truth），并生成 Intlayer 字典，使两个层在逐个组件迁移时保持同步。
...
<Question title="我是否需要在 Lit 孤岛内单独安装 i18n 库？">

不需要。`lit-intlayer` 读取与 Astro 侧相同的字典，因此您无需在孤岛中安装 `@lit/localize`。第 6 步展示了孤岛组件直接从页面接收活动语言环境，而无需在浏览器中重新检测。
