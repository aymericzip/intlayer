...
title: "next-i18next에서 Intlayer로 마이그레이션하기 | 국제화 (i18n)"
description: "기존 코드를 손상시키지 않고 단계별로 Next.js 애플리케이션을 next-i18next에서 Intlayer로 마이그레이션하는 방법을 알아봅니다. 원활한 전환을 위해 @intlayer/next-i18next 호환성 어댑터를 사용하세요."
keywords:
  - next-i18next
  - react-i18next
  - i18next
...
# next-i18next에서 Intlayer로 마이그레이션하기

## 왜 next-i18next에서 Intlayer로 마이그레이션해야 하나요?
...
`next-i18next`는 내부적으로 `react-i18next`와 `i18next`를 포장(wrapping)하므로, Intlayer로 마이그레이션하기 위한 두 가지 보완적 전략이 있습니다:

1. **호환성 어댑터 (기존 앱에 권장)** — `@intlayer/next-i18next`, `@intlayer/react-i18next`, `@intlayer/i18next`를 설치하세요. 이러한 패키지들은 기존과 **정확히 동일한 API**를 제공하면서 모든 번역 작업을 Intlayer에 위임합니다. `useTranslation`, `appWithTranslation`, `serverSideTranslations` 및 Next.js 페이지 라우팅에 대한 기존 호출은 그대로 유지됩니다. 변경 사항은 초기화 설정뿐입니다.

2. **전체 마이그레이션** — `next-i18next` API를 점진적으로 기본 Intlayer 훅(`useIntlayer`)으로 교체하고 컴포넌트와 함께 `.content.ts` 파일 내에 콘텐츠를 배치합니다.
...
다음 단계는 페이지와 컴포넌트 코드를 변경하지 않고 기존 Next.js Pages Router 앱을 Intlayer에서 실행하기 위해 필요한 최소한의 작업입니다.
...
> 마이그레이션 도중 별칭 설정이 완료될 때까지 `next-i18next`, `react-i18next`, `i18next`를 계속 설치해 둘 수 있습니다.

</Step>

<Step number={2} title="Intlayer 설정">

`intlayer init` 명령어는 초기 `intlayer.config.ts` 파일을 만듭니다. 기존 로케일(언어)과 일치하도록 업데이트하고 `syncJSON` 플러그인이 `next-i18next` 메시지 파일(일반적으로 `public/locales`)을 가리키도록 설정하세요:
...
const config: IntlayerConfig = {
  ...
  plugins: [
    syncJSON({
      // i18next 플레이스홀더 구문인 {{name}}과 일치시킵니다.
      format: "i18next",
...
> **`source`**는 로케일과 네임스페이스(`key`)를 해당 JSON 파일 경로로 매핑합니다. **`location`**은 Intlayer 관찰자(Watcher)에게 변경 사항을 모니터링할 폴더를 알려줍니다. `format: 'i18next'` 옵션은 `next-i18next`의 플레이스홀더가 올바르게 분석되도록 보장합니다.

</Step>

<Step number={3} title="Next.js 설정 업데이트">

기존 `next.config.ts` (또는 `.js`) 파일을 `@intlayer/next-i18next/plugin`의 `createNextI18nPlugin`으로 래핑하세요. 이 래퍼는 `withIntlayer`를 결합**하면서** 동시에 `next-i18next` / `react-i18next` / `i18next` → `@intlayer/*` 별칭을 주입합니다. 이를 통해 기존의 `import { useTranslation } from 'next-i18next'`와 같은 호출이 빌드 시 자동으로 리디렉션됩니다. 소스 코드를 변경할 필요가 없습니다.

```typescript fileName="next.config.ts" codeFormat={["typescript", "esm", "commonjs"]}
import type { NextConfig } from "next";
import { createNextI18nPlugin } from "@intlayer/next-i18next/plugin";
// next-i18next.config.js에서 가져오던 i18n 설정을 제거할 수 있습니다.
// import { i18n } from './next-i18next.config';

const withIntlayer = createNextI18nPlugin();

const nextConfig: NextConfig = {
  // Intlayer가 백그라운드에서 Next.js i18n 라우팅을 담당하므로
  // 여기서 더 이상 i18n 객체를 전달할 필요가 없습니다.
};

export default withIntlayer(nextConfig);
```

> **`next-i18next.config.js` 파일은 더 이상 필요하지 않습니다.** Intlayer는 모든 사전을 **빌드 시간**에 컴파일하고, 로케일 감지, 라우팅 및 사전 불러오기를 매끄럽게 처리합니다.
>
> `next-intlayer/server`의 순수한 `withIntlayer`를 사용하고 싶나요? 이 경우 사전은 컴파일되지만 `next-i18next` / `react-i18next` / `i18next`에 대한 별칭은 추가**되지 않습니다** — 따라서 수동으로 `@intlayer/*`를 참조하도록 가져오기 경로를 이름을 변경해야 합니다(단계 4 참고).

</Step>

</Steps>

이것으로 빠른 마이그레이션이 완료되었습니다. 이제 Next.js 앱은 `useTranslation`, `serverSideTranslations`, `appWithTranslation`에 대한 모든 호출을 그대로 유지한 채 Intlayer에서 작동합니다.
...
>
> ```tsx
> // Pages Router — 'about'은 등록된 사전 네임스페이스입니다.
> const { t } = useTranslation("about");
> t("counter.label"); // ✓ 자동 완성
> t("does.not.exist"); // ✗ TypeScript 오류
>
> // getStaticProps / getServerSideProps (i18next 인스턴스)
...
호환성 어댑터가 제자리에 배치되면 기존에 사용하던 표준 `next-i18next` 상용구를 삭제할 수 있습니다:

| 파일 / 패턴                              | 더 이상 필요 없는 이유                                                                                                                |
| ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| `next-i18next.config.js`                 | Intlayer는 `intlayer.config.ts`에 기반하여 라우팅, 사전 불러오기, 기본 로케일을 내부적으로 처리합니다.                                |
| `package.json` 안의 `next-i18next`       | 완전히 `@intlayer/next-i18next`와 별칭으로 교체되었습니다.                                                                            |
| JSON 언어 번들 (`public/locales/*.json`) | JSON 번들은 `syncJSON` 플러그인을 여전히 사용할 때만 필요합니다. `.content.ts` 파일로 이전을 완료하면 JSON 폴더를 삭제할 수 있습니다. |

한 단계 더 나아갈 준비가 되면 Intlayer는 **코드베이스 내의 모든 `.content.ts` 및 `.content.json` 파일을 자동으로 감지합니다**(기본적으로 `./src` 내의 어느 곳에서든). `MyComponent.tsx` 파일 바로 옆에 `my-component.content.ts` 파일을 추가하기만 하면 Intlayer는 별도 설정 없이 컴파일 타임에 이를 감지합니다 — 가져오기, 등록, 중앙 인덱스 파일이 필요 없습니다. 페이지와 컴포넌트에서 번역의 위치 통일이 매우 매끄러워집니다.
...
- **Intlayer와 Next.js (Pages Router)** — Next.js의 전체 설정 가이드: [intlayer_with_nextjs_page_router.md](https://github.com/aymericzip/intlayer/blob/main/docs/docs/ko/intlayer_with_nextjs_page_router.md)
