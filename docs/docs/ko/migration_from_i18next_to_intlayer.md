...
# i18next에서 Intlayer로 마이그레이션하기

## 왜 i18next에서 Intlayer로 마이그레이션해야 하나요?
...
`i18next`에서 Intlayer로 마이그레이션하기 위한 두 가지 보완적 전략이 있습니다:

1. **호환성 어댑터 (기존 앱에 권장)** — `@intlayer/i18next`를 설치하세요. 이 패키지는 `i18next`와 **정확히 동일한 API**를 제공하면서 모든 번역 작업을 Intlayer에 위임합니다. `i18next.t()`, `i18next.changeLanguage()` 및 `createInstance()`에 대한 기존 호출은 그대로 유지됩니다. 변경 사항은 가져오기 경로와 초기화뿐입니다.

2. **전체 마이그레이션** — `i18next` API를 점진적으로 기본 Intlayer 도구로 교체하고 컴포넌트와 함께 `.content.ts` 파일 내에 콘텐츠를 배치합니다.
...
다음 단계는 코드베이스를 변경하지 않고 기존 `i18next` 앱을 Intlayer에서 실행하기 위해 필요한 최소한의 작업입니다.
...
> `i18next`를 계속 설치해 둘 수 있습니다 — 호환성 어댑터는 TypeScript 타입을 위해 이를 `devDependency` / `peerDependency`로 사용합니다.
...
<Step number={3} title="번들러 별칭 업데이트 (선택 사항)">

번들러(Vite, Webpack, esbuild)를 사용하는 경우 모듈 별칭을 삽입하여 `import ... from 'i18next'`가 자동으로 `@intlayer/i18next`로 확인되게 할 수 있습니다. 이를 통해 코드베이스 전체에서 가져오기 경로를 수동으로 변경하지 않아도 됩니다.
...
> `i18nextVitePlugin()`은 `vite-intlayer`의 `intlayer()` 플러그인을 래핑하며, `i18next` → `@intlayer/i18next` 별칭을 자동으로 추가합니다. 일반적인 `vite-intlayer`의 `intlayer()` 플러그인을 사용하면 사전은 컴파일되지만 별칭은 추가**되지 않습니다** — 이 경우 수동으로 `@intlayer/i18next`를 참조하도록 가져오기 경로를 이름을 변경해야 합니다(다음 단계 참고).

</Step>

</Steps>

이것으로 빠른 마이그레이션이 완료되었습니다. 이제 앱은 모든 가져오기와 `i18next` API를 그대로 유지한 채 Intlayer에서 작동합니다.
...
소스 파일에서 명시적으로 의존성을 나타내고 싶거나, 가져오기를 별칭 지정하는 번들러 플러그인을 사용하지 않는다면 수동으로 가져오기 경로를 변경할 수 있습니다:
...
호환성 어댑터가 제자리에 배치되면 기존에 사용하던 표준 `i18next` 상용구를 삭제할 수 있습니다:

| 파일 / 패턴                       | 더 이상 필요 없는 이유                                                                                                                |
| --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| `i18next.init()` 호출             | Intlayer는 모든 것을 자동으로 초기화하며 런타임에 불러오는 단계가 없습니다.                                                           |
| `i18next.use(...)`                | Intlayer는 i18next 플러그인, 백엔드 또는 언어 탐지기를 사용하지 않습니다.                                                             |
| JSON 언어 번들 (`locales/*.json`) | JSON 번들은 `syncJSON` 플러그인을 여전히 사용할 때만 필요합니다. `.content.ts` 파일로 이전을 완료하면 JSON 폴더를 삭제할 수 있습니다. |

한 단계 더 나아갈 준비가 되면 Intlayer는 **코드베이스 내의 모든 `.content.ts` 및 `.content.json` 파일을 자동으로 감지합니다**(기본적으로 `./src` 내의 어느 곳에서든). 로직 파일 바로 옆에 `my-component.content.ts` 파일을 추가하기만 하면 Intlayer는 별도 설정 없이 컴파일 타임에 이를 감지합니다 — 가져오기, 등록, 중앙 인덱스 파일이 필요 없습니다. 번역의 위치 통일이 매우 매끄러워집니다.
