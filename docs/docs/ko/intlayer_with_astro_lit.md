...
priority: 9
title: "Astro + Lit i18n - 앱을 번역하는 완전 가이드"
...
Lit 커스텀 엘리먼트는 클라이언트 사이드 `<script>`를 통해 임포트되어 body에 배치됩니다.
...
<Step number={6} title="Lit 커스텀 엘리먼트 생성">
...
이 프로세스를 쉽게 하기 위해, Intlayer는 컴포넌트를 변환하고 콘텐츠를 추출하기 위한 [compiler](https://github.com/aymericzip/intlayer/blob/main/docs/docs/ko/compiler.md) / [extractor](https://github.com/aymericzip/intlayer/blob/main/docs/docs/ko/cli/extract.md)를 제안합니다.
...
const config: IntlayerConfig = {
  ...
  compiler: {
    ...
    /**
     * 컴포넌트를 변환한 후 저장해야 하는지 여부를 나타냅니다.
     *
     * - `true`인 경우, 컴파일러가 디스크의 컴포넌트 파일을 다시 작성합니다. 따라서 변환이 영구적이며, 컴파일러는 다음 프로세스에서 변환을 건너뜁니다. 이러한 방식으로 컴파일러는 앱을 변환한 다음 제거할 수 있습니다.
     *
     * - `false`인 경우, 컴파일러는 `useIntlayer()` 함수 호출을 빌드 출력의 코드에만 주입하고 기본 codebase를 유지합니다. 변환은 메모리에서만 수행됩니다.
     */
...
<Question title="Lit 아일랜드가 포함된 Astro 사이트를 국제화하는 데 사용할 수 있는 다양한 솔루션은 무엇인가요?">

Astro의 내장 `i18n` 옵션은 로케일 접두사와 리디렉션만 처리할 뿐 콘텐츠 자체는 관리하지 않으므로 메시지 레이어가 필요하며, 아일랜드 구조로 인해 두 번째 문제가 발생합니다: 아일랜드는 Astro가 아닌 Lit으로 실행된다는 점입니다.

- **Astro `i18n`과 직접 작성한 사전**, 그리고 아일랜드 내부의 **`@lit/localize`**: 동기화해야 할 두 개의 콘텐츠 소스가 생기고, 둘 사이에 공유된 타입 안전성이 없습니다.
- **`Intlayer`**: 둘 모두를 위한 단일 콘텐츠 레이어입니다. `astro-intlayer`는 `.astro` 페이지를 다루고 `lit-intlayer`는 Lit 아일랜드를 다루며, 둘 다 동일한 선언을 읽습니다.
...
<Question title="Lit 아일랜드 내부에 별도의 i18n 라이브러리가 필요한가요?">

아닙니다. `lit-intlayer`는 Astro 측과 동일한 사전을 읽으므로 `@lit/localize`를 함께 설치할 필요가 없습니다. 6단계에서 아일랜드 컴포넌트가 로케일을 다시 확인하는 대신 페이지로부터 활성 로케일을 직접 전달받는 모습을 보여줍니다.
