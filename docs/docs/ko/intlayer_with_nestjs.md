...
title: "NestJS i18n - 완전한 번역 가이드: NestJS"
description: 번들 크기, SEO, 성능 및 유지보수성을 위한 최고의 솔루션. 2026년에 NestJS 백엔드를 다국어로 만드세요, LLM 번역, Agent Skills & MCP.
keywords:
  - 국제화
  - 문서
  - Intlayer
...
> 콘텐츠 선언은 애플리케이션 내 어디에서나 정의할 수 있으며, `contentDir` 디렉토리(기본값 `./src`)에 포함되어 있고 콘텐츠 선언 파일 확장자(기본값 `.content.{json,ts,tsx,js,jsx,mjs,cjs}`)와 일치해야 합니다.
