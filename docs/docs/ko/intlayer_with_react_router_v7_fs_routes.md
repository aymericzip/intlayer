...
<Question title="React Router v7 앱을 국제화하는 데 사용할 수 있는 다양한 솔루션은 무엇인가요?">

- **`react-i18next` / `i18next`**: 런타임에 JSON 네임스페이스를 로드하며, 라우터와 수동으로 연결해야 하는 별도의 로케일 감지기가 필요합니다.
...
<Question title="본 가이드와 설정 기반 React Router 가이드의 차이점은 무엇인가요?">

라우트가 선언되는 방식만 다릅니다. 본 가이드는 파일 이름에서 로케일 세그먼트가 결정되는 파일 시스템 라우팅 규칙을 사용하는 반면, [설정 기반 가이드](https://github.com/aymericzip/intlayer/blob/main/docs/docs/ko/intlayer_with_react_router_v7.md)는 `routes.ts`에서 동일한 트리를 선언합니다. 콘텐츠 선언, 훅 및 설정은 두 방식 모두 동일하므로 i18n 코드를 수정하지 않고도 라우팅 규칙을 전환할 수 있습니다.

</Question>

<Question title="파일 시스템 라우트에서 로케일 세그먼트를 어떻게 추가하나요?">

4단계와 5단계에서 다룹니다: 로케일은 라우트 파일 이름에서 동적 세그먼트가 되어 그 아래의 모든 페이지가 활성 로케일을 수신합니다. `validatePrefix`는 해당 세그먼트가 선언된 로케일인지 여부를 확인하여 `/xx/about`이 중복 페이지를 렌더링하는 대신 실제 404를 반환하도록 하며, `getLocalizedUrl`은 모든 경로를 대상 언어로 다시 작성합니다.
...
8단계에서 컴포넌트를 보여줍니다. `useLocale`은 활성 로케일, 선언된 로케일 목록 및 선택 사항을 유지하는 setter를 노출하며, `getLocalizedUrl`은 현재 경로를 대상 언어로 다시 작성하므로 사용자가 홈으로 이동되지 않고 동일한 라우트에 머무를 수 있습니다.
