...
history:
  ...
  - version: 9.1.3
    date: 2026-08-06
    changes: "`routing.enableProxy`를 3상태로 변경: 미설정(auto), `true`, `false`"
  - version: 9.0.0
    date: 2026-07-11
...
const config: IntlayerConfig = {
  ...
  routing: {
    ...
    /**
     * Intlayer 로케일 라우팅 프록시(미들웨어)를 활성화합니다.
     * dev, preview, SSR에서 로케일 감지, 리디렉션, 리라이트를 처리합니다.
     * - 미설정(auto): dev 및 preview 서버는 쿠키와 헤더에 저장된 로케일을 무시하여
     *   라우팅을 URL 기반으로 유지합니다. 접두사 해석, 로케일 저장,
     *   Accept-Language 감지는 계속 동작합니다.
     *   프로덕션에서는 `true`와 동일하게 동작합니다.
     * - true: 모든 환경에서 완전한 동작.
     * - false: 로케일 라우팅 없음.
     * 기본값: undefined (auto)
     */
  ...
  analytics: {
     ...
     * `@intlayer/analytics`가 설치되어 있고 귀속(attribution)을 위해 `editor.clientId`가 설정되어 있어야 합니다.
     * 기본값: true
     */
    enabled: true,
...
| `enableProxy` | Intlayer 로케일 라우팅 프록시(미들웨어)를 활성화합니다.                                                                                                                            | `boolean` &#124; <br/> `undefined`                                                                                                                                                                           | `undefined` (auto)     | `true`                                                                                                                                                                                       | • 미설정(auto): dev 및 preview 서버는 쿠키/헤더에 저장된 로케일을 리디렉션 소스로 무시합니다. 접두사, 저장, `Accept-Language` 감지는 계속 동작합니다. 프로덕션에서는 `true`와 동일합니다.<br/>• `true`: 어디서나 완전한 동작.<br/>• `false`: 로케일 라우팅 없음. Next.js에서는 `intlayerProxy` 미들웨어가 패스스루가 됩니다. |
...
Intlayer의 내부 경로 및 출력 결과와 관련된 설정입니다. 이러한 설정은 일반적으로 내부용이며 사용자가 수정할 필요가 없습니다.

| Field                     | Description                                                          | Type     | Default                           | Example              | Note                                                  |
| ------------------------- | -------------------------------------------------------------------- | -------- | --------------------------------- | -------------------- | ----------------------------------------------------- |
| `baseDir`                 | 프로젝트의 기본 디렉토리입니다.                                      | `string` | `process.cwd()`                   | `'/path/to/project'` | 모든 Intlayer 관련 디렉토리를 해석하는 데 사용됩니다. |
| `dictionariesDir`         | 로컬라이제이션 딕셔너리를 저장할 디렉토리 경로입니다.                | `string` | `'.intlayer/dictionary'`          |                      |                                                       |
| `moduleAugmentationDir`   | 모듈 확대용 디렉토리이며, IDE 제안 및 타입 검사 기능을 향상시킵니다. | `string` | `'.intlayer/types'`               | `'intlayer-types'`   | `tsconfig.json`에 포함되어 있는지 확인하세요.         |
| `unmergedDictionariesDir` | 병합되지 않은 딕셔너리를 저장할 디렉토리입니다.                      | `string` | `'.intlayer/unmerged_dictionary'` |                      |                                                       |
| `typesDir`                | 딕셔너리 타입을 저장할 디렉토리입니다.                               | `string` | `'.intlayer/types'`               |                      |                                                       |
| `mainDir`                 | 주요 애플리케이션 파일이 저장된 디렉토리입니다.                      | `string` | `'.intlayer/main'`                |                      |                                                       |
| `configDir`               | 구성 파일이 저장된 디렉토리입니다.                                   | `string` | `'.intlayer/config'`              |                      |                                                       |
| `cacheDir`                | 캐시 파일이 저장된 디렉토리입니다.                                   | `string` | `'.intlayer/cache'`               |                      |                                                       |
