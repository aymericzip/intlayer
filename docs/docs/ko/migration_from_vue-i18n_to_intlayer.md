...
title: "vue-i18n에서 Intlayer로 마이그레이션하기 | 국제화 (i18n)"
description: "기존 코드를 손상시키지 않고 단계별로 Vue 또는 Nuxt 애플리케이션을 vue-i18n에서 Intlayer로 마이그레이션하는 방법을 알아봅니다. 원활한 전환을 위해 @intlayer/vue-i18n 호환성 어댑터를 사용하세요."
keywords:
  - vue-i18n
  - intlayer
  - 마이그레이션
...
`vue-i18n`에서 Intlayer로 마이그레이션하기 위한 두 가지 보완적 전략이 있습니다:

1. **호환성 어댑터 (기존 앱에 권장)** — `@intlayer/vue-i18n` (Vue 컴포넌트용)을 설치하세요. 이 패키지는 `vue-i18n`과 **정확히 동일한 API**를 제공하면서 모든 번역 작업을 Intlayer에 위임합니다. `$t`, `useI18n()`, `<i18n-t>`에 대한 기존 호출은 그대로 유지됩니다. 변경 사항은 가져오기 경로와 초기화뿐입니다.

2. **전체 마이그레이션** — `vue-i18n` API를 점진적으로 기본 Intlayer 훅(`useIntlayer`)으로 교체하고 컴포넌트와 함께 `.content.ts` 파일 내에 콘텐츠를 배치합니다.
...
다음 단계는 컴포넌트 코드를 변경하지 않고 기존 `vue-i18n` 앱을 Intlayer에서 실행하기 위해 필요한 최소한의 작업입니다.
...
> `vue-i18n`을 계속 설치해 둘 수 있습니다 — 호환성 어댑터는 TypeScript 타입을 위해 이를 `devDependency` / `peerDependency`로 사용합니다.
...
기존 번들러 구성을 호환성 플러그인으로 래핑하세요. 이는 핵심 Intlayer 플러그인을 결합하고 콘텐츠 관찰을 연결하며, 가장 중요하게는 **모듈 별칭을 주입**합니다. 따라서 기존의 `import … from 'vue-i18n'` 호출이 빌드 시 자동으로 `@intlayer/vue-i18n`으로 리디렉션됩니다. 소스 코드를 변경할 필요가 없습니다.
...
> `vueI18nVitePlugin()`은 `vite-intlayer`의 `intlayer()` 플러그인을 래핑하며, `vue-i18n` 별칭을 자동으로 추가합니다. 일반적인 `vite-intlayer`의 `intlayer()` 플러그인을 사용하면 사전은 컴파일되지만 별칭은 추가**되지 않습니다** — 이 경우 수동으로 `@intlayer/vue-i18n`을 참조하도록 가져오기 경로를 이름을 변경해야 합니다(단계 4 참고).

**Nuxt의 경우:**

`@nuxtjs/i18n` (Nuxt 통합)를 사용하는 경우 `nuxt-intlayer`를 설치하고 `nuxt.config.ts`에 추가하세요:
...
export default defineNuxtConfig({
  ...
  // @nuxtjs/i18n 모듈은 안전하게 제거할 수 있습니다
});
```

> **`createI18n()`나 수동 Provider 부트스트래핑이 더 이상 필요하지 않습니다.** Intlayer는 모든 사전을 **빌드 시간**에 컴파일하므로 런타임에 메시지를 불러오는 단계가 없습니다. 별칭으로 적용된 Provider가 초기화를 담당합니다.

</Step>

</Steps>

이것으로 빠른 마이그레이션이 완료되었습니다. 이제 앱은 모든 가져오기와 `vue-i18n` API를 그대로 유지한 채 Intlayer에서 작동합니다.

> **자동화된 타입의 번역 키.** Intlayer가 사전을 컴파일하면, `namespace` 옵션을 전달할 때 `useI18n`은 실제 콘텐츠에 대해 타입 검사를 수행합니다. 키는 IDE에서 자동 완성되며 유효하지 않은 경로는 빌드 시에 TypeScript 오류로 나타납니다 — 추가 설정이 필요 없습니다.
>
> ```ts
> // 'about'은 등록된 사전 키입니다.
...
호환성 어댑터가 제자리에 배치되면 기존에 사용하던 표준 `vue-i18n` 상용구를 삭제할 수 있습니다:

| 파일 / 패턴                         | 더 이상 필요 없는 이유                                                                                                                |
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| `createI18n()` 호출                 | Intlayer Provider는 모든 것을 자동으로 초기화하며 런타임에 불러오는 단계가 없습니다.                                                  |
| Vue 플러그인 등록 (`app.use(i18n)`) | Intlayer 플러그인이 내부적으로 의존성 주입(injection)을 담당합니다.                                                                   |
