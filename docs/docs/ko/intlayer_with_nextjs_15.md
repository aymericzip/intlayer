...
- **`IntlayerProvider`**는 로케일 레이아웃에 한 번만 마운트됩니다. 서버와 클라이언트 컴포넌트 모두에 로케일을 제공하므로 페이지가 더 이상 자체적으로 감싸지 않아도 됩니다.
- 서버 훅은 호출 위치에서 전달된 로케일, 프로바이더에 의해 설정된 서버 컨텍스트, 요청에 포함된 로케일(Intlayer 프록시에 의해 설정된 `x-intlayer-locale` 헤더, 그 다음 로케일 쿠키) 순서로 로케일을 확인합니다. 이 마지막 단계는 레이아웃(및 프로바이더)이 다시 실행되지 않는 페이지 세그먼트만 다시 렌더링하는 클라이언트 측 탐색에서도 콘텐츠가 올바르게 유지되도록 합니다.
...
Intlayer v9.5부터 `withIntlayer`가 content watcher를 자체적으로 시작하므로 추가 설정이 필요하지 않습니다.
