...
title: "Astro + Preact i18n - 앱을 번역하는 완전 가이드"
description: "i18next는 이제 그만. 2026년 다국어 (i18n) Astro + Preact 앱 구축 가이드. AI 에이전트로 번역하고 번들 크기, SEO, 성능을 최적화하세요."
keywords:
  - 국제화
  - 문서
  - Intlayer
...
# Intlayer를 사용하여 Astro + Preact 사이트 번역하기 | 국제화 (i18n)
...
## Astro + Preact에서 Intlayer 설정을 위한 단계별 가이드
...
- **@astrojs/preact**
  Preact 컴포넌트 아일랜드를 사용할 수 있게 해주는 공식 Astro 통합 도구입니다.
...
Astro 설정에 intlayer 플러그인과 Preact 통합을 추가합니다.
...
> `preact()` 통합은 `client:only="preact"`를 통해 Preact 컴포넌트 아일랜드를 사용할 수 있게 합니다.
...
> 콘텐츠 파일에 TSX 코드가 포함된 경우 `import { h } from "preact";`를 임포트하거나 JSX 프라그마가 Preact에 맞게 올바르게 설정되어 있는지 확인해야 할 수도 있습니다.

</Step>

<Step number={5} title="Astro에서 콘텐츠 사용">

`intlayer`에서 내보낸 핵심 헬퍼를 사용하여 `.astro` 파일에서 직접 사전을 소비할 수 있습니다. 또한 각 페이지에 hreflang 및 canonical 링크와 같은 SEO 메타데이터를 추가하고, 클라이언트 사이드의 인터랙티브 콘텐츠를 위해 Preact 아일랜드를 포함해야 합니다.
...
<html lang={locale} dir={getHTMLTextDir(locale)}>
  ...
  <body>
    <!-- Preact 아일랜드는 언어 전환기를 포함한 모든 인터랙티브 콘텐츠를 렌더링합니다 -->
    <PreactIsland locale={locale} client:only="preact" />
  </body>
</html>
```

> 콘텐츠를 `alt`, `title`, `href`, `aria-label` 등의 `string` 속성에 사용하려면 함수의 값을 다음과 같이 사용할 수 있습니다:
...
<Step number={6} title="Preact 아일랜드 컴포넌트 생성">

Preact 애플리케이션을 감싸고 서버에서 감지된 로케일을 전달받는 아일랜드 컴포넌트를 생성합니다:
...
> `locale` 프롭은 Astro 페이지(서버 감지)에서 `IntlayerProvider`로 전달되어 트리 내 모든 Preact 훅의 초기 로케일이 됩니다.

> 참고: Preact에서는 HTML 속성에 `className` 대신 `class`를 사용합니다.

</Step>

<Step number={7} title="언어 전환기 추가">

사용 가능한 로케일을 읽고 사용자가 새 언어를 선택할 때 로컬라이즈된 URL로 이동하는 Preact 컴포넌트 `LocaleSwitcher`를 생성합니다:
...
const config: IntlayerConfig = {
  ...
  compiler: {
     ...
     *
     * - `true`인 경우, 컴파일러는 컴포넌트 파일을 디스크에 다시 작성합니다. 따라서 변환은 영구적이며, 컴파일러는 다음 프로세스에서 변환을 건너뜁니다. 이러한 방식으로 컴파일러는 앱을 변환할 수 있으며, 그 후 제거될 수 있습니다.
     *
...
> v9부터 `intlayerCompiler`가 `intlayer` 플러그인에 포함되어 있습니다. 따라서 수동으로 추가할 필요가 없습니다.
...
```json5 fileName="tsconfig.json"
{
  compilerOptions: {
    // ...
    jsx: "react-jsx",
    jsxImportSource: "preact", // Preact 10+에서 권장됨
  },
  include: [
    // ... 기존 TypeScript 설정
...
<Question title="Preact 아일랜드가 포함된 Astro 사이트를 국제화하는 데 사용할 수 있는 다양한 솔루션은 무엇인가요?">

Astro의 내장 `i18n` 옵션은 로케일 접두사와 리디렉션만 처리할 뿐 콘텐츠 자체는 관리하지 않으므로 메시지 레이어가 필요하며, 아일랜드 구조로 인해 두 번째 문제가 발생합니다: 아일랜드는 Astro가 아닌 Preact로 실행된다는 점입니다.

- **Astro `i18n`과 직접 작성한 사전**, 그리고 아일랜드 내부의 **`preact-i18n`**: 동기화해야 할 두 개의 콘텐츠 소스가 생기고, 둘 사이에 공유된 타입 안전성이 없습니다.
- **`Intlayer`**: 둘 모두를 위한 단일 콘텐츠 레이어입니다. `astro-intlayer`는 `.astro` 페이지를 다루고 `preact-intlayer`는 Preact 아일랜드를 다루며, 둘 다 동일한 선언을 읽습니다.
...
대부분 가능합니다. 콘텐츠를 이전하려면 [react-i18next 마이그레이션 가이드](https://github.com/aymericzip/intlayer/blob/main/docs/docs/ko/migration_from_react-i18next_to_intlayer.md)를 따르세요. 점진적으로 마이그레이션할 수도 있습니다: [sync JSON 플러그인](https://github.com/aymericzip/intlayer/blob/main/docs/docs/ko/plugins/sync-json.md)은 기존 JSON 카탈로그를 단일 진실 공급원(source of truth)으로 유지하면서 Intlayer 사전을 생성하므로 컴포넌트를 하나씩 이전하는 동안 두 계층의 동기화가 유지됩니다.
...
<Question title="Preact 아일랜드 내부에 별도의 i18n 라이브러리가 필요한가요?">

아닙니다. `preact-intlayer`는 Astro 측과 동일한 사전을 읽으므로 `preact-i18n`을 함께 설치할 필요가 없습니다. 6단계에서 아일랜드 컴포넌트가 로케일을 다시 확인하는 대신 페이지로부터 활성 로케일을 직접 전달받는 모습을 보여줍니다.
