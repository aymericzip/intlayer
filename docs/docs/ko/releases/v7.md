...
## 자동 코드 포맷팅: formatCommand 구성

v7은 편집기 구성에서 `formatCommand` 옵션을 도입하여, Intlayer에 의해 작성될 때 콘텐츠 파일을 자동으로 포맷할 수 있도록 합니다. 이를 통해 콘텐츠 선언 파일 전체에서 일관된 코드 스타일과 포맷팅을 보장합니다.

설정되지 않은 경우, Intlayer는 포맷 명령을 자동으로 감지하려고 시도합니다. prettier, biome, eslint의 다음 명령들을 해석하려고 시도합니다.

### 설정

`formatCommand` 옵션은 `{{file}}`이 실제 파일 경로로 대체되는 문자열 템플릿을 허용합니다:
...
파일 경로를 인수로 받는 모든 코드 포매터를 사용할 수 있습니다:
...
- **일관된 형식 지정**: 모든 콘텐츠 파일이 프로젝트의 스타일 가이드라인에 따라 자동으로 형식이 지정됩니다
- **개발자 경험**: Intlayer가 파일을 작성한 후 수동으로 형식을 지정할 필요가 없습니다
- **팀 일관성**: 모든 팀 멤버가 동일한 형식을 콘텐츠 파일에 적용하도록 보장합니다
- **CI/CD 통합**: 콘텐츠 파일이 자동화된 워크플로우에서 일관된 형식을 유지합니다

### 작동 원리

Intlayer가 content declaration 파일(`.content.ts`, `.content.js` 등)을 작성하거나 업데이트할 때, 지정된 format 명령을 파일에 대해 자동으로 실행합니다. `{{file}}` placeholder는 실제 파일 경로로 대체되며, 명령은 프로젝트의 base directory에서 실행됩니다.

---

## Dictionary Configuration: 더 나은 구성과 구조

v7은 dictionary 관련 설정에 대한 더 나은 조직화와 개선된 컨텐츠 관리를 제공하는 새로운 전용 `dictionary` 설정 섹션을 도입합니다.

### 새로운 dictionary 구성 구조

`fill` 속성이 `content` 섹션에서 새로운 `dictionary` 섹션으로 이동되어 더 명확한 관심사의 분리를 제공합니다:
...
- **더 명확한 구성**: 사전 관련 설정이 이제 함께 그룹화되어 있습니다
- **더 나은 관심사의 분리**: 콘텐츠 발견과 사전 작업이 명확하게 분리됩니다
- **향상된 유지보수성**: 사전 관련 구성을 더 쉽게 이해하고 수정할 수 있습니다
- **향후 확장성**: 사전 섹션은 추가 사전 관련 설정을 수용할 수 있습니다
- **포괄적인 사전 관리**: 새로운 사전을 만들고 관리하기 위한 `title`, `live`, `priority`, `tags`, `version`, `description`과 같은 속성을 포함합니다

### 구성 사용

dictionary 구성은 두 가지 주요 목적을 수행합니다:

1. **기본값**: 콘텐츠 선언 파일을 생성할 때 기본값 정의
2. **Fallback 동작**: 특정 필드가 정의되지 않았을 때 fallback 값을 제공하여 dictionary 작동 동작을 전역적으로 정의 가능

dictionary 섹션에는 dictionary 관리를 위한 포괄적인 속성이 포함됩니다:

- **`fill`**: 콘텐츠 생성을 위한 자동 채우기 동작
- **`title`**: 새 dictionary의 기본 제목
- **`live`**: 실시간 업데이트를 위한 라이브 동기화 구성
- **`priority`**: dictionary 해석 우선순위 설정
- **`tags`**: 콘텐츠 조직화를 위한 기본 태그
- **`version`**: dictionary 업데이트를 위한 버전 관리
- **`description`**: 새 콘텐츠의 기본 설명

콘텐츠 선언 파일 및 구성 값이 적용되는 방법에 대한 자세한 정보는 [Content File Documentation](https://github.com/aymericzip/intlayer/blob/main/docs/docs/ko/dictionary/content_file.md)을 참조하세요.
...
- **`editor.formatCommand`**: 콘텐츠 파일의 자동 코드 포맷팅을 위한 새로운 옵션
