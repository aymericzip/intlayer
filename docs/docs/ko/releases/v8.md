...
자세한 내용은 [YAML 콘텐츠 파일 문서](https://github.com/aymericzip/intlayer/blob/main/docs/docs/ko/dictionary/content_file.md#yaml-content-file) 및 [Markdown 콘텐츠 파일 문서](https://github.com/aymericzip/intlayer/blob/main/docs/docs/ko/dictionary/markdown.md)를 참조하세요.
...
자세한 내용은 [복수형 문서](https://github.com/aymericzip/intlayer/blob/main/docs/docs/ko/dictionary/plurial.md)를 참조하세요.
...
```typescript
content.placeholder; // IntlayerNode<string> 반환
content.placeholder.value; // string 반환
content.placeholder.toString(); // string 반환
content.placeholder.toLowerCase(); // string 반환
String(content.placeholder); // string 반환
content.placeholder.toUpperCase(); // 대문자로 변환된 string 반환
content.placeholder.replace("a", "b"); // 수정된 string 반환
```

### Array 예제

```typescript
content.myArrayOfString             // IntlayerNode<Array<string>> 반환
content.myArrayOfString.find(...)   // 요소 반환
content.myArrayOfString.join(', ')  // 결합된 string 반환
```
...
- **Parcel Watcher**: 콘텐츠 감시자(watcher)가 `chokidar`에서 `@parcel/watcher`로 마이그레이션되어, 네이티브 C++ 파일 감시를 활용해 더 빠른 업데이트와 현저히 낮은 메모리 소비를 제공합니다.
- **새로운 언어 서버 프로토콜 (LSP)**: 완전히 새로운 LSP를 이제 사용할 수 있습니다. OXC 기반 파싱으로 구축되어, IDE 및 AI 에이전트가 `useIntlayer('my-key')` 호출을 해당하는 `.content` 파일과 원활하게 연결할 수 있도록 돕습니다.

자세한 내용은 [LSP 문서](https://github.com/aymericzip/intlayer/blob/main/docs/docs/ko/lsp.md)를 참조하세요.
...
Intlayer v8.7.0은 프로덕션 번들을 최적화하기 위해 설계된 새로운 실험적 빌드 기능을 `intlayer.config.ts`에 도입합니다:

- **딕셔너리 경량화 (Minification)**: 크기를 줄이기 위해 딕셔너리 파일을 압축합니다.
- **사용되지 않는 키 제거 (Unused Key Purging)**: 딕셔너리에서 사용되지 않는 번역 키를 스캔하여 제거하고, 활성 키의 이름을 짧은 문자(예: `"products" -> "a"`, `"pricing" -> "b"`)로 변경합니다. 이를 통해 번들 크기를 최대 5%까지 줄일 수 있습니다.
- **TypeScript 검사 비활성화**: 컴파일 단계에서 TypeScript 타입 검사를 비활성화하여 빌드 속도를 높입니다.
...
const config: IntlayerConfig = {
  ...
  build: {
    /**
     * 번들 크기를 줄이기 위해 딕셔너리를 경량화(minify)합니다.
     */
    minify: true,

    /**
     * 딕셔너리에서 사용되지 않는 키를 제거(purge)합니다.
     */
    purge: true,

    /**
     * 빌드 시 TypeScript 타입을 확인할지 여부를 나타냅니다.
     */
    checkTypes: false,
  },
};
...
구성에 대한 자세한 내용은 [Angular 환경 문서](https://github.com/aymericzip/intlayer/blob/main/docs/docs/ko/intlayer_with_angular_21.md)를 참조하세요.
