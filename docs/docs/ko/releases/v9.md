...
## Vite 플러그인: 번들로 제공되는 Compiler 및 Proxy

`intlayer()` Vite 플러그인은 이제 **compiler**와 **locale-routing proxy**를 직접 번들로 제공하므로 대부분의 프로젝트에서는 `vite.config.ts`에 단일 플러그인만 필요합니다:
...
- **Proxy**: 새로운 `routing.enableProxy` 옵션 (기본값 `true`)을 기반으로 자동으로 활성화됩니다. 개발, 미리보기 및 프로덕션 SSR에서 로캘 감지 / 리디렉션 / 재작성 미들웨어를 연결합니다. 더 이상 `intlayerProxy()`를 별도로 등록할 필요가 없습니다.

### `routing.enableProxy` 옵션

새로운 `routing.enableProxy` 옵션은 로캘 라우팅 프록시가 연결되는지 여부를 제어합니다. 기본값은 `true`입니다. 로캘 라우팅을 직접 처리하려면 이를 비활성화하세요:
...
독립 실행형 `intlayerCompiler()` 및 `intlayerProxy()` 플러그인은 고급 설정을 위해 계속 내보내집니다. 이를 `intlayer()`와 함께 등록하는 것은 안전합니다 — 각 플러그인은 자체적으로 중복을 제거하고 한 번만 실행됩니다.
