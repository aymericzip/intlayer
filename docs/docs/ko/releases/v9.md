...
Intlayer v9는 단일 명령으로 자신의 Intlayer 인스턴스를 실행할 수 있는 일급 지원을 제공합니다. Intlayer Cloud 계정이 필요하지 않습니다.

```sh
curl -fsSL https://intlayer.org/install.sh | sh
```

설치 프로그램은 `docker-compose.yml` 및 `.env`를 다운로드하고, 필요한 시크릿을 자동으로 생성하며, `docker compose up -d`를 실행합니다. 대시보드, API, 데이터베이스, 객체 스토리지, 트랜잭션 이메일 등 모든 것이 컨테이너에서 로컬로 실행됩니다.
