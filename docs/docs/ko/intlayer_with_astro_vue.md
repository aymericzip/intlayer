...
# Intlayer를 사용하여 Astro + Vue 사이트 번역하기 | 국제화 (i18n)
...
- **vue-intlayer**
  Intlayer를 Vue 애플리케이션과 통합하는 패키지입니다. Vue의 국제화를 위한 `installIntlayer`, `useIntlayer` 및 `useLocale` 컴포저블을 제공합니다.

- **@astrojs/vue**
  Vue 컴포넌트 아일랜드를 사용할 수 있게 해주는 공식 Astro 통합 도구입니다.
...
Astro 설정에 intlayer 플러그인과 Vue 통합을 추가합니다.
...
> `vue()` 통합은 `client:only="vue"`를 통해 Vue 컴포넌트 아일랜드를 사용할 수 있게 합니다.
...
<html lang={locale} dir={getHTMLTextDir(locale)}>
  ...
  <body>
    <!-- Vue 아일랜드는 언어 전환기를 포함한 모든 인터랙티브 콘텐츠를 렌더링합니다 -->
...
<Step number={6} title="Vue 아일랜드 컴포넌트 생성">

Vue 애플리케이션을 감싸고 서버에서 감지된 로케일을 전달받는 아일랜드 컴포넌트를 생성합니다. 컴포저블을 사용하기 전에 Vue 인스턴스에 Intlayer 플러그인을 등록하기 위해 `installIntlayer`를 호출해야 합니다.
...
<template>
  <div>
    <h1>{{ title }}</h1>
    <!-- 언어 전환기는 아일랜드 템플릿 내에 인라인으로 렌더링됩니다 -->
...
> `locale` 프롭은 Astro 페이지(서버 감지)에서 전달되어 `installIntlayer`를 초기화하는 데 사용되며, 이로 인해 컴포넌트 트리 내 모든 컴포저블의 기본 로케일이 결정됩니다.

</Step>
<Step number={7} title="언어 전환기 추가">

언어 전환 기능은 Vue 아일랜드 템플릿(위 6단계 참조)에 직접 내장되어 있습니다. `vue-intlayer`의 `useLocale` 컴포저블을 사용하며, 사용자가 새 언어를 선택할 때 로컬라이즈된 URL로 이동합니다:
...
기존 codebase가 있다면 수천 개의 파일을 변환하는 것은 시간이 걸릴 수 있습니다.

이 프로세스를 쉽게 하기 위해 Intlayer는 컴포넌트를 변환하고 콘텐츠를 추출할 수 있는 [compiler](https://github.com/aymericzip/intlayer/blob/main/docs/docs/ko/compiler.md) / [extractor](https://github.com/aymericzip/intlayer/blob/main/docs/docs/ko/cli/extract.md)를 제공합니다.
...
const config: IntlayerConfig = {
  ...
  compiler: {
     ...
     *
     * - `true`인 경우, 컴파일러는 컴포넌트 파일을 디스크에 다시 작성합니다. 따라서 변환은 영구적이고, 컴파일러는 다음 프로세스에서 변환을 건너뜁니다. 이렇게 하면 컴파일러가 앱을 변환한 다음 제거할 수 있습니다.
     *
...
<Question title="Vue 아일랜드가 포함된 Astro 사이트를 국제화하는 데 사용할 수 있는 다양한 솔루션은 무엇인가요?">

Astro의 내장 `i18n` 옵션은 로케일 접두사와 리디렉션만 처리할 뿐 콘텐츠 자체는 관리하지 않으므로 메시지 레이어가 필요하며, 아일랜드 구조로 인해 두 번째 문제가 발생합니다: 아일랜드는 Astro가 아닌 Vue로 실행된다는 점입니다.

- **Astro `i18n`과 직접 작성한 사전**, 그리고 아일랜드 내부의 **`vue-i18n`**: 동기화해야 할 두 개의 콘텐츠 소스가 생기고, 둘 사이에 공유된 타입 안전성이 없습니다.
- **`Intlayer`**: 둘 모두를 위한 단일 콘텐츠 레이어입니다. `astro-intlayer`는 `.astro` 페이지를 다루고 `vue-intlayer`는 Vue 아일랜드를 다루며, 둘 다 동일한 선언을 읽습니다.
...
<Question title="컴포넌트를 다시 작성하지 않고 vue-i18n에서 마이그레이션할 수 있나요?">

대부분 가능합니다. 콘텐츠를 이전하려면 [vue-i18n 마이그레이션 가이드](https://github.com/aymericzip/intlayer/blob/main/docs/docs/ko/migration_from_vue-i18n_to_intlayer.md)를 따르세요. 점진적으로 마이그레이션할 수도 있습니다: [sync JSON 플러그인](https://github.com/aymericzip/intlayer/blob/main/docs/docs/ko/plugins/sync-json.md)은 기존 JSON 카탈로그를 단일 진실 공급원(source of truth)으로 유지하면서 Intlayer 사전을 생성하므로 컴포넌트를 하나씩 이전하는 동안 두 계층의 동기화가 유지됩니다.
...
<Question title="Vue 아일랜드 내부에 별도의 i18n 라이브러리가 필요한가요?">

아닙니다. `vue-intlayer`는 Astro 측과 동일한 사전을 읽으므로 `vue-i18n`을 함께 설치할 필요가 없습니다. 6단계에서 아일랜드 컴포넌트가 로케일을 다시 확인하는 대신 페이지로부터 활성 로케일을 직접 전달받는 모습을 보여줍니다.
