...
| 사용 사례                                              | 헬퍼     |
| ------------------------------------------------------ | -------- |
| 로케일에 맞는 문법적 복수형 (one apple / two apples)   | `plural` |
| 사용자 정의 숫자 범위 (`<5`, `>=10`) 또는 비 CLDR 범주 | `enu`    |

영어만 대상으로 하는 경우(`one` / `other`만 있음), 둘 다 작동합니다. `few` / `many` / `two` 구분이 있는 언어의 경우 `plural`을 권장합니다.
