...
title: Astro + React i18n - Comment traduire une application Astro + React en 2026
description: Apprenez comment ajouter l'internationalisation (i18n) à votre site Astro + React en utilisant Intlayer. Suivez ce guide pour rendre votre site multilingue.
keywords:
...
history:
  ...
  - version: 6.2.0
    date: 2025-10-03
    changes: "Actualisation pour l'intégration Astro, configuration, utilisation"
---

# Traduisez votre site Astro + React avec Intlayer | Internationalisation (i18n)
...
- **react**, **react-dom**
  Les paquets React fondamentaux nécessaires pour rendre les composants React dans le navigateur.

- **react-intlayer**
  Le paquet qui intègre Intlayer avec les applications React. Il fournit `IntlayerProvider`, et les hooks `useIntlayer` et `useLocale` pour l'internationalisation React.

- **@astrojs/react**
  L'intégration Astro officielle qui permet d'utiliser des îles de composants React.
...
Ajoutez le plugin intlayer et l'intégration React dans votre configuration.
...
Vous pouvez consommer les dictionnaires directement dans les fichiers `.astro` en utilisant les helpers principaux exportés par `intlayer`. Vous devriez également ajouter des métadonnées SEO comme les liens hreflang et canoniques à chaque page, et intégrer l'île React pour le contenu interactif côté client.
...
<html lang={locale}>
  ...
  <body>
    <!-- L'île React rend tout le contenu interactif, y compris le sélecteur de locale -->
...
### Étape 6 : Créer le composant Île React

Créez le composant île qui enveloppe votre application React et reçoit la locale détectée par le serveur :
...
> La prop `locale` est transmise de la page Astro (détectée par le serveur) dans `IntlayerProvider`, ce qui en fait la locale initiale pour tous les hooks React de l'arbre.

### Étape 7 : Ajouter un Sélecteur de Locale

Créez un composant React `LocaleSwitcher` qui lit les locales disponibles et navigue vers l'URL localisée lorsque l'utilisateur choisit une nouvelle langue :
