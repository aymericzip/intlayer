...
## Guide étape par étape pour configurer Intlayer dans Astro + React
...
- **react-intlayer**
  Le package qui intègre Intlayer avec les applications React. Il fournit l'`IntlayerProvider`, ainsi que les hooks `useIntlayer` et `useLocale` pour l'internationalisation dans React.

- **@astrojs/react**
  L'intégration Astro officielle qui permet d'utiliser des îles (islands) de composants React.
...
Ajoutez le plugin intlayer et l'intégration React à votre configuration Astro.
...
> L'intégration `react()` permet d'utiliser des îles de composants React via `client:only="react"`.
...
<html lang={locale} dir={getHTMLTextDir(locale)}>
  ...
  <body>
    <!-- L'île React rend tout le contenu interactif, y compris le sélecteur de langue -->
...
<Step number={6} title="Créer le composant Île React">

Créez un composant île qui enveloppe votre application React et reçoit la locale détectée par le serveur :
...
> La prop `locale` est transmise de la page Astro (détection serveur) à l'`IntlayerProvider`, ce qui en fait la locale initiale pour tous les hooks React de l'arbre.

</Step>
<Step number={7} title="Ajouter un sélecteur de langue">

Créez un composant React `LocaleSwitcher` qui lit les locales disponibles et navigue vers l'URL localisée lorsqu'un utilisateur sélectionne une nouvelle langue :
...
export function LocaleSwitcher() {
  ...
  return (
    <div className="locale-switcher">
      <span className="switcher-label">Changer de langue :</span>
...
<Question title="Quelles sont les différentes solutions pour internationaliser un site Astro avec des îlots React ?">

L'option `i18n` intégrée d'Astro gère les préfixes de locale et les redirections, mais pas le contenu lui-même : vous avez donc toujours besoin d'une couche de messages, et un îlot ajoute un second problème : l'îlot exécute React, pas Astro.

- **`i18n` d'Astro plus des dictionnaires écrits à la main**, avec **`react-i18next`** à l'intérieur de l'îlot : deux sources de contenu à garder synchronisées, et aucun typage partagé entre elles.
- **`Intlayer`** : une seule couche de contenu pour les deux. `astro-intlayer` couvre les pages `.astro` et `react-intlayer` couvre les îlots React, en lisant les mêmes déclarations.
...
<Question title="Ai-je besoin d'une bibliothèque d'i18n séparée à l'intérieur de mon îlot React ?">

Non. `react-intlayer` lit les mêmes dictionnaires que le côté Astro, vous n'installez donc pas `react-i18next` à côté. L'étape 6 montre le composant d'îlot recevant la locale active de la page au lieu de la résoudre à nouveau.
