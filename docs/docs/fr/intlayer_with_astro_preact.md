...
description: Apprenez comment ajouter l'internationalisation (i18n) à votre site Astro + Preact en utilisant Intlayer. Suivez ce guide pour rendre votre site multilingue.
keywords:
...
# Traduisez votre site Astro + Preact avec Intlayer | Internationalisation (i18n)
...
- **preact**
  Le paquet Preact fondamental — une alternative rapide et légère à React.

- **preact-intlayer**
  Le paquet qui intègre Intlayer avec les applications Preact. Il fournit `IntlayerProvider`, et les hooks `useIntlayer` et `useLocale` pour l'internationalisation Preact.

- **@astrojs/preact**
  L'intégration Astro officielle qui permet d'utiliser des îles de composants Preact.
...
Ajoutez le plugin intlayer et l'intégration Preact dans votre configuration.
...
Vous pouvez consommer les dictionnaires directement dans les fichiers `.astro` en utilisant les helpers principaux exportés par `intlayer`. Vous devriez également ajouter des métadonnées SEO comme les liens hreflang et canoniques à chaque page, et intégrer l'île Preact pour le contenu interactif côté client.
...
<html lang={locale}>
  ...
  <body>
    <!-- L'île Preact rend tout le contenu interactif, y compris le sélecteur de locale -->
...
Créez le composant île qui enveloppe votre application Preact et reçoit la locale détectée par le serveur :
...
> La prop `locale` est transmise de la page Astro (détectée par le serveur) dans `IntlayerProvider`, ce qui en fait la locale initiale pour tous les hooks Preact de l'arbre.

> Note : Dans Preact, utilisez `class` au lieu de `className` pour les attributs HTML.

### Étape 7 : Ajouter un Sélecteur de Locale

Créez un composant Preact `LocaleSwitcher` qui lit les locales disponibles et navigue vers l'URL localisée lorsque l'utilisateur choisit une nouvelle langue :
