...
Voir [Modèle d'Application](https://github.com/aymericzip/intlayer-angular-22-template) sur GitHub.
...
Dans cette architecture, `angular-intlayer` fournit `provideIntlayer()`, enregistré dans `src/app/app.config.ts` afin que chaque composant puisse lire son contenu via le signal `useIntlayer`. L'Angular CLI est configurée avec le builder esbuild personnalisé, et `esbuild.plugins.ts` enregistre le plugin Intlayer dans le build pour observer et reconstruire vos déclarations de contenu. Les déclarations de contenu sont placées aux côtés des composants dans `src/app/`.
...
<Question title="Quelles sont les différentes solutions pour internationaliser une application Angular ?">
...
La raison habituelle de quitter `@angular/localize` est le modèle d'un build par locale. Intlayer conserve un seul build et change de langue à l'exécution. Voir [pourquoi Intlayer](https://github.com/aymericzip/intlayer/blob/main/docs/docs/fr/interest_of_intlayer.md).
...
Non. C'est le modèle de `@angular/localize`. Avec Intlayer, un seul build sert toutes les locales déclarées, la langue active est résolue à l'exécution à partir de l'URL, d'un cookie ou de l'en-tête `Accept-Language`, et seul le contenu de la locale affichée est téléchargé.
...
<Question title="Comment traduire une application Angular automatiquement avec l'IA ?">
