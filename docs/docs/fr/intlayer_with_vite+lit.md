...
<AccordionGroup>
<Accordion header="Couverture entièrement éclairée">
...
Voir [Application Template](https://github.com/aymericzip/intlayer-vite-lit-template) sur GitHub.
...
Dans cette architecture, `lit-intlayer` fournit des intégrations d'éléments personnalisés et des contrôleurs réactifs pour gérer les traductions dans les Web Components. Les déclarations de contenu résident dans `src/` aux côtés des éléments.
...
<Step number={6} title="Utiliser Intlayer dans votre LitElement">
...
<Question title="Quelles sont les différentes solutions pour internationaliser une application Vite et Lit ?">

Vite n'a pas d'avis sur l'i18n, le choix vient donc de l'écosystème Lit :

- **`@lit/localize`** : l'option officielle, basée sur l'extraction XLIFF et un bundle compilé par locale, si bien que changer de langue à l'exécution nécessite le mode runtime.
- **Un dictionnaire écrit à la main** importé dans chaque élément : aucune dépendance, aucun typage, aucun outil.
...
<Question title="Quel poids l'i18n ajoute-t-elle à la taille de mon bundle Lit ?">
...
<Question title="Comment utiliser du contenu traduit dans un composant Lit ?">

Lisez le contenu à l'intérieur de la méthode `render` de votre `LitElement` via la liaison Intlayer, comme le montre l'étape 6. Changer la locale déclenche une mise à jour des éléments qui la lisent, si bien que le changement de langue ne nécessite pas de recharger la page ni de livrer un second bundle.
