...
- **noPrefix** :
  ...
  - _Type_ : `boolean`
  - _Par défaut_ : `false`
  - _Description_ : Indique si les routes doivent être servies sans préfixe de locale.
  - _Exemple_ : `true`
  - _Remarque_ : Si activé, les routes ne seront pas préfixées par la locale (ex. `/fr/about` devient `/about`).

- **fallbackToDefaultLocale** :

  - _Type_ : `boolean`
  - _Par défaut_ : `true`
  - _Description_ : Indique si l'application doit utiliser la locale par défaut lorsqu'aucune locale valide n'est trouvée.
  - _Exemple_ : `false`
...
- **detectLocaleFromHeaders** :
  ...
  - _Type_ : `boolean`
  - _Par défaut_ : `true`
  - _Description_ : Active ou désactive la détection automatique de la locale à partir des en-têtes HTTP `Accept-Language`.
  - _Exemple_ : `false`
...
- **Remarque** : `@intlayer/babel` est disponible par défaut sur le package `vite-intlayer`, mais `@intlayer/swc` n'est pas installé par défaut sur le package `next-intlayer` car les plugins SWC sont encore expérimentaux sur Next.js.
...
- **optimize** :
  ...
  - _Exemple_ : `true`
  - _Remarque_ : Cela permettra d'importer uniquement les dictionnaires utilisés dans le bundle. Mais toutes les importations resteront des importations statiques pour éviter le traitement asynchrone lors du chargement des dictionnaires.
  - _Remarque_ : Lorsqu'activé, Intlayer optimisera le chunking des dictionnaires en remplaçant tous les appels de `useIntlayer` par `useDictionary` et `getIntlayer` par `getDictionary`.
  - _Remarque_ : Assurez-vous que toutes les clés sont déclarées statiquement dans les appels `useIntlayer`. Par exemple : `useIntlayer('navbar')`.
...
- **activateDynamicImport** :
  ...
  - _Exemple_ : `true`
  - _Remarque_ : Cela permettra d'importer dynamiquement le contenu du dictionnaire uniquement pour la langue actuelle.
  - _Remarque_ : Les importations dynamiques reposent sur React Suspense et peuvent légèrement impacter les performances de rendu. Mais si désactivé, toutes les langues seront chargées en même temps, même si elles ne sont pas utilisées.
  - _Remarque_ : Lorsqu'activé, Intlayer optimisera le chunking des dictionnaires en remplaçant tous les appels de `useIntlayer` par `useDynamicDictionary`.
  - _Remarque_ : Cette option sera ignorée si `optimize` est désactivé.
  - _Remarque_ : Assurez-vous que toutes les clés sont déclarées statiquement dans les appels `useIntlayer`. Par exemple : `useIntlayer('navbar')`.

- **traversePattern** :
  - _Type_ : `string[]`
  - _Par défaut_ : `['**/*.{js,ts,mjs,cjs,jsx,tsx,mjx,cjx,vue,svelte,svte}', '!**/node_modules/**']`
  ...
  - _Remarque_ : Utilisez ceci pour limiter l'optimisation aux fichiers de code pertinents et améliorer les performances de build.
  - _Remarque_ : Cette option sera ignorée si `optimize` est désactivé.
  - _Remarque_ : Utilisez le motif glob.
