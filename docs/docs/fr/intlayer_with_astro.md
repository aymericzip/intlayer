...
<Question title="Quelles sont les différentes solutions pour internationaliser un site Astro ?">

Astro fournit une option `i18n` au niveau du routage qui gère les préfixes de locale et les redirections, mais elle ne gère pas le contenu lui-même : vous avez donc toujours besoin d'une couche de messages :

- **L'`i18n` intégrée d'Astro** plus des dictionnaires JSON ou TypeScript écrits à la main : aucune dépendance, mais aucun typage, aucune règle de pluriel et aucun outil.
- **`i18next`** ou **`vue-i18n` / `svelte-i18n`** à l'intérieur des îlots : une bibliothèque complète par framework d'îlot, chacune avec son propre catalogue.
- **`Intlayer`** : une seule couche de contenu partagée par les pages Astro et tous les frameworks d'îlot, compilée au moment du build, entièrement typée, avec traduction par IA, un éditeur visuel et un CMS.
