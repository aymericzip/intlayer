...
Les fonctions `head` des routes s'exécutent **en dehors** de l'arbre React : `useIntlayer` n'y est pas disponible. Vous disposez de trois façons de lire un dictionnaire depuis `head`, chacune arbitrant entre taille du bundle et rapidité de résolution du `head` du document.
