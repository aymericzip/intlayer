...
Par rapport aux solutions principales telles que `vue-i18n` ou `i18next`, Intlayer est une solution qui vient avec des optimisations intégrées telles que :

<AccordionGroup>
<Accordion header="Support complet de Vue">

Intlayer est optimisé pour fonctionner parfaitement avec Vue en offrant une **portée du contenu au niveau des composants**, des **traductions réactives** et toutes les fonctionnalités nécessaires pour mettre à l'échelle votre internationalisation (i18n).
...
<iframe title="The best i18n solution for Vite et Vue? Discover Intlayer" class="m-auto aspect-16/9 w-full overflow-hidden rounded-lg border-0" allow="autoplay; gyroscope;" loading="lazy" width="1080" height="auto" src="https://www.youtube.com/embed/IE3XWkZ6a5U?autoplay=0&amp;origin=https://intlayer.org&amp;controls=0&amp;rel=1"/>
...
<iframe
  ...
  className="m-auto overflow-hidden rounded-lg border-0 max-md:size-full max-md:h-[700px] md:aspect-16/9 md:w-full"
  title="Démo - intlayer-vite-vue-template"
  sandbox="allow-forms allow-modals allow-popups allow-presentation allow-same-origin allow-scripts"
  loading="lazy"
/>
...
Dans cette architecture, `vue-intlayer` fournit le plugin installé dans `main.ts`. Les composants Vue déclarent et consomment les traductions localement à l'aide de composables, le rechargement à chaud Vite étant géré par `vite-intlayer`.
...
> Vous pouvez aussi appeler `installIntlayer(app)` directement comme une fonction si vous préférez :
>
> ```javascript
> import { intlayer } from "vue-intlayer";
> app.use(intlayer);
...
<Step number={9} title="Modifier les attributs Lang et Dir du HTML" isOptional={true}>
...
<Question title="Quelles sont les différentes solutions pour internationaliser une application Vue ?">

- **`vue-i18n`** : la bibliothèque de référence, avec des catalogues de messages enregistrés globalement et un formatage de style ICU.
...
<Question title="Intlayer fonctionne-t-il avec l'API de composition de Vue et script setup ?">

Oui. `useIntlayer` est un composable utilisé à l'intérieur de `<script setup>` comme n'importe quel autre, et le contenu renvoyé est réactif, si bien que changer la locale réaffiche les composants qui le lisent sans rechargement de page.

</Question>
<Question title="Comment mettre en place un routage localisé dans une SPA Vue ?">
...
L'étape 9 le couvre. `getHTMLTextDir` renvoie `rtl` pour les langues telles que l'arabe, l'hébreu, le persan et l'ourdou, vous pouvez donc lier `lang` et `dir` sur l'élément racine à partir de la locale active et laisser vos propriétés logiques CSS faire le reste.

</Question>
<Question title="Comment traduire une application Vue automatiquement avec l'IA ?">
