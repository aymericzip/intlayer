...
Vous devez également ajouter des métadonnées SEO, telles que hreflang et des liens canoniques, sur chaque page et intégrer une île Vue pour le contenu interactif côté client.
...
<Question title="Quelles sont les différentes solutions pour internationaliser un site Astro avec des îlots Vue ?">

L'option `i18n` intégrée d'Astro gère les préfixes de locale et les redirections, mais pas le contenu lui-même : vous avez donc toujours besoin d'une couche de messages, et un îlot ajoute un second problème : l'îlot exécute Vue, pas Astro.

- **`i18n` d'Astro plus des dictionnaires écrits à la main**, avec **`vue-i18n`** à l'intérieur de l'îlot : deux sources de contenu à garder synchronisées, et aucun typage partagé entre elles.
- **`Intlayer`** : une seule couche de contenu pour les deux. `astro-intlayer` couvre les pages `.astro` et `vue-intlayer` couvre les îlots Vue, en lisant les mêmes déclarations.
...
<Question title="Ai-je besoin d'une bibliothèque d'i18n séparée à l'intérieur de mon îlot Vue ?">

Non. `vue-intlayer` lit les mêmes dictionnaires que le côté Astro, vous n'installez donc pas `vue-i18n` à côté. L'étape 6 montre le composant d'îlot recevant la locale active de la page au lieu de la résoudre à nouveau.
