...
Voir plus d'options dans la documentation CLI : [Référence CLI](https://github.com/aymericzip/intlayer/blob/main/docs/docs/fr/cli/index.md) → "Tester les traductions manquantes". Ainsi que le guide [Tests](https://github.com/aymericzip/intlayer/blob/main/docs/docs/fr/testing.md).
...
Vous pouvez toujours affiner par dictionnaire en utilisant le champ `autoFill` dans les fichiers de contenu. Intlayer considérera d'abord la configuration par dictionnaire, puis basculera vers la configuration globale. Consultez la référence complète dans [Auto-fill](https://github.com/aymericzip/intlayer/blob/main/docs/docs/fr/autoFill.md) et [Content file reference](https://github.com/aymericzip/intlayer/blob/main/docs/docs/fr/dictionary/content_file.md).
...
Mise à jour du serveur MCP pour supporter les nouvelles fonctionnalités.
...
## Processus parallèle pour surveiller les fichiers sur turbopack

Lors de l'utilisation de Turbopack sur Next.js 14 et versions ultérieures comme serveur de développement avec la commande `next dev --turbopack`, les modifications du dictionnaire ne seront pas détectées automatiquement par défaut.

Cette limitation se produit parce que Turbopack ne peut pas exécuter les plugins webpack en parallèle pour surveiller les modifications dans vos fichiers de contenu.

Pour contourner ce problème, nous avons utilisé la commande `intlayer watch` avec l'option `--with` pour exécuter simultanément le serveur de développement et l'observateur de build Intlayer.
...
## Notes de migration de v6 à v7

Consultez les [notes de migration de v6 à v7](https://github.com/aymericzip/intlayer/blob/main/docs/docs/fr/releases/v7.md) pour plus d'informations.

---

## Liens utiles

- [Référence CLI](https://github.com/aymericzip/intlayer/blob/main/docs/docs/fr/cli/index.md)
