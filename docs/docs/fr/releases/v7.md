...
## Formatage automatique du code : configuration de formatCommand

v7 introduit l'option `formatCommand` dans la configuration de l'éditeur, vous permettant de formater automatiquement les fichiers de contenu lorsqu'ils sont écrits par Intlayer. Cela garantit un style de code et un formatage cohérents dans vos fichiers de déclaration de contenu.

S'il n'est pas défini, Intlayer essaiera de détecter automatiquement la commande de formatage. En essayant de résoudre les commandes suivantes : prettier, biome, eslint.

### Configuration

L'option `formatCommand` accepte un modèle de chaîne où `{{file}}` sera remplacé par le chemin de fichier réel :
...
### Formatters supportés

Vous pouvez utiliser n'importe quel formateur de code qui accepte les chemins de fichiers comme arguments :

**Utilisation de Biome :**
...
**Utilisation du formateur intégré de Bun :**
...
- **Formatage cohérent** : Tous les fichiers de contenu sont automatiquement formatés selon les directives de style de votre projet
- **Expérience développeur** : Pas besoin de formater manuellement les fichiers après qu'Intlayer les ait écrits
- **Cohérence d'équipe** : Assure que tous les membres de l'équipe ont le même formatage appliqué aux fichiers de contenu
- **Intégration CI/CD** : Les fichiers de contenu maintiennent un formatage cohérent dans les workflows automatisés

### Fonctionnement

Lorsqu'Intlayer écrit ou met à jour un fichier de déclaration de contenu (`.content.ts`, `.content.js`, etc.), il exécute automatiquement la commande de format spécifiée sur le fichier. L'espace réservé `{{file}}` est remplacé par le chemin d'accès réel du fichier, et la commande est exécutée dans le répertoire de base du projet.

---

## Configuration du dictionnaire : Meilleure organisation et structure

v7 introduit une nouvelle section de configuration `dictionary` dédiée qui offre une meilleure organisation des paramètres liés au dictionnaire et une gestion de contenu améliorée.

### Nouvelle structure de configuration du dictionnaire

La propriété `fill` a été déplacée de la section `content` vers une nouvelle section `dictionary`, offrant une séparation des responsabilités plus claire :
...
- **Organisation plus claire** : Les paramètres spécifiques au dictionnaire sont maintenant regroupés
- **Meilleure séparation des préoccupations** : La découverte de contenu et les opérations de dictionnaire sont clairement séparées
- **Maintenabilité améliorée** : Plus facile à comprendre et à modifier les configurations liées au dictionnaire
- **Extensibilité future** : La section dictionnaire peut accueillir des paramètres supplémentaires spécifiques au dictionnaire
- **Gestion complète du dictionnaire** : Inclut des propriétés comme `title`, `live`, `priority`, `tags`, `version`, et `description` pour créer et gérer de nouveaux dictionnaires

### Utilisation de la configuration

La configuration du dictionnaire sert deux objectifs principaux :

1. **Valeurs par défaut** : Définir les valeurs par défaut lors de la création de fichiers de déclaration de contenu
2. **Comportement de secours** : Fournir des valeurs de secours lorsque des champs spécifiques ne sont pas définis, vous permettant de définir le comportement du dictionnaire globalement

La section dictionnaire inclut des propriétés complètes pour la gestion du dictionnaire :

- **`fill`** : Comportement de remplissage automatique pour la génération de contenu
- **`title`** : Titre par défaut pour les nouveaux dictionnaires
- **`live`** : Configuration de synchronisation en direct pour les mises à jour en temps réel
- **`priority`** : Paramètres de priorité pour la résolution du dictionnaire
- **`tags`** : Tags par défaut pour l'organisation du contenu
- **`version`** : Gestion des versions pour les mises à jour du dictionnaire
- **`description`** : Description par défaut pour le nouveau contenu
...
- **`editor.formatCommand`**: Nouvelle option pour le formatage automatique du code des fichiers de contenu
