...
## `@intlayer/analytics` : suivi de l'exposition du contenu et tests A/B

Un nouveau package optionnel, [`@intlayer/analytics`](https://github.com/aymericzip/intlayer/blob/main/docs/docs/en/analytics.md), vous indique quel contenu est réellement affiché à vos visiteurs — et boucle la boucle avec **Collections & Variants** en vous permettant de faire des tests A/B sur le contenu et de mesurer les conversions par variante.

- **Suivi au niveau du provider** : `IntlayerProvider` (React / Next.js / React Native) enregistre automatiquement des événements `page_view` au chargement, au changement de route et au changement de langue.
- **Suivi au niveau du nœud** : chaque résolution de `useIntlayer` remonte un événement `content_exposure` — quelle clé de dictionnaire, quel chemin de clé, quelle langue et, le cas échéant, quelle **variante** A/B a été affichée. Les expositions répétées d'un même nœud dans une fenêtre d'envoi sont regroupées.
- **Attribution déterministe des variantes A/B** : `getVariant(experimentKey, variants)` attribue une variante à chaque session **sans aller-retour serveur**, donc aucun scintillement avant le premier rendu.
- **`useConversion()`** : attribue un objectif (inscription, clic, achat…) à la variante à laquelle la session a été exposée.
- **Coût nul si non installé** : même schéma tree-shakeable que `@intlayer/editor` — `import()` dynamique enveloppé dans un `try/catch`, plus une variable d'environnement à la compilation (`INTLAYER_ANALYTICS_ENABLED`) qui élimine complètement l'intégration du bundle pour les applications qui ne l'activent pas.
- **Aucun nouveau schéma de configuration** : réutilise `editor.backendURL` (point d'envoi / auto-hébergement) et `editor.clientId` (clé de projet, qui sert aussi d'interrupteur d'activation).
- **Respect de la vie privée** : hachage anonyme de session (SHA-256, aucun identifiant brut stocké), aucun stockage d'IP, géolocalisation grossière limitée au pays, requêtes envoyées par lots environ toutes les 20 secondes.
- **Tableau de bord** : une nouvelle page **Analytics** (affichée dans la barre latérale une fois un projet sélectionné) indique les utilisateurs actifs, les utilisateurs du jour, ceux des 7/30/90 derniers jours, un graphique d'évolution des visiteurs, ainsi que la répartition par langue et par localisation.
...
Disponible dès aujourd'hui pour React, Next.js et React Native (via `react-intlayer`) ; les intégrations pour d'autres frameworks (Vue, Svelte, Angular, …) sont prévues, en suivant le même schéma de déploiement que `@intlayer/editor`. Consultez la [documentation Analytics](https://github.com/aymericzip/intlayer/blob/main/docs/docs/en/analytics.md) pour le guide complet.
...
- [Analytics - Suivi de l'exposition du contenu et tests A/B](https://github.com/aymericzip/intlayer/blob/main/docs/docs/en/analytics.md)
