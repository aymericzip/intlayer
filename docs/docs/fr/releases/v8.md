...
- **Propriété `live`** : La propriété `live` dans les dictionnaires est dépréciée. Utilisez `importMode: 'fetch'` à la place.
- **API de Contenu Riche Unifiée** : La nouvelle méthode `.use()` est désormais utilisée pour les surcharges Markdown et HTML, offrant une API cohérente à travers tous les types de contenu riche.
