...
## Fichiers de Contenu YAML & Markdown (v8.10.0)

Vous pouvez désormais déclarer du contenu directement en utilisant les extensions de fichier `.content.md` et `.content.yaml`. C'est parfait pour les contenus longs, les blogs, la documentation et les politiques de confidentialité.

Ces fichiers sont entièrement modifiables depuis l'Éditeur Visuel, offrant un flux de travail simplifié pour les membres non techniques de l'équipe.

Pour plus de détails, consultez la [Documentation des Fichiers de Contenu YAML](https://github.com/aymericzip/intlayer/blob/main/docs/docs/fr/dictionary/content_file.md#yaml-content-file) et la [Documentation des Fichiers de Contenu Markdown](https://github.com/aymericzip/intlayer/blob/main/docs/docs/fr/dictionary/markdown.md).
...
## Règles de Pluriels Unicode CLDR (v8.8.0)

Un nouvel assistant de nœud `plural` a été introduit, basé sur les règles de pluriel définies par le projet Unicode CLDR, améliorant le support des langues complexes.
...
Vous pouvez ensuite passer le nombre dynamiquement dans votre composant de framework :
...
Pour plus d'informations, consultez la [Documentation sur la Pluralisation](https://github.com/aymericzip/intlayer/blob/main/docs/docs/fr/dictionary/plurial.md).
...
## TypeScript : Accesseurs Primitifs IntlayerNode

Le type `IntlayerNode` a été mis à jour pour permettre l'appel de méthodes JavaScript primitives directement sur les nœuds de contenu. Cela permet d'interagir avec votre contenu localisé directement comme s'il s'agissait d'une primitive standard de type chaîne, nombre, booléen ou tableau.

Pris en charge sur React, Preact, Solid, Svelte, Vue et Angular.

### Exemple de Chaîne de Caractères

```typescript
content.placeholder; // Retourne IntlayerNode<string>
content.placeholder.value; // Retourne string
content.placeholder.toString(); // Retourne string
content.placeholder.toLowerCase(); // Retourne string
String(content.placeholder); // Retourne string
content.placeholder.toUpperCase(); // Retourne la chaîne en majuscules
content.placeholder.replace("a", "b"); // Retourne la chaîne modifiée
```

### Exemple de Tableau

```typescript
content.myArrayOfString             // Retourne IntlayerNode<Array<string>>
content.myArrayOfString.find(...)   // Retourne l'élément
content.myArrayOfString.join(', ')  // Retourne la chaîne jointe
```
...
## Outils : Watcher C++ & LSP basé sur OXC (v8.12.0)

Intlayer v8.12.0 apporte des améliorations majeures à l'expérience développeur :

- **Parcel Watcher** : Le watcher de contenu a été migré de `chokidar` vers `@parcel/watcher`, exploitant la surveillance de fichiers native en C++ pour fournir des mises à jour plus rapides et une consommation de mémoire considérablement réduite.
- **Nouveau Language Server Protocol (LSP)** : Un tout nouveau LSP est désormais disponible. Conçu avec une analyse basée sur OXC, il aide votre IDE et vos agents IA à lier de manière transparente les appels `useIntlayer('my-key')` à leurs fichiers `.content` correspondants, et vice versa.

Pour plus de détails, consultez la [Documentation LSP](https://github.com/aymericzip/intlayer/blob/main/docs/docs/fr/lsp.md).
...
## Optimisations de Build Expérimentales : Minify & Purge (v8.7.0)

Intlayer v8.7.0 introduit de nouvelles fonctionnalités de build expérimentales dans votre `intlayer.config.ts` conçues pour optimiser votre bundle de production :

- **Minification des Dictionnaires** : Minifie les fichiers de dictionnaire pour réduire leur taille.
- **Purge des Clés Inutilisées** : Analyse et supprime les clés de traduction inutilisées des dictionnaires, en renommant les clés actives avec des caractères courts (par exemple, `"products" -> "a"`, `"pricing" -> "b"`). Cela réduit la taille du bundle jusqu'à 5 %.
- **Désactivation des Vérifications TypeScript** : Accélère les builds en désactivant la vérification des types TypeScript pendant l'étape de compilation.
...
## Support Angular : Bundler Vite (v8.11.0)

Pour les utilisateurs d'Angular, Intlayer prend désormais en charge nativement le bundler Vite via un nouveau plugin esbuild personnalisé, accélérant les temps de développement et de build.

Pour plus de détails sur la configuration, consultez la [Documentation de l'Environnement Angular](https://github.com/aymericzip/intlayer/blob/main/docs/docs/fr/intlayer_with_angular_21.md).

## Génération de Sitemap (v8.6.0)

Intlayer génère désormais automatiquement des sitemaps en fonction de vos locales et de votre mode de routage. C'est particulièrement utile pour l'optimisation SEO dans les architectures de routeurs comme TanStack Router :
