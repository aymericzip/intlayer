...
Dans cette architecture, `preact-intlayer` fournit le `IntlayerProvider` monté dans `main.tsx` pour envelopper l'arborescence Preact. Les déclarations de contenu résident dans `src/` aux côtés des composants.
