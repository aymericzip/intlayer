...
history:
  ...
  - version: 8.7.7
    date: 2026-04-24
    changes: "Documentation initiale pour Astro + Svelte"
author: aymericzip
---
...
- **svelte-intlayer**
  Le package qui intègre Intlayer avec les applications Svelte. Il fournit `setupIntlayer`, ainsi que les stores `useIntlayer` et `useLocale` pour l’internationalisation dans Svelte.

- **@astrojs/svelte**
  L’intégration Astro officielle qui permet d’utiliser des îles (islands) de composants Svelte.
...
<html lang={locale} dir={getHTMLTextDir(locale)}>
  ...
  <body>
    <!-- L’île Svelte rend tout le contenu interactif, y compris le sélecteur de langue -->
...
<Step number={6} title="Créer le composant Île Svelte">
...
<div>
  <h1>{$content.title}</h1>
  <!-- Le sélecteur de langue est rendu en ligne dans l'île -->
...
<script lang="ts">
  ...
  // Réutilisation de la même configuration locale/setupIntlayer que dans l'Étape 6 ci-dessus...
...
<Question title="Quelles sont les différentes solutions pour internationaliser un site Astro avec des îlots Svelte ?">

L'option `i18n` intégrée d'Astro gère les préfixes de locale et les redirections, mais pas le contenu lui-même : vous avez donc toujours besoin d'une couche de messages, et un îlot ajoute un second problème : l'îlot exécute Svelte, pas Astro.

- **`i18n` d'Astro plus des dictionnaires écrits à la main**, avec **`svelte-i18n`** à l'intérieur de l'îlot : deux sources de contenu à garder synchronisées, et aucun typage partagé entre elles.
- **`Intlayer`** : une seule couche de contenu pour les deux. `astro-intlayer` couvre les pages `.astro` et `svelte-intlayer` couvre les îlots Svelte, en lisant les mêmes déclarations.
...
<Question title="Puis-je migrer depuis `svelte-i18n` sans réécrire mes composants ?">
...
<Question title="Ai-je besoin d'une bibliothèque d'i18n séparée à l'intérieur de mon îlot Svelte ?">

Non. `svelte-intlayer` lit les mêmes dictionnaires que le côté Astro, vous n'installez donc pas `svelte-i18n` à côté. L'étape 6 montre le composant d'îlot recevant la locale active de la page au lieu de la résoudre à nouveau.
