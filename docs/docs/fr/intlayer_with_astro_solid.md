...
priority: 9
title: "Astro + Solid i18n - Guide complet pour traduire votre application"
description: "Oubliez i18next. Le guide 2026 pour créer une application Astro + Solid multilingue (i18n). Traduisez avec des agents IA et optimisez la taille du bundle, le SEO et les performances."
keywords:
...
## Guide étape par étape pour configurer Intlayer dans Astro + Solid
...
- **solid-js**
  Le package Solid de base.

- **solid-intlayer**
  Le package qui intègre Intlayer avec les applications Solid. Il fournit l’`IntlayerProvider`, ainsi que les primitives `useIntlayer` et `useLocale` pour l’internationalisation dans Solid.

- **@astrojs/solid-js**
  L’intégration Astro officielle qui permet d’utiliser des îles (islands) de composants Solid via `client:only="solid-js"`.
...
Dans cette architecture, l'intégration `intlayer()` enregistrée dans `astro.config.ts` compile vos dictionnaires et ajoute un middleware qui résout la locale de chaque requête et l'expose sur `Astro.locals.intlayer`. Les pages résident sous un segment rest `src/pages/[...locale]/`, ainsi la locale par défaut est servie sans préfixe et chaque autre locale dispose de sa propre URL dédiée. Les fichiers `.astro` lisent le contenu avec les hooks `useIntlayer` / `useLocale` d'`astro-intlayer` pour le rendu serveur, et l'îlot Solid (`src/components/solid/SolidIsland.tsx`) reçoit la locale détectée par le serveur en prop et la transmet à `solid-intlayer` pour la partie interactive. Les déclarations de contenu sont placées aux côtés de vos composants dans `src/`.
...
Ajoutez le plugin intlayer et l’intégration Solid à votre configuration Astro.
...
> L'intégration `solid()` permet d'utiliser des îles de composants Solid via `client:only="solid-js"`.
...
Vous devez également ajouter des métadonnées SEO, telles que hreflang et des liens canoniques, sur chaque page et intégrer une île Solid pour le contenu interactif côté client.
...
<html lang={locale} dir={getHTMLTextDir(locale)}>
  ...
  <body>
    <!-- L’île Solid rend tout le contenu interactif, y compris le sélecteur de langue -->
...
<Step number={6} title="Créer le composant Île Solid">

Créez un composant île qui enveloppe votre application Solid et reçoit la locale détectée par le serveur :
...
> La prop `locale` est transmise de la page Astro (détection serveur) à l’`IntlayerProvider`, ce qui en fait la locale initiale pour toutes les primitives Solid de l’arbre.

> Dans Solid, `useIntlayer` retourne une fonction **accessor** (ex : `content.). Vous devez appeler cette fonction pour accéder au contenu réactif.

</Step>
<Step number={7} title="Ajouter un sélecteur de langue">

Créez un composant Solid `LocaleSwitcher` qui lit les locales disponibles et navigue vers l’URL localisée lorsqu’un utilisateur sélectionne une nouvelle langue :
...
> **Note sur la réactivité de Solid :**
> Dans Solid, `locale` est un signal accessor réactif - appelez-le toujours comme `locale()` pour obtenir sa valeur actuelle.
...
Pour plus d'informations sur l'utilisation de l'extension, consultez la [documentation de l'extension VS Code Intlayer](https://github.com/aymericzip/intlayer/blob/main/docs/docs/fr/vs_code_extension.md).
...
<Question title="Quelles sont les différentes solutions pour internationaliser un site Astro avec des îlots Solid ?">

L'option `i18n` intégrée d'Astro gère les préfixes de locale et les redirections, mais pas le contenu lui-même : vous avez donc toujours besoin d'une couche de messages, et un îlot ajoute un second problème : l'îlot exécute Solid, pas Astro.

- **`i18n` d'Astro plus des dictionnaires écrits à la main**, avec **`@solid-primitives/i18n`** à l'intérieur de l'îlot : deux sources de contenu à garder synchronisées, et aucun typage partagé entre elles.
- **`Intlayer`** : une seule couche de contenu pour les deux. `astro-intlayer` couvre les pages `.astro` et `solid-intlayer` couvre les îlots Solid, en lisant les mêmes déclarations.
...
<Question title="Ai-je besoin d'une bibliothèque d'i18n séparée à l'intérieur de mon îlot Solid ?">

Non. `solid-intlayer` lit les mêmes dictionnaires que le côté Astro, vous n'installez donc pas `@solid-primitives/i18n` à côté. L'étape 6 montre le composant d'îlot recevant la locale active de la page au lieu de la résoudre à nouveau.
