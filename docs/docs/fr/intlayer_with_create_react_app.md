...
Par rapport aux principales solutions comme `react-i18next` ou `i18next`, Intlayer est une solution qui intègre des optimisations telles que :

<AccordionGroup>
<Accordion header="Couverture complète de React">
...
Au lieu de charger des fichiers JSON massifs dans vos pages, chargez uniquement le contenu nécessaire. Intlayer vous aide à **réduire la taille de votre bundle et de vos pages jusqu'à 50%**.
...
La connexion de fichiers JSON massifs aux composants peut entraîner des problèmes de performance et de réactivité. Intlayer optimise le chargement de votre contenu au moment de la construction.
...
- **intlayer**
  ...
  Le package principal qui fournit des outils d'internationalisation pour la gestion de configuration, la traduction, [la déclaration de contenu](https://github.com/aymericzip/intlayer/blob/main/docs/docs/fr/dictionary/content_file.md), la transpilation et les [commandes CLI](https://github.com/aymericzip/intlayer/blob/main/docs/docs/fr/cli/index.md).
...
<Question title="Quelles sont les différentes solutions pour internationaliser un projet Create React App ?">
...
- **`Intlayer`** : contenu déclaré à côté de chaque composant et compilé au moment du build via `react-scripts-intlayer`, entièrement typé, avec traduction par IA, un éditeur visuel et un CMS.
...
<Question title="Create React App n'est plus maintenu. Dois-je d'abord passer à Vite ?">
