...
You should also add SEO metadata like hreflang and canonical links to each page, and embed the Preact island for interactive client-side content.
...
<Step number={6} title="Create the Preact Island component">
...
<Question title="What are the different solutions available to internationalize an Astro site with Preact islands?">

Astro's built-in `i18n` option handles locale prefixes and redirects but not the content itself, so you still need a message layer, and an island adds a second problem: the island runs Preact, not Astro.

- **Astro `i18n` plus hand written dictionaries**, with **`preact-i18n`** inside the island: two content sources to keep in sync, and no shared typing between them.
- **`Intlayer`**: one content layer for both. `astro-intlayer` covers the `.astro` pages and `preact-intlayer` covers the Preact islands, reading the same declarations.
...
<Question title="Do I need a separate i18n library inside my Preact island?">

No. `preact-intlayer` reads the same dictionaries as the Astro side, so you do not install `preact-i18n` alongside it. Step 6 shows the island component receiving the active locale from the page instead of resolving it again.
