...
- `editor.clientId` — the public project key attributed to every ingested event. It also acts as an **enable switch**: analytics stays fully disabled (and tree-shaken, see below) until `clientId` is configured.
