...
description: "No more i18next. The 2026 guide to building a multilingual (i18n) Express app. Translate with AI agents and optimize bundle size, SEO and performances."
keywords:
...
<Question title="What are the different solutions available to internationalize a Express backend?">

The historical option is `i18next` with `i18next-http-middleware`, which loads JSON catalogs per namespace and stores the locale on the request. The alternative is `Intlayer` through `express-intlayer`, which declares content in typed files shared with your frontend, resolves the locale per request, and adds AI translation and a CMS.
...
<Question title="How much does i18n add to my Express server bundle size?">
...
<Question title="Does it work with an existing Express app and other middleware?">

Yes. `express-intlayer` is a standard Express middleware, so it composes with your existing stack. Register it before the routes that read content, so the locale is resolved by the time a handler calls `t()` or `getIntlayer()`.
...
Yes, and that is the usual setup. `express-intlayer` works alongside `react-intlayer`, `next-intlayer` and `vite-intlayer` on the same declared content, so a label used both in an API response and in a page is declared once. See [how Intlayer works](https://github.com/aymericzip/intlayer/blob/main/docs/docs/en/how_works_intlayer.md).
