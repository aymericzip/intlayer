...
In this architecture, `vanilla-intlayer` or core `intlayer` provides JavaScript APIs to manage translations and update DOM content dynamically. Content declarations live in `src/`.
...
- **`i18next`**: mature and framework agnostic, but it adds a runtime and loads catalogs as JSON.
...
No. Run `npx intlayer extract` and Intlayer reads your components, pulls the user facing strings out and writes a `.content` file next to each one, so you review a diff instead of copying strings into a catalog one at a time. Step 13 of this guide walks through it.
...
Yes. Step 12 covers lazy loading dictionaries per locale, so the initial payload carries one language and the others are fetched only if the visitor switches. See [dynamic dictionaries](https://github.com/aymericzip/intlayer/blob/main/docs/docs/en/dynamic_dictionaries/index.md).
