...
description: "No more i18next. The 2026 guide to building a multilingual (i18n) AdonisJS app. Translate with AI agents and optimize bundle size, SEO and performances."
keywords:
...
> Your content declarations can be defined anywhere in your application as soon as they are included into the `contentDir` directory (by default, `./src` or `./app`). And match the content declaration file extension (by default, `.content.{json,ts,tsx,js,jsx,mjs,cjs,md,mdx,yaml,yml}`).
...
<Question title="What are the different solutions available to internationalize a AdonisJS backend?">

AdonisJS ships `@adonisjs/i18n`, which covers ICU messages in `resources/lang` files with a request scoped service. The alternative is `Intlayer` through `adonis-intlayer`, which declares content in typed files shared with your frontend, resolves the locale per request, and adds AI translation, missing translation checks and a CMS.
...
<Question title="How much does i18n add to my AdonisJS server bundle size?">
...
<Question title="How do I use translations inside a controller or a service?">

Call `getIntlayer` with your dictionary key, or wrap a message in `t()`. The active locale comes from the request context the middleware installed, so there is no service to inject and no locale argument to pass down.

</Question>

<Question title="Does it work with Edge templates?">

Yes. Resolve the content in the controller and pass it to the view, so the template renders already translated values rather than resolving keys itself.
...
Yes, and that is the usual setup. `adonis-intlayer` works alongside `react-intlayer`, `next-intlayer` and `vite-intlayer` on the same declared content, so a label used both in an API response and in a page is declared once. See [how Intlayer works](https://github.com/aymericzip/intlayer/blob/main/docs/docs/en/how_works_intlayer.md).
