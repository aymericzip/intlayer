...
- **Content and Code Directory Separation**: Split of `contentDir` into `contentDir` (for content definitions) and `codeDir` (for code transformation) to improve build performance and clarity.
...
export default {
  content: {
    contentDir: ["src"], // Used for both content and code
  },
};
```
...
export default {
  content: {
    contentDir: ["src/content"], // Only watch for src/content/*.content.* files here
    codeDir: ["src"], // Only scan for code transformation here
  },
};
```
