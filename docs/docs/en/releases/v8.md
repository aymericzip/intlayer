...
HTML content can be rendered directly as a React node, or customized using the `.use()` method or a provider.
...
## Markdown & HTML: `.use()` and MDX Support
...
const formattedPrice = new intl.NumberFormat({
  style: "currency",
  currency: "USD",
  // locale: "en-US", (optional, will be automatically injected if not provided)
}).format(123.45);
```
...
return (
  <div>
    {myInsertion({
      name: <span className="highlight">John</span>,
    })}
  </div>
);
```

You can also mix different types of values:
...
### Framework: Preact Support for Enhanced Insertion Values

In addition to React, Intlayer v8 now fully supports Preact VNodes in insertion values. This ensures a consistent experience across both React and Preact ecosystems, allowing for seamless integration of rich components in your translations.
