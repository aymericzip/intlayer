...
<Step number={5} title="Create Route Files with File-System Conventions">
...
<Step number={8} title="Create a Locale Switcher Component">
...
<Step number={9} title="Add HTML Attributes Management">
...
Only how the routes are declared. This guide uses the file system route conventions, where the locale segment comes from the file names, while the [config based guide](https://github.com/aymericzip/intlayer/blob/main/docs/docs/en/intlayer_with_react_router_v7.md) declares the same tree in `routes.ts`. The content declarations, the hooks and the configuration are identical in both, so you can switch conventions without touching your i18n.
