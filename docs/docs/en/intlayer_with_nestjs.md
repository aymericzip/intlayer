...
NestJS has `nestjs-i18n`, which is the common choice and covers JSON or YAML catalogs with a request scoped service. The alternative is `Intlayer` through `express-intlayer`, which uses the same declared content as your frontend, is typed against your dictionaries, and comes with AI translation and a CMS.
