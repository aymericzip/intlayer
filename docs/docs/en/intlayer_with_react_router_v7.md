...
In this architecture, routes are defined programmatically in `app/routes.ts` with optional `/:locale?` dynamic segments. Localized pages, layouts, and content declarations are organized within the `app/` directory:
...
In this architecture, routes follow React Router v7 file-system conventions using `flatRoutes`, where dots (`.`) represent URL segments and `($locale)` denotes the optional localized segment:
...
export default [
  route("/:locale?", "routes/page.tsx"), // Localized home page
  route("/:locale?/about", "routes/about/page.tsx"), // Localized about page
...
- **`Intlayer`**: the most advanced solution. Content declared anywhere in your codebase ([next to each component or centralized](https://github.com/aymericzip/intlayer/blob/main/docs/blog/en/per-component_vs_centralized_i18n.md)), compiled at build time, typed end to end, with locale aware routing helpers, AI translation, a visual editor and a CMS.
