...
## Step-by-Step Guide to Set Up Intlayer in Astro + Solid
...
- **solid-js**
  The core Solid package.

- **solid-intlayer**
  The package that integrates Intlayer with Solid applications. It provides `IntlayerProvider`, `useIntlayer`, and `useLocale` primitives for Solid internationalization.

- **@astrojs/solid-js**
  The official Astro integration that enables Solid component islands.
...
Add the intlayer plugin and the Solid integration into your configuration.
...
> The `solid()` integration enables Solid component islands via `client:only="solid-js"`.
...
You should also add SEO metadata like hreflang and canonical links to each page, and embed the Solid island for interactive client-side content.
...
<html lang={locale} dir={getHTMLTextDir(locale)}>
  ...
  <body>
    <!-- The Solid island renders all interactive content, including the locale switcher -->
...
<Step number={6} title="Create the Solid Island component">

Create the island component that wraps your Solid app and receives the server-detected locale:
...
Create a `LocaleSwitcher` Solid component that reads the available locales and navigates to the localized URL when the user picks a new language:
