...
title: "Astro + Svelte i18n - Complete guide to translate your app"
...
Consume your dictionaries in `.astro` files with the hooks exported by `astro-intlayer`. They share the signatures of the framework bindings: `useIntlayer("key")` returns the content of a dictionary and `useLocale()` the current locale, with no argument to pass around.
...
You should also add SEO metadata like hreflang and canonical links to each page, and embed the Svelte island for interactive client-side content.
...
<Step number={6} title="Create the Svelte Island component">
...
<Question title="What are the different solutions available to internationalize an Astro site with Svelte islands?">
...
- **Astro `i18n` plus hand written dictionaries**, with **`svelte-i18n`** inside the island: two content sources to keep in sync, and no shared typing between them.
- **`Intlayer`**: one content layer for both. `astro-intlayer` covers the `.astro` pages and `svelte-intlayer` covers the Svelte islands, reading the same declarations.

Declaring a label once and using it in both a static page and an interactive island is the reason to pick a single content layer here. See [why Intlayer](https://github.com/aymericzip/intlayer/blob/main/docs/docs/en/interest_of_intlayer.md).
...
<Question title="Can I migrate from `svelte-i18n` without rewriting my components?">
...
<Question title="Do I need a separate i18n library inside my Svelte island?">

No. `svelte-intlayer` reads the same dictionaries as the Astro side, so you do not install `svelte-i18n` alongside it. Step 6 shows the island component receiving the active locale from the page instead of resolving it again.
