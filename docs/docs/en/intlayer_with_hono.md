...
description: "No more i18next. The 2026 guide to building a multilingual (i18n) Hono app. Translate with AI agents and optimize bundle size, SEO and performances."
keywords:
...
Hono targets all of them, and Intlayer resolves content from dictionaries compiled at build time rather than reading catalog files from disk at runtime, which is what usually breaks on edge runtimes. Keep `dictionary.importMode` at its default `"static"` so the content is bundled with the worker.
...
Yes, and that is the usual setup. `hono-intlayer` works alongside `react-intlayer`, `next-intlayer` and `vite-intlayer` on the same declared content, so a label used both in an API response and in a page is declared once. See [how Intlayer works](https://github.com/aymericzip/intlayer/blob/main/docs/docs/en/how_works_intlayer.md).
