...
<Step number={5} title="Utilize Intlayer in Your Components">
...
> `getLocaleFromStorage` will check the locale from header or cookie depending on your configuration. See [Configuration](https://github.com/aymericzip/intlayer/blob/main/docs/docs/en/configuration.md) for more details.
...
<Step number={8} title="Internationalized Links" isOptional={true}>
...
- **`svelte-i18n`** and **`typesafe-i18n`**: store based message catalogs, wired into the load functions by hand.
- **`Paraglide`**: compiled messages with a strong typing story, focused on the message layer only.
- **`Intlayer`**: the most advanced solution. Content declared anywhere in your codebase ([next to each component or centralized](https://github.com/aymericzip/intlayer/blob/main/docs/blog/en/per-component_vs_centralized_i18n.md)) and compiled at build time, with locale aware routing, server side locale detection, AI translation, a visual editor and a CMS.

On SvelteKit the difference shows in the server side pieces: locale detection in hooks, localized links and the editor integration come with the library instead of being assembled per project. See [why Intlayer](https://github.com/aymericzip/intlayer/blob/main/docs/docs/en/interest_of_intlayer.md) and the [Svelte i18n benchmark](https://github.com/aymericzip/intlayer/blob/main/docs/docs/en/benchmark/svelte.md).
