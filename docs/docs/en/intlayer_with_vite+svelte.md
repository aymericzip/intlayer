...
In this architecture, `svelte-intlayer` provides reactive stores and utilities to manage translations within Svelte components. Content declarations live in `src/` alongside components.
...
<Question title="What are the different solutions available to internationalize a Vite and Svelte app?">

Vite has no opinion on i18n, so the choice comes from the Svelte ecosystem:

- **`svelte-i18n`** and **`typesafe-i18n`**: store based message catalogs registered globally.
...
No. Run `npx intlayer extract` and Intlayer reads your components, pulls the user facing strings out and writes a `.content` file next to each one, so you review a diff instead of copying strings into a catalog one at a time. Step 10 of this guide walks through it.
