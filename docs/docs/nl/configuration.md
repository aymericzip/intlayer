...
const config: IntlayerConfig = {
  ...
  routing: {
    ...
    /**
     * Schakelt de Intlayer locale-routing proxy (middleware) in.
     * Verzorgt localedetectie, redirects en rewrites in dev, preview en SSR.
     * - niet ingesteld (auto): dev- en preview-servers houden de routing URL-gestuurd
     *   en negeren de locale die in cookies en headers is opgeslagen. Prefixen worden
     *   nog steeds opgelost, de locale wordt nog steeds bewaard en
     *   Accept-Language-detectie blijft gelden. In productie gedraagt het zich als `true`.
     * - true: volledig gedrag in elke omgeving.
     * - false: geen locale-routing.
     * Standaard: undefined (auto)
     */
  ...
  ai: {
     ...
     * Opties: 'openai', 'anthropic', 'mistral', 'deepseek', 'gemini', 'ollama', 'openrouter', 'alibaba', 'fireworks', 'groq', 'huggingface', 'bedrock', 'googlevertex', 'togetherai', 'lmstudio', 'moonshotai'
     * Standaard: 'openai'
     */
    provider: "openai",
  ...
  build: {
    ...
    /**
     * Groepeer de dictionary-chunks per taal op de code-splitting-grens die ze
     * gebruikt, zodat een lazy geladen pagina haar inhoud in één verzoek ophaalt.
     * Standaard: true
     *
     * Opmerking:
     * - Geldt alleen voor dictionaries die `importMode: 'dynamic'` gebruiken.
     */
    chunkGrouping: true,

    /**
     * Laad een dictionary samen met de chunk die hem gebruikt, in plaats van hem
     * op te halen zodra die chunk rendert. Lezers renderen synchroon in plaats van
     * te suspenden, zodat navigeren geen laadstatus meer laat flitsen.
     * Standaard: true
     *
     * Opmerking:
     * - Alleen de opgeloste taal wordt afgewacht, dus de pagina downloadt alleen
     *   de taal die ze toont.
     */
...
| `enableProxy` | Schakelt de Intlayer locale-routing proxy (middleware) in.                                                                                                                                                            | `boolean` &#124; <br/> `undefined`                                                                                                                                                                           | `undefined` (auto)     | `true`                                                                                                                                                                                               | • Niet ingesteld (auto): dev- en preview-servers negeren de in cookies/headers opgeslagen locale als redirectbron; prefixen, persistentie en `Accept-Language`-detectie blijven gelden. In productie gedraagt het zich als `true`.<br/>• `true`: volledig gedrag overal.<br/>• `false`: geen locale-routing. Bij Next.js wordt de `intlayerProxy`-middleware een doorgeefluik. |
...
| `contentDir`     | Pad naar de map waar contentdefinitiebestanden (`.content.*`) worden bewaard.                                      | `string[]` | `['.']`                                                                                                                                                                   | `['src', '../../ui-library', require.resolve("@my-package/content"), '@my-package/content']`                                                                                          | Wordt gebruikt voor het volgen van contentbestanden en regeneratie van woordenboeken.                                                                                               |
...
| Veld                 | Beschrijving                                                                                                                        | Type                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Standaard   | Voorbeeld                                                     | Opmerking                                                                                                                                                                                                   |
| -------------------- | ----------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- | ------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `provider`           | Provider gebruikt voor AI-functies van Intlayer.                                                                                    | `'openai'` &#124; <br/> `'anthropic'` &#124; <br/> `'mistral'` &#124; <br/> `'deepseek'` &#124; <br/> `'gemini'` &#124; <br/> `'ollama'` &#124; <br/> `'openrouter'` &#124; <br/> `'alibaba'` &#124; <br/> `'fireworks'` &#124; <br/> `'groq'` &#124; <br/> `'huggingface'` &#124; <br/> `'bedrock'` &#124; <br/> `'googleaistudio'` &#124; <br/> `'googlevertex'` &#124; <br/> `'togetherai'` &#124; <br/> `'lmstudio'` &#124; <br/> `'moonshotai'` | `undefined` | `'anthropic'`                                                 | Verschillende providers vereisen verschillende API-sleutels en hebben verschillende prijzen.                                                                                                                |
...
| `chunkGrouping`       | Geeft aan of de dictionary-chunks per taal gegroepeerd moeten worden op de code-splitting-grens die ze gebruikt.                                | `boolean`                        | `true`                                                                                                                                                                            |                                                                               | • Zonder groepering doet een pagina die uit veel componenten bestaat één verzoek per dictionary.<br/>• Dictionaries die vanuit meerdere grenzen bereikt worden, gaan naar een gedeelde chunk, zodat geen pagina de inhoud van een andere meelevert.<br/>• Geldt alleen voor dictionaries die `importMode: 'dynamic'` gebruiken.<br/>• Geldt alleen voor de client-build, en alleen bij het bundelen (niet in dev).                                                                                                                              |
| `dictionariesPreload` | Geeft aan of een dictionary samen met de chunk die hem gebruikt geladen moet worden, in plaats van opgehaald te worden zodra die chunk rendert. | `boolean`                        | `true`                                                                                                                                                                            |                                                                               | • Het gegenereerde entry point wacht de browse-taal op topniveau af, zodat een lazy geladen route pas als geladen geldt wanneer de inhoud er is.<br/>• Lezers renderen synchroon in plaats van te suspenden, zodat navigeren geen laadstatus meer laat flitsen.<br/>• Alleen de opgeloste taal wordt afgewacht, dus de pagina downloadt alleen de taal die ze toont.<br/>• Geldt alleen voor dictionaries die `importMode: 'dynamic'` gebruiken, in de client-build.<br/>• Vereist een bundler die top-level await ondersteunt (Vite, esbuild). |
