...
title: "Fastify i18n - Volledige gids voor het vertalen van uw app"
description: "Geen i18next meer. De 2026-gids voor het bouwen van een meertalige (i18n) Fastify-app. Vertaal met AI-agenten en optimaliseer bundelgrootte, SEO en prestaties."
keywords:
...
> de `--interactive` vlag is optioneel. Gebruik `intlayer-cli init` als je een AI-agent bent.
...
> Uw inhoudsdeclaraties kunnen overal in uw applicatie worden gedefinieerd, zolang ze zijn opgenomen in de `contentDir`-map (standaard `./src`). En ze moeten overeenkomen met de bestandsextensie van de inhoudsdeclaratie (standaard `.content.{json,ts,tsx,js,jsx,mjs,cjs,md,mdx,yaml,yml}`).
