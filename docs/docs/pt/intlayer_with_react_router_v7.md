...
title: React Router v7 i18n - Guia completo para traduzir React Router
description: A melhor solução para tamanho de bundle, SEO, desempenho & manutenibilidade. Torne seu React Router v7 aplicativo multilíngue em 2026, tradução LLM, Agent Skills & MCP.
keywords:
  - Internacionalização
  - Documentação
  - Intlayer
...
> Suas declarações de conteúdo podem ser definidas em qualquer lugar da sua aplicação, desde que estejam incluídas no diretório `contentDir` (por padrão, `./app`). E correspondam à extensão do arquivo de declaração de conteúdo (por padrão, `.content.{json,ts,tsx,js,jsx,mjs,cjs}`).
