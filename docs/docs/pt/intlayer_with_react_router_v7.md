...
<iframe title="How to translate an React Router v7 app using Intlayer" class="m-auto aspect-16/9 w-full overflow-hidden rounded-lg border-0" allow="autoplay; gyroscope;" loading="lazy" width="1080" height="auto" src="https://www.youtube.com/embed/dS9L7uJeak4?autoplay=0&amp;origin=https://intlayer.org&amp;controls=0&amp;rel=1"/>
  ...
  <Tab label="Código (baseado em configuração)" value="code-config">
...
/>
  ...
  <Tab label="Código (rotas baseadas no sistema de arquivos)" value="code-fs">
...
/>
  ...
  <Tab label="Demo (baseado em configuração)" value="demo">
...
Nesta arquitetura, as rotas são definidas programaticamente em `app/routes.ts` com segmentos dinâmicos opcionais `/:locale?`. Páginas localizadas, layouts e declarações de conteúdo são organizados dentro do diretório `app/`:
...
Crie um arquivo de configuração `intlayer.config.ts` para declarar os idiomas suportados pela sua aplicação:
...
Nesta arquitetura, as rotas seguem as convenções do sistema de arquivos do React Router v7 usando `flatRoutes`, onde pontos (`.`) representam segmentos de URL e `($locale)` indica o segmento localizado opcional:
...
> O plugin Vite `intlayer()` é usado para integrar Intlayer com Vite. Ele garante a construção de arquivos de declaração de conteúdo e os monitora em modo de desenvolvimento. Define variáveis de ambiente do Intlayer dentro da aplicação Vite. Além disso, fornece aliases para otimizar o desempenho.

</Step>
<Step number={4} title="Configurar Rotas do React Router v7">
...
export default [
  route("/:locale?", "routes/page.tsx"), // Página inicial localizada
  route("/:locale?/about", "routes/about/page.tsx"), // Página sobre localizada
