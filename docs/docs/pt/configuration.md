...
history:
  - version: 9.1.3
    date: 2026-08-06
    changes: "Tornar `routing.enableProxy` de três estados: não definido (auto), `true`, `false`"
  - version: 9.0.0
    date: 2026-07-11
    changes: "Adicionada a configuração `analytics`"
  - version: 9.0.0
    date: 2026-06-24
    changes: "Add `enableProxy` option to the routing configuration"
  - version: 8.10.0
    date: 2026-06-17
    changes: "Adicionada a opção `format` à configuração do dicionário"
  - version: 8.9.4
    date: 2026-05-12
...
const config: IntlayerConfig = {
  ...
  routing: {
    ...
    /**
     * Ativa o proxy de roteamento de locales do Intlayer (middleware).
     * Trata deteção de locale, redirecionamentos e rewrites em dev, preview e SSR.
     * - não definido (auto): os servidores de dev e preview mantêm o roteamento
     *   guiado pela URL, ignorando o locale armazenado em cookies e cabeçalhos.
     *   Os prefixos continuam a ser resolvidos, o locale continua a ser persistido
     *   e a deteção Accept-Language continua a aplicar-se. Em produção age como `true`.
     * - true: comportamento completo em todos os ambientes.
     * - false: sem roteamento de locales.
     * Padrão: undefined (auto)
     */
  ...
  analytics: {
    /**
     * Se a recolha de analytics está ativada (visualizações de página, exposições de conteúdo, eventos A/B).
     * Requer que `editor.clientId` esteja definido para a atribuição.
     * Padrão: false
     */
    enabled: true,

    /**
     * Milissegundos entre os envios automáticos em lote para o backend.
     * Padrão: 20000
     */
    flushInterval: 20000,

    /**
     * Fração de sessões a registar, de 0 (nenhuma) a 1 (todas).
     * Padrão: 1
     */
    sampleRate: 1,
  },
  ...
  ai: {
     ...
     * Opções: 'openai', 'anthropic', 'mistral', 'deepseek', 'gemini', 'ollama', 'openrouter', 'alibaba', 'fireworks', 'groq', 'huggingface', 'bedrock', 'googlevertex', 'togetherai', 'lmstudio', 'moonshotai'
     * Padrão: 'openai'
     */
    provider: "openai",
  ...
  dictionary: {
     ...
     * - "static": Importado estaticamente no momento da build.
     * - "dynamic": Importado dinamicamente usando Suspense.
     * - "fetch": Buscado dinamicamente via API de sincronização ao vivo.
     */
    importMode: "static",

    /**
     * O formato de mensagem padrão para todos os dicionários no projeto.
     * - 'intlayer': Formato nativo do intlayer (padrão).
     * - 'icu': Formato de mensagem ICU.
...
Os analytics são estritamente opt-in: nada é recolhido a menos que `analytics.enabled` esteja explicitamente definido como `true` **e** uma chave de projeto (`editor.clientId`) esteja configurada para a atribuição. Quando desativado (padrão), toda a integração de analytics é eliminada do bundle da sua aplicação (dead-code elimination).

| Campo           | Descrição                                                                                    | Tipo      | Padrão  | Exemplo | Nota                                                                                                                                                 |
| --------------- | -------------------------------------------------------------------------------------------- | --------- | ------- | ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| `enabled`       | Ativa a recolha de analytics (visualizações de página, exposições de conteúdo, eventos A/B). | `boolean` | `false` | `true`  | Requer que `editor.clientId` esteja definido para a atribuição; caso contrário, os analytics permanecem desativados mesmo que `enabled` seja `true`. |
| `flushInterval` | Milissegundos entre os envios automáticos em lote para o backend.                            | `number`  | `20000` | `10000` |                                                                                                                                                      |
| `sampleRate`    | Fração de sessões a registar, de `0` (nenhuma) a `1` (todas).                                | `number`  | `1`     | `0.5`   | A amostragem é determinística por sessão, pelo que uma sessão registada reporta todos os seus eventos (sem funis parciais).                          |
...
| `enableProxy` | Ativa o proxy de roteamento de locales do Intlayer (middleware).                                                                                                                                                        | `boolean` &#124; <br/> `undefined`                                                                                                                                                                           | `undefined` (auto)     | `true`                                                                                                                                                                                      | • Não definido (auto): os servidores de dev e preview ignoram o locale armazenado em cookies/cabeçalhos como fonte de redirecionamento; prefixos, persistência e deteção `Accept-Language` continuam a aplicar-se. Em produção age como `true`.<br/>• `true`: comportamento completo em todo o lado.<br/>• `false`: sem roteamento de locales. No Next.js, o middleware `intlayerProxy` passa a ser transparente. |
...
| `format`                    | O formato de mensagem padrão para todos os dicionários no projeto.                                                                                     | `'intlayer'` &#124; <br/> `'icu'` &#124; <br/> `'i18next'` &#124; <br/> `'vue-i18n'` &#124; <br/> `'po'`        | `'intlayer'` | `'icu'`                                                                                     | • `'intlayer'`: Formato nativo intlayer.<br/>• `'icu'`: Formato de mensagem ICU.<br/>• `'i18next'`: Formato i18next.<br/>• `'vue-i18n'`: Formato Vue I18n.<br/>• `'po'`: Formato GNU Gettext PO.                                                                                                                                                                                                        |
...
| Campo                | Descrição                                                                                                                             | Tipo                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Padrão      | Exemplo                                                       | Nota                                                                                                                                                                                                  |
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- | ------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `provider`           | O provedor a utilizar para as funcionalidades de IA do Intlayer.                                                                      | `'openai'` &#124; <br/> `'anthropic'` &#124; <br/> `'mistral'` &#124; <br/> `'deepseek'` &#124; <br/> `'gemini'` &#124; <br/> `'ollama'` &#124; <br/> `'openrouter'` &#124; <br/> `'alibaba'` &#124; <br/> `'fireworks'` &#124; <br/> `'groq'` &#124; <br/> `'huggingface'` &#124; <br/> `'bedrock'` &#124; <br/> `'googleaistudio'` &#124; <br/> `'googlevertex'` &#124; <br/> `'togetherai'` &#124; <br/> `'lmstudio'` &#124; <br/> `'moonshotai'` | `undefined` | `'anthropic'`                                                 | Diferentes provedores requerem diferentes chaves de API e têm preços diferentes.                                                                                                                      |
