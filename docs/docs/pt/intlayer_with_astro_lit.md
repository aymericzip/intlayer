...
priority: 9
title: "Astro + Lit i18n - Guia completo para traduzir seu aplicativo"
...
Os custom elements Lit são importados via um `<script>` no cliente e colocados no corpo da página.
...
// Conteúdo do dicionário 'lit-demo' para este locale
...
<Question title="Quais são as diferentes soluções disponíveis para internacionalizar um site Astro com ilhas Lit?">

A opção nativa de `i18n` do Astro cuida dos prefixos de URL e redirecionamentos, mas não gerencia as mensagens, e uma ilha traz uma complicação a mais por rodar Lit em vez de Astro.

- **`i18n` integrado do Astro com dicionários manuais**, junto com **`@lit/localize`** dentro da ilha: duas fontes de conteúdo para manter e sem tipagem compartilhada entre si.
- **`Intlayer`**: uma camada única de conteúdo para ambos. O `astro-intlayer` cuida das páginas `.astro` e o `lit-intlayer` das ilhas Lit, consumindo as mesmas declarações.

Declarar um texto uma única vez e utilizá-lo tanto em uma página estática quanto em uma ilha interativa é a grande razão para adotar uma camada unificada de conteúdo. Consulte [por que Intlayer](https://github.com/aymericzip/intlayer/blob/main/docs/docs/pt/interest_of_intlayer.md).
...
<Question title="Posso migrar do @lit/localize sem reescrever meus componentes?">
...
<Question title="Preciso de uma biblioteca de i18n separada dentro da minha ilha Lit?">

Não. O `lit-intlayer` lê os mesmos dicionários do lado Astro, portanto você não precisa instalar o `@lit/localize` em paralelo. O passo 6 mostra o componente da ilha recebendo o locale ativo diretamente da página sem precisar resolvê-lo novamente.
