...
- **preact**
  Pacote principal do Preact - uma alternativa rápida e leve ao React.

- **preact-intlayer**
  Pacote para integrar o Intlayer em aplicações Preact. Ele fornece o `IntlayerProvider`, além dos hooks `useIntlayer` e `useLocale` para internacionalização no Preact.

- **@astrojs/preact**
  Integração oficial do Astro que permite o uso de islands de componentes Preact.
...
Adicione o plugin `intlayer` e a integração do Preact à sua configuração do Astro.
...
> A integração `preact()` permite o uso de islands de componentes Preact via `client:only="preact"`.
...
> Se seus arquivos de conteúdo incluírem código TSX, você pode precisar importar `import { h } from "preact";` ou garantir que seu pragma JSX esteja configurado corretamente para o Preact.
...
Você também deve adicionar metadados de SEO (como hreflang e links canônicos) a cada página e introduzir uma island de Preact para conteúdo interativo no lado do cliente.
...
<html lang={locale} dir={getHTMLTextDir(locale)}>
  ...
  <body>
    <!-- A island de Preact renderiza todo o conteúdo interativo, incluindo o seletor de idioma -->
...
<Step number={6} title="Criar um componente Island de Preact">

Crie um componente de island que envolva sua aplicação Preact e receba a localidade detectada pelo servidor:
...
> A prop `locale` é passada da página Astro (detecção no servidor) para o `IntlayerProvider`, servindo como o idioma inicial para todos os hooks Preact na árvore.

> Nota: No Preact, a propriedade HTML `class` é usada em vez de `className`.

</Step>
<Step number={7} title="Adicionar um Seletor de Idioma">

Crie um componente Preact `LocaleSwitcher` que leia os idiomas disponíveis e navegue para a URL localizada quando um usuário selecionar um novo idioma:
...
Intlayer utiliza module augmentation para aproveitar os benefícios do TypeScript e tornar seu codebase mais robusto.
...
<Question title="Quais são as diferentes soluções disponíveis para internacionalizar um site Astro com ilhas Preact?">

Adicionar Preact ao Astro introduz opções adicionais de internacionalização:

- **`i18n` integrado do Astro com dicionários manuais**, junto com **`preact-i18n`** dentro da ilha: duas fontes de conteúdo para manter e sem tipagem compartilhada entre si.
- **`Intlayer`**: uma camada única de conteúdo para ambos. O `astro-intlayer` atende às páginas `.astro` e o `preact-intlayer` atende às ilhas Preact, lendo as mesmas declarações.

Declarar um texto uma única vez e utilizá-lo tanto em uma página estática quanto em uma ilha interativa é o grande diferencial de contar com uma camada unificada de conteúdo. Consulte [por que Intlayer](https://github.com/aymericzip/intlayer/blob/main/docs/docs/pt/interest_of_intlayer.md).
...
<Question title="Preciso de uma biblioteca de i18n separada dentro da minha ilha Preact?">

Não. O `preact-intlayer` lê os mesmos dicionários do lado Astro, portanto você não precisa instalar o `preact-i18n` em conjunto. O passo 6 mostra o componente da ilha recebendo o locale ativo diretamente da página em vez de tentar detectá-lo novamente.
