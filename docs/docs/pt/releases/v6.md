...
Veja mais opções na documentação do CLI: [Referência do CLI](https://github.com/aymericzip/intlayer/blob/main/docs/docs/pt/cli/index.md) → "Testar traduções ausentes". Assim como o guia de [Testes](https://github.com/aymericzip/intlayer/blob/main/docs/docs/pt/testing.md).
...
Você ainda pode ajustar por dicionário usando o campo `autoFill` em arquivos de conteúdo. O Intlayer primeiro considerará a configuração por dicionário e depois voltará para a configuração global. Veja a referência completa em [Auto-fill](https://github.com/aymericzip/intlayer/blob/main/docs/docs/pt/autoFill.md) e [Referência de arquivo de conteúdo](https://github.com/aymericzip/intlayer/blob/main/docs/docs/pt/dictionary/content_file.md).
...
Atualizado o servidor MCP para suportar os novos recursos.
...
> O servidor MCP ajuda na automatização de projetos usando Intlayer. Testando traduções ausentes para código autogenerado. Ajuda a entender sua configuração e incorpora a documentação do Intlayer no seu IDE.
...
## Processo paralelo para monitorar arquivos no turbopack

Ao usar Turbopack no Next.js 14 e superior como seu servidor de desenvolvimento com o comando `next dev --turbopack`, as mudanças no dicionário não serão detectadas automaticamente por padrão.

Esta limitação ocorre porque o Turbopack não consegue executar plugins do webpack em paralelo para monitorar mudanças em seus arquivos de conteúdo.

Para contornar isso, usamos o comando `intlayer watch`, usando a opção `--with` para executar simultaneamente tanto o servidor de desenvolvimento quanto o watcher de build do Intlayer.
...
## Notas de migração de v6 para v7

Verifique as [notas de migração de v6 para v7](https://github.com/aymericzip/intlayer/blob/main/docs/docs/pt/releases/v7.md) para mais informações.

---

## Links úteis

- [Referência CLI](https://github.com/aymericzip/intlayer/blob/main/docs/docs/pt/cli/index.md)
