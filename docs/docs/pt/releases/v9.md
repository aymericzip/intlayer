...
## Plugin Vite: Compilador e Proxy Embutidos

O plugin Vite `intlayer()` agora embutiu diretamente o **compilador** e o **proxy de roteamento de localidade**, de modo que a maioria dos projetos só precisa de um único plugin em `vite.config.ts`:
...
- **Proxy**: Ativa automaticamente com base na nova opção `routing.enableProxy` (`true` por padrão). Ele liga o middleware de detecção / redirecionamento / reescrita em desenvolvimento, visualização e SSR em produção. Você não precisa mais registrar `intlayerProxy()` separadamente.

### Opção `routing.enableProxy`

Uma nova opção `routing.enableProxy` controla se o proxy de roteamento de localidade está conectado. O padrão é `true`. Desative-o quando quiser lidar com o roteamento você mesmo:
...
Os plugins independentes `intlayerCompiler()` e `intlayerProxy()` continuam exportados para configurações avançadas. Registrá-los ao lado de `intlayer()` é seguro — cada plugin se desduplica e é executado apenas uma vez.
