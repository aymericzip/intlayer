...
- **Propriedade `live`**: A propriedade `live` em dicionários está obsoleta. Use `importMode: 'fetch'` em vez disso.
- **API de Conteúdo Rico Unificada**: O novo método `.use()` é agora utilizado para substituições tanto de Markdown quanto de HTML, proporcionando uma API consistente em todos os tipos de conteúdo rico.
