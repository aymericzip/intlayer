...
> Suas declarações de conteúdo podem ser definidas em qualquer lugar da sua aplicação, desde que estejam incluídas no diretório `contentDir` (por padrão, `./src` ou `./app`) e correspondam à extensão de arquivo de declaração de conteúdo (por padrão, `.content.{json,ts,tsx,js,jsx,mjs,cjs,md,mdx,yaml,yml}`).
...
<Question title="Quais são as diferentes soluções disponíveis para internacionalizar um backend AdonisJS?">

O AdonisJS disponibiliza o `@adonisjs/i18n`, que suporta mensagens no formato ICU em arquivos dentro de `resources/lang` com um serviço de escopo por requisição. A alternativa é o `Intlayer` através do `adonis-intlayer`, que declara o conteúdo em arquivos tipados compartilhados com o seu frontend, resolve o locale por requisição e adiciona tradução por IA, verificação de chaves faltantes e um CMS.
...
<Question title="Quanto a i18n adiciona ao tamanho do pacote (bundle) do meu servidor AdonisJS?">
...
<Question title="Como utilizo traduções dentro de um controller ou serviço?">

Chame `getIntlayer` com a chave do seu dicionário ou envolva a mensagem em `t()`. O locale ativo é obtido do contexto da requisição estabelecido pelo middleware, eliminando a necessidade de injetar serviços adicionais ou passar argumentos de locale manualmente.

</Question>

<Question title="Funciona com templates Edge?">

Sim. Resolva o conteúdo no controller e repasse-o à view, permitindo que o template renderize diretamente os valores traduzidos em vez de precisar resolver chaves por conta própria.
