...
title: "Astro + Vue i18n - Guia completo para traduzir seu aplicativo"
description: "Sem mais i18next. O guia 2026 para criar uma aplicação Astro + Vue multilíngue (i18n). Traduza com agentes de IA e otimize o tamanho do bundle, SEO e desempenho."
keywords:
  - internacionalização
  - documentação
  - Intlayer
...
history:
  - version: 9.5.5
    date: 2026-09-19
    changes: "Uso dos hooks useIntlayer / useLocale do astro-intlayer na página Astro"
  - version: 8.9.0
    date: 2026-05-04
...
# Traduza o seu site Astro + Vue com o Intlayer | Internacionalização (i18n)
...
## Guia passo a passo para configurar o Intlayer no Astro + Vue
...
- **vue**
  Pacote principal do Vue.

- **vue-intlayer**
  Pacote para integrar o Intlayer em aplicações Vue. Ele fornece o `installIntlayer`, além dos composables `useIntlayer` e `useLocale` para internacionalização no Vue.

- **@astrojs/vue**
  Integração oficial do Astro que permite o uso de islands de componentes Vue.
...
Adicione o plugin `intlayer` e a integração do Vue à sua configuração do Astro.
...
> A integração `vue()` permite o uso de islands de componentes Vue via `client:only="vue"`.
...
Você também deve adicionar metadados de SEO (como hreflang e links canônicos) a cada página e introduzir uma island de Vue para conteúdo interativo no lado do cliente.
...
<html lang={locale} dir={getHTMLTextDir(locale)}>
  ...
  <body>
    <!-- A island de Vue renderiza todo o conteúdo interativo, incluindo o seletor de idioma -->
...
<Step number={6} title="Criar um componente Island de Vue">

Crie um componente de island que envolva sua aplicação Vue e receba a localidade detectada pelo servidor. Você deve registrar o plugin Intlayer na sua instância Vue chamando `installIntlayer` antes de usar quaisquer composables.
...
<template>
  <div>
    <h1>{{ title }}</h1>
    <!-- O seletor de idioma é renderizado diretamente dentro do template da island -->
...
> A prop `locale` é passada da página Astro (detecção no servidor) e usada para inicializar o `installIntlayer`, definindo o idioma inicial para todos os composables na árvore.

</Step>
<Step number={7} title="Adicionar um Seletor de Idioma">

A funcionalidade de troca de idioma está integrada diretamente no template da island de Vue (veja o passo 6 acima). Ela usa o composable `useLocale` do `vue-intlayer` e navega para a URL localizada quando um usuário seleciona um novo idioma:
...
// Reutiliza a mesma lógica de prop/setup app do passo 6...
...
<Question title="Quais são as diferentes soluções disponíveis para internacionalizar um site Astro com ilhas Vue?">

A opção integrada de `i18n` do Astro gerencia prefixos de locale e redirecionamentos, mas não o conteúdo em si, o que exige uma camada de mensagens. Uma ilha introduz um ponto adicional: a ilha executa Vue, não Astro.

- **`i18n` integrado do Astro com dicionários manuais**, junto com **`vue-i18n`** dentro da ilha: duas fontes de conteúdo para sincronizar e sem tipagem compartilhada entre elas.
- **`Intlayer`**: uma camada única de conteúdo para ambos. O `astro-intlayer` atende às páginas `.astro` e o `vue-intlayer` atende às ilhas Vue, consumindo as mesmas declarações.

Declarar um rótulo uma única vez e utilizá-lo tanto em uma página estática quanto em uma ilha interativa é a grande vantagem de adotar uma camada unificada aqui. Consulte [por que Intlayer](https://github.com/aymericzip/intlayer/blob/main/docs/docs/pt/interest_of_intlayer.md).
...
Em grande parte, sim. Siga o [guia de migração do vue-i18n](https://github.com/aymericzip/intlayer/blob/main/docs/docs/pt/migration_from_vue-i18n_to_intlayer.md) para migrar o conteúdo. Você também pode migrar gradualmente: o [plugin sync JSON](https://github.com/aymericzip/intlayer/blob/main/docs/docs/pt/plugins/sync-json.md) mantém seus catálogos JSON existentes como fonte de verdade e gera dicionários Intlayer a partir deles, mantendo ambas as camadas sincronizadas enquanto você migra componentes um a um.
...
<Question title="Preciso de uma biblioteca de i18n separada dentro da minha ilha Vue?">

Não. O `vue-intlayer` lê os mesmos dicionários do lado Astro, portanto você não precisa instalar o `vue-i18n` em paralelo. O passo 6 mostra o componente da ilha recebendo o locale ativo diretamente da página em vez de tentar detectá-lo novamente.
