...
> Se sua aplicação já existe, você pode usar o [Intlayer Compiler](https://github.com/aymericzip/intlayer/blob/main/docs/docs/pt/compiler.md), assim como o [comando extract](https://github.com/aymericzip/intlayer/blob/main/docs/docs/pt/cli/extract.md), para transformar milhares de componentes em um segundo.
...
<Step number={10} title="Adicionar middleware">
...
- **`react-i18next` / `i18next`**: namespaces JSON carregados em tempo de execução, com um detector de locale separado conectado manualmente ao roteador.
- **`react-intl`** e **`Lingui`**: mensagens no formato ICU com uma etapa de extração.
- **`Intlayer`**: a solução mais avançada. O conteúdo pode ser declarado em qualquer lugar da sua base de código ([ao lado de cada componente ou centralizado](https://intlayer.org/blog/per-component-vs-centralized-i18n)) e compilado em tempo de build, tipado de ponta a ponta, com funções auxiliares de roteamento cientes de locale, tradução por IA, editor visual e CMS.
...
<Question title="Qual é a diferença entre este guia e o guia do React Router baseado em configuração?">

Apenas na forma como as rotas são declaradas. Este guia adota as convenções de roteamento baseadas no sistema de arquivos, onde o segmento de locale decorre do nome dos arquivos, enquanto o [guia baseado em configuração](https://github.com/aymericzip/intlayer/blob/main/docs/docs/pt/intlayer_with_react_router_v7.md) declara a mesma árvore em `routes.ts`. As declarações de conteúdo, os hooks e a configuração são rigorosamente idênticos em ambos, de modo que você pode trocar de convenção sem alterar sua configuração de i18n.

</Question>

<Question title="Como adiciono um segmento de locale com rotas no sistema de arquivos?">

Os passos 4 e 5 cobrem isso: o locale torna-se um segmento dinâmico nos nomes dos arquivos de rota, de forma que todas as páginas sob ele recebem o locale ativo. A função `validatePrefix` informa se o segmento corresponde a um locale declarado, permitindo retornar um 404 real para `/xx/about` em vez de renderizar páginas duplicadas, e `getLocalizedUrl` reescreve qualquer caminho para o idioma de destino.

</Question>

<Question title="Preciso colocar o locale na URL?">

Não. A configuração `routing.mode` aceita `"prefix-no-default"` (o padrão), `"prefix-all"`, `"no-prefix"` e `"search-params"`, e `routing.domains` mapeia cada idioma para seu domínio próprio. Consulte a [referência de configuração](https://github.com/aymericzip/intlayer/blob/main/docs/docs/pt/configuration.md).
...
O passo 8 mostra o componente. O hook `useLocale` expõe o idioma ativo, os declarados e uma função para salvar a escolha, enquanto `getLocalizedUrl` reescreve o caminho atual para o novo idioma, mantendo o leitor na mesma rota em vez de enviá-lo para a página inicial.

</Question>

<Question title="Como traduzo o app automaticamente com IA?">

Execute `npx intlayer fill`. Ele preenche traduções ausentes com o LLM de sua preferência, usando sua própria chave de API e provedor, e a opção `--git-diff` limita a execução ao conteúdo alterado na branch atual. Consulte o [comando fill](https://github.com/aymericzip/intlayer/blob/main/docs/docs/pt/cli/fill.md) e a [integração CI/CD](https://github.com/aymericzip/intlayer/blob/main/docs/docs/pt/CI_CD.md).
