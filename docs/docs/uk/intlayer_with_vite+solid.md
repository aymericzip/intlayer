...
У цій архітектурі `solid-intlayer` надає `IntlayerProvider`, змонтований у `index.tsx` для обгортання дерева Solid. Оголошення контенту розміщуються в `src/` поруч із компонентами.
...
<Question title="Які є різні рішення для інтернаціоналізації додатків Vite та Solid?">

Vite не має особливих вимог щодо i18n, тому варіанти відповідають екосистемі Solid:

- **`@solid-primitives/i18n`**: базові примітиви для Solid.
- **`Intlayer`**: повна інтеграція з реактивністю Solid, компіляція під час збирання, переклад AI, візуальний редактор та CMS.

Див. [чому Intlayer](https://github.com/aymericzip/intlayer/blob/main/docs/docs/uk/interest_of_intlayer.md).

</Question>
<Question title="Скільки i18n додає до розміру бандла Solid?">
...
<Question title="Як використовувати перекладений контент у компоненті Solid?">

Викликайте `useIntlayer` у вашому компоненті. Контент побудований на сигналах Solid, тому зміна локалі оновлює лише ті вузли DOM, які читають контент, без повторного рендерингу всього компонента.
