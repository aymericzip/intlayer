...
## Vite Plugin: Вбудований компілятор і проксі

Плагін Vite `intlayer()` тепер містить безпосередньо **компілятор** та **проксі маршрутизації локалей**, тому більшості проектів потрібен лише один плагін у `vite.config.ts`:
...
- **Компілятор**: Активується автоматично, коли встановлено `compiler.enabled` та шлях `compiler.output`. Вам більше не потрібно реєструвати `intlayerCompiler()` окремо.
- **Проксі**: Активується автоматично на основі нової опції `routing.enableProxy` (`true` за замовчуванням). Він підключає middleware виявлення / перенаправлення / перезапису локалі в development, preview та production SSR. Вам більше не потрібно реєструвати `intlayerProxy()` окремо.

### Опція `routing.enableProxy`

Нова опція `routing.enableProxy` керує тим, чи підключений проксі маршрутизації локалей. Значення за замовчуванням — `true`. Вимкніть її, якщо хочете самостійно керувати маршрутизацією локалей:
...
Окремі плагіни `intlayerCompiler()` та `intlayerProxy()` продовжують експортуватися для розширених конфігурацій. Їхня реєстрація поруч із `intlayer()` безпечна — кожен плагін автоматично дедублюється і запускається лише один раз.
