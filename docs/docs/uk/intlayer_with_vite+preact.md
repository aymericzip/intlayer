...
У цій архітектурі `preact-intlayer` надає `IntlayerProvider`, змонтований у `main.tsx` для обгортання дерева Preact. Оголошення контенту розміщуються в `src/` поруч із компонентами.
...
Застосовуючи ці зміни, ваша програма буде:
...
```json5 fileName="tsconfig.json"
{
  ...
  "compilerOptions": {
    // ...
    "jsx": "react-jsx",
    "jsxImportSource": "preact", // Рекомендується для Preact 10+
    // ...
  },
  "include": [
    // ... Ваші існуючі конфігурації TypeScript
...
> Переконайтеся, що ваш `tsconfig.json` налаштований для Preact, особливо `jsx` та `jsxImportSource` або `jsxFactory`/`jsxFragmentFactory` для старіших версій Preact, якщо ви не використовуєте за умовчанням `preset-vite`.

### Конфігурація Git

Рекомендується ігнорувати файли, згенеровані Intlayer. Це дозволить уникнути їх фіксації в Git репозиторію.
...
- **Вбудовані передпросмотри** перекладеного вмісту.
...
<Question title="Як використовувати перекладений контент у компоненті Preact?">

Викликайте `useIntlayer` у вашому компоненті так само, як у React. `preact-intlayer` є нативною зв'язкою для Preact. Крок 5 демонструє використання, а крок 11 охоплює Markdown та HTML.
