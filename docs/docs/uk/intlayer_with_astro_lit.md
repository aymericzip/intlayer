...
Lit custom elements імпортуються за допомогою блоку `<script>` на стороні клієнта і розміщуються в body.
...
// Вміст словника 'lit-demo' для цієї локалі
...
<Step number={6} title="Створення Lit Custom Element">
...
<Question title="Які є різні рішення для інтернаціоналізації сайту Astro з островами Lit?">

Вбудована функція `i18n` в Astro керує префіксами та перенаправленнями, але не контентом. Додавання острова створює додаткове питання: острів запускає не Astro, а Lit.

- **Astro `i18n` плюс власні словники**, а в острові **`@lit/localize`**: два окремих джерела даних без спільної типізації.
- **`Intlayer`**: єдиний шар контенту для обох частин. `astro-intlayer` обслуговує сторінки `.astro`, а `lit-intlayer` читає ті самі оголошення для островів Lit.
...
Більшою мірою так. Скористайтеся [посібником з міграції](https://github.com/aymericzip/intlayer/blob/main/docs/docs/uk/compat/index.md). Ви також можете мігрувати поступово: [sync JSON плагін](https://github.com/aymericzip/intlayer/blob/main/docs/docs/uk/plugins/sync-json.md) зберігає існуючі каталоги JSON як джерело істини та генерує словники Intlayer.
...
<Question title="Чи потрібна мені окрема бібліотека i18n всередині мого острова Lit?">

Ні. Пакет `lit-intlayer` читає ті самі словники, що й Astro, тому встановлювати `@lit/localize` поруч не потрібно. Крок 6 показує, як компонент острова отримує локаль безпосередньо зі сторінки.
