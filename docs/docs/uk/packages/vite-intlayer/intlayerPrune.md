...
> Плагін вже включений та налаштований автоматично, коли ви використовуєте [`intlayer()`](https://github.com/aymericzip/intlayer/blob/main/docs/docs/uk/packages/vite-intlayer/intlayer.md). Вам потрібно зареєструвати його вручну лише в тому випадку, якщо ви самостійно складаєте стек плагінів.
...
Він залишається активним, коли `editor.enabled` становить `true`: візуальний редактор визначає кожну правку через `dictionaryKey` + `keyPath` відносно необ'єднаних словників, яких цей плагін ніколи не торкається, а очищене поле — це поле, яке не читає жоден компонент, тому воно ніколи не відображається і не може бути вибраним на сторінці.
