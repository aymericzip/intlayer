...
Коли `editor.enabled` становить `true`, плагін продовжує працювати, але **пропускає перейменування полів**: перейменування переписує ключі контенту, якими проходить інтерпретатор, тому `keyPath`, який отримує візуальний редактор, більше не відповідатиме необ'єднаним словникам, які він редагує. Стиснення пробілів та видалення метаданих верхнього рівня залишаються активними.
