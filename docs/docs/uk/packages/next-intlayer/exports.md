...
history:
  - version: 9.4.0
    date: 2026-08-22
    changes: "Оновлення до архітектури Next.js >= 9.4.0"
  - version: 10.0.0
    date: 2026-06-23
...
| ------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------- |
| `IntlayerProvider`       | Єдиний провайдер для Next.js App Router. Монтується один раз у макеті локалі, ініціалізує контекст сервера в межах запиту _та_ монтує клієнтський провайдер. (Intlayer >= 9.4)          | -                      |
| `IntlayerClientProvider` | **Застаріло** — використовуйте `IntlayerProvider` з `next-intlayer/server`. Провайдер для клієнтських компонентів у Next.js App Router. Обгортає `IntlayerProvider` з `react-intlayer`. | -                      |
| `IntlayerServerProvider` | **Застаріло** — використовуйте `IntlayerProvider` з `next-intlayer/server`. Надає контекст локалі на сервері. (Intlayer < 9.4)                                                          | -                      |
...
| `MarkdownRenderer`       | Відтворює вміст markdown з користувацькими компонентами.                                                                                                                                | -                      |
...
| `usePathname`          | Повертає поточний шлях без префікса локалі та без параметрів запиту локалі.                                                               | [usePathname](https://github.com/aymericzip/intlayer/blob/main/docs/docs/uk/packages/next-intlayer/usePathname.md)      |
