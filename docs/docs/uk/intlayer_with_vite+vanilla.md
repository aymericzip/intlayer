...
<Step number={7} title="Змініть мову вашого вмісту" isOptional={true}>
...
<Step number={8} title="Рендеринг вмісту Markdown та HTML" isOptional={true}>
...
<Step number={9} title="Додайте локалізовану маршрутизацію (Localized Routing) до свого додатка" isOptional={true}>
...
<Step number={10} title="Зміна URL при зміні локалі" isOptional={true}>
...
<Step number={12} title="Ліниве завантаження словників за локалями" isOptional={true}>
...
<Step number={13} title="Отримайте вміст ваших компонентів" isOptional={true}>
...
<Question title="Які є різні рішення для інтернаціоналізації додатків Vite та Vanilla JS?">

Vite не має власних обмежень щодо i18n, тому вибір походить з екосистеми Vanilla JS:

- **Власні об'єкти словників**: імпорт JSON/TS без типізації та інструментів тестування.
- **`i18next`**: універсальна бібліотека з завантаженням у runtime.
- **`Intlayer`**: компіляція під час збирання, повна типізація, переклад AI та візуальний редактор.
...
<Question title="Як використовувати перекладений контент у додатку Vanilla JS?">

Ініціалізуйте Intlayer у вашій точці входу, як показано на кроці 4, читайте контент через `useIntlayer` та оновлюйте DOM самостійно.
