...
## Налаштування через Intlayer CLI (рекомендовано)
...
2. Запитає, який метод транспортування ви хочете використовувати (локальний сервер (stdio) або віддалений сервер (SSE)).
...
4. Виберіть MCP для встановлення (stdio, SSE)
5. Натисніть Enter.
