...
const nextConfig: NextConfig = {/* опції конфігурації тут */};

export default withIntlayer(nextConfig);
```

> Плагін `withIntlayer()` Next.js використовується для інтеграції Intlayer з Next.js. Він забезпечує збирання файлів декларацій контенту та моніторить їх у режимі розробки. Він визначає змінні середовища Intlayer в середовищах [Webpack](https://webpack.js.org/) або [Turbopack](https://nextjs.org/docs/app/api-reference/turbopack). Крім того, він надає псевдоніми для оптимізації продуктивності та забезпечує сумісність з серверними компонентами.
...
> Один `IntlayerProvider` охоплює обидві половини дерева: він посідає контекст сервера з областю запиту, який читають серверні хуки, і монтує клієнтського провайдера, щоб клієнтські компоненти отримували ту саму локаль.
...
> Ваші декларації вмісту можуть бути визначені будь-де у вашому додатку, як тільки вони будуть додані до директорії `contentDir` (за замовчуванням `./src`). І вони повинні відповідати розширенню файлу декларації вмісту (за замовчуванням `.content.{json,ts,tsx,js,jsx,mjs,cjs,md,mdx,yaml,yml}`).
...
- **`IntlayerProvider`** монтується один раз в корневому layout. Він надає locale як server, так і client компонентам, тому сторінки більше не обгортають себе.
- Без сегмента шляху `[locale]` locale завжди походить із запиту — заголовка `x-intlayer-locale`, встановленого проксі Intlayer, потім з cookie locale — який серверні хуки читають самостійно, коли провайдер не запущений.
...
export const ServerComponentExample: FC = () => {
  const content = useIntlayer("server-component-example"); // Створіть пов'язану декларацію контенту
...
> `next-intlayer` — це ізоморфний шлях імпорту: умова експорту `react-server` надає серверним компонентам реалізацію з ambient-locale, тоді як клієнтські компоненти отримують реалізацію, підтримувану контекстом. Один і той же виклик працює на обох сторонах.
...
> Починаючи з Intlayer v9, цей middleware дотримується опції `routing.enableProxy` (`true` за замовчуванням). Встановіть `routing.enableProxy: false` у вашій конфігурації, щоб перетворити його на pass-through без видалення цього файлу. Див. [примітки до випуску v9](https://github.com/aymericzip/intlayer/blob/main/docs/docs/uk/releases/v9.md).
...
- **`next-intl`**, **`next-i18next` / `i18next`** та **`react-intl`**: Каталоги JSON або ICU, що завантажуються для кожного простору імен.
- **`Lingui`**: Працює на основі вилучення, з повідомленнями ICU, скомпільованими під час збирання.
- **`Intlayer`**: Найбільш передове рішення. Контент оголошується в будь-якому місці вашої кодової бази ([поруч із кожним компонентом або централізовано](https://intlayer.org/blog/per-component-vs-centralized-i18n)) та компілюється для кожного компонента, повністю типізований, з AI-перекладом, візуальним редактором та CMS.

Цей посібник охоплює налаштування без локалі в шляху. Див. [чому Intlayer](https://github.com/aymericzip/intlayer/blob/main/docs/docs/uk/interest_of_intlayer.md) та [бенчмарк i18n Next.js](https://github.com/aymericzip/intlayer/blob/main/docs/docs/uk/benchmark/nextjs.md).
...
Ні. Запустіть `npx intlayer extract`, і Intlayer прочитає ваші компоненти, витягне призначені для користувача рядки і створить файл `.content` поруч із кожним із них, завдяки чому ви переглядаєте diff замість копіювання рядків у каталог по одному.

Для повністю автоматизованого пайплайну [Intlayer Compiler](https://github.com/aymericzip/intlayer/blob/main/docs/docs/uk/compiler.md) робить те саме під час збирання: він сканує ваші вихідні файли JSX, TSX, Vue та Svelte під час кожної зміни, генерує словники та підтримує їх синхронізацію за допомогою гарячої заміни модулів (HMR), тому взагалі немає ключів, які потрібно підтримувати вручну.
...
<Question title="Чому варто надавати доступ до додатка без локалі в URL?">

Тому що локаль не завжди є частиною ідентичності сторінки. Автентифікована панель приладів, внутрішній інструмент або додаток за авторизацією не мають причин показувати `/fr/` у кожному URL: мова є перевагою користувача, а не окремим документом. Відмова від префікса також зберігає ваші маршрути, посилання та аналітику в єдиному наборі шляхів.

</Question>

<Question title="Які наслідки для SEO має відсутність локалі в URL?">

Вони реальні, тому вибирайте свідомо. Без окремого URL для кожної мови пошукові системи не мають окремої сторінки для індексації кожної локалі, `hreflang` не має на що вказувати, а пошуковий робот бачить лише ту мову, яку надає ваше виявлення за замовчуванням. Це нормально для вмісту за авторизацією, який все одно не індексується, і погано підходить для загальнодоступного маркетингового сайту або документації. Якщо органічний трафік за мовами має значення, натомість використовуйте налаштування з префіксом у [посібнику з Next.js 16](https://github.com/aymericzip/intlayer/blob/main/docs/docs/uk/intlayer_with_nextjs_16.md).

</Question>

<Question title="У чому різниця між режимами `no-prefix` та `search-params`?">

`"no-prefix"` повністю зберігає локаль поза межами URL та визначає її з cookie, заголовка або домену, тому всі мови мають одну адресу. `"search-params"` поміщає її в рядок запиту як `/dashboard?locale=fr`, що все одно дає кожній мові окремий URL, на який можна посилатися та додавати в закладки, не змінюючи дерево маршрутів. Віддавайте перевагу `"search-params"`, коли вам потрібні посилання для кожної мови.

</Question>

<Question title="Як у такому разі визначається локаль?">

З джерел, перелічених у `routing.storage`, за замовчуванням спочатку cookie, потім заголовок `Accept-Language`, з поверненням до вашої локалі за замовчуванням. Крок 7 додає проксі, який застосовує це. Мова, яку користувач вибирає явно, зберігається, тому вона залишається під час наступного відвідування. Див. [довідник конфігурації](https://github.com/aymericzip/intlayer/blob/main/docs/docs/uk/configuration.md).

</Question>

<Question title="Чи можна замість цього зіставити кожну мову з власним доменом?">

Так, і це варіант, який варто розглянути, якщо ви прибрали префікс з косметичних міркувань, але все ж прагнете SEO. `routing.domains` зіставляє локаль з іменем хоста, тому домен визначає мову, до шляху не додається префікс, а кожна мова отримує індексований URL, на який може вказувати `hreflang`.
