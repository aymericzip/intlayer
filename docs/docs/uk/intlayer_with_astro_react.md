...
Вам також слід додати метадані SEO (такі як hreflang та канонічні посилання) на кожній сторінці та додати React island для інтерактивного вмісту на стороні клієнта.
...
<Step number={6} title="Створення компонента React Island">
...
<Question title="Які є різні рішення для інтернаціоналізації сайту Astro з островами React?">

Вбудована функція `i18n` в Astro керує префіксами та перенаправленнями, але не контентом. Додавання острова створює додаткове питання: острів запускає не Astro, а React.

- **Astro `i18n` плюс власні словники**, а в острові **`react-i18next`**: два окремих джерела даних без спільної типізації.
- **`Intlayer`**: єдиний шар контенту для обох частин. `astro-intlayer` обслуговує сторінки `.astro`, а `react-intlayer` читає ті самі оголошення для островів React.
...
<Question title="Чи можу я мігрувати з `react-i18next` без переписування компонентів?">
...
<Question title="Чи потрібна мені окрема бібліотека i18n всередині мого острова React?">

Ні. Пакет `react-intlayer` читає ті самі словники, що й Astro, тому встановлювати `react-i18next` поруч не потрібно. Крок 6 показує, як компонент острова отримує локаль безпосередньо зі сторінки.
