...
У цій архітектурі `react-intlayer` надає `IntlayerProvider`, змонтований у `main.tsx` для обгортання дерева React. Оголошення контенту розташовуються поруч із компонентами в `src/`, а Vite обробляє перебудову контенту та HMR за допомогою `vite-intlayer`.
...
Vite не має особливих вимог щодо i18n, тому варіанти відповідають екосистемі React:

- **`react-i18next`**: завантаження JSON під час виконання.
- **`react-intl`**: формат ICU.
- **`Intlayer`**: оголошення вмісту поруч із компонентом, компіляція плагіном Vite, підтримка HMR, переклад AI та візуальний редактор.
...
Ні. Запустіть `npx intlayer extract`, і Intlayer прочитає ваші компоненти, витягне призначені для користувача рядки і створить файл `.content` поруч із кожним, завдяки чому ви переглядаєте diff замість копіювання рядків у каталог вручну. Крок 11 цього посібника пояснює це.
...
Так. Плагін Vite відстежує файли `.content.ts` та оновлює словники й типи в реальному часі без перезавантаження сторінки.

</Question>
<Question title="Як налаштувати маршрутизацію з урахуванням локалі у Vite React SPA?">
