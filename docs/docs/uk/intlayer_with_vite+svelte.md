...
У цій архітектурі `svelte-intlayer` надає реактивні сховища та утиліти для керування перекладами всередині компонентів Svelte. Оголошення контенту розміщуються в `src/` поруч із компонентами.
...
<Question title="Які є різні рішення для інтернаціоналізації додатків Vite та Svelte?">

Vite не має особливих вимог щодо i18n, тому варіанти відповідають екосистемі Svelte:

- **`svelte-i18n`** або **`typesafe-i18n`**: рішення на основі сховищ (stores) Svelte.
- **`Intlayer`**: оголошення поруч із компонентом, компіляція під час збирання, повна реактивність Svelte та AI переклад.
...
<Question title="Як використовувати перекладений контент у компоненті Svelte?">

Викликайте `useIntlayer` у вашому компоненті. Значення реактивне, тому зміна мови оновлює інтерфейс на льоту без перезавантаження сторінки.
