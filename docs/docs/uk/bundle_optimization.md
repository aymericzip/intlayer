...
| **`minify`**          | `boolean`                        | `true`                          | Чи мініфікувати словники для зменшення розміру бандла.                                                                                                                                                            |
| **`purge`**           | `boolean`                        | `true`                          | Чи видаляти невикористовувані ключі в словниках.                                                                                                                                                                  |
...
## Мініфікація та очищення словників

Intlayer надає опції для подальшої оптимізації вашого бандла шляхом мініфікації словників та очищення невикористовуваних ключів (purge).

### Мініфікація (Minification)

Мініфікація словників видаляє непотрібні пробіли, коментарі та зменшує розмір JSON-контенту. Це особливо корисно для великих словників.
...
> Примітка: Мініфікація ігнорується, якщо `optimize` вимкнено або якщо увімкнено Візуальний редактор (оскільки редактору потрібен повний вміст для можливості редагування).

### Очищення (Purging)

Очищення гарантує, що лише ті ключі, які фактично використовуються у вашому коді, будуть включені у фінальний бандл словників. Це може значно зменшити розмір вашого бандла, якщо у вас є великі словники з багатьма ключами, які не використовуються в кожній частині вашого застосунку.
...
> Примітка: Очищення ігнорується, якщо `optimize` вимкнено.
