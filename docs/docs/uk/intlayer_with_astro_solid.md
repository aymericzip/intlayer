...
title: "Astro + Solid i18n - Повний посібник з перекладу вашого застосунку"
description: "Більше ніякого i18next. Посібник 2026 зі створення багатомовного (i18n) застосунку Astro + Solid. Перекладайте за допомогою ШІ-агентів та оптимізуйте розмір бандлу, SEO та продуктивність."
keywords:
  - інтернаціоналізація
  - документація
  - Intlayer
...
- **solid-js**
  Основний пакет Solid.

- **solid-intlayer**
  Пакет для інтеграції Intlayer у додатки Solid. Він надає `IntlayerProvider`, а також примітиви `useIntlayer` та `useLocale` для інтернаціоналізації в Solid.

- **@astrojs/solid-js**
  Офіційна інтеграція Astro, яка дозволяє використовувати Solid component islands.
...
> Інтеграція `solid()` дозволяє використовувати Solid component islands за допомогою `client:only="solid-js"`.
...
Вам також слід додати метадані SEO (такі як hreflang та канонічні посилання) на кожній сторінці та додати Solid island для інтерактивного вмісту на стороні клієнта.
...
<html lang={locale} dir={getHTMLTextDir(locale)}>
  ...
  <body>
    <!-- Solid island рендерить увесь інтерактивний вміст, включаючи перемикач мов -->
...
<Step number={6} title="Створення компонента Solid Island">
...
<Question title="Які є різні рішення для інтернаціоналізації сайту Astro з островами Solid?">

Вбудована функція `i18n` в Astro керує префіксами та перенаправленнями, але не контентом. Додавання острова створює додаткове питання: острів запускає не Astro, а Solid.

- **Astro `i18n` плюс власні словники**, а в острові **`@solid-primitives/i18n`**: два окремих джерела даних без спільної типізації.
- **`Intlayer`**: єдиний шар контенту для обох частин. `astro-intlayer` обслуговує сторінки `.astro`, а `solid-intlayer` читає ті самі оголошення для островів Solid.
...
Більшою мірою так. Скористайтеся [посібником з міграції](https://github.com/aymericzip/intlayer/blob/main/docs/docs/uk/migration_from_i18next_to_intlayer.md). Ви також можете мігрувати поступово: [sync JSON плагін](https://github.com/aymericzip/intlayer/blob/main/docs/docs/uk/plugins/sync-json.md) зберігає існуючі каталоги JSON як джерело істини та генерує словники Intlayer.
...
<Question title="Чи потрібна мені окрема бібліотека i18n всередині мого острова Solid?">

Ні. Пакет `solid-intlayer` читає ті самі словники, що й Astro, тому встановлювати `@solid-primitives/i18n` поруч не потрібно. Крок 6 показує, як компонент острова отримує локаль безпосередньо зі сторінки.
