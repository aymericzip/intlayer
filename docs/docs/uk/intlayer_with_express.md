...
- **`i18next` / `i18next-http-middleware`**: поширене рішення для runtime з використанням JSON файлів.
- **Власні словники**: без типізації та автоматичних інструментів.
- **`Intlayer`**: middleware `express-intlayer`, типізація TypeScript, переклад за допомогою AI, CMS та спільні словники з клієнтською частиною.
