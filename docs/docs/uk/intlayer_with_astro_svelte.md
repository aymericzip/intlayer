...
Вам також слід додати метадані SEO (такі як hreflang та канонічні посилання) на кожній сторінці та додати Svelte island для інтерактивного вмісту на стороні клієнта.
...
<Question title="Які є різні рішення для інтернаціоналізації сайту Astro з островами Svelte?">

Вбудована функція `i18n` в Astro керує префіксами та перенаправленнями, але не контентом. Додавання острова створює додаткове питання: острів запускає не Astro, а Svelte.

- **Astro `i18n` плюс власні словники**, а в острові **`svelte-i18n`**: два окремих джерела даних без спільної типізації.
- **`Intlayer`**: єдиний шар контенту для обох частин. `astro-intlayer` обслуговує сторінки `.astro`, а `svelte-intlayer` читає ті самі оголошення для островів Svelte.
...
<Question title="Чи можу я мігрувати з `svelte-i18n` без переписування компонентів?">

Більшою мірою так. Скористайтеся [посібником з міграції](https://github.com/aymericzip/intlayer/blob/main/docs/docs/uk/compat/svelte-i18n.md). Ви також можете мігрувати поступово: [sync JSON плагін](https://github.com/aymericzip/intlayer/blob/main/docs/docs/uk/plugins/sync-json.md) зберігає існуючі каталоги JSON як джерело істини та генерує словники Intlayer.
...
<Question title="Чи потрібна мені окрема бібліотека i18n всередині мого острова Svelte?">

Ні. Пакет `svelte-intlayer` читає ті самі словники, що й Astro, тому встановлювати `svelte-i18n` поруч не потрібно. Крок 6 показує, як компонент острова отримує локаль безпосередньо зі сторінки.
