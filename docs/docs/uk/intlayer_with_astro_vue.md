...
description: "Більше ніякого i18next. Посібник 2026 зі створення багатомовного (i18n) застосунку Astro + Vue. Перекладайте за допомогою ШІ-агентів та оптимізуйте розмір бандлу, SEO та продуктивність."
keywords:
  - інтернаціоналізація
  - документація
  - Intlayer
...
history:
  - version: 9.5.5
    date: 2026-09-19
    changes: "Використання хуків useIntlayer / useLocale з astro-intlayer на сторінці Astro"
  - version: 8.9.0
    date: 2026-05-04
    changes: "Оновлення використання API useIntlayer у Solid для прямого доступу до властивостей"
  - version: 8.7.7
    date: 2026-04-24
    changes: "Початкова документація для Astro + Vue"
author: aymericzip
---

# Перекладіть свій сайт Astro + Vue за допомогою Intlayer | Інтернаціоналізація (i18n)
...
## Покрокова інструкція з налаштування Intlayer в Astro + Vue
...
- **astro-intlayer**
  Включає плагін інтеграції Astro для інтеграції Intlayer із [бандлером Vite](https://vite.dev/guide/why.html#why-bundle-for-production), middleware, що розпізнає локаль кожного запиту в `Astro.locals.intlayer`, та хуки `useIntlayer` / `useDictionary` / `useLocale`. Той самий шлях імпорту розпізнається як серверна реалізація у фронтматтері `.astro` та як клієнтська (на базі `vanilla-intlayer`) у блоках `<script>`.

- **vue**
  Основний пакет Vue.

- **vue-intlayer**
  Пакет для інтеграції Intlayer у додатки Vue. Він надає `installIntlayer`, а також composables `useIntlayer` та `useLocale` для інтернаціоналізації у Vue.

- **@astrojs/vue**
  Офіційна інтеграція Astro, яка дозволяє використовувати Vue component islands.
...
Додайте плагін `intlayer` до вашої конфігурації Astro та інтеграцію Vue.
...
> Інтеграція `vue()` дозволяє використовувати Vue component islands за допомогою `client:only="vue"`.
...
Вам також слід додати метадані SEO (такі як hreflang та канонічні посилання) на кожній сторінці та додати Vue island для інтерактивного вмісту на стороні клієнта.
...
<html lang={locale} dir={getHTMLTextDir(locale)}>
  ...
  <body>
    <!-- Vue island рендерить увесь інтерактивний вміст, включаючи перемикач мов -->
...
<Step number={6} title="Створення компонента Vue Island">

Створіть компонент island, який обгортає ваш додаток Vue і приймає мову, визначену сервером. Ви повинні викликати `installIntlayer`, щоб зареєструвати плагін Intlayer в екземплярі Vue перед використанням будь-яких composables.
...
<template>
  <div>
    <h1>{{ title }}</h1>
    <!-- Перемикач мов рендериться безпосередньо в шаблоні island -->
...
> Проп `locale` передається зі сторінки Astro (визначення на стороні сервера) і використовується для ініціалізації `installIntlayer`, встановлюючи початкову мову для всіх composables всередині дерева.

</Step>
<Step number={7} title="Додавання перемикача мов">

Функціональність перемикання мов інтегрована безпосередньо в шаблон Vue island (див. крок 6 вище). Він використовує composable `useLocale` з `vue-intlayer` і переходить за локалізованою URL-адресою, коли користувач обирає нову мову:
...
// Припустимо таку ж логіку props/setup додатка з кроку 6...
...
<Question title="Які є різні рішення для інтернаціоналізації сайту Astro з островами Vue?">

Вбудована функція `i18n` в Astro керує префіксами та перенаправленнями, але не контентом. Додавання острова створює додаткове питання: острів запускає не Astro, а Vue.

- **Astro `i18n` плюс власні словники**, а в острові **`vue-i18n`**: два окремих джерела даних без спільної типізації.
- **`Intlayer`**: єдиний шар контенту для обох частин. `astro-intlayer` обслуговує сторінки `.astro`, а `vue-intlayer` читає ті самі оголошення для островів Vue.
...
Більшою мірою так. Скористайтеся [посібником з міграції](https://github.com/aymericzip/intlayer/blob/main/docs/docs/uk/migration_from_vue-i18n_to_intlayer.md). Ви також можете мігрувати поступово: [sync JSON плагін](https://github.com/aymericzip/intlayer/blob/main/docs/docs/uk/plugins/sync-json.md) зберігає існуючі каталоги JSON як джерело істини та генерує словники Intlayer.
...
<Question title="Чи потрібна мені окрема бібліотека i18n всередині мого острова Vue?">

Ні. Пакет `vue-intlayer` читає ті самі словники, що й Astro, тому встановлювати `vue-i18n` поруч не потрібно. Крок 6 показує, як компонент острова отримує локаль безпосередньо зі сторінки.

</Question>
<Question title="Як острів дізнається, якою мовою була відрендерена сторінка?">

Сторінка Astro передає локаль як проп, і провайдер Intlayer в острові використовує її, тому острів гідратується тією самою мовою, яку відрендерив сервер. Це запобігає спалаху мови за замовчуванням (flash of default language).
...
<Question title="Як автоматично перекласти сайт за допомогою AI?">
