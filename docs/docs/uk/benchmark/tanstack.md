...
> [Переглянути повні дані бенчмарку](https://intlayer.org/markdown?url=https%3A%2F%2Fraw.githubusercontent.com%2Fintlayer-org%2Fbenchmark-i18n%2Fmain%2Freport%2Fscripts%2Fsummarize-tanstack.md)
...
- **Intlayer**: Забезпечує найкращу продуктивність і найменший розмір бандла (v9.5.10) для TanStack Start.
...
- [`react-intlayer`](https://github.com/aymericzip/intlayer/blob/main/docs/docs/uk/packages/react-intlayer/exports.md) (v9.5.10)
- [`@intlayer/use-intl`](https://github.com/aymericzip/intlayer/blob/main/docs/docs/uk/compat/next-intl.md) (v9.5.10)
- [`@intlayer/lingui`](https://github.com/aymericzip/intlayer/blob/main/docs/docs/uk/compat/lingui.md) (v9.5.10)
- [`@intlayer/react-i18next`](https://github.com/aymericzip/intlayer/blob/main/docs/docs/uk/compat/react-i18next.md) (v9.5.10)
...
Я запустив одну й ту саму багатомовну програму в реальному браузері для кожного стеку, а потім записав, що насправді показувалося в мережі та скільки часу це займало. Розміри повідомляються **після звичайного стиснення в мережі**, оскільки це ближче до того, що люди завантажують, ніж до підрахунків сирцевого коду.

- **Розмір бібліотеки інтернаціоналізації**: Після bundling, tree-shaking та мініфікації розмір бібліотеки i18n — це розмір постачальників (наприклад, `NextIntlClientProvider`) + код хуків (наприклад, `useTranslations`) в пустому компоненті. Він не включає завантаження файлів перекладу. Це показує, наскільки дорога бібліотека перш ніж ваш контент потрапить в картину.

- **JavaScript на сторінку**: Для кожного маршруту бенчмарку, скільки скрипту браузер завантажує під час цього відвідування, усереднено по сторінках в наборі (і по локалям, де звіт їх об'єднує). Важкі сторінки — повільні сторінки.

- **Витік з інших локалей**: Це контент однієї й тієї ж сторінки, але іншою мовою, який був би випадково завантажений у перевіреній сторінці. Цей контент не потрібний і його слід уникати. (наприклад, контент сторінки `/fr/about` у пакеті сторінки `/en/about`)

- **Витік з інших маршрутів**: Та сама ідея для **інших екранів** у програмі: чи їхній копія йде разом, коли ви відкрили лише одну сторінку. (наприклад, контент сторінки `/en/about` у пакеті сторінки `/en/contact`). Високий бал натякає на слабке розділення або надто широкі пакети.

- **Середній розмір пакету компонента**: Загальні частини користувацького інтерфейсу вимірюються **один за одним** замість того, щоб приховуватися всередину одного гігантського номера програми. Це показує, чи інтернаціоналізація тихо збільшує звичайні компоненти. Наприклад, якщо ваш компонент перерендерується, він завантажить всі ці дані з пам'яті. Приєднання великого JSON до будь-якого компонента подібно до підключення великого сховища непотрібних даних, що уповільнить продуктивність ваших компонентів.

- **Чутливість перемикання мови**: Я змінюю мову за допомогою власного елемента керування програми та вимірюю, скільки часу пройде, поки сторінка явно не переключиться, що помітив би відвідувач, а не лабораторна мікрокроку.

- **Робота з рендерингом після зміни мови**: Вужча подальша діяльність: скільки зусиль потребував інтерфейс, щоб перемалювати для нової мови, коли перемикання йде. Корисно, коли «відчутний» час та вартість фреймворку розходяться.

- **Час завантаження початкової сторінки**: Від навігації до браузера, враховуючи, що сторінка повністю завантажена для сценаріїв, які я тестував. Добре для порівняння холодних запусків.

- **Час гідратації**: Коли програма це розкриває, скільки часу клієнт витрачає на перетворення серверної HTML на те, що можна насправді клацнути. Тире в таблицях означає, що реалізація не надала надійної цифри гідратації у цьому бенчмарку.
...
- Для додатку розміром близько 111 КБ `gt-react` може додати понад 170 КБ зайвих даних (порядок величини зафіксовано в реалізації Next.js у тому ж бенчмарку).
...
Пакет досить важкий (~11.1 kb, що більш ніж у 2.9 рази більше за `react-intlayer`).
...
Пакет особливо важкий (~18.4kb, що становить близько 3.5× `react-intlayer`).
...
Пакет також важкий (~14.4kb, що становить близько 3× `react-intlayer`).
