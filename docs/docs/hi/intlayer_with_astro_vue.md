...
history:
  ...
  - version: 8.7.7
    date: 2026-04-24
    changes: "Astro + Vue के लिए प्रारंभिक दस्तावेज़ीकरण"
author: aymericzip
---
...
## Astro + Vue में Intlayer को कॉन्फ़िगर करने के लिए चरण-दर-चरण मार्गदर्शिका
...
- **vue-intlayer**
  Vue अनुप्रयोगों में Intlayer को एकीकृत करने के लिए पैकेज। यह Vue में अंतर्राष्ट्रीयकरण के लिए `installIntlayer` के साथ-साथ `useIntlayer` और `useLocale` कंपोजेबल्स (composables) प्रदान करता है।

- **@astrojs/vue**
  आधिकारिक Astro एकीकरण जो Vue घटक islands के उपयोग की अनुमति देता है।
...
अपने Astro कॉन्फ़िगरेशन में `intlayer` प्लगइन और Vue एकीकरण जोड़ें।
...
> `vue()` एकीकरण `client:only="vue"` के माध्यम से Vue घटक islands के उपयोग की अनुमति देता है।
...
आपको हर पृष्ठ पर SEO मेटाडेटा (जैसे hreflang और कैनोनिकल लिंक) भी जोड़ना चाहिए और इंटरैक्टिव क्लाइंट-साइड सामग्री के लिए एक Vue island पेश करना चाहिए।
...
<html lang={locale} dir={getHTMLTextDir(locale)}>
  ...
  <body>
    <!-- Vue island भाषा स्विचर सहित सभी इंटरैक्टिव सामग्री को रेंडर करता है -->
...
<Step number={6} title="एक Vue Island घटक बनाएँ">

एक island घटक बनाएँ जो आपके Vue एप्लिकेशन को लपेटता है और सर्वर-संसूचित लोकेल प्राप्त करता है। आपको किसी भी कंपोजेबल्स का उपयोग करने से पहले `installIntlayer` को कॉल करके अपने Vue इंस्टेंस में Intlayer प्लगइन पंजीकृत करना होगा।
...
<template>
  <div>
    <h1>{{ title }}</h1>
    <!-- भाषा स्विचर सीधे island टेम्पलेट के भीतर रेंडर किया गया है -->
...
> `locale` प्रॉप को Astro पृष्ठ (सर्वर पहचान) से पारित किया जाता है और `installIntlayer` को इनिशियलाइज़ करने के लिए उपयोग किया जाता है, जिससे पेड़ के भीतर सभी कंपोजेबल्स के लिए प्रारंभिक भाषा सेट हो जाती है।

</Step>
<Step number={7} title="एक भाषा स्विचर जोड़ना">

भाषा स्विचिंग कार्यक्षमता सीधे Vue island टेम्पलेट के भीतर एकीकृत है (ऊपर चरण 6 देखें)। यह `vue-intlayer` से `useLocale` कंपोजेबल का उपयोग करता है और उपयोगकर्ता द्वारा एक नई भाषा चुनने पर स्थानीयकृत URL पर नेविगेट करता है:
...
// चरण 6 से समान प्रॉप/सेटअप ऐप लॉजिक का पुन: उपयोग करें...
...
एक्सटेंशन का उपयोग करने के बारे में अधिक जानकारी के लिए, [VS Code एक्सटेंशन दस्तावेज़](https://github.com/aymericzip/intlayer/blob/main/docs/docs/hi/vs_code_extension.md) देखें।
...
<Question title="Vue आइलैंड वाली Astro साइट के अंतर्राष्ट्रीयकरण के लिए कौन से विभिन्न समाधान उपलब्ध हैं?">

Astro का अंतर्निहित `i18n` विकल्प उपसर्गों और पुनर्निर्देशों को संभालता है लेकिन सामग्री का प्रबंधन नहीं करता है। आइलैंड जोड़ने से एक अतिरिक्त चुनौती पैदा होती है: वे Astro नहीं बल्कि Vue चलाते हैं।

- **Astro `i18n` प्लस कस्टम शब्दकोश**, और आइलैंड में **`vue-i18n`**: बिना साझा प्रकारों के दो अलग-अलग सामग्री स्रोत।
- **`Intlayer`**: दोनों के लिए एक साझा सामग्री परत। `astro-intlayer` `.astro` पृष्ठों को सेवा प्रदान करता है, और `vue-intlayer` Vue आइलैंड के लिए समान घोषणा पढ़ता है।
...
<Question title="क्या मैं अपने घटकों को फिर से लिखे बिना `vue-i18n` से माइग्रेट कर सकता हूँ?">

काफी हद तक हाँ। [माइग्रेशन गाइड](https://github.com/aymericzip/intlayer/blob/main/docs/docs/hi/migration_from_vue-i18n_to_intlayer.md) का पालन करें। आप धीरे-धीरे भी माइग्रेट कर सकते हैं: [sync JSON plugin](https://github.com/aymericzip/intlayer/blob/main/docs/docs/hi/plugins/sync-json.md) आपके JSON कैटलॉग को सत्य का स्रोत बनाए रखता है और Intlayer शब्दकोश उत्पन्न करता है।
...
<Question title="क्या मुझे अपने Vue आइलैंड के अंदर एक अलग i18n लाइब्रेरी की आवश्यकता है?">

नहीं। `vue-intlayer` पैकेज Astro के समान शब्दकोश पढ़ता है, इसलिए आपको इसके साथ `vue-i18n` स्थापित करने की आवश्यकता नहीं है। चरण 6 दिखाता है कि कैसे आइलैंड घटक सीधे पृष्ठ से लोकेल प्राप्त करते हैं।
