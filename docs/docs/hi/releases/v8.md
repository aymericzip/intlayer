...
## YAML और Markdown सामग्री फ़ाइलें (v8.10.0)

अब आप सीधे `.content.md` और `.content.yaml` फ़ाइल एक्सटेंशन का उपयोग करके सामग्री घोषित कर सकते हैं। यह लंबी सामग्री, ब्लॉग, दस्तावेज़ और गोपनीयता नीतियों के लिए बिल्कुल सही है।

ये फ़ाइलें विज़ुअल एडिटर (Visual Editor) से पूरी तरह से संपादन योग्य हैं, जो गैर-तकनीकी टीम के सदस्यों के लिए एक सुव्यवस्थित वर्कफ़्लो प्रदान करती हैं।

अधिक जानकारी के लिए, [YAML सामग्री फ़ाइल दस्तावेज़](https://github.com/aymericzip/intlayer/blob/main/docs/docs/hi/dictionary/content_file.md#yaml-content-file) और [Markdown सामग्री फ़ाइल दस्तावेज़](https://github.com/aymericzip/intlayer/blob/main/docs/docs/hi/dictionary/markdown.md) देखें।
...
## Unicode CLDR बहुवचन नियम (v8.8.0)

Unicode CLDR प्रोजेक्ट द्वारा परिभाषित बहुवचन नियमों के आधार पर एक नया `plural` नोड हेल्पर पेश किया गया है, जो जटिल भाषाओं के लिए समर्थन में सुधार करता है।
...
इसके बाद आप अपने फ्रेमवर्क घटक में गतिशील रूप से (dynamically) काउंट पास कर सकते हैं:
...
अधिक जानकारी के लिए, [बहुवचनीकरण दस्तावेज़ (Pluralization Documentation)](https://github.com/aymericzip/intlayer/blob/main/docs/docs/hi/dictionary/plurial.md) देखें।
...
## TypeScript: IntlayerNode प्रिमिटिव एक्सेसर्स (Primitive Accessors)

सामग्री नोड्स पर सीधे प्रिमिटिव जावास्क्रिप्ट विधियों (primitive JavaScript methods) को कॉल करने की अनुमति देने के लिए `IntlayerNode` प्रकार को अपडेट किया गया है। यह आपकी स्थानीयकृत सामग्री के साथ सीधे बातचीत करना संभव बनाता है जैसे कि यह एक मानक स्ट्रिंग, नंबर, बूलियन या एरे प्रिमिटिव हो।

React, Preact, Solid, Svelte, Vue, और Angular में समर्थित।

### स्ट्रिंग उदाहरण (String Example)

```typescript
content.placeholder; // IntlayerNode<string> लौटाता है
content.placeholder.value; // string लौटाता है
content.placeholder.toString(); // string लौटाता है
content.placeholder.toLowerCase(); // string लौटाता है
String(content.placeholder); // string लौटाता है
content.placeholder.toUpperCase(); // अपरकेस स्ट्रिंग लौटाता है
content.placeholder.replace("a", "b"); // संशोधित स्ट्रिंग लौटाता है
```

### एरे उदाहरण (Array Example)

```typescript
content.myArrayOfString             // IntlayerNode<Array<string>> लौटाता है
content.myArrayOfString.find(...)   // तत्व (element) लौटाता है
content.myArrayOfString.join(', ')  // संयुक्त स्ट्रिंग लौटाता है
```
...
## टूलिंग: C++ वॉचर और OXC-आधारित LSP (v8.12.0)

Intlayer v8.12.0 डेवलपर अनुभव में बड़े सुधार लाता है:

- **Parcel Watcher**: सामग्री वॉचर (content watcher) को `chokidar` से `@parcel/watcher` पर माइग्रेट कर दिया गया है, जो तेज़ अपडेट और काफी कम मेमोरी खपत प्रदान करने के लिए मूल C++ फ़ाइल वॉचिंग का लाभ उठाता है।
- **नया लैंग्वेज सर्वर प्रोटोकॉल (LSP)**: एक बिल्कुल नया LSP अब उपलब्ध है। OXC-आधारित पार्सिंग के साथ निर्मित, यह आपके IDE और AI एजेंटों को `useIntlayer('my-key')` कॉल को उनकी संबंधित `.content` फ़ाइलों के साथ सहजता से जोड़ने में मदद करता है, और इसके विपरीत भी।

अधिक जानकारी के लिए, [LSP दस्तावेज़](https://github.com/aymericzip/intlayer/blob/main/docs/docs/hi/lsp.md) देखें।
...
## प्रायोगिक बिल्ड अनुकूलन: Minify और Purge (v8.7.0)

Intlayer v8.7.0 आपके प्रोडक्शन बंडल को अनुकूलित करने के लिए डिज़ाइन की गई आपकी `intlayer.config.ts` में नई प्रायोगिक बिल्ड सुविधाएँ पेश करता है:

- **डिक्शनरी मिनिफिकेशन (Dictionary Minification)**: आकार घटाने के लिए डिक्शनरी फ़ाइलों को छोटा (minify) करता है।
- **अनुपयोगी कुंजी पर्जिंग (Unused Key Purging)**: डिक्शनरी से अनुपयोगी अनुवाद कुंजियों को स्कैन और हटाता है, सक्रिय कुंजियों का नाम बदलकर छोटे वर्णों में कर देता है (उदा. `"products" -> "a"`, `"pricing" -> "b"`)। यह बंडल के आकार को 5% तक कम कर देता है।
- **TypeScript जांच अक्षम करें**: संकलन (compilation) चरण के दौरान TypeScript टाइपचेकिंग को अक्षम करके बिल्ड को तेज़ करें।
...
const config: IntlayerConfig = {
  ...
  build: {
    /**
     * बंडल के आकार को कम करने के लिए डिक्शनरी को छोटा (minify) करें।
     */
    minify: true,

    /**
     * डिक्शनरी में अनुपयोगी कुंजियों को हटाएँ (purge करें)
     */
    purge: true,

    /**
     * इंगित करता है कि क्या बिल्ड को TypeScript प्रकारों की जांच करनी चाहिए
     */
    checkTypes: false,
  },
};
...
Intlayer v8 सामग्री परिभाषा फ़ाइलों के कॉन्फ़िगरेशन को कोड परिवर्तन के कॉन्फ़िगरेशन से अलग करता है। यह अधिक सटीक निगरानी और स्कैनिंग की अनुमति देता, जिससे बिल्ड प्रदर्शन में सुधार होता है।
...
## Angular समर्थन: Vite बंडलर (v8.11.0)

Angular उपयोगकर्ताओं के लिए, Intlayer अब एक नए कस्टम esbuild प्लगइन के माध्यम से मूल रूप से Vite बंडलर का समर्थन करता, जिससे विकास और बिल्ड समय तेज़ हो जाता है।

कॉन्फ़िगरेशन के विवरण के लिए, [Angular पर्यावरण दस्तावेज़ (Angular Environment Documentation)](https://github.com/aymericzip/intlayer/blob/main/docs/docs/hi/intlayer_with_angular_21.md) देखें।

## साइटमैप जनरेशन (v8.6.0)

Intlayer अब आपके लोकेल्स और राउटिंग मोड के आधार पर स्वचालित रूप से साइटमैप जेनरेट करता है। यह विशेष रूप से TanStack Router जैसे राउटर आर्किटेक्चर में SEO अनुकूलन के लिए उपयोगी है:
