...
デフォルトでは、辞書が読み込まれるとき、すべてのロケールのコンテンツをインポートします。
このオプションがtrueに設定されている場合、現在のロケールの辞書コンテンツのみが
動的インポートを介して取得されます。この場合、Intlayerはすべての
`useIntlayer`呼び出しを`useDynamicDictionary`に置き換えます。
