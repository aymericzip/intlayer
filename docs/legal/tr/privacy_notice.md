...
## Intlayer i18n Scanner (Chrome Uzantısı)

**Intlayer i18n Scanner** Chrome uzantısı, geçerli sekmede açık olan sayfanın uluslararasılaştırma yapılandırmasını inceler.

- **Yerel analiz:** Uzantı açılır penceresini açtığınızda, geçerli sayfa (dil öznitelikleri, hreflang bağlantıları, meta etiketleri, framework global değişkenleri) doğrudan tarayıcınızda okunur. Bu sayfa içeriği sunucularımıza gönderilmez.
- **Denetim taraması:** Yalnızca tarama düğmesine tıkladığınızda, geçerli sayfanın URL'si denetimi gerçekleştirmek için Intlayer API'sine (`back.intlayer.org`) gönderilir. Toplu istatistikler hesaplamak amacıyla yalnızca taranan sayfanın **alan adını**, elde edilen **puanı** ve taramanın **tarihini** saklarız. Bu kayıt kimliğinizle veya bir Intlayer hesabıyla ilişkilendirilmez.
- **Başka veri toplanmaz:** Uzantı kişisel bilgi, kimlik doğrulama verisi veya kullanıcı etkinliği toplamaz, ziyaret ettiğiniz sayfaları izlemez, çerez veya analiz aracı kullanmaz ve uzak kod çalıştırmaz.
