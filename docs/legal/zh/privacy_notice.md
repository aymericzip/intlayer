...
## Intlayer i18n Scanner（Chrome 扩展）

**Intlayer i18n Scanner** Chrome 扩展用于检查当前标签页中所打开页面的国际化配置。

- **本地分析：** 打开扩展弹出窗口时，扩展会直接在您的浏览器中读取当前页面（语言属性、hreflang 链接、meta 标签、框架全局变量）。这些页面内容不会发送到我们的服务器。
- **审计扫描：** 仅当您点击扫描按钮时，当前页面的 URL 才会发送至 Intlayer API（`back.intlayer.org`）以执行审计。为计算汇总统计数据，我们仅保存被扫描页面的**域名**、得出的**分数**以及扫描**日期**。该记录不会与您的身份或 Intlayer 账户关联。
- **无其他数据收集：** 该扩展不收集个人信息、认证数据或用户活动，不跟踪您访问的页面，不使用 Cookie 或分析工具，也不运行远程代码。
