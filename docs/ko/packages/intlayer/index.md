# intlayer: 다국어 사전을 관리하기 위한 NPM 패키지 (i18n)

**Intlayer**는 JavaScript 개발자를 위해 특별히 설계된 패키지 모음입니다. React, Next.js, Express.js와 같은 프레임워크와 호환됩니다.

**`intlayer` 패키지**는 코드 어디에서나 콘텐츠를 선언할 수 있도록 합니다. 다국어 콘텐츠 선언을 구조화된 사전으로 변환하여 애플리케이션에 원활하게 통합할 수 있습니다. TypeScript와 함께 **Intlayer**는 더 강력하고 효율적인 도구를 제공하여 개발을 향상시킵니다.
...
- **통합된 콘텐츠 파일**: 번역을 해당 구성 요소와 가까이 유지하여 유지보수성과 명확성을 향상시킵니다.
...
이 명령은 모든 `*.content.*` 파일을 스캔하고 컴파일하며 결과를 **`intlayer.config.ts`**에 지정된 디렉토리에 작성합니다 (기본값: `./.intlayer`).
...
.
└── .intlayer
    ├── dictionary  # 콘텐츠 사전이 포함됨
    │   ├── client-component.json
    │   └── server-component.json
    ├── main  # 애플리케이션에서 사용할 사전 진입점 포함
    │   ├── dictionary.cjs
    │   └── dictionary.mjs
...
예를 들어, **en/client-component.json**은 다음과 같습니다:
...
Intlayer는 CLI 도구를 제공합니다:

- 콘텐츠 선언을 감사하고 누락된 번역을 완료합니다.
- 콘텐츠 선언에서 사전을 빌드합니다.
- CMS에서 원격 사전을 로컬 프로젝트로 푸시 및 풀합니다.

자세한 내용은 [intlayer-cli](https://github.com/aymericzip/intlayer/blob/main/docs/ko/intlayer_cli.md)를 참조하세요.

## 애플리케이션에서 Intlayer 사용

콘텐츠를 선언한 후, 애플리케이션에서 Intlayer 사전을 사용할 수 있습니다.

Intlayer는 애플리케이션을 위한 패키지로 제공됩니다.
