/* AUTO-GENERATED – DO NOT EDIT */
/* REGENERATE USING `pnpm prepare` */
import type { LocalesValues } from '@intlayer/config';
import { existsSync } from 'fs';
import { readFile } from 'fs/promises';
import { dirname, join } from 'path';
import { fileURLToPath } from 'url';

const isESModule = typeof import.meta.url === 'string';
const dir = isESModule ? dirname(fileURLToPath(import.meta.url)) : __dirname;

export const blogEntry = {
  './blog/en/index.md': {
    en: (() => {
      const target = join(dir, '../../../blog/en/index.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(join(dir, '../../../blog/en/index.md'), 'utf8')
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    fr: (() => {
      const target = join(dir, '../../../blog/fr/index.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(join(dir, '../../../blog/en/index.md'), 'utf8')
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ru: (() => {
      const target = join(dir, '../../../blog/ru/index.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(join(dir, '../../../blog/en/index.md'), 'utf8')
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ja: (() => {
      const target = join(dir, '../../../blog/ja/index.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(join(dir, '../../../blog/en/index.md'), 'utf8')
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ko: (() => {
      const target = join(dir, '../../../blog/ko/index.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(join(dir, '../../../blog/en/index.md'), 'utf8')
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    zh: (() => {
      const target = join(dir, '../../../blog/zh/index.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(join(dir, '../../../blog/en/index.md'), 'utf8')
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    es: (() => {
      const target = join(dir, '../../../blog/es/index.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(join(dir, '../../../blog/en/index.md'), 'utf8')
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    de: (() => {
      const target = join(dir, '../../../blog/de/index.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(join(dir, '../../../blog/en/index.md'), 'utf8')
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ar: (() => {
      const target = join(dir, '../../../blog/ar/index.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(join(dir, '../../../blog/en/index.md'), 'utf8')
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    pt: (() => {
      const target = join(dir, '../../../blog/pt/index.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(join(dir, '../../../blog/en/index.md'), 'utf8')
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    'en-GB': (() => {
      const target = join(dir, '../../../blog/en-GB/index.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(join(dir, '../../../blog/en/index.md'), 'utf8')
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    it: (() => {
      const target = join(dir, '../../../blog/it/index.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(join(dir, '../../../blog/en/index.md'), 'utf8')
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    hi: (() => {
      const target = join(dir, '../../../blog/hi/index.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(join(dir, '../../../blog/en/index.md'), 'utf8')
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    tr: (() => {
      const target = join(dir, '../../../blog/tr/index.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(join(dir, '../../../blog/en/index.md'), 'utf8')
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
  } as unknown as Record<LocalesValues, Promise<string>>,
  './blog/en/internationalization_and_SEO.md': {
    en: (() => {
      const target = join(
        dir,
        '../../../blog/en/internationalization_and_SEO.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/internationalization_and_SEO.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    fr: (() => {
      const target = join(
        dir,
        '../../../blog/fr/internationalization_and_SEO.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/internationalization_and_SEO.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ru: (() => {
      const target = join(
        dir,
        '../../../blog/ru/internationalization_and_SEO.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/internationalization_and_SEO.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ja: (() => {
      const target = join(
        dir,
        '../../../blog/ja/internationalization_and_SEO.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/internationalization_and_SEO.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ko: (() => {
      const target = join(
        dir,
        '../../../blog/ko/internationalization_and_SEO.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/internationalization_and_SEO.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    zh: (() => {
      const target = join(
        dir,
        '../../../blog/zh/internationalization_and_SEO.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/internationalization_and_SEO.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    es: (() => {
      const target = join(
        dir,
        '../../../blog/es/internationalization_and_SEO.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/internationalization_and_SEO.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    de: (() => {
      const target = join(
        dir,
        '../../../blog/de/internationalization_and_SEO.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/internationalization_and_SEO.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ar: (() => {
      const target = join(
        dir,
        '../../../blog/ar/internationalization_and_SEO.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/internationalization_and_SEO.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    pt: (() => {
      const target = join(
        dir,
        '../../../blog/pt/internationalization_and_SEO.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/internationalization_and_SEO.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    'en-GB': (() => {
      const target = join(
        dir,
        '../../../blog/en-GB/internationalization_and_SEO.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/internationalization_and_SEO.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    it: (() => {
      const target = join(
        dir,
        '../../../blog/it/internationalization_and_SEO.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/internationalization_and_SEO.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    hi: (() => {
      const target = join(
        dir,
        '../../../blog/hi/internationalization_and_SEO.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/internationalization_and_SEO.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    tr: (() => {
      const target = join(
        dir,
        '../../../blog/tr/internationalization_and_SEO.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/internationalization_and_SEO.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
  } as unknown as Record<LocalesValues, Promise<string>>,
  './blog/en/intlayer_with_i18next.md': {
    en: (() => {
      const target = join(dir, '../../../blog/en/intlayer_with_i18next.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_i18next.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    fr: (() => {
      const target = join(dir, '../../../blog/fr/intlayer_with_i18next.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_i18next.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ru: (() => {
      const target = join(dir, '../../../blog/ru/intlayer_with_i18next.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_i18next.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ja: (() => {
      const target = join(dir, '../../../blog/ja/intlayer_with_i18next.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_i18next.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ko: (() => {
      const target = join(dir, '../../../blog/ko/intlayer_with_i18next.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_i18next.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    zh: (() => {
      const target = join(dir, '../../../blog/zh/intlayer_with_i18next.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_i18next.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    es: (() => {
      const target = join(dir, '../../../blog/es/intlayer_with_i18next.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_i18next.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    de: (() => {
      const target = join(dir, '../../../blog/de/intlayer_with_i18next.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_i18next.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ar: (() => {
      const target = join(dir, '../../../blog/ar/intlayer_with_i18next.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_i18next.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    pt: (() => {
      const target = join(dir, '../../../blog/pt/intlayer_with_i18next.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_i18next.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    'en-GB': (() => {
      const target = join(dir, '../../../blog/en-GB/intlayer_with_i18next.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_i18next.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    it: (() => {
      const target = join(dir, '../../../blog/it/intlayer_with_i18next.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_i18next.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    hi: (() => {
      const target = join(dir, '../../../blog/hi/intlayer_with_i18next.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_i18next.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    tr: (() => {
      const target = join(dir, '../../../blog/tr/intlayer_with_i18next.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_i18next.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
  } as unknown as Record<LocalesValues, Promise<string>>,
  './blog/en/intlayer_with_next-i18next.md': {
    en: (() => {
      const target = join(
        dir,
        '../../../blog/en/intlayer_with_next-i18next.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_next-i18next.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    fr: (() => {
      const target = join(
        dir,
        '../../../blog/fr/intlayer_with_next-i18next.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_next-i18next.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ru: (() => {
      const target = join(
        dir,
        '../../../blog/ru/intlayer_with_next-i18next.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_next-i18next.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ja: (() => {
      const target = join(
        dir,
        '../../../blog/ja/intlayer_with_next-i18next.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_next-i18next.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ko: (() => {
      const target = join(
        dir,
        '../../../blog/ko/intlayer_with_next-i18next.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_next-i18next.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    zh: (() => {
      const target = join(
        dir,
        '../../../blog/zh/intlayer_with_next-i18next.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_next-i18next.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    es: (() => {
      const target = join(
        dir,
        '../../../blog/es/intlayer_with_next-i18next.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_next-i18next.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    de: (() => {
      const target = join(
        dir,
        '../../../blog/de/intlayer_with_next-i18next.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_next-i18next.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ar: (() => {
      const target = join(
        dir,
        '../../../blog/ar/intlayer_with_next-i18next.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_next-i18next.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    pt: (() => {
      const target = join(
        dir,
        '../../../blog/pt/intlayer_with_next-i18next.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_next-i18next.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    'en-GB': (() => {
      const target = join(
        dir,
        '../../../blog/en-GB/intlayer_with_next-i18next.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_next-i18next.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    it: (() => {
      const target = join(
        dir,
        '../../../blog/it/intlayer_with_next-i18next.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_next-i18next.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    hi: (() => {
      const target = join(
        dir,
        '../../../blog/hi/intlayer_with_next-i18next.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_next-i18next.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    tr: (() => {
      const target = join(
        dir,
        '../../../blog/tr/intlayer_with_next-i18next.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_next-i18next.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
  } as unknown as Record<LocalesValues, Promise<string>>,
  './blog/en/intlayer_with_next-intl.md': {
    en: (() => {
      const target = join(dir, '../../../blog/en/intlayer_with_next-intl.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_next-intl.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    fr: (() => {
      const target = join(dir, '../../../blog/fr/intlayer_with_next-intl.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_next-intl.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ru: (() => {
      const target = join(dir, '../../../blog/ru/intlayer_with_next-intl.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_next-intl.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ja: (() => {
      const target = join(dir, '../../../blog/ja/intlayer_with_next-intl.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_next-intl.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ko: (() => {
      const target = join(dir, '../../../blog/ko/intlayer_with_next-intl.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_next-intl.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    zh: (() => {
      const target = join(dir, '../../../blog/zh/intlayer_with_next-intl.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_next-intl.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    es: (() => {
      const target = join(dir, '../../../blog/es/intlayer_with_next-intl.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_next-intl.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    de: (() => {
      const target = join(dir, '../../../blog/de/intlayer_with_next-intl.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_next-intl.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ar: (() => {
      const target = join(dir, '../../../blog/ar/intlayer_with_next-intl.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_next-intl.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    pt: (() => {
      const target = join(dir, '../../../blog/pt/intlayer_with_next-intl.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_next-intl.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    'en-GB': (() => {
      const target = join(
        dir,
        '../../../blog/en-GB/intlayer_with_next-intl.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_next-intl.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    it: (() => {
      const target = join(dir, '../../../blog/it/intlayer_with_next-intl.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_next-intl.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    hi: (() => {
      const target = join(dir, '../../../blog/hi/intlayer_with_next-intl.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_next-intl.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    tr: (() => {
      const target = join(dir, '../../../blog/tr/intlayer_with_next-intl.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_next-intl.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
  } as unknown as Record<LocalesValues, Promise<string>>,
  './blog/en/intlayer_with_react-i18next.md': {
    en: (() => {
      const target = join(
        dir,
        '../../../blog/en/intlayer_with_react-i18next.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_react-i18next.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    fr: (() => {
      const target = join(
        dir,
        '../../../blog/fr/intlayer_with_react-i18next.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_react-i18next.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ru: (() => {
      const target = join(
        dir,
        '../../../blog/ru/intlayer_with_react-i18next.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_react-i18next.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ja: (() => {
      const target = join(
        dir,
        '../../../blog/ja/intlayer_with_react-i18next.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_react-i18next.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ko: (() => {
      const target = join(
        dir,
        '../../../blog/ko/intlayer_with_react-i18next.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_react-i18next.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    zh: (() => {
      const target = join(
        dir,
        '../../../blog/zh/intlayer_with_react-i18next.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_react-i18next.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    es: (() => {
      const target = join(
        dir,
        '../../../blog/es/intlayer_with_react-i18next.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_react-i18next.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    de: (() => {
      const target = join(
        dir,
        '../../../blog/de/intlayer_with_react-i18next.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_react-i18next.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ar: (() => {
      const target = join(
        dir,
        '../../../blog/ar/intlayer_with_react-i18next.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_react-i18next.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    pt: (() => {
      const target = join(
        dir,
        '../../../blog/pt/intlayer_with_react-i18next.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_react-i18next.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    'en-GB': (() => {
      const target = join(
        dir,
        '../../../blog/en-GB/intlayer_with_react-i18next.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_react-i18next.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    it: (() => {
      const target = join(
        dir,
        '../../../blog/it/intlayer_with_react-i18next.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_react-i18next.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    hi: (() => {
      const target = join(
        dir,
        '../../../blog/hi/intlayer_with_react-i18next.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_react-i18next.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    tr: (() => {
      const target = join(
        dir,
        '../../../blog/tr/intlayer_with_react-i18next.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_react-i18next.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
  } as unknown as Record<LocalesValues, Promise<string>>,
  './blog/en/intlayer_with_react-intl.md': {
    en: (() => {
      const target = join(dir, '../../../blog/en/intlayer_with_react-intl.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_react-intl.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    fr: (() => {
      const target = join(dir, '../../../blog/fr/intlayer_with_react-intl.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_react-intl.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ru: (() => {
      const target = join(dir, '../../../blog/ru/intlayer_with_react-intl.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_react-intl.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ja: (() => {
      const target = join(dir, '../../../blog/ja/intlayer_with_react-intl.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_react-intl.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ko: (() => {
      const target = join(dir, '../../../blog/ko/intlayer_with_react-intl.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_react-intl.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    zh: (() => {
      const target = join(dir, '../../../blog/zh/intlayer_with_react-intl.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_react-intl.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    es: (() => {
      const target = join(dir, '../../../blog/es/intlayer_with_react-intl.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_react-intl.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    de: (() => {
      const target = join(dir, '../../../blog/de/intlayer_with_react-intl.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_react-intl.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ar: (() => {
      const target = join(dir, '../../../blog/ar/intlayer_with_react-intl.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_react-intl.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    pt: (() => {
      const target = join(dir, '../../../blog/pt/intlayer_with_react-intl.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_react-intl.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    'en-GB': (() => {
      const target = join(
        dir,
        '../../../blog/en-GB/intlayer_with_react-intl.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_react-intl.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    it: (() => {
      const target = join(dir, '../../../blog/it/intlayer_with_react-intl.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_react-intl.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    hi: (() => {
      const target = join(dir, '../../../blog/hi/intlayer_with_react-intl.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_react-intl.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    tr: (() => {
      const target = join(dir, '../../../blog/tr/intlayer_with_react-intl.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/intlayer_with_react-intl.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
  } as unknown as Record<LocalesValues, Promise<string>>,
  './blog/en/list_i18n_technologies/CMS/drupal.md': {
    en: (() => {
      const target = join(
        dir,
        '../../../blog/en/list_i18n_technologies/CMS/drupal.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/list_i18n_technologies/CMS/drupal.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    fr: (() => {
      const target = join(
        dir,
        '../../../blog/fr/list_i18n_technologies/CMS/drupal.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/list_i18n_technologies/CMS/drupal.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ru: (() => {
      const target = join(
        dir,
        '../../../blog/ru/list_i18n_technologies/CMS/drupal.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/list_i18n_technologies/CMS/drupal.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ja: (() => {
      const target = join(
        dir,
        '../../../blog/ja/list_i18n_technologies/CMS/drupal.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/list_i18n_technologies/CMS/drupal.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ko: (() => {
      const target = join(
        dir,
        '../../../blog/ko/list_i18n_technologies/CMS/drupal.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/list_i18n_technologies/CMS/drupal.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    zh: (() => {
      const target = join(
        dir,
        '../../../blog/zh/list_i18n_technologies/CMS/drupal.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/list_i18n_technologies/CMS/drupal.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    es: (() => {
      const target = join(
        dir,
        '../../../blog/es/list_i18n_technologies/CMS/drupal.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/list_i18n_technologies/CMS/drupal.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    de: (() => {
      const target = join(
        dir,
        '../../../blog/de/list_i18n_technologies/CMS/drupal.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/list_i18n_technologies/CMS/drupal.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ar: (() => {
      const target = join(
        dir,
        '../../../blog/ar/list_i18n_technologies/CMS/drupal.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/list_i18n_technologies/CMS/drupal.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    pt: (() => {
      const target = join(
        dir,
        '../../../blog/pt/list_i18n_technologies/CMS/drupal.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/list_i18n_technologies/CMS/drupal.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    'en-GB': (() => {
      const target = join(
        dir,
        '../../../blog/en-GB/list_i18n_technologies/CMS/drupal.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/list_i18n_technologies/CMS/drupal.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    it: (() => {
      const target = join(
        dir,
        '../../../blog/it/list_i18n_technologies/CMS/drupal.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/list_i18n_technologies/CMS/drupal.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    hi: (() => {
      const target = join(
        dir,
        '../../../blog/hi/list_i18n_technologies/CMS/drupal.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/list_i18n_technologies/CMS/drupal.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    tr: (() => {
      const target = join(
        dir,
        '../../../blog/tr/list_i18n_technologies/CMS/drupal.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/list_i18n_technologies/CMS/drupal.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
  } as unknown as Record<LocalesValues, Promise<string>>,
  './blog/en/list_i18n_technologies/CMS/wix.md': {
    en: (() => {
      const target = join(
        dir,
        '../../../blog/en/list_i18n_technologies/CMS/wix.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/list_i18n_technologies/CMS/wix.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    fr: (() => {
      const target = join(
        dir,
        '../../../blog/fr/list_i18n_technologies/CMS/wix.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/list_i18n_technologies/CMS/wix.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ru: (() => {
      const target = join(
        dir,
        '../../../blog/ru/list_i18n_technologies/CMS/wix.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/list_i18n_technologies/CMS/wix.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ja: (() => {
      const target = join(
        dir,
        '../../../blog/ja/list_i18n_technologies/CMS/wix.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/list_i18n_technologies/CMS/wix.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ko: (() => {
      const target = join(
        dir,
        '../../../blog/ko/list_i18n_technologies/CMS/wix.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/list_i18n_technologies/CMS/wix.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    zh: (() => {
      const target = join(
        dir,
        '../../../blog/zh/list_i18n_technologies/CMS/wix.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/list_i18n_technologies/CMS/wix.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    es: (() => {
      const target = join(
        dir,
        '../../../blog/es/list_i18n_technologies/CMS/wix.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/list_i18n_technologies/CMS/wix.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    de: (() => {
      const target = join(
        dir,
        '../../../blog/de/list_i18n_technologies/CMS/wix.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/list_i18n_technologies/CMS/wix.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ar: (() => {
      const target = join(
        dir,
        '../../../blog/ar/list_i18n_technologies/CMS/wix.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/list_i18n_technologies/CMS/wix.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    pt: (() => {
      const target = join(
        dir,
        '../../../blog/pt/list_i18n_technologies/CMS/wix.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/list_i18n_technologies/CMS/wix.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    'en-GB': (() => {
      const target = join(
        dir,
        '../../../blog/en-GB/list_i18n_technologies/CMS/wix.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/list_i18n_technologies/CMS/wix.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    it: (() => {
      const target = join(
        dir,
        '../../../blog/it/list_i18n_technologies/CMS/wix.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/list_i18n_technologies/CMS/wix.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    hi: (() => {
      const target = join(
        dir,
        '../../../blog/hi/list_i18n_technologies/CMS/wix.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/list_i18n_technologies/CMS/wix.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    tr: (() => {
      const target = join(
        dir,
        '../../../blog/tr/list_i18n_technologies/CMS/wix.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/list_i18n_technologies/CMS/wix.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
  } as unknown as Record<LocalesValues, Promise<string>>,
  './blog/en/list_i18n_technologies/CMS/wordpress.md': {
    en: (() => {
      const target = join(
        dir,
        '../../../blog/en/list_i18n_technologies/CMS/wordpress.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/CMS/wordpress.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    fr: (() => {
      const target = join(
        dir,
        '../../../blog/fr/list_i18n_technologies/CMS/wordpress.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/CMS/wordpress.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ru: (() => {
      const target = join(
        dir,
        '../../../blog/ru/list_i18n_technologies/CMS/wordpress.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/CMS/wordpress.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ja: (() => {
      const target = join(
        dir,
        '../../../blog/ja/list_i18n_technologies/CMS/wordpress.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/CMS/wordpress.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ko: (() => {
      const target = join(
        dir,
        '../../../blog/ko/list_i18n_technologies/CMS/wordpress.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/CMS/wordpress.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    zh: (() => {
      const target = join(
        dir,
        '../../../blog/zh/list_i18n_technologies/CMS/wordpress.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/CMS/wordpress.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    es: (() => {
      const target = join(
        dir,
        '../../../blog/es/list_i18n_technologies/CMS/wordpress.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/CMS/wordpress.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    de: (() => {
      const target = join(
        dir,
        '../../../blog/de/list_i18n_technologies/CMS/wordpress.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/CMS/wordpress.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ar: (() => {
      const target = join(
        dir,
        '../../../blog/ar/list_i18n_technologies/CMS/wordpress.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/CMS/wordpress.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    pt: (() => {
      const target = join(
        dir,
        '../../../blog/pt/list_i18n_technologies/CMS/wordpress.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/CMS/wordpress.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    'en-GB': (() => {
      const target = join(
        dir,
        '../../../blog/en-GB/list_i18n_technologies/CMS/wordpress.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/CMS/wordpress.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    it: (() => {
      const target = join(
        dir,
        '../../../blog/it/list_i18n_technologies/CMS/wordpress.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/CMS/wordpress.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    hi: (() => {
      const target = join(
        dir,
        '../../../blog/hi/list_i18n_technologies/CMS/wordpress.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/CMS/wordpress.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    tr: (() => {
      const target = join(
        dir,
        '../../../blog/tr/list_i18n_technologies/CMS/wordpress.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/CMS/wordpress.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
  } as unknown as Record<LocalesValues, Promise<string>>,
  './blog/en/list_i18n_technologies/frameworks/angular.md': {
    en: (() => {
      const target = join(
        dir,
        '../../../blog/en/list_i18n_technologies/frameworks/angular.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/angular.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    fr: (() => {
      const target = join(
        dir,
        '../../../blog/fr/list_i18n_technologies/frameworks/angular.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/angular.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ru: (() => {
      const target = join(
        dir,
        '../../../blog/ru/list_i18n_technologies/frameworks/angular.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/angular.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ja: (() => {
      const target = join(
        dir,
        '../../../blog/ja/list_i18n_technologies/frameworks/angular.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/angular.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ko: (() => {
      const target = join(
        dir,
        '../../../blog/ko/list_i18n_technologies/frameworks/angular.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/angular.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    zh: (() => {
      const target = join(
        dir,
        '../../../blog/zh/list_i18n_technologies/frameworks/angular.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/angular.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    es: (() => {
      const target = join(
        dir,
        '../../../blog/es/list_i18n_technologies/frameworks/angular.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/angular.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    de: (() => {
      const target = join(
        dir,
        '../../../blog/de/list_i18n_technologies/frameworks/angular.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/angular.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ar: (() => {
      const target = join(
        dir,
        '../../../blog/ar/list_i18n_technologies/frameworks/angular.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/angular.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    pt: (() => {
      const target = join(
        dir,
        '../../../blog/pt/list_i18n_technologies/frameworks/angular.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/angular.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    'en-GB': (() => {
      const target = join(
        dir,
        '../../../blog/en-GB/list_i18n_technologies/frameworks/angular.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/angular.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    it: (() => {
      const target = join(
        dir,
        '../../../blog/it/list_i18n_technologies/frameworks/angular.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/angular.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    hi: (() => {
      const target = join(
        dir,
        '../../../blog/hi/list_i18n_technologies/frameworks/angular.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/angular.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    tr: (() => {
      const target = join(
        dir,
        '../../../blog/tr/list_i18n_technologies/frameworks/angular.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/angular.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
  } as unknown as Record<LocalesValues, Promise<string>>,
  './blog/en/list_i18n_technologies/frameworks/flutter.md': {
    en: (() => {
      const target = join(
        dir,
        '../../../blog/en/list_i18n_technologies/frameworks/flutter.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/flutter.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    fr: (() => {
      const target = join(
        dir,
        '../../../blog/fr/list_i18n_technologies/frameworks/flutter.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/flutter.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ru: (() => {
      const target = join(
        dir,
        '../../../blog/ru/list_i18n_technologies/frameworks/flutter.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/flutter.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ja: (() => {
      const target = join(
        dir,
        '../../../blog/ja/list_i18n_technologies/frameworks/flutter.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/flutter.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ko: (() => {
      const target = join(
        dir,
        '../../../blog/ko/list_i18n_technologies/frameworks/flutter.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/flutter.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    zh: (() => {
      const target = join(
        dir,
        '../../../blog/zh/list_i18n_technologies/frameworks/flutter.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/flutter.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    es: (() => {
      const target = join(
        dir,
        '../../../blog/es/list_i18n_technologies/frameworks/flutter.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/flutter.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    de: (() => {
      const target = join(
        dir,
        '../../../blog/de/list_i18n_technologies/frameworks/flutter.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/flutter.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ar: (() => {
      const target = join(
        dir,
        '../../../blog/ar/list_i18n_technologies/frameworks/flutter.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/flutter.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    pt: (() => {
      const target = join(
        dir,
        '../../../blog/pt/list_i18n_technologies/frameworks/flutter.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/flutter.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    'en-GB': (() => {
      const target = join(
        dir,
        '../../../blog/en-GB/list_i18n_technologies/frameworks/flutter.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/flutter.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    it: (() => {
      const target = join(
        dir,
        '../../../blog/it/list_i18n_technologies/frameworks/flutter.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/flutter.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    hi: (() => {
      const target = join(
        dir,
        '../../../blog/hi/list_i18n_technologies/frameworks/flutter.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/flutter.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    tr: (() => {
      const target = join(
        dir,
        '../../../blog/tr/list_i18n_technologies/frameworks/flutter.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/flutter.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
  } as unknown as Record<LocalesValues, Promise<string>>,
  './blog/en/list_i18n_technologies/frameworks/react-native.md': {
    en: (() => {
      const target = join(
        dir,
        '../../../blog/en/list_i18n_technologies/frameworks/react-native.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/react-native.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    fr: (() => {
      const target = join(
        dir,
        '../../../blog/fr/list_i18n_technologies/frameworks/react-native.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/react-native.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ru: (() => {
      const target = join(
        dir,
        '../../../blog/ru/list_i18n_technologies/frameworks/react-native.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/react-native.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ja: (() => {
      const target = join(
        dir,
        '../../../blog/ja/list_i18n_technologies/frameworks/react-native.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/react-native.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ko: (() => {
      const target = join(
        dir,
        '../../../blog/ko/list_i18n_technologies/frameworks/react-native.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/react-native.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    zh: (() => {
      const target = join(
        dir,
        '../../../blog/zh/list_i18n_technologies/frameworks/react-native.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/react-native.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    es: (() => {
      const target = join(
        dir,
        '../../../blog/es/list_i18n_technologies/frameworks/react-native.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/react-native.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    de: (() => {
      const target = join(
        dir,
        '../../../blog/de/list_i18n_technologies/frameworks/react-native.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/react-native.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ar: (() => {
      const target = join(
        dir,
        '../../../blog/ar/list_i18n_technologies/frameworks/react-native.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/react-native.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    pt: (() => {
      const target = join(
        dir,
        '../../../blog/pt/list_i18n_technologies/frameworks/react-native.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/react-native.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    'en-GB': (() => {
      const target = join(
        dir,
        '../../../blog/en-GB/list_i18n_technologies/frameworks/react-native.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/react-native.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    it: (() => {
      const target = join(
        dir,
        '../../../blog/it/list_i18n_technologies/frameworks/react-native.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/react-native.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    hi: (() => {
      const target = join(
        dir,
        '../../../blog/hi/list_i18n_technologies/frameworks/react-native.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/react-native.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    tr: (() => {
      const target = join(
        dir,
        '../../../blog/tr/list_i18n_technologies/frameworks/react-native.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/react-native.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
  } as unknown as Record<LocalesValues, Promise<string>>,
  './blog/en/list_i18n_technologies/frameworks/react.md': {
    en: (() => {
      const target = join(
        dir,
        '../../../blog/en/list_i18n_technologies/frameworks/react.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/react.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    fr: (() => {
      const target = join(
        dir,
        '../../../blog/fr/list_i18n_technologies/frameworks/react.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/react.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ru: (() => {
      const target = join(
        dir,
        '../../../blog/ru/list_i18n_technologies/frameworks/react.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/react.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ja: (() => {
      const target = join(
        dir,
        '../../../blog/ja/list_i18n_technologies/frameworks/react.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/react.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ko: (() => {
      const target = join(
        dir,
        '../../../blog/ko/list_i18n_technologies/frameworks/react.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/react.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    zh: (() => {
      const target = join(
        dir,
        '../../../blog/zh/list_i18n_technologies/frameworks/react.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/react.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    es: (() => {
      const target = join(
        dir,
        '../../../blog/es/list_i18n_technologies/frameworks/react.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/react.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    de: (() => {
      const target = join(
        dir,
        '../../../blog/de/list_i18n_technologies/frameworks/react.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/react.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ar: (() => {
      const target = join(
        dir,
        '../../../blog/ar/list_i18n_technologies/frameworks/react.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/react.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    pt: (() => {
      const target = join(
        dir,
        '../../../blog/pt/list_i18n_technologies/frameworks/react.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/react.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    'en-GB': (() => {
      const target = join(
        dir,
        '../../../blog/en-GB/list_i18n_technologies/frameworks/react.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/react.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    it: (() => {
      const target = join(
        dir,
        '../../../blog/it/list_i18n_technologies/frameworks/react.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/react.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    hi: (() => {
      const target = join(
        dir,
        '../../../blog/hi/list_i18n_technologies/frameworks/react.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/react.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    tr: (() => {
      const target = join(
        dir,
        '../../../blog/tr/list_i18n_technologies/frameworks/react.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/react.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
  } as unknown as Record<LocalesValues, Promise<string>>,
  './blog/en/list_i18n_technologies/frameworks/svelte.md': {
    en: (() => {
      const target = join(
        dir,
        '../../../blog/en/list_i18n_technologies/frameworks/svelte.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/svelte.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    fr: (() => {
      const target = join(
        dir,
        '../../../blog/fr/list_i18n_technologies/frameworks/svelte.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/svelte.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ru: (() => {
      const target = join(
        dir,
        '../../../blog/ru/list_i18n_technologies/frameworks/svelte.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/svelte.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ja: (() => {
      const target = join(
        dir,
        '../../../blog/ja/list_i18n_technologies/frameworks/svelte.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/svelte.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ko: (() => {
      const target = join(
        dir,
        '../../../blog/ko/list_i18n_technologies/frameworks/svelte.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/svelte.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    zh: (() => {
      const target = join(
        dir,
        '../../../blog/zh/list_i18n_technologies/frameworks/svelte.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/svelte.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    es: (() => {
      const target = join(
        dir,
        '../../../blog/es/list_i18n_technologies/frameworks/svelte.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/svelte.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    de: (() => {
      const target = join(
        dir,
        '../../../blog/de/list_i18n_technologies/frameworks/svelte.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/svelte.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ar: (() => {
      const target = join(
        dir,
        '../../../blog/ar/list_i18n_technologies/frameworks/svelte.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/svelte.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    pt: (() => {
      const target = join(
        dir,
        '../../../blog/pt/list_i18n_technologies/frameworks/svelte.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/svelte.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    'en-GB': (() => {
      const target = join(
        dir,
        '../../../blog/en-GB/list_i18n_technologies/frameworks/svelte.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/svelte.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    it: (() => {
      const target = join(
        dir,
        '../../../blog/it/list_i18n_technologies/frameworks/svelte.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/svelte.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    hi: (() => {
      const target = join(
        dir,
        '../../../blog/hi/list_i18n_technologies/frameworks/svelte.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/svelte.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    tr: (() => {
      const target = join(
        dir,
        '../../../blog/tr/list_i18n_technologies/frameworks/svelte.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/svelte.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
  } as unknown as Record<LocalesValues, Promise<string>>,
  './blog/en/list_i18n_technologies/frameworks/vue.md': {
    en: (() => {
      const target = join(
        dir,
        '../../../blog/en/list_i18n_technologies/frameworks/vue.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/vue.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    fr: (() => {
      const target = join(
        dir,
        '../../../blog/fr/list_i18n_technologies/frameworks/vue.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/vue.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ru: (() => {
      const target = join(
        dir,
        '../../../blog/ru/list_i18n_technologies/frameworks/vue.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/vue.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ja: (() => {
      const target = join(
        dir,
        '../../../blog/ja/list_i18n_technologies/frameworks/vue.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/vue.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ko: (() => {
      const target = join(
        dir,
        '../../../blog/ko/list_i18n_technologies/frameworks/vue.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/vue.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    zh: (() => {
      const target = join(
        dir,
        '../../../blog/zh/list_i18n_technologies/frameworks/vue.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/vue.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    es: (() => {
      const target = join(
        dir,
        '../../../blog/es/list_i18n_technologies/frameworks/vue.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/vue.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    de: (() => {
      const target = join(
        dir,
        '../../../blog/de/list_i18n_technologies/frameworks/vue.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/vue.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ar: (() => {
      const target = join(
        dir,
        '../../../blog/ar/list_i18n_technologies/frameworks/vue.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/vue.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    pt: (() => {
      const target = join(
        dir,
        '../../../blog/pt/list_i18n_technologies/frameworks/vue.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/vue.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    'en-GB': (() => {
      const target = join(
        dir,
        '../../../blog/en-GB/list_i18n_technologies/frameworks/vue.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/vue.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    it: (() => {
      const target = join(
        dir,
        '../../../blog/it/list_i18n_technologies/frameworks/vue.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/vue.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    hi: (() => {
      const target = join(
        dir,
        '../../../blog/hi/list_i18n_technologies/frameworks/vue.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/vue.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    tr: (() => {
      const target = join(
        dir,
        '../../../blog/tr/list_i18n_technologies/frameworks/vue.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/list_i18n_technologies/frameworks/vue.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
  } as unknown as Record<LocalesValues, Promise<string>>,
  './blog/en/next-i18next_vs_next-intl_vs_intlayer.md': {
    en: (() => {
      const target = join(
        dir,
        '../../../blog/en/next-i18next_vs_next-intl_vs_intlayer.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/next-i18next_vs_next-intl_vs_intlayer.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    fr: (() => {
      const target = join(
        dir,
        '../../../blog/fr/next-i18next_vs_next-intl_vs_intlayer.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/next-i18next_vs_next-intl_vs_intlayer.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ru: (() => {
      const target = join(
        dir,
        '../../../blog/ru/next-i18next_vs_next-intl_vs_intlayer.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/next-i18next_vs_next-intl_vs_intlayer.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ja: (() => {
      const target = join(
        dir,
        '../../../blog/ja/next-i18next_vs_next-intl_vs_intlayer.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/next-i18next_vs_next-intl_vs_intlayer.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ko: (() => {
      const target = join(
        dir,
        '../../../blog/ko/next-i18next_vs_next-intl_vs_intlayer.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/next-i18next_vs_next-intl_vs_intlayer.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    zh: (() => {
      const target = join(
        dir,
        '../../../blog/zh/next-i18next_vs_next-intl_vs_intlayer.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/next-i18next_vs_next-intl_vs_intlayer.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    es: (() => {
      const target = join(
        dir,
        '../../../blog/es/next-i18next_vs_next-intl_vs_intlayer.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/next-i18next_vs_next-intl_vs_intlayer.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    de: (() => {
      const target = join(
        dir,
        '../../../blog/de/next-i18next_vs_next-intl_vs_intlayer.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/next-i18next_vs_next-intl_vs_intlayer.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ar: (() => {
      const target = join(
        dir,
        '../../../blog/ar/next-i18next_vs_next-intl_vs_intlayer.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/next-i18next_vs_next-intl_vs_intlayer.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    pt: (() => {
      const target = join(
        dir,
        '../../../blog/pt/next-i18next_vs_next-intl_vs_intlayer.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/next-i18next_vs_next-intl_vs_intlayer.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    'en-GB': (() => {
      const target = join(
        dir,
        '../../../blog/en-GB/next-i18next_vs_next-intl_vs_intlayer.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/next-i18next_vs_next-intl_vs_intlayer.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    it: (() => {
      const target = join(
        dir,
        '../../../blog/it/next-i18next_vs_next-intl_vs_intlayer.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/next-i18next_vs_next-intl_vs_intlayer.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    hi: (() => {
      const target = join(
        dir,
        '../../../blog/hi/next-i18next_vs_next-intl_vs_intlayer.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/next-i18next_vs_next-intl_vs_intlayer.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    tr: (() => {
      const target = join(
        dir,
        '../../../blog/tr/next-i18next_vs_next-intl_vs_intlayer.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/next-i18next_vs_next-intl_vs_intlayer.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
  } as unknown as Record<LocalesValues, Promise<string>>,
  './blog/en/rag_powered_documentation_assistant.md': {
    en: (() => {
      const target = join(
        dir,
        '../../../blog/en/rag_powered_documentation_assistant.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/rag_powered_documentation_assistant.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    fr: (() => {
      const target = join(
        dir,
        '../../../blog/fr/rag_powered_documentation_assistant.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/rag_powered_documentation_assistant.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ru: (() => {
      const target = join(
        dir,
        '../../../blog/ru/rag_powered_documentation_assistant.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/rag_powered_documentation_assistant.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ja: (() => {
      const target = join(
        dir,
        '../../../blog/ja/rag_powered_documentation_assistant.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/rag_powered_documentation_assistant.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ko: (() => {
      const target = join(
        dir,
        '../../../blog/ko/rag_powered_documentation_assistant.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/rag_powered_documentation_assistant.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    zh: (() => {
      const target = join(
        dir,
        '../../../blog/zh/rag_powered_documentation_assistant.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/rag_powered_documentation_assistant.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    es: (() => {
      const target = join(
        dir,
        '../../../blog/es/rag_powered_documentation_assistant.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/rag_powered_documentation_assistant.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    de: (() => {
      const target = join(
        dir,
        '../../../blog/de/rag_powered_documentation_assistant.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/rag_powered_documentation_assistant.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ar: (() => {
      const target = join(
        dir,
        '../../../blog/ar/rag_powered_documentation_assistant.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/rag_powered_documentation_assistant.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    pt: (() => {
      const target = join(
        dir,
        '../../../blog/pt/rag_powered_documentation_assistant.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/rag_powered_documentation_assistant.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    'en-GB': (() => {
      const target = join(
        dir,
        '../../../blog/en-GB/rag_powered_documentation_assistant.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/rag_powered_documentation_assistant.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    it: (() => {
      const target = join(
        dir,
        '../../../blog/it/rag_powered_documentation_assistant.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/rag_powered_documentation_assistant.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    hi: (() => {
      const target = join(
        dir,
        '../../../blog/hi/rag_powered_documentation_assistant.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/rag_powered_documentation_assistant.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    tr: (() => {
      const target = join(
        dir,
        '../../../blog/tr/rag_powered_documentation_assistant.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/rag_powered_documentation_assistant.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
  } as unknown as Record<LocalesValues, Promise<string>>,
  './blog/en/react-i18next_vs_react-intl_vs_intlayer.md': {
    en: (() => {
      const target = join(
        dir,
        '../../../blog/en/react-i18next_vs_react-intl_vs_intlayer.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/react-i18next_vs_react-intl_vs_intlayer.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    fr: (() => {
      const target = join(
        dir,
        '../../../blog/fr/react-i18next_vs_react-intl_vs_intlayer.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/react-i18next_vs_react-intl_vs_intlayer.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ru: (() => {
      const target = join(
        dir,
        '../../../blog/ru/react-i18next_vs_react-intl_vs_intlayer.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/react-i18next_vs_react-intl_vs_intlayer.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ja: (() => {
      const target = join(
        dir,
        '../../../blog/ja/react-i18next_vs_react-intl_vs_intlayer.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/react-i18next_vs_react-intl_vs_intlayer.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ko: (() => {
      const target = join(
        dir,
        '../../../blog/ko/react-i18next_vs_react-intl_vs_intlayer.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/react-i18next_vs_react-intl_vs_intlayer.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    zh: (() => {
      const target = join(
        dir,
        '../../../blog/zh/react-i18next_vs_react-intl_vs_intlayer.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/react-i18next_vs_react-intl_vs_intlayer.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    es: (() => {
      const target = join(
        dir,
        '../../../blog/es/react-i18next_vs_react-intl_vs_intlayer.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/react-i18next_vs_react-intl_vs_intlayer.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    de: (() => {
      const target = join(
        dir,
        '../../../blog/de/react-i18next_vs_react-intl_vs_intlayer.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/react-i18next_vs_react-intl_vs_intlayer.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ar: (() => {
      const target = join(
        dir,
        '../../../blog/ar/react-i18next_vs_react-intl_vs_intlayer.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/react-i18next_vs_react-intl_vs_intlayer.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    pt: (() => {
      const target = join(
        dir,
        '../../../blog/pt/react-i18next_vs_react-intl_vs_intlayer.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/react-i18next_vs_react-intl_vs_intlayer.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    'en-GB': (() => {
      const target = join(
        dir,
        '../../../blog/en-GB/react-i18next_vs_react-intl_vs_intlayer.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/react-i18next_vs_react-intl_vs_intlayer.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    it: (() => {
      const target = join(
        dir,
        '../../../blog/it/react-i18next_vs_react-intl_vs_intlayer.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/react-i18next_vs_react-intl_vs_intlayer.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    hi: (() => {
      const target = join(
        dir,
        '../../../blog/hi/react-i18next_vs_react-intl_vs_intlayer.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/react-i18next_vs_react-intl_vs_intlayer.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    tr: (() => {
      const target = join(
        dir,
        '../../../blog/tr/react-i18next_vs_react-intl_vs_intlayer.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(
              dir,
              '../../../blog/en/react-i18next_vs_react-intl_vs_intlayer.md'
            ),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
  } as unknown as Record<LocalesValues, Promise<string>>,
  './blog/en/vue-i18n_vs_intlayer.md': {
    en: (() => {
      const target = join(dir, '../../../blog/en/vue-i18n_vs_intlayer.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/vue-i18n_vs_intlayer.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    fr: (() => {
      const target = join(dir, '../../../blog/fr/vue-i18n_vs_intlayer.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/vue-i18n_vs_intlayer.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ru: (() => {
      const target = join(dir, '../../../blog/ru/vue-i18n_vs_intlayer.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/vue-i18n_vs_intlayer.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ja: (() => {
      const target = join(dir, '../../../blog/ja/vue-i18n_vs_intlayer.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/vue-i18n_vs_intlayer.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ko: (() => {
      const target = join(dir, '../../../blog/ko/vue-i18n_vs_intlayer.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/vue-i18n_vs_intlayer.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    zh: (() => {
      const target = join(dir, '../../../blog/zh/vue-i18n_vs_intlayer.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/vue-i18n_vs_intlayer.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    es: (() => {
      const target = join(dir, '../../../blog/es/vue-i18n_vs_intlayer.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/vue-i18n_vs_intlayer.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    de: (() => {
      const target = join(dir, '../../../blog/de/vue-i18n_vs_intlayer.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/vue-i18n_vs_intlayer.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ar: (() => {
      const target = join(dir, '../../../blog/ar/vue-i18n_vs_intlayer.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/vue-i18n_vs_intlayer.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    pt: (() => {
      const target = join(dir, '../../../blog/pt/vue-i18n_vs_intlayer.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/vue-i18n_vs_intlayer.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    'en-GB': (() => {
      const target = join(dir, '../../../blog/en-GB/vue-i18n_vs_intlayer.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/vue-i18n_vs_intlayer.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    it: (() => {
      const target = join(dir, '../../../blog/it/vue-i18n_vs_intlayer.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/vue-i18n_vs_intlayer.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    hi: (() => {
      const target = join(dir, '../../../blog/hi/vue-i18n_vs_intlayer.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/vue-i18n_vs_intlayer.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    tr: (() => {
      const target = join(dir, '../../../blog/tr/vue-i18n_vs_intlayer.md');
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/vue-i18n_vs_intlayer.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
  } as unknown as Record<LocalesValues, Promise<string>>,
  './blog/en/what_is_internationalization.md': {
    en: (() => {
      const target = join(
        dir,
        '../../../blog/en/what_is_internationalization.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/what_is_internationalization.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    fr: (() => {
      const target = join(
        dir,
        '../../../blog/fr/what_is_internationalization.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/what_is_internationalization.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ru: (() => {
      const target = join(
        dir,
        '../../../blog/ru/what_is_internationalization.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/what_is_internationalization.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ja: (() => {
      const target = join(
        dir,
        '../../../blog/ja/what_is_internationalization.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/what_is_internationalization.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ko: (() => {
      const target = join(
        dir,
        '../../../blog/ko/what_is_internationalization.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/what_is_internationalization.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    zh: (() => {
      const target = join(
        dir,
        '../../../blog/zh/what_is_internationalization.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/what_is_internationalization.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    es: (() => {
      const target = join(
        dir,
        '../../../blog/es/what_is_internationalization.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/what_is_internationalization.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    de: (() => {
      const target = join(
        dir,
        '../../../blog/de/what_is_internationalization.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/what_is_internationalization.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    ar: (() => {
      const target = join(
        dir,
        '../../../blog/ar/what_is_internationalization.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/what_is_internationalization.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    pt: (() => {
      const target = join(
        dir,
        '../../../blog/pt/what_is_internationalization.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/what_is_internationalization.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    'en-GB': (() => {
      const target = join(
        dir,
        '../../../blog/en-GB/what_is_internationalization.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/what_is_internationalization.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    it: (() => {
      const target = join(
        dir,
        '../../../blog/it/what_is_internationalization.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/what_is_internationalization.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    hi: (() => {
      const target = join(
        dir,
        '../../../blog/hi/what_is_internationalization.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/what_is_internationalization.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
    tr: (() => {
      const target = join(
        dir,
        '../../../blog/tr/what_is_internationalization.md'
      );
      if (!existsSync(target)) {
        console.error('File not found: ' + target);
        return Promise.resolve(
          readFile(
            join(dir, '../../../blog/en/what_is_internationalization.md'),
            'utf8'
          )
        );
      }
      return Promise.resolve(readFile(target, 'utf8'));
    })(),
  } as unknown as Record<LocalesValues, Promise<string>>,
} as const;
