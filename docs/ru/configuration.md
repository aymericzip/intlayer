...
По умолчанию, когда словарь загружается, он импортирует содержимое для всех локалей.
Если эта опция установлена в true, только содержимое словаря текущей локали
будет получено через динамический импорт. В этом случае Intlayer заменит все
вызовы `useIntlayer` на `useDynamicDictionary`.
...
- **traversePattern**:
  - _Тип_: `string[]`
  - _По умолчанию_: `['**/*.{js,ts,mjs,cjs,jsx,tsx,mjx,cjx,vue,svelte,svte}', '!**/node_modules/**']`
  ...
  - _Примечание_: Эта опция будет проигнорирована, если `optimize` отключен.
  - _Примечание_: Используйте glob шаблон.
