...
- **strictMode**:
  ...
  - _Примечание_: Если установлено на "inclusive", функция перевода `t` потребует от каждой объявленной локали быть определенной. Если одна локаль отсутствует, это вызовет предупреждение. Но будет принято, если локаль не объявленная в вашей конфигурации, но существует.
