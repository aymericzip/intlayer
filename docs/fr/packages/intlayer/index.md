...
Cette commande scanne tous les fichiers `*.content.*`, les compile et écrit les résultats dans le répertoire spécifié dans votre fichier **`intlayer.config.ts`** (par défaut, `./.intlayer`).
