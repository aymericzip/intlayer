...
1. URL path prefix (e.g. `/fr/about` → `fr`).
...
- Requests whose path ends with a file extension (static assets). If a locale prefix is present on a static asset path (e.g. `/fr/logo.png`), it is stripped so the file can be served correctly.
