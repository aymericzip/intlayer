...
tsximport { createServerFn } from '@tanstack/react-start'
import {
...
tsxconst getGreeting = createServerFn().handler(async () => {
return renderToReadableStream(<h1>Hello from the server</h1>)
})
...
Server#
tsximport { createCompositeComponent } from '@tanstack/react-start/rsc'
...
Client#
tsximport { CompositeComponent } from '@tanstack/react-start/rsc'
