...
export const Route = createFileRoute('/{-$locale}/intlayer-message-formatter')({
  loader: async ({ params }) => {
    const { locale = defaultLocale } = params;

    const [metadata, siteStructuredData, softwareStructuredData] =
      await Promise.all([
        getIntlayerAsync('intlayer-message-formatter-metadata', locale),
  ...
  head: ({ params, loaderData }) => {
    ...
    const { locale = defaultLocale } = params;
    const path = Website_IntlayerFormatter_Path;
        ...
        ...getSiteStructuredDataScripts(siteStructuredData),
        {
          ...
          children: JSON.stringify(
            buildSoftwareApplicationJsonLd({
              name: 'Intlayer Message Formatter & Editor',
              url: `${Website_Home}/intlayer-message-formatter`,
              ...
              operatingSystem: 'Web',
              mainEntityUrl: `${Website_Home}/intlayer-message-formatter`,
              offersPrice: '0.00',
            })
          ),
        },
  ...
  component: IntlayerFormatterRoute,
});

function IntlayerFormatterRoute() {
  return (
    <PageLayout>
      <MessageFormatterPage dialect="intlayer" />
    </PageLayout>
  );
}
