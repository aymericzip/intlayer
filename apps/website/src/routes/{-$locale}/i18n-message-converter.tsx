...
import { MessageConverterPage } from '~/components/MessageConverterPage';
...
export const Route = createFileRoute('/{-$locale}/i18n-message-converter')({
  loader: async ({ params }) => {
    const { locale = defaultLocale } = params;

    const [metadata, siteStructuredData, softwareStructuredData] =
      await Promise.all([
        getIntlayerAsync('i18n-message-converter-metadata', locale),
  ...
  head: ({ params, loaderData }) => {
    ...
    const { locale = defaultLocale } = params;
    const path = Website_MessageConverter_Path;
        ...
        ...getSiteStructuredDataScripts(siteStructuredData),
        {
          ...
          children: JSON.stringify(
            buildSoftwareApplicationJsonLd({
              name: 'Intlayer i18n Message Format Converter',
              url: `${Website_Home}/i18n-message-converter`,
              ...
              operatingSystem: 'Web',
              mainEntityUrl: `${Website_Home}/i18n-message-converter`,
              offersPrice: '0.00',
            })
          ),
        },
  ...
  component: MessageConverterRoute,
});

function MessageConverterRoute() {
  return (
    <PageLayout>
      <MessageConverterPage />
    </PageLayout>
  );
}
