    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <div className="mt-1 text-foreground/70 text-sm">{description}</div>
        </div>
      </div>
...
export const SSRSection: FC = () => {
  const { title, description, server, client } = useIntlayer(
    'ssr-section-markdown'
  );
    ...
    <section className="mx-auto max-w-6xl px-4 py-24 md:px-8 lg:px-12">
      ...
      >
        <StepCard
          ...
          codeBlockTitle={server.codeBlockTitle}
          code={server.code.value}
          lang="typescript"
        />
        <StepCard
          icon={<MonitorSmartphone className="size-5 text-foreground" />}
          title={client.title}
          description={client.description}
          codeBlockTitle={client.codeBlockTitle}
