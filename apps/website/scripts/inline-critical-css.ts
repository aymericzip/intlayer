 ...
 * @param prodCssHref - The actual compiled CSS asset path, used to override broken dev paths.
 */
export const inlineStylesheetLinks = async (
  html: string,
  loadStylesheet: StylesheetLoader,
  prodCssHref: string | null = null
  ...
  let alreadyInlinedProdCss = false;

  for (const tag of stylesheetLinks) {
    let href = readAttribute(tag, 'href');
    ...
    // Intercept broken TanStack dev paths injected during SSR prerender
    const isBrokenDevLink =
      href.startsWith('/src/') || href.startsWith('/@tanstack-start/');
    ...
    if (isBrokenDevLink) {
      ...
      // TanStack may inject multiple dev links. Inline the CSS chunk once,
      // and silently strip out the subsequent broken dev links to prevent duplication.
      if (alreadyInlinedProdCss) {
        rewritten = rewritten.replace(tag, '');
        continue;
      }

      href = prodCssHref;
      alreadyInlinedProdCss = true;
    }
...
const inlineCriticalCss = async (): Promise<void> => {
  ...
  // Locate the actual CSS file generated by the Vite client build
  const compiledCssPath = allFiles.find(
    (f) => f.includes('/assets/') && f.endsWith('.css')
  );
  const prodCssHref = compiledCssPath
    ? `/${relative(PUBLIC_DIRECTORY, compiledCssPath).replace(/\\/g, '/')}`
    : null;
  ...
  const outcomes = await mapWithConcurrency<string, PageOutcome>(
    ...
    async (page) => {
      ...
      if (didInline || result.html.length > 0) {
        // Always write if we altered the HTML (e.g. stripped dev tags), even if we didn't inline.
