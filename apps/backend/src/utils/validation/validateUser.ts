...
const userZodSchema = z.object({
  ...
  email: z
    ...
    .check(z.email('User Email must be a valid email address.')),
  phone: z
    .string({
      message: 'User Phone must be a string.',
    })
    .check(
      z.minLength(8, 'User Phone must be at least 8 characters long.'),
      z.maxLength(20, 'User Phone must be at most 20 characters long.'),
      z.regex(
        /^\+?\(?\d{3}\)?[-\s.]?\d{3}[-\s.]?\d{4,6}$/,
...
export const validateUser = (
  ...
  const schema = z.pick(userZodSchema, mask as any);
