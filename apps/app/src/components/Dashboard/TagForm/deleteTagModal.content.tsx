import { type Dictionary, t } from 'intlayer';

const deleteTagModalContent = {
  key: 'delete-tag-modal',
  content: {
    title: t({
      ar: 'حذف الوسم',
      de: 'Tag löschen',
      en: 'Delete tag',
      'en-GB': 'Delete tag',
      es: 'Eliminar etiqueta',
      fr: 'Supprimer le tag',
      hi: 'टैग हटाएं',
      it: 'Elimina tag',
      ja: 'タグを削除',
      ko: '태그 삭제',
      pt: 'Excluir tag',
      ru: 'Удалить тег',
      tr: 'Tagı sil',
      zh: '删除标签',
      pl: 'Usuń tag',
      id: 'Hapus tag',
      vi: 'Xóa tag',
      uk: 'Видалити тег',
    }),

    description: {
      single: t({
        en: 'Are you sure you want to delete this tag?',
        'en-GB': 'Are you sure you want to delete this tag?',
        fr: 'Êtes-vous sûr de vouloir supprimer ce tag ?',
        es: '¿Está seguro de que desea eliminar esta etiqueta?',
        de: 'Sind Sie sicher, dass Sie diesen Tag löschen möchten?',
        ja: 'このタグを削除してもよろしいですか？',
        ko: '이 태그를 삭제하시겠습니까?',
        zh: '您确定要删除此标签吗？',
        it: 'Sei sicuro di voler eliminare questo tag?',
        pt: 'Tem certeza de que deseja excluir este tag?',
        hi: 'क्या आप इस टैग को हटाना चाहते हैं?',
        ar: 'هل أنت متأكد من أنك تريد حذف هذا الوسم؟',
        ru: 'Вы уверены, что хотите удалить этот тег?',
        tr: 'Bu etiketi silmek istediğinize emin misiniz?',
        pl: 'Czy na pewno chcesz usunąć ten tag?',
        id: 'Apakah Anda yakin ingin menghapus tag ini?',
        vi: 'Bạn có chắc chắn muốn xóa tag này không?',
        uk: 'Ви впевнені, що хочете видалити цей тег?',
      }),
      bulk: t({
        en: 'Are you sure you want to delete the selected tags?',
        'en-GB': 'Are you sure you want to delete the selected tags?',
        fr: 'Êtes-vous sûr de vouloir supprimer les tags sélectionnés ?',
        es: '¿Está seguro de que desea eliminar las etiquetas seleccionadas?',
        de: 'Sind Sie sicher, dass Sie die ausgewählten Tags löschen möchten?',
        ja: '選択したタグを削除してもよろしいですか？',
        ko: '선택한 태그를 삭제하시겠습니까?',
        zh: '您确定要删除所选标签吗？',
        it: 'Sei sicuro di voler eliminare i tag selezionati?',
        pt: 'Tem certeza de que deseja excluir os tags selecionados?',
        hi: 'क्या आप चयनित टैग्स को हटाना चाहते हैं?',
        ar: 'هل أنت متأكد من أنك تريد حذف الوسوم المختارة؟',
        ru: 'Вы уверены, что хотите удалить выбранные теги?',
        tr: 'Seçilen etiketleri silmek istediğinizden emin misiniz?',
        pl: 'Czy na pewno chcesz usunąć wybrane tagi?',
        id: 'Apakah Anda yakin ingin menghapus tag yang dipilih?',
        vi: 'Bạn có chắc chắn muốn xóa các tag đã chọn không?',
        uk: 'Ви впевнені, що хочете видалити вибрані теги?',
      }),
    },

    confirmButton: {
      text: t({
        en: 'Delete',
        fr: 'Supprimer',
        es: 'Eliminar',
        'en-GB': 'Delete',
        de: 'Löschen',
        ja: '削除',
        ko: '삭제',
        zh: '删除',
        it: 'Elimina',
        pt: 'Excluir',
        hi: 'हटाएं',
        ar: 'حذف',
        ru: 'Удалить',
        tr: 'Sil',
        pl: 'Usuń',
        id: 'Hapus',
        vi: 'Xóa',
        uk: 'Видалити',
      }),
      label: t({
        en: 'Click to delete tag',
        fr: 'Cliquez pour supprimer le tag',
        es: 'Haga clic para eliminar la etiqueta',
        'en-GB': 'Click to delete tag',
        de: 'Klicken Sie, um das Tag zu löschen',
        ja: 'タグを削除するにはクリックしてください',
        ko: '태그를 삭제하려면 클릭하세요',
        zh: '点击删除标签',
        it: 'Clicca per eliminare il tag',
        pt: 'Clique para excluir o tag',
        hi: 'टैग को हटाने के लिए क्लिक करें',
        ar: 'انقر لحذف الوسم',
        ru: 'Нажмите, чтобы удалить тег',
        tr: 'Tagı silmek için tıklayın',
        pl: 'Kliknij, aby usunąć tag',
        id: 'Klik untuk menghapus tag',
        vi: 'Nhấp để xóa tag',
        uk: 'Натисніть, щоб видалити тег',
      }),
    },

    cancelButton: {
      text: t({
        en: 'Cancel',
        fr: 'Annuler',
        es: 'Cancelar',
        'en-GB': 'Cancel',
        de: 'Abbrechen',
        ja: 'キャンセル',
        ko: '취소',
        zh: '取消',
        it: 'Annulla',
        pt: 'Cancelar',
        hi: 'रद्द करें',
        ar: 'إلغاء',
        ru: 'Отменить',
        tr: 'İptal',
        pl: 'Anuluj',
        id: 'Batal',
        vi: 'Hủy',
        uk: 'Скасувати',
      }),
      label: t({
        en: 'Click to cancel',
        fr: 'Cliquez pour annuler',
        es: 'Haga clic para cancelar',
        'en-GB': 'Click to cancel',
        de: 'Klicken Sie, um abzubrechen',
        ja: 'キャンセルをクリックしてください',
        ko: '취소를 클릭하세요',
        zh: '点击取消',
        it: 'Clicca per annullare',
        pt: 'Clique para cancelar',
        hi: 'रद्द करने के लिए क्लिक करें',
        ar: 'انقر لإلغاء',
        ru: 'Нажмите, чтобы отменить',
        tr: 'İptal etmek için tıklayın',
        pl: 'Kliknij, aby anulować',
        id: 'Klik untuk membatalkan',
        vi: 'Nhấp để hủy',
        uk: 'Натисніть, щоб скасувати',
      }),
    },
  },
  title: 'Delete tag confirmation modal',
  description:
    'Modal content related to the confirmation process before deleting a tag in the dashboard. Includes text for title, buttons, and labels.',
  tags: ['dashboard', 'tag management', 'modal'],
} satisfies Dictionary;

export default deleteTagModalContent;
