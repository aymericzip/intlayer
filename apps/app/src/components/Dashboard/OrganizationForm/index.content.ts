import { type Dictionary, t } from 'intlayer';

const organizationFormContent = {
  key: 'organization-form',
  content: {
    title: t({
      en: 'Organization details',
      'en-GB': 'Organization details',
      fr: "Détails de l'organisation",
      es: 'Detalles de la organización',
      de: 'Organisationsdetails',
      ja: '組織の詳細',
      ko: '조직 세부정보',
      zh: '组织详情',
      it: "Dettagli dell'organizzazione",
      pt: 'Detalhes da organização',
      hi: 'संगठन विवरण',
      ar: 'تفاصيل المنظمة',
      ru: 'Детали организации',
      tr: 'Organizasyon detayları',
      pl: 'Szczegóły organizacji',
      id: 'Detail organisasi',
      vi: 'Chi tiết tổ chức',
      uk: 'Деталі організації',
    }),
    nameInput: {
      label: t({
        en: 'Name',
        'en-GB': 'Name',
        fr: 'Nom',
        es: 'Nombre',
        de: 'Name',
        ja: '名前',
        ko: '이름',
        zh: '名称',
        it: 'Nome',
        pt: 'Nome',
        hi: 'नाम',
        ar: 'اسم',
        ru: 'Имя',
        tr: 'İsim',
        pl: 'Nazwa',
        id: 'Nama',
        vi: 'Tên',
        uk: 'Назва',
      }),
      placeholder: t({
        en: 'Enter the name of your organization',
        'en-GB': 'Enter the name of your organization',
        fr: 'Entrez le nom de votre organisation',
        es: 'Ingrese el nombre de su organización',
        de: 'Geben Sie den Namen Ihrer Organisation ein',
        ja: '組織の名前を入力してください',
        ko: '조직의 이름을 입력하세요',
        zh: '输入您的组织名称',
        it: 'Inserisci il nome della tua organizzazione',
        pt: 'Insira o nome da sua organização',
        hi: 'अपने संगठन का नाम दर्ज करें',
        ar: 'أدخل اسم منظمتك',
        ru: 'Введите название вашей организации',
        tr: 'Organizasyonunuzun adını girin',
        pl: 'Wprowadź nazwę swojej organizacji',
        id: 'Masukkan nama organisasi Anda',
        vi: 'Nhập tên tổ chức của bạn',
        uk: 'Введіть назву вашої організації',
      }),
    },

    createOrganizationTitle: t({
      en: 'No organization',
      'en-GB': 'No organization',
      fr: 'Aucune organisation',
      es: 'Sin organización',
      de: 'Keine Organisation',
      ja: '組織なし',
      ko: '조직 없음',
      zh: '没有组织',
      it: 'Nessuna organizzazione',
      pt: 'Nenhuma organização',
      hi: 'कोई संगठन नहीं',
      ar: 'لا توجد منظمة',
      ru: 'Нет организации',
      tr: 'Organizasyon yok',
      pl: 'Brak organizacji',
      id: 'Belum ada organisasi',
      vi: 'Chưa có tổ chức',
      uk: 'Немає організації',
    }),

    createOrganizationDescription: t({
      en: 'Create your first organization to start using Intlayer',
      'en-GB': 'Create your first organization to start using Intlayer',
      fr: 'Créez votre première organisation pour commencer à utiliser Intlayer',
      es: 'Cree su primera organización para comenzar a usar Intlayer',
      de: 'Erstellen Sie Ihre erste Organisation, um Intlayer zu verwenden',
      ja: 'Intlayerの使用を開始するために最初の組織を作成してください',
      ko: 'Intlayer 사용을 시작하려면 첫 번째 조직을 만드십시오',
      zh: '创建第一个组织以开始使用Intlayer',
      it: 'Crea la tua prima organizzazione per iniziare a utilizzare Intlayer',
      pt: 'Crie sua primeira organização para começar a usar o Intlayer',
      hi: 'Intlayer का उपयोग शुरू करने के लिए अपना पहला संगठन बनाएं',
      ar: 'قم بإنشاء منظمتك الأولى لبدء استخدام Intlayer',
      ru: 'Создайте свою первую организацию, чтобы начать использовать Intlayer',
      tr: 'Intlayer kullanmaya başlamak için ilk organizasyonunuzu oluşturun',
      pl: 'Utwórz swoją pierwszą organizację, aby zacząć korzystać z Intlayer',
      id: 'Buat organisasi pertama Anda untuk mulai menggunakan Intlayer',
      vi: 'Tạo tổ chức đầu tiên để bắt đầu sử dụng Intlayer',
      uk: 'Створіть свою першу організацію, щоб почати користуватися Intlayer',
    }),

    createOrganizationButton: {
      text: t({
        en: 'Create organization',
        'en-GB': 'Create organization',
        fr: 'Créer une organisation',
        es: 'Crear una organización',
        de: 'Organisation erstellen',
        ja: '組織を作成',
        ko: '조직 만들기',
        zh: '创建组织',
        it: 'Crea organizzazione',
        pt: 'Criar organização',
        hi: 'संगठन बनाएँ',
        ar: 'إنشاء منظمة',
        ru: 'Создать организацию',
        tr: 'Organizasyon oluştur',
        pl: 'Utwórz organizację',
        id: 'Buat organisasi',
        vi: 'Tạo tổ chức',
        uk: 'Створити організацію',
      }),
      ariaLabel: t({
        en: 'Click to create organization',
        'en-GB': 'Click to create organization',
        fr: 'Cliquez pour créer une organisation',
        es: 'Haga clic para crear una organización',
        de: 'Klicken Sie, um eine Organisation zu erstellen',
        ja: '組織を作成するにはクリックしてください',
        ko: '조직을 만들려면 클릭하세요',
        zh: '单击以创建组织',
        it: "Fai clic per creare un'organizzazione",
        pt: 'Clique para criar organização',
        hi: 'संगठन बनाने के लिए क्लिक करें',
        ar: 'انقر لإنشاء منظمة',
        ru: 'Нажмите, чтобы создать организацию',
        tr: 'Organizasyon oluşturmak için tıklayın',
        pl: 'Kliknij, aby utworzyć organizację',
        id: 'Klik untuk membuat organisasi',
        vi: 'Nhấp để tạo tổ chức',
        uk: 'Натисніть, щоб створити організацію',
      }),
    },

    editButton: {
      text: t({
        en: 'Edit organization',
        'en-GB': 'Edit organization',
        fr: "Modifier l'organisation",
        es: 'Editar la organización',
        de: 'Organisation bearbeiten',
        ja: '組織を編集',
        ko: '조직 편집',
        zh: '编辑组织',
        it: 'Modifica organizzazione',
        pt: 'Editar organização',
        hi: 'संगठन संपादित करें',
        ar: 'تحرير المنظمة',
        ru: 'Редактировать организацию',
        tr: 'Organizasyonu düzenle',
        pl: 'Edytuj organizację',
        id: 'Edit organisasi',
        vi: 'Chỉnh sửa tổ chức',
        uk: 'Редагувати організацію',
      }),
      ariaLabel: t({
        en: 'Click to edit',
        'en-GB': 'Click to edit',
        fr: 'Cliquez pour modifier',
        es: 'Haga clic para editar',
        de: 'Klicken Sie, um zu bearbeiten',
        ja: '編集するにはクリックしてください',
        ko: '편집하려면 클릭하세요',
        zh: '单击以编辑',
        it: 'Fai clic per modificare',
        pt: 'Clique para editar',
        hi: 'संपादित करने के लिए क्लिक करें',
        ar: 'انقر لتحرير',
        ru: 'Нажмите, чтобы отредактировать',
        tr: 'Düzenlemek için tıklayın',
        pl: 'Kliknij, aby edytować organizację',
        id: 'Klik untuk mengedit',
        vi: 'Nhấp để chỉnh sửa',
        uk: 'Натисніть, щоб редагувати',
      }),
    },

    selectButton: {
      text: t({
        en: 'Select',
        'en-GB': 'Select',
        fr: 'Sélectionner',
        es: 'Seleccionar',
        de: 'Auswählen',
        ja: '選択',
        ko: '선택',
        zh: '选择',
        it: 'Seleziona',
        pt: 'Selecionar',
        hi: 'चुनें',
        ar: 'اختر',
        ru: 'Выбрать',
        tr: 'Seç',
        pl: 'Wybierz',
        id: 'Pilih',
        vi: 'Chọn',
        uk: 'Вибрати',
      }),
      ariaLabel: t({
        en: 'Click to select',
        'en-GB': 'Click to select',
        fr: 'Cliquez pour sélectionner',
        es: 'Haga clic para seleccionar',
        de: 'Klicken Sie, um auszuwählen',
        ja: '選択するにはクリックしてください',
        ko: '선택하려면 클릭하세요',
        zh: '单击以选择',
        it: 'Fai clic per selezionare',
        pt: 'Clique para selecionar',
        hi: 'चुनने के लिए क्लिक करें',
        ar: 'انقر للاختيار',
        ru: 'Нажмите, чтобы выбрать',
        tr: 'Seçmek için tıklayın',
        pl: 'Kliknij, aby wybrać',
        id: 'Klik untuk memilih',
        vi: 'Nhấp để chọn',
        uk: 'Натисніть, щоб вибрати',
      }),
      selected: t({
        en: 'Selected',
        'en-GB': 'Selected',
        fr: 'Sélectionné',
        es: 'Seleccionado',
        de: 'Ausgewählt',
        ja: '選択済み',
        ko: '선택됨',
        zh: '已选择',
        it: 'Selezionato',
        pt: 'Selecionado',
        hi: 'चुना हुआ',
        ar: 'محدد',
        ru: 'Выбран',
        tr: 'Seçildi',
        pl: 'Wybrano',
        id: 'Terpilih',
        vi: 'Đã chọn',
        uk: 'Вибрано',
      }),
      unselected: t({
        en: 'Select',
        'en-GB': 'Select',
        fr: 'Sélectionner',
        es: 'Seleccionar',
        de: 'Auswählen',
        ja: '選択',
        ko: '선택',
        zh: '选择',
        it: 'Seleziona',
        pt: 'Selecionar',
        hi: 'चुनें',
        ar: 'اختر',
        ru: 'Выбрать',
        tr: 'Seç',
        pl: 'Wybierz',
        id: 'Pilih',
        vi: 'Chọn',
        uk: 'Вибрати',
      }),
    },
    addOrganizationButton: {
      text: t({
        en: 'Add organization',
        'en-GB': 'Add organization',
        fr: 'Ajouter une organisation',
        de: 'Organisation hinzufügen',
        es: 'Agregar organización',
        it: 'Aggiungi organizzazione',
        pt: 'Adicionar organização',
        hi: 'संगठन जोड़ें',
        ar: 'إضافة المنظمة',
        ru: 'Добавить организацию',
        ja: '組織を追加',
        ko: '조직 추가',
        zh: '添加组织',
        tr: 'Organizasyon ekle',
        pl: 'Dodaj organizację',
        id: 'Tambahkan organisasi',
        vi: 'Thêm tổ chức',
        uk: 'Додати організацію',
      }),
      ariaLabel: t({
        en: 'Click to add organization',
        'en-GB': 'Click to add organization',
        fr: 'Cliquez pour ajouter une organisation',
        de: 'Klicken Sie, um eine Organisation hinzuzufügen',
        es: 'Haga clic para agregar una organización',
        it: "Fai clic per aggiungere un'organizzazione",
        pt: 'Clique para adicionar uma organização',
        hi: 'संगठन जोड़ने के लिए क्लिक करें',
        ar: 'انقر لإضافة المنظمة',
        ru: 'Нажмите, чтобы добавить организацию',
        ja: '組織を追加するにはクリック',
        ko: '조직을 추가하려면 클릭',
        zh: '单击添加组织',
        tr: 'Organizasyon eklemek için tıklayın',
        pl: 'Kliknij, aby dodać organizację',
        id: 'Klik untuk menambahkan organisasi',
        vi: 'Nhấp để thêm tổ chức',
        uk: 'Натисніть, щоб додати організацію',
      }),
    },
    dangerZoneTitle: t({
      en: 'Danger Zone',
      'en-GB': 'Danger Zone',
      fr: 'Zone dangereuse',
      es: 'Zona de peligro',
      de: 'Gefahrenzone',
      ja: '危険ゾーン',
      ko: '위험 구역',
      zh: '危险区域',
      it: 'Zona di pericolo',
      pt: 'Zona de perigo',
      hi: 'खतरनाक क्षेत्र',
      ar: 'منطقة الخطر',
      ru: 'Опасная зона',
      tr: 'Tehlike Bölgesi',
      pl: 'Strefa niebezpieczeństwa',
      id: 'Zona Bahaya',
      vi: 'Vùng nguy hiểm',
      uk: 'Небезпечна зона',
    }),
    dangerZoneDescription: t({
      en: 'Deleting this organization is permanent and cannot be undone. All members, projects, and data will be permanently removed.',
      'en-GB':
        'Deleting this organization is permanent and cannot be undone. All members, projects, and data will be permanently removed.',
      fr: 'La suppression de cette organisation est définitive et ne peut pas être annulée. Tous les membres, projets et données seront définitivement supprimés.',
      es: 'Eliminar esta organización es permanente y no se puede deshacer. Todos los miembros, proyectos y datos se eliminarán permanentemente.',
      de: 'Das Löschen dieser Organisation ist dauerhaft und kann nicht rückgängig gemacht werden. Alle Mitglieder, Projekte und Daten werden dauerhaft entfernt.',
      ja: 'この組織の削除は永久的で元に戻せません。すべてのメンバー、プロジェクト、データが完全に削除されます。',
      ko: '이 조직을 삭제하면 영구적으로 복구할 수 없습니다. 모든 멤버, 프로젝트 및 데이터가 영구적으로 삭제됩니다.',
      zh: '删除此组织是永久性的，无法撤销。所有成员、项目和数据将被永久删除。',
      it: 'Eliminare questa organizzazione è permanente e non può essere annullato. Tutti i membri, i progetti e i dati verranno eliminati definitivamente.',
      pt: 'Excluir esta organização é permanente e não pode ser desfeito. Todos os membros, projetos e dados serão removidos permanentemente.',
      hi: 'इस संगठन को हटाना स्थायी है और इसे पूर्ववत नहीं किया जा सकता। सभी सदस्य, प्रोजेक्ट और डेटा स्थायी रूप से हटा दिए जाएंगे।',
      ar: 'حذف هذه المنظمة نهائي ولا يمكن التراجع عنه. ستتم إزالة جميع الأعضاء والمشاريع والبيانات بشكل دائم.',
      ru: 'Удаление этой организации необратимо. Все участники, проекты и данные будут безвозвратно удалены.',
      tr: 'Bu organizasyonu silmek kalıcıdır ve geri alınamaz. Tüm üyeler, projeler ve veriler kalıcı olarak kaldırılacaktır.',
      pl: 'Usunięcie tej organizacji jest trwałe i nie można go cofnąć. Wszyscy członkowie, projekty i dane zostaną trwale usunięte.',
      id: 'Menghapus organisasi ini bersifat permanen dan tidak dapat dibatalkan. Semua anggota, proyek, dan data akan dihapus secara permanen.',
      vi: 'Xóa tổ chức này là vĩnh viễn và không thể hoàn tác. Tất cả thành viên, dự án và dữ liệu sẽ bị xóa vĩnh viễn.',
      uk: 'Видалення цієї організації є незворотним. Усіх учасників, проєкти та дані буде видалено безповоротно.',
    }),
    deleteOrganizationButton: {
      text: t({
        en: 'Delete Organization',
        'en-GB': 'Delete Organization',
        fr: "Supprimer l'organisation",
        es: 'Eliminar la organización',
        de: 'Organisation löschen',
        ja: '組織を削除',
        ko: '조직 삭제',
        zh: '删除组织',
        it: 'Elimina organizzazione',
        pt: 'Excluir organização',
        hi: 'संगठन हटाएं',
        ar: 'حذف المنظمة',
        ru: 'Удалить организацию',
        tr: 'Organizasyonu sil',
        pl: 'Usuń organizację',
        id: 'Hapus organisasi',
        vi: 'Xóa tổ chức',
        uk: 'Видалити організацію',
      }),
      ariaLabel: t({
        en: 'Click to delete organization',
        'en-GB': 'Click to delete organization',
        fr: "Cliquez pour supprimer l'organisation",
        es: 'Haga clic para eliminar la organización',
        de: 'Klicken Sie, um die Organisation zu löschen',
        ja: '組織を削除するにはクリックしてください',
        ko: '조직을 삭제하려면 클릭하세요',
        zh: '单击删除组织',
        it: "Clicca per eliminare l'organizzazione",
        pt: 'Clique para excluir a organização',
        hi: 'संगठन को हटाने के लिए क्लिक करें',
        ar: 'انقر لحذف المنظمة',
        ru: 'Нажмите, чтобы удалить организацию',
        tr: 'Organizasyonu silmek için tıklayın',
        pl: 'Kliknij, aby usunąć organizację',
        id: 'Klik untuk menghapus organisasi',
        vi: 'Nhấp để xóa tổ chức',
        uk: 'Натисніть, щоб видалити організацію',
      }),
    },
  },
  title: 'Organization form',
  description:
    'Content declaration for the organization form in the dashboard. Includes titles, button labels, and input placeholders for creating, editing, selecting, and deleting organizations.',
  tags: ['dashboard', 'organization management', 'form'],
} satisfies Dictionary;

export default organizationFormContent;
