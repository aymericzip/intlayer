...
export const useExperiment = (
  ...
  const variant = writable(variants[0] ?? '');
  const isAssigned = writable(false);
  ...
  return { variant: readonly(variant), isAssigned: readonly(isAssigned) };
};
