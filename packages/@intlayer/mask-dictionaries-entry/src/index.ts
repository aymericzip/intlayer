/**
 * @intlayer/mask-dictionaries-entry is a package that only returns the unmerged dictionary entry file.
...
export const getDictionaryMask = () => {
  const { content } = getConfiguration();

  const dictionariesPath = join(content.mainDir, 'masks.cjs');
...
export default (() => getDictionaryMask())();
