...
import {
  type IntlayerPluginOptions,
  intlayer,
  intlayerProxy,
} from 'vite-intlayer';

/** Options of the `nuxt-intlayer` module (`intlayer` key of `nuxt.config`). */
export type NuxtIntlayerModuleOptions = {
  /**
   * Compat-adapter callers forwarded to the Vite plugin, so their call sites
   * take part in build optimization. Set by compat modules such as
   * `@intlayer/nuxtjs-i18n`, which install this module.
   */
  compatCallers?: IntlayerPluginOptions['compatCallers'];
};
 ...
 * ```
 */
export const module: NuxtModule<NuxtIntlayerModuleOptions> =
  defineNuxtModule<NuxtIntlayerModuleOptions>({
    meta: {
      name: 'nuxt-intlayer',
      configKey: 'intlayer',
    },
    setup(options, nuxt) {
      ...
      nuxt.hook('vite:extendConfig', (viteConfig, { isServer }) => {
        ...
        viteConfig.plugins?.push(
          intlayer({ compatCallers: options.compatCallers })
        );
      });
